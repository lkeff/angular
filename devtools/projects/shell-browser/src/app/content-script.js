...
const detectAngularMessageBus = new same_page_message_bus_1.SamePageMessageBus(`angular-devtools-content-script-${location.href}`, `angular-devtools-detect-angular-${location.href}`);
...
const localMessageBus = new same_page_message_bus_1.SamePageMessageBus(`angular-devtools-content-script-${location.href}`, `angular-devtools-backend-${location.href}`);
const chromeMessageBus = new chrome_message_bus_1.ChromeMessageBus(port);
    ...
    if (event.source === window && event.data) {
        try {
            ...
            if (message.includes('Extension context invalidated.')) {
                console.error('Angular DevTools: Disconnecting content script due to invalid extension context. Please reload the page.');
