"use strict";
/**
 * @license
 * Copyright Google LLC All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.dev/license
 */
Object.defineProperty(exports, "__esModule", { value: true });
const angular_check_1 = require("./angular-check");
const setNgVersion = (version = '12.0.0') => document.documentElement.setAttribute('ng-version', version);
const removeNgVersion = () => document.documentElement.removeAttribute('ng-version');
describe('angular-check', () => {
    afterEach(() => removeNgVersion());
    describe('getAngularVersion', () => {
        it('should return the angular version', () => {
            setNgVersion('11.1.1');
            expect((0, angular_check_1.getAngularVersion)()).toBe('11.1.1');
        });
    });
    describe('appIsSupportedAngularVersion', () => {
        it('should work with g3', () => {
            setNgVersion('0.0.0-placeholder');
            expect((0, angular_check_1.appIsSupportedAngularVersion)()).toBeTrue();
        });
        it('should work with new versions', () => {
            setNgVersion('12.0.0');
            expect((0, angular_check_1.appIsSupportedAngularVersion)()).toBeTrue();
        });
        it('should return false for older version', () => {
            setNgVersion('9.0.0');
            expect((0, angular_check_1.appIsSupportedAngularVersion)()).toBeFalse();
        });
        it('should return false for no version', () => {
            expect((0, angular_check_1.appIsSupportedAngularVersion)()).toBeFalse();
        });
    });
    describe('appIsAngular', () => {
        it('should return true for older version', () => {
            setNgVersion('8.0.0');
            expect((0, angular_check_1.appIsAngular)()).toBeTrue();
        });
        it('should return false for no version', () => {
            expect((0, angular_check_1.appIsAngular)()).toBeFalse();
        });
    });
    describe('appIsAngularIvy', () => {
        it('should not recognize VE apps', () => {
            window.ng = {
                probe() { },
            };
            setNgVersion();
            expect((0, angular_check_1.appIsAngularIvy)()).toBeFalse();
        });
        it('should not recognize no Angular apps', () => {
            expect((0, angular_check_1.appIsAngularIvy)()).toBeFalse();
        });
        it('should recognize Ivy apps', () => {
            const el = document.createElement('div');
            el.setAttribute('ng-version', '0.0.0-PLACEHOLDER');
            el.__ngContext__ = 0;
            document.body.append(el);
            expect((0, angular_check_1.appIsAngularIvy)()).toBeTrue();
            el.remove();
        });
    });
    describe('appIsAngularInDevMode', () => {
        afterEach(() => {
            delete window.ng;
        });
        it('should detect VE apps', () => {
            window.ng = {
                probe() { },
            };
            setNgVersion();
            expect((0, angular_check_1.appIsAngularInDevMode)()).toBeTrue();
        });
        it('should detect Ivy apps', () => {
            window.ng = {
                getComponent() { },
            };
            setNgVersion();
            expect((0, angular_check_1.appIsAngularInDevMode)()).toBeTrue();
        });
        it('should not detect apps if `ng` is not an object with the right shape', () => {
            setNgVersion();
            window.ng = {};
            expect((0, angular_check_1.appIsAngularInDevMode)()).toBeFalse();
            window.ng = () => { };
            expect((0, angular_check_1.appIsAngularInDevMode)()).toBeFalse();
        });
    });
});
