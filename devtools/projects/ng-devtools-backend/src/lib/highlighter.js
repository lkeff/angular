...
exports.findComponentAndHost = findComponentAndHost;
exports.getDirectiveName = getDirectiveName;
exports.highlightSelectedElement = highlightSelectedElement;
exports.highlightHydrationElement = highlightHydrationElement;
exports.unHighlight = unHighlight;
exports.removeHydrationHighlights = removeHydrationHighlights;
exports.inDoc = inDoc;
const ng_debug_api_1 = require("./ng-debug-api/ng-debug-api");
let hydrationOverlayItems = [];
let selectedElementOverlay = null;
...
function findComponentAndHost(el) {
...
function highlightHydrationElement(el, status) {
    let overlay = null;
    if ((status === null || status === void 0 ? void 0 : status.status) === 'skipped') {
        overlay = addHighlightForElement(el, COLORS.grey, status === null || status === void 0 ? void 0 : status.status);
    }
    else if ((status === null || status === void 0 ? void 0 : status.status) === 'mismatched') {
        overlay = addHighlightForElement(el, COLORS.red, status === null || status === void 0 ? void 0 : status.status);
    }
    else if ((status === null || status === void 0 ? void 0 : status.status) === 'hydrated') {
        overlay = addHighlightForElement(el, COLORS.blue, status === null || status === void 0 ? void 0 : status.status);
    }
    if (overlay) {
...
function addHighlightForElement(el, color = COLORS.blue, overlayType) {
    const cmp = findComponentAndHost(el).component;
    const rect = getComponentRect(el);
    if ((rect === null || rect === void 0 ? void 0 : rect.height) === 0 || (rect === null || rect === void 0 ? void 0 : rect.width) === 0) {
    ...
    if (overlayType) {
        if (rect.width > MINIMAL_OVERLAY_CONTENT_SIZE.width &&
...
function showOverlay(overlay, overlayContent, dimensions, content, labelPosition) {
...
function positionOverlayContent(overlayContent, dimensions, labelPosition) {
...
function toCSSColor(red, green, blue, alpha = 1) {
    return `rgba(${red},${green},${blue},${alpha})`;
}
function createOverlaySvgElement(type) {
    let icon;
    if (type === 'hydrated') {
        icon = HYDRATION_SVG;
    }
    ...
    const svg = new DOMParser().parseFromString(icon, 'image/svg+xml').childNodes[0];
