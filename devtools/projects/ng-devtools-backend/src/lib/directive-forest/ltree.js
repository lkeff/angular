"use strict";
/**
 * @license
 * Copyright Google LLC All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.dev/license
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.LTreeStrategy = exports.getDirectiveHostElement = exports.METADATA_PROPERTY_NAME = exports.isLContainer = void 0;
exports.getLViewFromDirectiveOrElementInstance = getLViewFromDirectiveOrElementInstance;
const semver_dsl_1 = require("semver-dsl");
const highlighter_1 = require("../highlighter");
const utils_1 = require("../utils");
const version_1 = require("../version");
let HEADER_OFFSET = 19;
const latest = () => {
    HEADER_OFFSET = 20;
};
(0, semver_dsl_1.SemVerDSL)(version_1.VERSION).gte('10.0.0-next.4', latest);
// In g3 everyone has version 0.0.0, using the currently synced commits in the g3 codebase.
(0, semver_dsl_1.SemVerDSL)(version_1.VERSION).eq('0.0.0', latest);
const TYPE = 1;
const ELEMENT = 0;
const LVIEW_TVIEW = 1;
const isLContainer = (value) => {
    return Array.isArray(value) && value[TYPE] === true;
};
exports.isLContainer = isLContainer;
const isLView = (value) => {
    return Array.isArray(value) && typeof value[TYPE] === 'object';
};
exports.METADATA_PROPERTY_NAME = '__ngContext__';
function getLViewFromDirectiveOrElementInstance(dir) {
    if (!dir) {
        return null;
    }
    const context = dir[exports.METADATA_PROPERTY_NAME];
    if (!context) {
        return null;
    }
    if (isLView(context)) {
        return context;
    }
    return context.lView;
}
const getDirectiveHostElement = (dir) => {
    if (!dir) {
        return false;
    }
    const ctx = dir[exports.METADATA_PROPERTY_NAME];
    if (!ctx) {
        return false;
    }
    if (ctx[0] !== null) {
        return ctx[0];
    }
    const components = ctx[LVIEW_TVIEW].components;
    if (!components || components.length !== 1) {
        return false;
    }
    return ctx[components[0]][0];
};
exports.getDirectiveHostElement = getDirectiveHostElement;
class LTreeStrategy {
    supports(element) {
        return typeof element.__ngContext__ !== 'undefined';
    }
    _getNode(lView, data, idx) {
        const directives = [];
        let component = null;
        const tNode = data[idx];
        const node = lView[idx][ELEMENT];
        const element = (node.tagName || node.nodeName).toLowerCase();
        if (!tNode) {
            return {
                nativeElement: node,
                children: [],
                element,
                directives: [],
                component: null,
                hydration: null, // We know there is no hydration if we use the LTreeStrategy
                defer: null, // neither there will be any defer
            };
        }
        for (let i = tNode.directiveStart; i < tNode.directiveEnd; i++) {
            const instance = lView[i];
            const dirMeta = data[i];
            if (dirMeta && dirMeta.template) {
                component = {
                    name: element,
                    isElement: (0, utils_1.isCustomElement)(node),
                    instance,
                };
            }
            else if (dirMeta) {
                directives.push({
                    name: (0, highlighter_1.getDirectiveName)(instance),
                    instance,
                });
            }
        }
        return {
            nativeElement: node,
            children: [],
            element,
            directives,
            component,
            hydration: null, // We know there is no hydration if we use the LTreeStrategy
            defer: null, // neither there will be any defer
        };
    }
    _extract(lViewOrLContainer, nodes = []) {
        if ((0, exports.isLContainer)(lViewOrLContainer)) {
            for (let i = 9; i < lViewOrLContainer.length; i++) {
                if (lViewOrLContainer[i]) {
                    this._extract(lViewOrLContainer[i], nodes);
                }
            }
            return nodes;
        }
        const lView = lViewOrLContainer;
        const tView = lView[LVIEW_TVIEW];
        for (let i = HEADER_OFFSET; i < lView.length; i++) {
            const lViewItem = lView[i];
            if (tView.data && Array.isArray(lViewItem) && lViewItem[ELEMENT] instanceof Node) {
                const node = this._getNode(lView, tView.data, i);
                // TODO(mgechev): verify if this won't make us skip projected content.
                if (node.component || node.directives.length) {
                    nodes.push(node);
                    this._extract(lViewItem, node.children);
                }
            }
        }
        return nodes;
    }
    build(element, nodes = []) {
        var _a;
        const ctx = element.__ngContext__;
        const rootLView = (_a = ctx.lView) !== null && _a !== void 0 ? _a : ctx;
        return this._extract(rootLView);
    }
}
exports.LTreeStrategy = LTreeStrategy;
