...
exports.mutateNestedProp = mutateNestedProp;
 ...
 */
function mutateNestedProp(obj, keyPath, newValue) {
    if (keyPath.length === 0)
    ...
    const signalCount = nestedProps.filter((prop) => (0, utils_1.isSignal)(prop.value)).length;
    if (signalCount > 1)
        throw new Error('Cannot mutate nested signals.');
    // Check for a single signal.
    const signalIndex = nestedProps.findIndex((prop) => (0, utils_1.isSignal)(prop.value));
    if (signalIndex !== -1) {
...
function* getNestedProps(receiver, keyPath) {
    ...
    while (keys.length !== 0) {
        ...
        receiver = (0, utils_1.isSignal)(value) ? value() : value;
    }
}
/**
...
function immutableUpdate(props, newValue) {
    ...
    if (Array.isArray(prop.receiver)) {
        ...
        return Object.assign(Object.assign({}, prop.receiver), { [prop.key]: immutableUpdate(remainingProps, newValue) });
    }
    else {
