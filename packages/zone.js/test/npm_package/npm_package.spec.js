...
function checkInSubFolder(subFolder, testFn) {
    shelljs_1.default.cd(subFolder);
    testFn();
    shelljs_1.default.cd('../');
}
describe('Zone.js npm_package', () => {
    beforeEach(() => {
        shelljs_1.default.cd(path_1.default.dirname(runfiles_1.runfiles.resolve('angular/packages/zone.js/npm_package/package.json')));
    });
    describe('misc root files', () => {
        describe('README.md', () => {
            it('should have a README.md file with basic info', () => {
                expect(shelljs_1.default.cat('README.md')).toContain(`Zone`);
            });
        });
    });
    describe('primary entry-point', () => {
        ...
        it('should have a package.json file', () => {
            expect(shelljs_1.default.grep('"name":', packageJson)).toContain(`zone.js`);
        });
        ...
        it('should contain module resolution mappings', () => {
            expect(shelljs_1.default.grep('"main":', packageJson)).toContain(`zone.umd.js`);
        });
        it('should contain typings', () => {
            expect(shelljs_1.default.grep('"typings":', packageJson)).toContain(`./zone.d.ts`);
        });
    });
    describe('check npm_package root folder', () => {
        describe('typescript support', () => {
            it('should include types', () => {
                ...
                expect(shelljs_1.default.cat('zone.d.ts')).toContain('lib/zone');
                expect(shelljs_1.default.cat('zone.d.ts')).toContain('lib/zone.api.extensions');
                expect(shelljs_1.default.cat('zone.d.ts')).toContain('lib/zone.configurations.api');
                // Defines globals.
                expect(shelljs_1.default.cat('lib/zone.d.ts')).toContain('declare global {');
                expect(shelljs_1.default.cat('lib/zone.d.ts')).toContain('const Zone');
                // Defines extensions.
                expect(shelljs_1.default.cat('lib/zone.api.extensions.d.ts')).toContain('interface EventTarget');
                expect(shelljs_1.default.cat('lib/zone.configurations.api.d.ts')).toContain('ZoneGlobalConfigurations');
            });
        });
        describe('rxjs patch', () => {
            it('should not contain rxjs source', () => {
                checkInSubFolder('./bundles', () => {
                    expect(shelljs_1.default.cat('zone-patch-rxjs.umd.js')).not.toContain('_enable_super_gross_mode_that_will_cause_bad_things');
                });
                checkInSubFolder('./fesm2015', () => {
                    expect(shelljs_1.default.cat('zone-patch-rxjs.js')).not.toContain('_enable_super_gross_mode_that_will_cause_bad_things');
                });
            });
        });
        describe('es5', () => {
            it('zone.js(es5) should not contain es6 spread code', () => {
                checkInSubFolder('./bundles', () => {
                    expect(shelljs_1.default.cat('zone.umd.js')).not.toContain('let value of values');
                });
            });
            it('zone.js(es5) should not contain source map comment', () => {
                checkInSubFolder('./bundles', () => {
                    expect(shelljs_1.default.cat('zone.umd.js')).not.toContain('sourceMappingURL');
                });
            });
            it('zone.js(es5) should contain use strict', () => {
                checkInSubFolder('./bundles', () => {
                    expect(shelljs_1.default.cat('zone.umd.js')).toMatch(/^\s*'use strict';/);
                });
            });
        });
        describe('es2015', () => {
            it('zone.js(es2015) should contain es6 code', () => {
                checkInSubFolder('./fesm2015', () => {
                    expect(shelljs_1.default.cat('zone.js')).toContain('let value of values');
                });
            });
            it('zone.js(es2015) should not contain source map comment', () => {
                checkInSubFolder('./fesm2015', () => {
                    expect(shelljs_1.default.cat('zone.js')).not.toContain('sourceMappingURL');
                });
            });
            it('zone.js(es2015) should contain use strict', () => {
                checkInSubFolder('./fesm2015', () => {
                    expect(shelljs_1.default.cat('zone.js')).toMatch(/^\s*'use strict';/);
                });
            });
            it('zone-patch-rxjs.js should have rxjs external', () => {
                checkInSubFolder('./fesm2015', () => {
                    expect(shelljs_1.default.cat('zone-patch-rxjs.js')).toContain(` from 'rxjs'`);
                    expect(shelljs_1.default.cat('zone-patch-rxjs.js')).toContain(`Zone.__load_patch('rxjs',`);
                });
            });
        });
        describe('bundles file list', () => {
            it('should contain all files', () => {
                ...
                checkInSubFolder('./bundles', () => {
                    const list = shelljs_1.default.ls('./').stdout.split('\n').sort().slice(1);
