...
describe('Zone', function () {
    ...
    describe('task', () => {
        ...
        it('task can only run in the zone of creation', () => {
            ...
            }).toThrowError('A task can only be run in the zone of creation! (Creation: createZone; Execution: anotherZone)');
        ...
        it('task can only cancel in the zone of creation', () => {
            ...
            }).toThrowError('A task can only be cancelled in the zone of creation! (Creation: createZone; Execution: anotherZone)');
            task.zone.cancelTask(task);
        });
        it('should prevent double cancellation', () => {
            const task = zone.scheduleMacroTask('test', () => log.push('macroTask'), undefined, noop, noop);
            zone.cancelTask(task);
            try {
                zone.cancelTask(task);
            }
            catch (e) {
                expect(e.message).toContain("macroTask 'test': can not transition to 'canceling', expecting state 'scheduled' or 'running', was 'notScheduled'.");
            }
        });
        it('should not decrement counters on periodic tasks', () => {
            zone.run(() => {
                const task = zone.scheduleMacroTask('test', () => log.push('macroTask'), { isPeriodic: true }, noop, noop);
                zone.runTask(task);
                zone.runTask(task);
                zone.cancelTask(task);
            });
        ...
        it('should notify of queue status change', () => {
            zone.run(() => {
                ...
                z.cancelTask(z.scheduleMacroTask('test', () => log.push('macroTask'), undefined, noop, noop));
                z.cancelTask(z.scheduleEventTask('test', () => log.push('eventTask'), undefined, noop, noop));
            });
            expect(log).toEqual([
        ...
        it('should allow rescheduling a task on a separate zone', () => {
            ...
            const rchild = right.fork({
                ...
                onScheduleTask: (delegate, current, target, task) => {
                    ...
                    expect(task._zoneDelegates.map((zd) => zd.zone.name)).toEqual([
                        'left',
                        'test-root',
                        'ProxyZone',
                    ]);
    ...
    describe('invoking tasks', () => {
        ...
        it('should call onHandleError callback when zoneSpec onHasTask throw error', () => {
            ...
            const hasTaskZone = Zone.current.fork({
                name: 'hasTask',
                onHasTask: (delegate, currentZone, targetZone, hasTasState) => {
