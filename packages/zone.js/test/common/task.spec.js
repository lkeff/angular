...
const logTransitionTo = function (toState, fromState1, fromState2) {
    log.push({
...
function testFnWithLoggedTransitionTo(testFn) {
    return function () {
...
describe('task lifecycle', () => {
    describe('event task lifecycle', () => {
        ...
        it('task should transit from notScheduled to scheduling then to scheduled state when scheduleTask', testFnWithLoggedTransitionTo(() => {
        ...
        it('task should transit from scheduling to unknown when zoneSpec onScheduleTask callback throw error', testFnWithLoggedTransitionTo(() => {
            Zone.current
                .fork({
        ...
        it('task should transit from scheduled to running when task is invoked then from running to scheduled after invoke', testFnWithLoggedTransitionTo(() => {
            Zone.current.fork({ name: 'testEventTaskZone' }).run(() => {
        ...
        it('task should transit from scheduled to canceling then from canceling to notScheduled when task is canceled before running', testFnWithLoggedTransitionTo(() => {
            Zone.current.fork({ name: 'testEventTaskZone' }).run(() => {
                const task = Zone.current.scheduleEventTask('testEventTask', noop, undefined, noop, noop);
        ...
        it('task should transit from running to canceling then from canceling to notScheduled when task is canceled in running state', testFnWithLoggedTransitionTo(() => {
            Zone.current.fork({ name: 'testEventTaskZone' }).run(() => {
        ...
        it('task should transit from running to scheduled when task.callback throw error', testFnWithLoggedTransitionTo(() => {
            Zone.current.fork({ name: 'testEventTaskZone' }).run(() => {
                const task = Zone.current.scheduleEventTask('testEventTask', () => {
        ...
        it('task should transit from canceling to unknown when zoneSpec.onCancelTask throw error before task running', testFnWithLoggedTransitionTo(() => {
            Zone.current.fork({ name: 'testEventTaskZone' }).run(() => {
                const task = Zone.current.scheduleEventTask('testEventTask', noop, undefined, noop, () => {
        ...
        it('task should transit from canceling to unknown when zoneSpec.onCancelTask throw error in running state', testFnWithLoggedTransitionTo(() => {
            Zone.current.fork({ name: 'testEventTaskZone' }).run(() => {
                const task = Zone.current.scheduleEventTask('testEventTask', noop, undefined, noop, () => {
                    throw Error('cancel task');
                });
        ...
        it('task should transit from notScheduled to scheduled if zoneSpec.onHasTask throw error when scheduleTask', testFnWithLoggedTransitionTo(() => {
            Zone.current
                .fork({
                name: 'testEventTaskZone',
        ...
        it('task should transit to notScheduled state if zoneSpec.onHasTask throw error when task is canceled', testFnWithLoggedTransitionTo(() => {
            let task;
            Zone.current
                .fork({
                name: 'testEventTaskZone',
    ...
    describe('non periodical macroTask lifecycle', () => {
        ...
        it('task should transit from scheduled to running when task is invoked then from running to noScheduled after invoke', testFnWithLoggedTransitionTo(() => {
            Zone.current.fork({ name: 'testMacroTaskZone' }).run(() => {
        ...
        it('task should transit from running to noScheduled when task.callback throw error', testFnWithLoggedTransitionTo(() => {
            Zone.current.fork({ name: 'testMacroTaskZone' }).run(() => {
                const task = Zone.current.scheduleMacroTask('testMacroTask', () => {
                    throw Error('invoke error');
                }, undefined, noop, noop);
        ...
        it('task should transit from canceling to unknown when zoneSpec.onCancelTask throw error before task running', testFnWithLoggedTransitionTo(() => {
            Zone.current.fork({ name: 'testMacroTaskZone' }).run(() => {
                const task = Zone.current.scheduleMacroTask('testMacroTask', noop, undefined, noop, () => {
                    throw Error('cancel task');
                });
                try {
                    Zone.current.cancelTask(task);
        ...
        it('task should transit from notScheduled to scheduling then to scheduled if zoneSpec.onHasTask throw error when scheduleTask', testFnWithLoggedTransitionTo(() => {
            Zone.current
                .fork({
                name: 'testMacroTaskZone',
                onHasTask: (delegate, currZone, targetZone, hasTaskState) => {
        ...
        it('task should transit to notScheduled state if zoneSpec.onHasTask throw error after task.callback being invoked', testFnWithLoggedTransitionTo(() => {
            let task;
            Zone.current
                .fork({
                name: 'testMacroTaskZone',
        ...
        it('task should transit to notScheduled state if zoneSpec.onHasTask throw error when task is canceled before running', testFnWithLoggedTransitionTo(() => {
            let task;
            Zone.current
                .fork({
                name: 'testMacroTaskZone',
    ...
    describe('periodical macroTask lifecycle', () => {
        ...
        it('task should transit from notScheduled to scheduling then to scheduled state when scheduleTask', testFnWithLoggedTransitionTo(() => {
            Zone.current.fork({ name: 'testPeriodicalTaskZone' }).run(() => {
                task = Zone.current.scheduleMacroTask('testPeriodicalTask', noop, { isPeriodic: true }, noop, noop);
            });
            expect(log.map((item) => {
                return { toState: item.toState, fromState: item.fromState };
            })).toEqual([
        ...
        it('task should transit from canceling to unknown when zoneSpec.onCancelTask throw error before task running', testFnWithLoggedTransitionTo(() => {
            Zone.current.fork({ name: 'testPeriodicalTaskZone' }).run(() => {
                task = Zone.current.scheduleMacroTask('testPeriodicalTask', noop, { isPeriodic: true }, noop, () => {
                    throw Error('cancel task');
                });
                try {
                    Zone.current.cancelTask(task);
    ...
    describe('microTask lifecycle', () => {
        ...
        it('task should transit from running to notScheduled when task.callback throw error', testFnWithLoggedTransitionTo(() => {
            Zone.current.fork({ name: 'testMicroTaskZone' }).run(() => {
        ...
        it('task should not run if task transite to notScheduled state which was canceled', testFnWithLoggedTransitionTo(() => {
            let task;
    ...
    describe('reschedule zone', () => {
        ...
        it('should be able to reschedule zone when in scheduling state, after that, task will completely go to new zone, has nothing to do with original one', testFnWithLoggedTransitionTo(() => {
            zone.run(() => {
                const t = Zone.current.scheduleMacroTask('testRescheduleZoneTask', noop, undefined, noop, noop);
                t.invoke();
            });
        ...
        it('should not be able to reschedule task in notScheduled / running / canceling state', testFnWithLoggedTransitionTo(() => {
            Zone.current.fork({ name: 'rescheduleNotScheduled' }).run(() => {
                ...
                }).toThrow(Error(`macroTask 'testRescheduleZoneTask': can not transition to ` +
        ...
        it('can not reschedule a task to a zone which is the descendants of the original zone', testFnWithLoggedTransitionTo(() => {
            ...
            }).toThrowError('can not reschedule task to rescheduleZone which is descendants of the original zone originalZone');
        }));
    });
});
