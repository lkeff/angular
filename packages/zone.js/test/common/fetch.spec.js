...
describe('fetch', utils_1.isNode
    ? () => {
        it('is untested for node as the fetch implementation is experimental', () => { });
    }
    : (0, test_util_1.ifEnvSupports)('fetch', function () {
        let testZone;
        beforeEach(() => {
        ...
        it('should work for text response', function (done) {
            testZone.run(function () {
                global['fetch']('/base/angular/packages/zone.js/test/assets/sample.json').then(function (response) {
        ...
        it('should throw error when send crendential', (0, test_util_1.ifEnvSupportsWithDone)(test_util_1.isFirefox, function (done) {
            testZone.run(function () {
                global['fetch']('http://user:password@example.com').then(function (response) {
        ...
        describe('macroTask', () => {
            ...
            it('fetch should be considered as macroTask', (done) => {
                fetchZone.run(() => {
                    global['fetch']('/base/angular/packages/zone.js/test/assets/sample.json').then(function (response) {
                        expect(Zone.current.name).toBe(fetchZone.name);
                        expect(logs).toEqual([
            ...
            it('cancel fetch should invoke onCancelTask', (0, test_util_1.ifEnvSupportsWithDone)('AbortController', (done) => {
                if ((0, test_util_1.isSafari)()) {
            ...
            it('cancel fetchTask should trigger abort', (0, test_util_1.ifEnvSupportsWithDone)('AbortController', (done) => {
                if ((0, test_util_1.isSafari)()) {
                    // safari not work with AbortController
                    done();
                    return;
