 ...
 * correctly.
 */
function patchNgHostWithFileNameToModuleName(ngHost, compilerOpts, rootDirs, workspaceName, compilationTargetSrc, useManifestPathsAsModuleName) {
    ...
    function doFileNameToModuleName(importedFilePath, containingFilePath) {
        const relativeTargetPath = relativeToRootDirs(importedFilePath, rootDirs).replace(exports.EXT, '');
        ...
        if (importedFilePath.indexOf('node_modules') >= 0) {
            const maybeMetadataFile = importedFilePath.replace(exports.EXT, '') + '.metadata.json';
