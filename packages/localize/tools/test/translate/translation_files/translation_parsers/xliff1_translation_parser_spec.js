...
const xliff1_translation_parser_1 = require("../../../../src/translate/translation_files/translation_parsers/xliff1_translation_parser");
describe('Xliff1TranslationParser', () => {
    describe('analyze()', () => {
        it('should return true only if the file contains an <xliff> element with version="1.2" attribute', () => {
            const parser = new xliff1_translation_parser_1.Xliff1TranslationParser();
            expect(parser.analyze('/some/file.xlf', '<xliff version="1.2" xmlns="urn:oasis:names:tc:xliff:document:1.2">').canParse).toBeTrue();
            expect(parser.analyze('/some/file.json', '<xliff version="1.2" xmlns="urn:oasis:names:tc:xliff:document:1.2">').canParse).toBeTrue();
    ...
    describe('analyze()', () => {
        it('should return a success object if the file contains an <xliff> element with version="1.2" attribute', () => {
            const parser = new xliff1_translation_parser_1.Xliff1TranslationParser();
            expect(parser.analyze('/some/file.xlf', '<xliff version="1.2">')).toEqual(jasmine.objectContaining({ canParse: true, hint: jasmine.any(Object) }));
            expect(parser.analyze('/some/file.json', '<xliff version="1.2">')).toEqual(jasmine.objectContaining({ canParse: true, hint: jasmine.any(Object) }));
            expect(parser.analyze('/some/file.xliff', '<xliff version="1.2">')).toEqual(jasmine.objectContaining({ canParse: true, hint: jasmine.any(Object) }));
            expect(parser.analyze('/some/file.json', '<xliff version="1.2">')).toEqual(jasmine.objectContaining({ canParse: true, hint: jasmine.any(Object) }));
        });
        it('should return a failure object if the file cannot be parsed as XLIFF 1.2', () => {
            ...
            expect(parser.analyze('/some/file.xlf', '<xliff version="2.0">')).toEqual(jasmine.objectContaining({ canParse: false }));
        ...
        it('should return a diagnostics object when the file is not a valid format', () => {
            ...
            expect(result.diagnostics.messages).toEqual([
                {
                    type: 'warning',
                    message: 'The <xliff> node does not have the required attribute: version="1.2". ("[WARNING ->]<xliff version="2.0">"): /some/file.xlf@0:0',
                },
            ]);
            ...
            expect(result.diagnostics.messages).toEqual([
                {
                    type: 'error',
                    message: 'Unexpected closing tag "file". It may happen when the tag has already been closed by another tag. For more info see https://www.w3.org/TR/html5/syntax.html#closing-elements-that-have-implied-end-tags ("<xliff version="1.2">[ERROR ->]</file>"): /some/file.xlf@0:21',
                },
            ]);
        });
    });
    describe(`parse()`, () => {
      ...
      `;
            const result = doParse('/some/file.xlf', XLIFF);
            const id = (0, index_1.ɵcomputeMsgId)('{$START_TAG_APP_MY_COMPONENT}{$CLOSE_TAG_APP_MY_COMPONENT} Welcome');
            expect(result.translations[id]).toEqual((0, index_1.ɵmakeParsedTranslation)(['', '', ' Translate'], ['START_TAG_APP_MY_COMPONENT', 'CLOSE_TAG_APP_MY_COMPONENT']));
        });
     ...
     `;
            const result = doParse('/some/file.xlf', XLIFF);
            expect(result.translations['registration.submit']).toEqual((0, index_1.ɵmakeParsedTranslation)(['Weiter']));
        });
