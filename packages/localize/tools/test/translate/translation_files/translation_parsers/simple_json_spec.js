...
describe('SimpleJsonTranslationParser', () => {
    describe('analyze()', () => {
        it('should return true if the file extension  is `.json` and contains top level `locale` and `translations` properties', () => {
            const parser = new simple_json_translation_parser_1.SimpleJsonTranslationParser();
            ...
            expect(parser.analyze('/some/file.json', '{ "locale" : "fr", "translations" : {}}').canParse).toBeTrue();
        });
    });
    describe('analyze()', () => {
        it('should return a success object if the file extension  is `.json` and contains top level `locale` and `translations` properties', () => {
            const parser = new simple_json_translation_parser_1.SimpleJsonTranslationParser();
            expect(parser.analyze('/some/file.json', '{ "locale" : "fr", "translations" : {}}')).toEqual(jasmine.objectContaining({ canParse: true, hint: jasmine.any(Object) }));
        });
        it('should return a failure object if the file is not a valid format', () => {
            ...
            expect(parser.analyze('/some/file.json', '{ "translations" : {} }')).toEqual(jasmine.objectContaining({ canParse: false }));
            expect(parser.analyze('/some/file.json', '{ "locale" : "fr" }')).toEqual(jasmine.objectContaining({ canParse: false }));
        });
    });
    describe(`parse()`, () => {
        const doParse = (fileName, contents) => {
            const parser = new simple_json_translation_parser_1.SimpleJsonTranslationParser();
