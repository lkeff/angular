...
const xliff2_translation_parser_1 = require("../../../../src/translate/translation_files/translation_parsers/xliff2_translation_parser");
describe('Xliff2TranslationParser', () => {
    describe('analyze()', () => {
        it('should return true if the file contains an <xliff> element with version="2.0" attribute', () => {
            const parser = new xliff2_translation_parser_1.Xliff2TranslationParser();
            expect(parser.analyze('/some/file.xlf', '<xliff version="2.0" xmlns="urn:oasis:names:tc:xliff:document:2.0">').canParse).toBeTrue();
            expect(parser.analyze('/some/file.json', '<xliff version="2.0" xmlns="urn:oasis:names:tc:xliff:document:2.0">').canParse).toBeTrue();
    ...
    describe('analyze', () => {
        it('should return a success object if the file contains an <xliff> element with version="2.0" attribute', () => {
            const parser = new xliff2_translation_parser_1.Xliff2TranslationParser();
            expect(parser.analyze('/some/file.xlf', '<xliff version="2.0" xmlns="urn:oasis:names:tc:xliff:document:2.0">')).toEqual(jasmine.objectContaining({ canParse: true, hint: jasmine.any(Object) }));
            expect(parser.analyze('/some/file.json', '<xliff version="2.0" xmlns="urn:oasis:names:tc:xliff:document:2.0">')).toEqual(jasmine.objectContaining({ canParse: true, hint: jasmine.any(Object) }));
            expect(parser.analyze('/some/file.xliff', '<xliff version="2.0">')).toEqual(jasmine.objectContaining({ canParse: true, hint: jasmine.any(Object) }));
            expect(parser.analyze('/some/file.json', '<xliff version="2.0">')).toEqual(jasmine.objectContaining({ canParse: true, hint: jasmine.any(Object) }));
        });
        it('should return a failure object if the file cannot be parsed as XLIFF 2.0', () => {
            ...
            expect(parser.analyze('/some/file.xlf', '<xliff version="1.2">')).toEqual(jasmine.objectContaining({ canParse: false }));
        ...
        it('should return a diagnostics object when the file is not a valid format', () => {
            ...
            expect(result.diagnostics.messages).toEqual([
                {
                    type: 'warning',
                    message: 'The <xliff> node does not have the required attribute: version="2.0". ("[WARNING ->]<xliff version="1.2">"): /some/file.xlf@0:0',
                },
            ]);
            ...
            expect(result.diagnostics.messages).toEqual([
                {
                    type: 'error',
                    message: 'Unexpected closing tag "file". It may happen when the tag has already been closed by another tag. For more info see https://www.w3.org/TR/html5/syntax.html#closing-elements-that-have-implied-end-tags ("<xliff version="2.0">[ERROR ->]</file>"): /some/file.xlf@0:21',
                },
            ]);
        });
    });
    describe(`parse()`, () => {
        const doParse = (fileName, XLIFF) => {
            const parser = new xliff2_translation_parser_1.Xliff2TranslationParser();
        ...
        it('should extract basic messages', () => {
            ...
            const XLIFF = `
              ...
              `;
            const result = doParse('/some/file.xlf', XLIFF);
            expect(result.translations[(0, index_1.ɵcomputeMsgId)('translatable attribute', '')]).toEqual((0, index_1.ɵmakeParsedTranslation)(['etubirtta elbatalsnart']));
        });
        it('should extract translations with simple placeholders', () => {
            ...
            const result = doParse('/some/file.xlf', XLIFF);
            expect(result.translations[(0, index_1.ɵcomputeMsgId)('translatable element {$START_BOLD_TEXT}with placeholders{$CLOSE_BOLD_TEXT} {$INTERPOLATION}')]).toEqual((0, index_1.ɵmakeParsedTranslation)(['', ' tnemele elbatalsnart ', 'sredlohecalp htiw', ''], ['INTERPOLATION', 'START_BOLD_TEXT', 'CLOSE_BOLD_TEXT']));
        });
        ...
        it('should extract translations with simple ICU expressions', () => {
            ...
            const XLIFF = `
              ...
              `;
            const result = doParse('/some/file.xlf', XLIFF);
            expect(result.translations[(0, index_1.ɵcomputeMsgId)('{VAR_PLURAL, plural, =0 {{START_PARAGRAPH}test{CLOSE_PARAGRAPH}}}')]).toEqual((0, index_1.ɵmakeParsedTranslation)(['{VAR_PLURAL, plural, =0 {{START_PARAGRAPH}TEST{CLOSE_PARAGRAPH}}}'], []));
        });
        ...
        it('should extract translations with <mrk> elements', () => {
            const XLIFF = `
              ...
              `;
            const result = doParse('/some/file.xlf', XLIFF);
            expect(result.translations['mrk-test']).toEqual((0, index_1.ɵmakeParsedTranslation)(['Translated first sentence.']));
            expect(result.translations['mrk-test2']).toEqual((0, index_1.ɵmakeParsedTranslation)(['Translated first sentence.']));
        });
        it('should merge messages from each `<file>` element', () => {
            ...
            const result = doParse('/some/file.xlf', XLIFF);
            expect(result.translations[(0, index_1.ɵcomputeMsgId)('translatable attribute', '')]).toEqual((0, index_1.ɵmakeParsedTranslation)(['etubirtta elbatalsnart']));
            expect(result.translations[(0, index_1.ɵcomputeMsgId)('translatable element {$START_BOLD_TEXT}with placeholders{$LOSE_BOLD_TEXT} {$INTERPOLATION}')]).toEqual((0, index_1.ɵmakeParsedTranslation)(['', ' tnemele elbatalsnart ', 'sredlohecalp htiw', ''], ['INTERPOLATION', 'START_BOLD_TEXT', 'CLOSE_BOLD_TEXT']));
        });
        describe('[structure errors]', () => {
            ...
            it('should provide a diagnostic error when a trans-unit has no translation target nor source', () => {
                ...
                expectToFail('/some/file.xlf', XLIFF, /Missing required element: one of <target> or <source> is required/, [
