...
describe('utils', () => {
    describe('makeTemplateObject', () => {
        it('should return an array containing the cooked items', () => {
            const template = (0, __1.makeTemplateObject)(['cooked-a', 'cooked-b', 'cooked-c'], ['raw-a', 'raw-b', 'raw-c']);
    ...
    describe('makeParsedTranslation()', () => {
        it('should compute a template object from the parts', () => {
            expect((0, __1.makeParsedTranslation)(['a', 'b', 'c'], ['ph1', 'ph2']).messageParts).toEqual((0, __1.makeTemplateObject)(['a', 'b', 'c'], ['a', 'b', 'c']));
        });
        it('should include the placeholder names', () => {
            expect((0, __1.makeParsedTranslation)(['a', 'b', 'c'], ['ph1', 'ph2']).placeholderNames).toEqual([
                'ph1',
                'ph2',
            ]);
        });
        it('should compute the message string from the parts and placeholder names', () => {
            expect((0, __1.makeParsedTranslation)(['a', 'b', 'c'], ['ph1', 'ph2']).text).toEqual('a{$ph1}b{$ph2}c');
        });
    });
    describe('parseTranslation', () => {
        it('should extract the messageParts as a TemplateStringsArray', () => {
            const translation = (0, __1.parseTranslation)('a{$one}b{$two}c');
        ...
        it('should extract the messageParts with leading expression as a TemplateStringsArray', () => {
            const translation = (0, __1.parseTranslation)('{$one}a{$two}b');
        ...
        it('should extract the messageParts with trailing expression as a TemplateStringsArray', () => {
            const translation = (0, __1.parseTranslation)('a{$one}b{$two}');
            expect(translation.messageParts).toEqual(['a', 'b', '']);
            expect(translation.messageParts.raw).toEqual(['a', 'b', '']);
        });
        it('should extract the messageParts with escaped characters as a TemplateStringsArray', () => {
            const translation = (0, __1.parseTranslation)('a{$one}\nb\n{$two}c');
        ...
        it('should extract the messageParts with escaped placeholders as a TemplateStringsArray', () => {
            const translation = (0, __1.parseTranslation)('a{$one}:marker:b{$two}c');
        ...
        it('should handle a translation with no substitutions', () => {
            const translation = (0, __1.parseTranslation)('abc');
        ...
        it('should handle a translation with only substitutions', () => {
            const translation = (0, __1.parseTranslation)('{$one}{$two}');
    ...
    describe('translate', () => {
        it('should throw an error if there is no matching translation', () => {
            expect(() => doTranslate({}, parts `abc`)).toThrowError('No translation found for "2674653928643152084" ("abc").');
            expect(() => doTranslate({}, parts `:@@customId:abc`)).toThrowError('No translation found for "customId" ("abc").');
            expect(() => doTranslate({}, parts `:␟d42e3c2d3aa340581d42f53c46eb49ecb3d3beb4␟3896949568605777881:abc`)).toThrowError('No translation found for "2674653928643152084" ["d42e3c2d3aa340581d42f53c46eb49ecb3d3beb4", "3896949568605777881"] ("abc").');
            expect(() => doTranslate({}, parts `:meaning|:abc`)).toThrowError('No translation found for "1071947593002928768" ("abc" - "meaning").');
        });
        it('should throw an error if the translation contains placeholders that are not in the message', () => {
            expect(() => doTranslate({ 'abc{$INTERPOLATION}def': 'a{$PH}bc' }, parts `abc${1 + 2}:INTERPOLATION:def`)).toThrowError(`There is a placeholder name mismatch with the translation provided for the message "8986527425650846693" ("abc{$INTERPOLATION}def").\n` +
        ...
        it('(with identity translations) should render template literals as-is', () => {
            ...
            expect(doTranslate(translations, parts `abc${1 + 2 + 3}def`)).toEqual(parts `abc${1 + 2 + 3}def`);
            expect(doTranslate(translations, parts `abc${1 + 2 + 3}def${4 + 5 + 6}`)).toEqual(parts `abc${1 + 2 + 3}def${4 + 5 + 6}`);
            const getName = () => 'World';
            expect(doTranslate(translations, parts `Hello, ${getName()}!`)).toEqual(parts `Hello, ${'World'}!`);
        });
        ...
        it('(with translations to reverse expressions) should render template literals with expressions reversed', () => {
            ...
            const getName = () => 'World';
            expect(doTranslate(translations, parts `abc${1 + 2 + 3}def${4 + 5 + 6} - Hello, ${getName()}!`)).toEqual(parts `abc${'World'}def${4 + 5 + 6} - Hello, ${1 + 2 + 3}!`);
        });
        it('(with translations to remove expressions) should render template literals with expressions removed', () => {
            ...
            const getName = () => 'World';
            expect(doTranslate(translations, parts `abc${1 + 2 + 3}def${4 + 5 + 6} - Hello, ${getName()}!`)).toEqual(parts `abc${1 + 2 + 3} - Hello, ${'World'}!`);
        });
        function parts(messageParts, ...substitutions) {
            return [messageParts, substitutions];
        }
        function parseTranslations(translations) {
            const parsedTranslations = {};
            Object.keys(translations).forEach((key) => {
                parsedTranslations[(0, __1.computeMsgId)(key, '')] = (0, __1.parseTranslation)(translations[key]);
            });
            return parsedTranslations;
        }
        function doTranslate(translations, message) {
            return (0, __1.translate)(parseTranslations(translations), message[0], message[1]);
        }
    });
});
