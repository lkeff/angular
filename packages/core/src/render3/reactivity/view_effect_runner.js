...
exports.runEffectsInView = runEffectsInView;
const view_1 = require("../interfaces/view");
function runEffectsInView(view) {
    if (view[view_1.EFFECTS] === null) {
        return;
    }
    ...
    while (tryFlushEffects) {
        ...
        for (const effect of view[view_1.EFFECTS]) {
            if (!effect.dirty) {
                continue;
            }
        ...
        // become dirty.
        tryFlushEffects = foundDirtyEffect && !!(view[view_1.FLAGS] & 8192 /* LViewFlags.HasChildViewsToRefresh */);
    }
}
