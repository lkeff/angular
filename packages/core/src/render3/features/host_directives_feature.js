...
function resolveHostDirectives(matches) {
    ...
    for (let i = 0; i < matches.length; i++) {
        ...
        if (def.hostDirectives !== null) {
            ...
            hostDirectiveDefs !== null && hostDirectiveDefs !== void 0 ? hostDirectiveDefs : (hostDirectiveDefs = new Map());
            hostDirectiveRanges !== null && hostDirectiveRanges !== void 0 ? hostDirectiveRanges : (hostDirectiveRanges = new Map());
        ...
        if (i === 0 && (0, type_checks_1.isComponentDef)(def)) {
            hasComponent = true;
...
function trackHostDirectiveDef(def, matchedDefs, hostDirectiveDefs) {
    const hostDirectiveDef = (0, def_getters_1.getDirectiveDef)(def.directive);
...
function createHostDirectiveDef(config) {
    return typeof config === 'function'
        ? { directive: (0, di_1.resolveForwardRef)(config), inputs: empty_1.EMPTY_OBJ, outputs: empty_1.EMPTY_OBJ }
        : {
            directive: (0, di_1.resolveForwardRef)(config.directive),
    ...
    for (const publicName in exposedInputs) {
        if (exposedInputs.hasOwnProperty(publicName)) {
            ...
            if ((typeof ngDevMode === 'undefined' || ngDevMode) &&
                ...
                (0, assert_1.assertEqual)(declaredInputs[remappedPublicName], declaredInputs[publicName], `Conflicting host directive input alias ${publicName}.`);
            }
...
function validateHostDirective(hostDirectiveConfig, directiveDef) {
    ...
    if (directiveDef === null) {
        if ((0, def_getters_1.getComponentDef)(type) !== null) {
            throw new errors_1.RuntimeError(310 /* RuntimeErrorCode.HOST_DIRECTIVE_COMPONENT */, `Host directive ${type.name} cannot be a component.`);
        }
        throw new errors_1.RuntimeError(307 /* RuntimeErrorCode.HOST_DIRECTIVE_UNRESOLVABLE */, `Could not resolve metadata for host directive ${type.name}. ` +
            `Make sure that the ${type.name} class is annotated with an @Directive decorator.`);
    }
    if (!directiveDef.standalone) {
        throw new errors_1.RuntimeError(308 /* RuntimeErrorCode.HOST_DIRECTIVE_NOT_STANDALONE */, `Host directive ${directiveDef.type.name} must be standalone.`);
    }
    ...
    for (const publicName in hostDirectiveBindings) {
        if (hostDirectiveBindings.hasOwnProperty(publicName)) {
            if (!bindings.hasOwnProperty(publicName)) {
                throw new errors_1.RuntimeError(311 /* RuntimeErrorCode.HOST_DIRECTIVE_UNDEFINED_BINDING */, `Directive ${className} does not have an ${bindingType} with a public name of ${publicName}.`);
            }
            const remappedPublicName = hostDirectiveBindings[publicName];
            if (bindings.hasOwnProperty(remappedPublicName) && remappedPublicName !== publicName) {
                throw new errors_1.RuntimeError(312 /* RuntimeErrorCode.HOST_DIRECTIVE_CONFLICTING_ALIAS */, `Cannot alias ${bindingType} ${publicName} of host directive ${className} to ${remappedPublicName}, because it already has a different ${bindingType} with the same public name.`);
            }
        }
    }
}
