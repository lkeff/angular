...
function cacheMatchingLocalNames(tNode, localRefs, exportsMap) {
    ...
    for (let i = 0; i < localRefs.length; i += 2) {
        ...
        if (index == null)
            throw new errors_1.RuntimeError(-301 /* RuntimeErrorCode.EXPORT_NOT_FOUND */, ngDevMode && `Export of name '${localRefs[i + 1]}' not found!`);
...
function markAsComponentHost(tView, hostTNode, componentOffset) {
    var _a;
    ngDevMode && (0, assert_2.assertFirstCreatePass)(tView);
    ngDevMode && (0, assert_1.assertGreaterThan)(componentOffset, -1, 'componentOffset must be great than -1');
...
function initializeDirectives(tView, lView, tNode, directives, exportsMap, hostDirectiveDefs, hostDirectiveRanges) {
    ...
    for (let i = 0; i < directivesLength; i++) {
        ...
        if (!hasSeenComponent && (0, type_checks_1.isComponentDef)(def)) {
            hasSeenComponent = true;
            markAsComponentHost(tView, tNode, i);
        }
        (0, di_1.diPublicInInjector)((0, di_1.getOrCreateNodeInjectorForNode)(tNode, lView), tView, def.type);
    }
    ...
    let directiveIdx = (0, construction_1.allocExpando)(tView, lView, directivesLength, null);
    ngDevMode &&
        (0, assert_1.assertSame)(directiveIdx, tNode.directiveStart, 'TNode.directiveStart should point to just allocated space');
    ...
    for (let i = 0; i < directivesLength; i++) {
        ...
        tNode.mergedAttrs = (0, attrs_utils_1.mergeHostAttrs)(tNode.mergedAttrs, def.hostAttrs);
    ...
    for (const publicName in aliasMap) {
        if (aliasMap.hasOwnProperty(publicName)) {
            ...
            if (mode === 0 /* BindingType.Inputs */) {
                bindings = (_a = tNode.inputs) !== null && _a !== void 0 ? _a : (tNode.inputs = {});
            }
            else {
                bindings = (_b = tNode.outputs) !== null && _b !== void 0 ? _b : (tNode.outputs = {});
            }
            (_c = bindings[publicName]) !== null && _c !== void 0 ? _c : (bindings[publicName] = []);
...
function setupHostDirectiveInputsOrOutputs(mode, tNode, config, directiveIndex) {
    ...
    for (const initialName in aliasMap) {
        if (aliasMap.hasOwnProperty(initialName)) {
            ...
            let bindings;
            if (mode === 0 /* BindingType.Inputs */) {
                bindings = (_a = tNode.hostDirectiveInputs) !== null && _a !== void 0 ? _a : (tNode.hostDirectiveInputs = {});
            }
            else {
                bindings = (_b = tNode.hostDirectiveOutputs) !== null && _b !== void 0 ? _b : (tNode.hostDirectiveOutputs = {});
            }
            (_c = bindings[publicName]) !== null && _c !== void 0 ? _c : (bindings[publicName] = []);
...
function setupInitialInputs(tNode, directiveIndex, isHostDirective) {
    ...
    if (attrs === null ||
        ...
        (0, node_selector_matcher_1.isInlineTemplate)(tNode)) {
        (_a = tNode.initialInputs) !== null && _a !== void 0 ? _a : (tNode.initialInputs = []);
        ...
        if (!isHostDirective && inputs.hasOwnProperty(attrName)) {
            ...
            for (const index of inputConfig) {
                if (index === directiveIndex) {
                    inputsToStore !== null && inputsToStore !== void 0 ? inputsToStore : (inputsToStore = []);
    ...
    (_b = tNode.initialInputs) !== null && _b !== void 0 ? _b : (tNode.initialInputs = []);
...
function configureViewWithDirective(tView, tNode, lView, directiveIndex, def) {
    ngDevMode &&
        (0, assert_1.assertGreaterThanOrEqual)(directiveIndex, view_1.HEADER_OFFSET, 'Must be in Expando section');
    tView.data[directiveIndex] = def;
    const directiveFactory = def.factory || (def.factory = (0, definition_factory_1.getFactoryDef)(def.type, true));
    // Even though `directiveFactory` will already be using `ɵɵdirectiveInject` in its generated code,
    // we also want to support `inject()` directly from the directive constructor context so we set
    // `ɵɵdirectiveInject` as the inject implementation here too.
    const nodeInjectorFactory = new injector_1.NodeInjectorFactory(directiveFactory, (0, type_checks_1.isComponentDef)(def), di_2.ɵɵdirectiveInject);
    tView.blueprint[directiveIndex] = nodeInjectorFactory;
    lView[directiveIndex] = nodeInjectorFactory;
    registerHostBindingOpCodes(tView, tNode, directiveIndex, (0, construction_1.allocExpando)(tView, lView, def.hostVars, tokens_1.NO_CHANGE), def);
}
/**
...
function initTNodeFlags(tNode, index, numberOfDirectives) {
    ngDevMode &&
        (0, assert_1.assertNotEqual)(numberOfDirectives, tNode.directiveEnd - tNode.directiveStart, 'Reached the max number of directives');
