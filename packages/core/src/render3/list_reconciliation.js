...
exports.UniqueValueMultiKeyMap = exports.LiveCollection = void 0;
...
function reconcile(liveCollection, newCollection, trackByFn) {
    ...
    if (Array.isArray(newCollection)) {
        ...
        while (liveStartIdx <= liveEndIdx && liveStartIdx <= newEndIdx) {
            ...
            // collections.
            detachedItems !== null && detachedItems !== void 0 ? detachedItems : (detachedItems = new UniqueValueMultiKeyMap());
            liveKeysInTheFuture !== null && liveKeysInTheFuture !== void 0 ? liveKeysInTheFuture : (liveKeysInTheFuture = initLiveItemsInTheFuture(liveCollection, liveStartIdx, liveEndIdx, trackByFn));
    ...
    detachedItems === null || detachedItems === void 0 ? void 0 : detachedItems.forEach((item) => {
        ...
        for (const [key, idxSet] of duplicateKeys) {
            if (idxSet.size > 1) {
                ...
                for (let i = 1; i < idx.length; i++) {
                    duplicatedKeysMsg.push(`key "${(0, stringify_utils_1.stringifyForError)(key)}" at index "${idx[i - 1]}" and "${idx[i]}"`);
                }
            }
        }
        if (duplicatedKeysMsg.length > 0) {
            const message = (0, errors_1.formatRuntimeError)(-955 /* RuntimeErrorCode.LOOP_TRACK_DUPLICATE_KEYS */, 'The provided track expression resulted in duplicated keys for a given collection. ' +
...
exports.UniqueValueMultiKeyMap = UniqueValueMultiKeyMap;
