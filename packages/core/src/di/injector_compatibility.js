...
exports.SOURCE = exports.NG_TEMP_TOKEN_PATH = exports.RetrievingInjector = exports.setCurrentInjector = exports.getCurrentInjector = exports.THROW_IF_NOT_FOUND = void 0;
...
const di_1 = require("../../primitives/di");
const _THROW_IF_NOT_FOUND = {};
exports.THROW_IF_NOT_FOUND = _THROW_IF_NOT_FOUND;
var di_2 = require("../../primitives/di");
Object.defineProperty(exports, "getCurrentInjector", { enumerable: true, get: function () { return di_2.getCurrentInjector; } });
Object.defineProperty(exports, "setCurrentInjector", { enumerable: true, get: function () { return di_2.setCurrentInjector; } });
/*
...
class RetrievingInjector {
    ...
    retrieve(token, options) {
        ...
        try {
            ...
            (flags & 8 /* InternalInjectFlags.Optional */ ? null : exports.THROW_IF_NOT_FOUND), flags);
        }
        catch (e) {
...
exports.RetrievingInjector = RetrievingInjector;
...
function injectInjectorOnly(token, flags = 0 /* InternalInjectFlags.Default */) {
    const currentInjector = (0, di_1.getCurrentInjector)();
    if (currentInjector === undefined) {
        throw new errors_1.RuntimeError(-203 /* RuntimeErrorCode.MISSING_INJECTION_CONTEXT */, ngDevMode &&
            `The \`${(0, stringify_1.stringify)(token)}\` token injection failed. \`inject()\` function must be called from an injection context such as a constructor, a factory function, a field initializer, or a function used with \`runInInjectionContext\`.`);
    }
    else if (currentInjector === null) {
        return (0, inject_switch_1.injectRootLimpMode)(token, undefined, flags);
    }
    else {
        const options = convertToInjectOptions(flags);
        const value = currentInjector.retrieve(token, options);
        ngDevMode && (0, injector_profiler_1.emitInjectEvent)(token, value, flags);
        if ((0, di_1.isNotFound)(value)) {
            if (options.optional) {
                return null;
            }
            throw value;
...
function ɵɵinject(token, flags = 0 /* InternalInjectFlags.Default */) {
    return ((0, inject_switch_1.getInjectImplementation)() || injectInjectorOnly)((0, forward_ref_1.resolveForwardRef)(token), flags);
}
/**
...
function ɵɵinvalidFactoryDep(index) {
    throw new errors_1.RuntimeError(202 /* RuntimeErrorCode.INVALID_FACTORY_DEPENDENCY */, ngDevMode &&
...
function injectArgs(types) {
    ...
    for (let i = 0; i < types.length; i++) {
        const arg = (0, forward_ref_1.resolveForwardRef)(types[i]);
        if (Array.isArray(arg)) {
            if (arg.length === 0) {
                throw new errors_1.RuntimeError(900 /* RuntimeErrorCode.INVALID_DIFFER_INPUT */, ngDevMode && 'Arguments array must have arguments.');
            }
            let type = undefined;
...
function formatError(text, obj, injectorErrorName, source = null) {
    ...
    let context = (0, stringify_1.stringify)(obj);
    if (Array.isArray(obj)) {
        ...
        for (let key in obj) {
            if (obj.hasOwnProperty(key)) {
                let value = obj[key];
                parts.push(key + ':' + (typeof value === 'string' ? JSON.stringify(value) : (0, stringify_1.stringify)(value)));
            }
        }
        context = `{${parts.join(', ')}}`;
    }
