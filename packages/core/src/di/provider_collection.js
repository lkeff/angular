...
exports.USE_VALUE = void 0;
exports.makeEnvironmentProviders = makeEnvironmentProviders;
exports.provideEnvironmentInitializer = provideEnvironmentInitializer;
...
function internalImportProvidersFrom(checkForStandaloneCmp, ...sources) {
    ...
    (0, array_utils_1.deepForEach)(sources, (source) => {
        if ((typeof ngDevMode === 'undefined' || ngDevMode) && checkForStandaloneCmp) {
            const cmpDef = (0, def_getters_1.getComponentDef)(source);
            if (cmpDef === null || cmpDef === void 0 ? void 0 : cmpDef.standalone) {
                throw new errors_1.RuntimeError(800 /* RuntimeErrorCode.IMPORT_PROVIDERS_FROM_STANDALONE */, `Importing providers supports NgModule or ModuleWithProviders but got a standalone component "${(0, stringify_utils_1.stringifyForError)(source)}"`);
            }
        }
...
function processInjectorTypesWithProviders(typesWithProviders, visitor) {
    for (let i = 0; i < typesWithProviders.length; i++) {
        ...
        deepForEachProvider(providers, (provider) => {
            ngDevMode && validateProvider(provider, providers || empty_1.EMPTY_ARRAY, ngModule);
...
function walkProviderTree(container, visitor, parents, dedup) {
    ...
    let defType = null;
    let injDef = (0, defs_1.getInjectorDef)(container);
    const cmpDef = !injDef && (0, def_getters_1.getComponentDef)(container);
    if (!injDef && !cmpDef) {
        ...
        const ngModule = container
            .ngModule;
        injDef = (0, defs_1.getInjectorDef)(ngModule);
        if (injDef) {
            defType = ngModule;
        }
        else {
    ...
    if (ngDevMode && parents.indexOf(defType) !== -1) {
        const defName = (0, stringify_1.stringify)(defType);
        const path = parents.map(stringify_1.stringify);
        (0, errors_di_1.throwCyclicDependencyError)(defName, path);
    }
    ...
    if (cmpDef) {
        ...
        if (injDef.imports != null && !isDuplicate) {
            ...
            try {
                (0, array_utils_1.deepForEach)(injDef.imports, (imported) => {
        ...
        if (!isDuplicate) {
            ...
            const factory = (0, definition_factory_1.getFactoryDef)(defType) || (() => new defType());
            // Append extra providers to make more info available for consumers (to retrieve an injector
            // type), as well as internally (to calculate an injection scope correctly and eagerly
            // instantiate a `defType` when an injector is created).
            // Provider to create `defType` using its factory.
            visitor({ provide: defType, useFactory: factory, deps: empty_1.EMPTY_ARRAY }, defType);
            // Make this `defType` available to an internal logic that calculates injector scope.
            visitor({ provide: internal_tokens_1.INJECTOR_DEF_TYPES, useValue: defType, multi: true }, defType);
            // Provider to eagerly instantiate `defType` via `INJECTOR_INITIALIZER`.
            visitor({ provide: initializer_token_1.ENVIRONMENT_INITIALIZER, useValue: () => (0, injector_compatibility_1.ɵɵinject)(defType), multi: true }, defType);
        }
...
function validateProvider(provider, providers, containerType) {
    ...
    const classRef = (0, forward_ref_1.resolveForwardRef)(provider && (provider.useClass || provider.provide));
    if (!classRef) {
        (0, errors_di_1.throwInvalidProviderError)(containerType, providers, provider);
    }
}
function deepForEachProvider(providers, fn) {
    for (let provider of providers) {
        if ((0, provider_1.isEnvironmentProviders)(provider)) {
...
exports.USE_VALUE = (0, property_1.getClosureSafeProperty)({
    provide: String,
    useValue: property_1.getClosureSafeProperty,
});
function isValueProvider(value) {
    return value !== null && typeof value == 'object' && exports.USE_VALUE in value;
}
