...
exports.validateMatchingNode = validateMatchingNode;
exports.validateSiblingNodeExists = validateSiblingNodeExists;
exports.validateNodeExists = validateNodeExists;
exports.nodeNotFoundError = nodeNotFoundError;
exports.nodeNotFoundAtPathError = nodeNotFoundAtPathError;
exports.unsupportedProjectionOfDomNodes = unsupportedProjectionOfDomNodes;
exports.invalidSkipHydrationHost = invalidSkipHydrationHost;
...
function validateMatchingNode(node, nodeType, tagName, lView, tNode, isViewContainerAnchor = false) {
    ...
    if (!node ||
        ...
        let actual = '';
        const componentHostElement = (0, view_utils_1.unwrapRNode)(lView[view_1.HOST]);
        if (!node) {
            // No node found during hydration.
            header += `the node was not found.\n\n`;
            // Since the node is missing, we use the closest node to attach the error to
            (0, utils_1.markRNodeAsHavingHydrationMismatch)(componentHostElement, expectedDom);
        }
        else {
            ...
            (0, utils_1.markRNodeAsHavingHydrationMismatch)(componentHostElement, expectedDom, actualDom);
        }
        const footer = getHydrationErrorFooter(componentClassName);
        const message = header + expected + actual + getHydrationAttributeNote() + footer;
        throw new errors_1.RuntimeError(-500 /* RuntimeErrorCode.HYDRATION_NODE_MISMATCH */, message);
    }
}
/**
...
function validateSiblingNodeExists(node) {
    ...
    if (!node.nextSibling) {
        ...
        (0, utils_1.markRNodeAsHavingHydrationMismatch)(node, '', actual);
        throw new errors_1.RuntimeError(-501 /* RuntimeErrorCode.HYDRATION_MISSING_SIBLINGS */, message);
    }
}
/**
...
function validateNodeExists(node, lView = null, tNode = null) {
    if (!node) {
        ...
        if (lView !== null && tNode !== null) {
            ...
            // Since the node is missing, we use the closest node to attach the error to
            (0, utils_1.markRNodeAsHavingHydrationMismatch)((0, view_utils_1.unwrapRNode)(lView[view_1.HOST]), expected, '');
        }
        throw new errors_1.RuntimeError(-502 /* RuntimeErrorCode.HYDRATION_MISSING_NODE */, `${header}${expected}\n\n${footer}`);
    }
}
/**
...
function nodeNotFoundError(lView, tNode) {
    ...
    const footer = getHydrationErrorFooter();
    throw new errors_1.RuntimeError(-502 /* RuntimeErrorCode.HYDRATION_MISSING_NODE */, header + expected + footer);
}
/**
...
function nodeNotFoundAtPathError(host, path) {
    ...
    const footer = getHydrationErrorFooter();
    (0, utils_1.markRNodeAsHavingHydrationMismatch)(host);
    throw new errors_1.RuntimeError(-502 /* RuntimeErrorCode.HYDRATION_MISSING_NODE */, header + footer);
}
/**
...
function unsupportedProjectionOfDomNodes(rNode) {
    ...
    return new errors_1.RuntimeError(-503 /* RuntimeErrorCode.UNSUPPORTED_PROJECTION_DOM_NODES */, message);
}
/**
...
function invalidSkipHydrationHost(rNode) {
    ...
    const message = header + actual + footer;
    return new errors_1.RuntimeError(-504 /* RuntimeErrorCode.INVALID_SKIP_HYDRATION_HOST */, message);
}
...
function describeExpectedDom(lView, tNode, isViewContainerAnchor) {
    ...
    content += spacer + '…\n';
    const parentRNode = tNode.type ? (0, node_manipulation_1.getParentRElement)(lView[view_1.TVIEW], tNode, lView) : null;
    if (parentRNode) {
