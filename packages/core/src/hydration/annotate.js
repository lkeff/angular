...
exports.annotateForHydration = annotateForHydration;
...
const collect_native_nodes_1 = require("../render3/collect_native_nodes");
const def_getters_1 = require("../render3/def_getters");
const container_1 = require("../render3/interfaces/container");
const node_1 = require("../render3/interfaces/node");
...
const skip_hydration_1 = require("./skip_hydration");
...
function calcNumRootNodes(tView, lView, tNode) {
    const rootNodes = [];
    (0, collect_native_nodes_1.collectNativeNodes)(tView, lView, tNode, rootNodes);
...
function calcNumRootNodesInLContainer(lContainer) {
    const rootNodes = [];
    (0, collect_native_nodes_1.collectNativeNodesInLContainer)(lContainer, rootNodes);
    return rootNodes.length;
}
/**
...
function annotateComponentLViewForHydration(lView, context, injector) {
    const hostElement = lView[view_1.HOST];
    // Root elements might also be annotated with the `ngSkipHydration` attribute,
    // check if it's present before starting the serialization process.
    if (hostElement && !hostElement.hasAttribute(skip_hydration_1.SKIP_HYDRATION_ATTR_NAME)) {
...
function annotateLContainerForHydration(lContainer, context, injector) {
    const componentLView = (0, view_utils_1.unwrapLView)(lContainer[view_1.HOST]);
    ...
    const hostElement = (0, view_utils_1.unwrapRNode)(componentLView[view_1.HOST]);
    // Serialize all views within this view container.
    const rootLView = lContainer[view_1.PARENT];
    const rootLViewNghIndex = annotateHostElementForHydration(hostElement, rootLView, null, context);
    const renderer = componentLView[view_1.RENDERER];
    ...
    renderer.setAttribute(hostElement, utils_3.NGH_ATTR_NAME, finalIndex);
}
/**
...
function annotateForHydration(appRef, doc) {
    ...
    const isI18nHydrationEnabledVal = (0, i18n_1.isI18nHydrationEnabled)(injector);
    const isIncrementalHydrationEnabledVal = (0, utils_3.isIncrementalHydrationEnabled)(injector);
    const serializedViewCollection = new SerializedViewCollection();
    const corruptedTextNodes = new Map();
    const viewRefs = appRef._views;
    const shouldReplayEvents = injector.get(tokens_1.IS_EVENT_REPLAY_ENABLED, tokens_1.EVENT_REPLAY_ENABLED_DEFAULT);
    const eventTypesToReplay = {
        regular: new Set(),
        capture: new Set(),
    };
    const deferBlocks = new Map();
    const appId = appRef.injector.get(application_tokens_1.APP_ID);
    for (const viewRef of viewRefs) {
        const lNode = (0, utils_3.getLNodeForHydration)(viewRef);
    ...
    transferState.set(utils_3.NGH_DATA_KEY, serializedViews);
    if (deferBlocks.size > 0) {
        const blocks = {};
        for (const [id, info] of deferBlocks.entries()) {
            blocks[id] = info;
        }
        transferState.set(utils_3.NGH_DEFER_BLOCKS_KEY, blocks);
    }
...
function serializeLContainer(lContainer, tNode, lView, parentDeferBlockId, context) {
    ...
    for (let i = container_1.CONTAINER_HEADER_OFFSET; i < lContainer.length; i++) {
        ...
        let numRootNodes;
        let serializedView;
        if ((0, type_checks_1.isRootView)(childLView)) {
            // If this is a root view, get an LView for the underlying component,
            // because it contains information about the view to serialize.
            childLView = childLView[view_1.HEADER_OFFSET];
            ...
            if ((0, type_checks_1.isLContainer)(childLView)) {
                ...
                annotateLContainerForHydration(childLView, context, lView[view_1.INJECTOR]);
                const componentLView = (0, view_utils_1.unwrapLView)(childLView[view_1.HOST]);
                serializedView = {
                    [interfaces_2.TEMPLATE_ID]: componentLView[view_1.TVIEW].ssrId,
                    [interfaces_2.NUM_ROOT_NODES]: numRootNodes,
                };
            }
        }
        if (!serializedView) {
            const childTView = childLView[view_1.TVIEW];
            ...
            serializedView = {
                [interfaces_2.TEMPLATE_ID]: template,
                [interfaces_2.NUM_ROOT_NODES]: numRootNodes,
            };
            let isHydrateNeverBlock = false;
            // If this is a defer block, serialize extra info.
            if ((0, utils_1.isDeferBlock)(lView[view_1.TVIEW], tNode)) {
                ...
                if (context.isIncrementalHydrationEnabled && tDetails.hydrateTriggers !== null) {
                    ...
                    const deferBlockInfo = {
                        [interfaces_2.NUM_ROOT_NODES]: rootNodes.length,
                        [interfaces_2.DEFER_BLOCK_STATE]: lDetails[interfaces_1.DEFER_BLOCK_STATE],
                    };
                    const serializedTriggers = serializeHydrateTriggers(tDetails.hydrateTriggers);
                    if (serializedTriggers.length > 0) {
                        deferBlockInfo[interfaces_2.DEFER_HYDRATE_TRIGGERS] = serializedTriggers;
                    }
                    if (parentDeferBlockId !== null) {
                        // Serialize parent id only when it's present.
                        deferBlockInfo[interfaces_2.DEFER_PARENT_BLOCK_ID] = parentDeferBlockId;
                    }
                    context.deferBlocks.set(deferBlockId, deferBlockInfo);
                    const node = (0, view_utils_1.unwrapRNode)(lContainer);
                    if (node !== undefined) {
                        ...
                        ngDevMode && (0, error_handling_1.validateNodeExists)(node, childLView, tNode);
                        ngDevMode &&
                            (0, error_handling_1.validateMatchingNode)(node, Node.COMMENT_NODE, null, childLView, tNode, true);
                        annotateDeferBlockAnchorForHydration(node, deferBlockId);
                    }
                    ...
                    serializedView[interfaces_2.DEFER_BLOCK_ID] = deferBlockId;
                }
                ...
                serializedView[interfaces_2.DEFER_BLOCK_STATE] = lDetails[interfaces_1.DEFER_BLOCK_STATE];
            }
            if (!isHydrateNeverBlock) {
        ...
        if (views.length > 0 && currentViewAsString === lastViewAsString) {
            ...
            (_a = previousView[interfaces_2.MULTIPLIER]) !== null && _a !== void 0 ? _a : (previousView[interfaces_2.MULTIPLIER] = 1);
            previousView[interfaces_2.MULTIPLIER]++;
        }
        else {
...
function appendSerializedNodePath(ngh, tNode, lView, excludedParentNodes) {
    ...
    (_a = ngh[interfaces_2.NODES]) !== null && _a !== void 0 ? _a : (ngh[interfaces_2.NODES] = {});
    // Ensure we don't calculate the path multiple times.
    (_b = (_c = ngh[interfaces_2.NODES])[noOffsetIndex]) !== null && _b !== void 0 ? _b : (_c[noOffsetIndex] = (0, node_lookup_utils_1.calcPathForNode)(tNode, lView, excludedParentNodes));
}
/**
...
function appendDisconnectedNodeIndex(ngh, tNodeOrNoOffsetIndex) {
    ...
    const noOffsetIndex = typeof tNodeOrNoOffsetIndex === 'number'
        ...
        : tNodeOrNoOffsetIndex.index - view_1.HEADER_OFFSET;
    (_a = ngh[interfaces_2.DISCONNECTED_NODES]) !== null && _a !== void 0 ? _a : (ngh[interfaces_2.DISCONNECTED_NODES] = []);
    if (!ngh[interfaces_2.DISCONNECTED_NODES].includes(noOffsetIndex)) {
        ngh[interfaces_2.DISCONNECTED_NODES].push(noOffsetIndex);
    }
}
/**
...
function serializeLView(lView, parentDeferBlockId = null, context) {
    ...
    const i18nChildren = (0, i18n_1.getOrComputeI18nChildren)(tView, context);
    const nativeElementsToEventTypes = context.shouldReplayEvents
        ? (0, event_replay_1.collectDomEventsInfo)(tView, lView, context.eventTypesToReplay)
        : null;
    // Iterate over DOM element references in an LView.
    for (let i = view_1.HEADER_OFFSET; i < tView.bindingStartIndex; i++) {
        const tNode = tView.data[i];
        const noOffsetIndex = i - view_1.HEADER_OFFSET;
        // Attempt to serialize any i18n data for the given slot. We do this first, as i18n
        // has its own process for serialization.
        const i18nData = (0, i18n_1.trySerializeI18nBlock)(lView, i, context);
        if (i18nData) {
            (_a = ngh[interfaces_2.I18N_DATA]) !== null && _a !== void 0 ? _a : (ngh[interfaces_2.I18N_DATA] = {});
            ngh[interfaces_2.I18N_DATA][noOffsetIndex] = i18nData.caseQueue;
            ...
            for (const nodeNoOffsetIndex of i18nData.disjointNodes) {
                const tNode = tView.data[nodeNoOffsetIndex + view_1.HEADER_OFFSET];
                ngDevMode && (0, assert_1.assertTNode)(tNode);
        ...
        if (!(0, node_1.isTNodeShape)(tNode)) {
            continue;
        }
        ...
        if ((0, utils_2.isDetachedByI18n)(tNode)) {
            continue;
        }
        ...
        if ((0, node_lookup_utils_1.isDisconnectedNode)(tNode, lView) && isContentProjectedNode(tNode)) {
        ...
        if (Array.isArray(tNode.projection)) {
            for (const projectionHeadTNode of tNode.projection) {
                ...
                if (!Array.isArray(projectionHeadTNode)) {
                    ...
                    if (!(0, type_checks_1.isProjectionTNode)(projectionHeadTNode) &&
                        !(0, skip_hydration_1.isInSkipHydrationBlock)(projectionHeadTNode)) {
                        if ((0, node_lookup_utils_1.isDisconnectedNode)(projectionHeadTNode, lView)) {
                    ...
                    throw (0, error_handling_1.unsupportedProjectionOfDomNodes)((0, view_utils_1.unwrapRNode)(lView[i]));
                }
            }
        }
        ...
        if ((0, type_checks_1.isLContainer)(lView[i])) {
            ...
            if (embeddedTView !== null) {
                (_b = ngh[interfaces_2.TEMPLATES]) !== null && _b !== void 0 ? _b : (ngh[interfaces_2.TEMPLATES] = {});
                ngh[interfaces_2.TEMPLATES][noOffsetIndex] = getSsrId(embeddedTView);
            }
            // Serialize views within this LContainer.
            const hostNode = lView[i][view_1.HOST]; // host node of this container
            // LView[i][HOST] can be of 2 different types:
            // - either a DOM node
            // - or an array that represents an LView of a component
            if (Array.isArray(hostNode)) {
                // This is a component, serialize info about it.
                const targetNode = (0, view_utils_1.unwrapRNode)(hostNode);
                if (!targetNode.hasAttribute(skip_hydration_1.SKIP_HYDRATION_ATTR_NAME)) {
                    annotateHostElementForHydration(targetNode, hostNode, parentDeferBlockId, context);
                }
            }
            (_c = ngh[interfaces_2.CONTAINERS]) !== null && _c !== void 0 ? _c : (ngh[interfaces_2.CONTAINERS] = {});
            ngh[interfaces_2.CONTAINERS][noOffsetIndex] = serializeLContainer(lView[i], tNode, lView, parentDeferBlockId, context);
        }
        else if (Array.isArray(lView[i]) && !(0, node_1.isLetDeclaration)(tNode)) {
            // This is a component, annotate the host node with an `ngh` attribute.
            // Note: Let declarations that return an array are also storing an array in the LView,
            // we need to exclude them.
            const targetNode = (0, view_utils_1.unwrapRNode)(lView[i][view_1.HOST]);
            if (!targetNode.hasAttribute(skip_hydration_1.SKIP_HYDRATION_ATTR_NAME)) {
            ...
            if (tNode.type & 8 /* TNodeType.ElementContainer */) {
                ...
                (_d = ngh[interfaces_2.ELEMENT_CONTAINERS]) !== null && _d !== void 0 ? _d : (ngh[interfaces_2.ELEMENT_CONTAINERS] = {});
                ngh[interfaces_2.ELEMENT_CONTAINERS][noOffsetIndex] = calcNumRootNodes(tView, lView, tNode.child);
            }
                ...
                if (nextTNode && !(0, skip_hydration_1.isInSkipHydrationBlock)(nextTNode)) {
                    // Handle a tNode after the `<ng-content>` slot.
                    appendSerializedNodePath(ngh, nextTNode, lView, i18nChildren);
                }
            }
            else if (tNode.type & 1 /* TNodeType.Text */) {
                const rNode = (0, view_utils_1.unwrapRNode)(lView[i]);
                (0, utils_3.processTextNodeBeforeSerialization)(context, rNode);
            }
        }
        ...
        if (nativeElementsToEventTypes && tNode.type & 2 /* TNodeType.Element */) {
            const nativeElement = (0, view_utils_1.unwrapRNode)(lView[i]);
            if (nativeElementsToEventTypes.has(nativeElement)) {
                (0, event_delegation_utils_1.setJSActionAttributes)(nativeElement, nativeElementsToEventTypes.get(nativeElement), parentDeferBlockId);
            }
        }
    }
    return ngh;
...
function conditionallyAnnotateNodePath(ngh, tNode, lView, excludedParentNodes) {
    if ((0, type_checks_1.isProjectionTNode)(tNode)) {
    ...
    if (tNode.projectionNext &&
        ...
        !(0, skip_hydration_1.isInSkipHydrationBlock)(tNode.projectionNext)) {
    ...
    if (tNode.prev === null &&
        tNode.parent !== null &&
        (0, node_lookup_utils_1.isDisconnectedNode)(tNode.parent, lView) &&
        !(0, node_lookup_utils_1.isDisconnectedNode)(tNode, lView)) {
...
function componentUsesShadowDomEncapsulation(lView) {
    ...
    return (instance === null || instance === void 0 ? void 0 : instance.constructor)
        ? ((_a = (0, def_getters_1.getComponentDef)(instance.constructor)) === null || _a === void 0 ? void 0 : _a.encapsulation) === metadata_1.ViewEncapsulation.ShadowDom
        : false;
}
/**
...
function annotateHostElementForHydration(element, lView, parentDeferBlockId, context) {
    ...
    if (((0, type_checks_1.hasI18n)(lView) && !(0, i18n_1.isI18nHydrationSupportEnabled)()) ||
        ...
        //   would exactly match
        renderer.setAttribute(element, skip_hydration_1.SKIP_HYDRATION_ATTR_NAME, '');
        return null;
    }
    else {
        const ngh = serializeLView(lView, parentDeferBlockId, context);
        const index = context.serializedViewCollection.add(ngh);
        renderer.setAttribute(element, utils_3.NGH_ATTR_NAME, index.toString());
        return index;
    }
}
/**
...
function isContentProjectedNode(tNode) {
    ...
    while (currentTNode != null) {
        ...
        if ((0, type_checks_1.isComponentHost)(currentTNode)) {
            return true;
        }
...
function annotateDeferBlockRootNodesWithJsAction(tDetails, rootNodes, parentDeferBlockId, context) {
    const actionList = (0, utils_3.convertHydrateTriggersToJsAction)(tDetails.hydrateTriggers);
    ...
    if (actionList.length > 0) {
        ...
        for (let rNode of elementNodes) {
            (0, event_delegation_utils_1.setJSActionAttributes)(rNode, actionList, parentDeferBlockId);
        }
    }
}
