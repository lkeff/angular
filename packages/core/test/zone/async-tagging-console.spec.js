 ...
 */
const async_stack_tagging_1 = require("../../src/zone/async-stack-tagging");
describe('AsyncTaggingConsoleTest', () => {
    describe('should call console async stack tagging API', () => {
        ...
        let asyncStackTaggingZone;
        beforeEach(() => {
            scheduleAsyncTaskSpy.calls.reset();
            startAsyncTaskSpy.calls.reset();
            finishAsyncTaskSpy.calls.reset();
            asyncStackTaggingZone = Zone.current.fork(new async_stack_tagging_1.AsyncStackTaggingZoneSpec('test', {
        ...
        if (document &&
            ...
            document.addEventListener[Zone.__symbol__('OriginalDelegate')]) {
            it('button click', () => {
                asyncStackTaggingZone.run(() => {
                    ...
                    expect(scheduleAsyncTaskSpy).toHaveBeenCalledWith('Zone - HTMLButtonElement.addEventListener:click');
                    expect(startAsyncTaskSpy.calls.count()).toBe(2);
                    expect(finishAsyncTaskSpy.calls.count()).toBe(2);
                });
            });
