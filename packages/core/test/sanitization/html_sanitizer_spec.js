...
const html_sanitizer_1 = require("../../src/sanitization/html_sanitizer");
const inert_body_1 = require("../../src/sanitization/inert_body");
function sanitizeHtml(defaultDoc, unsafeHtmlInput) {
    return (0, html_sanitizer_1._sanitizeHtml)(defaultDoc, unsafeHtmlInput).toString();
}
describe('HTML sanitizer', () => {
    ...
    beforeEach(() => {
        ...
        console.warn = (msg) => logMsgs.push(msg);
    });
    afterEach(() => {
        console.warn = originalLog;
    });
    it('serializes nested structures', () => {
        expect(sanitizeHtml(defaultDoc, '<div alt="x"><p>a</p>b<b>c<a alt="more">d</a></b>e</div>')).toEqual('<div alt="x"><p>a</p>b<b>c<a alt="more">d</a></b>e</div>');
    ...
    it('supports namespaced attributes', () => {
        expect(sanitizeHtml(defaultDoc, '<a xlink:href="something">t</a>')).toEqual('<a xlink:href="something">t</a>');
        expect(sanitizeHtml(defaultDoc, '<a xlink:evil="something">t</a>')).toEqual('<a>t</a>');
        expect(sanitizeHtml(defaultDoc, '<a xlink:href="javascript:foo()">t</a>')).toEqual('<a xlink:href="unsafe:javascript:foo()">t</a>');
    });
    it('supports HTML5 elements', () => {
        expect(sanitizeHtml(defaultDoc, '<main><summary>Works</summary></main>')).toEqual('<main><summary>Works</summary></main>');
    });
    it('supports ARIA attributes', () => {
        expect(sanitizeHtml(defaultDoc, '<h1 role="presentation" aria-haspopup="true">Test</h1>')).toEqual('<h1 role="presentation" aria-haspopup="true">Test</h1>');
        expect(sanitizeHtml(defaultDoc, '<i aria-label="Info">Info</i>')).toEqual('<i aria-label="Info">Info</i>');
        expect(sanitizeHtml(defaultDoc, '<img src="pteranodon.jpg" aria-details="details">')).toEqual('<img src="pteranodon.jpg" aria-details="details">');
    });
    it('ignores srcset attributes', () => {
        // Modern browsers can handle `srcset` safely without any additional sanitization.
        expect(sanitizeHtml(defaultDoc, '<img srcset="/foo.png 400px, javascript:evil() 23px">')).toEqual('<img srcset="/foo.png 400px, javascript:evil() 23px">');
    ...
    it('escapes entities', () => {
        ...
        expect(sanitizeHtml(defaultDoc, '<p>Hello < World</p>')).toEqual('<p>Hello &lt; World</p>');
        expect(sanitizeHtml(defaultDoc, '<p alt="% &amp; &quot; !">Hello</p>')).toEqual('<p alt="% &amp; &#34; !">Hello</p>'); // NB: quote encoded as ASCII &#34;.
    });
    ...
    it('ignores content of script elements', () => {
        ...
        expect(sanitizeHtml(defaultDoc, '<script>var foo="<p>bar</p>"</script><div>hi</div>')).toEqual('<div>hi</div>');
        expect(sanitizeHtml(defaultDoc, '<style><!-- something-->hi</style>')).toEqual('');
    });
    it('ignores content of style elements', () => {
        expect(sanitizeHtml(defaultDoc, '<style><!-- foobar --></style><div>hi</div>')).toEqual('<div>hi</div>');
    ...
    it('should sanitize the content when `nextSibling` or `firstChild` were clobbered', () => {
        const nextSibling = () => sanitizeHtml(defaultDoc, '<input name="nextSibling" form="a">A<form id="a"></form>');
        const firstChild = () => sanitizeHtml(defaultDoc, '<object form="a" id="firstChild"></object>B<form id="a"></form>');
    ...
    it('should not allow JavaScript hidden in badly formed HTML to get through sanitization (Firefox bug)', () => {
        expect(sanitizeHtml(defaultDoc, '<svg><p><style><img src="</style><img src=x onerror=alert(1)//">')).toEqual((0, inert_body_1.isDOMParserAvailable)()
    ...
    it('should prevent mXSS attacks', function () {
        ...
        expect(sanitizeHtml(defaultDoc, '<a href="&#x3000;javascript:alert(1)">CLICKME</a>')).toMatch(/<a href="unsafe:(&#12288;)?javascript:alert\(1\)">CLICKME<\/a>/);
    });
    if ((0, inert_body_1.isDOMParserAvailable)()) {
        it('should work even if DOMParser returns a null body', () => {
            ...
            spyOn(window.DOMParser.prototype, 'parseFromString').and.returnValue({ body: null });
            expect(sanitizeHtml(defaultDoc, 'Hello, World')).toEqual('Hello, World');
        });
    }
});
