...
describe('computed', () => {
    it('should create computed', () => {
        ...
        const double = (0, core_1.computed)(() => `${counter() * 2}:${++computedRunCount}`);
    ...
    it('should not re-compute if there are no dependencies', () => {
        let tick = 0;
        const c = (0, core_1.computed)(() => ++tick);
        expect(c()).toEqual(1);
        expect(c()).toEqual(1);
    });
    ...
    it('should chain computed', () => {
        const name = (0, core_1.signal)('abc');
        const reverse = (0, core_1.computed)(() => name().split('').reverse().join(''));
        const upper = (0, core_1.computed)(() => reverse().toUpperCase());
        expect(upper()).toEqual('CBA');
        name.set('foo');
        expect(upper()).toEqual('OOF');
    });
    it('should evaluate computed only when subscribing', () => {
        const name = (0, core_1.signal)('John');
        const age = (0, core_1.signal)(25);
        const show = (0, core_1.signal)(true);
        let computeCount = 0;
        const displayName = (0, core_1.computed)(() => `${show() ? `${name()} aged ${age()}` : 'anonymous'}:${++computeCount}`);
    ...
    it('should detect simple dependency cycles', () => {
        const a = (0, core_1.computed)(() => a());
        expect(() => a()).toThrowError('Detected cycle in computations.');
    });
    it('should detect deep dependency cycles', () => {
        const a = (0, core_1.computed)(() => b());
        const b = (0, core_1.computed)(() => c());
        const c = (0, core_1.computed)(() => d());
        const d = (0, core_1.computed)(() => a());
        expect(() => a()).toThrowError('Detected cycle in computations.');
    });
    it('should cache exceptions thrown until computed gets dirty again', () => {
        const toggle = (0, core_1.signal)('KO');
    ...
    it('should not mark dirty computed signals that are dirty already', () => {
        const source = (0, core_1.signal)('a');
        const derived = (0, core_1.computed)(() => source().toUpperCase());
        let watchCount = 0;
    ...
    it('should disallow writing to signals within computed', () => {
        const source = (0, core_1.signal)(0);
        const illegal = (0, core_1.computed)(() => {
            source.set(1);
            return 0;
        });
    ...
    it('should set debugName when a debugName is provided', () => {
        const primitiveSignal = (0, core_1.signal)(0);
        const node = (0, core_1.computed)(() => primitiveSignal(), { debugName: 'computedSignal' })[signals_1.SIGNAL];
        expect(node.debugName).toBe('computedSignal');
    });
    describe('with custom equal', () => {
        it('should cache exceptions thrown by equal', () => {
            const s = (0, core_1.signal)(0);
            let computedRunCount = 0;
            let equalRunCount = 0;
            const c = (0, core_1.computed)(() => {
                computedRunCount++;
        ...
        it('should not track signal reads inside equal', () => {
            const value = (0, core_1.signal)(1);
            const epsilon = (0, core_1.signal)(0.5);
            let innerRunCount = 0;
            let equalRunCount = 0;
            const inner = (0, core_1.computed)(() => {
                innerRunCount++;
                return value();
            }, {
                equal: (a, b) => {
        ...
        it('should recover from exception', () => {
            ...
            const source = (0, core_1.signal)(0);
            const derived = (0, core_1.computed)(source, {
                equal: (a, b) => {
                    if (shouldThrow) {
                        throw new Error('equal');
                    }
                    return (0, signals_1.defaultEquals)(a, b);
                },
            });
    ...
    it('should call the post-producer-created fn when signal is called', () => {
        const producerKindsCreated = [];
        const prev = (0, signals_1.setPostProducerCreatedFn)((node) => producerKindsCreated.push(node.kind));
        const count = (0, core_1.signal)(0);
        (0, core_1.computed)(() => count() % 2 === 0);
