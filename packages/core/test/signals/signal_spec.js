...
describe('signals', () => {
    it('should be a getter which reflects the set value', () => {
        const state = (0, core_1.signal)(false);
    ...
    it('should not update signal when new value is equal to the previous one', () => {
        const state = (0, core_1.signal)('aaa', { equal: (a, b) => a.length === b.length });
    ...
    it('should not propagate change when the new signal value is equal to the previous one', () => {
        const state = (0, core_1.signal)('aaa', { equal: (a, b) => a.length === b.length });
        const upper = (0, core_1.computed)(() => state().toUpperCase());
        // set to a "different" value that is "equal" to the previous one
        // there should be no change in the signal's value as the new value is determined to be equal
        // to the previous one
        state.set('bbb');
    ...
    it('should consider objects as equal based on their identity with the default equality function', () => {
        let stateValue = {};
        const state = (0, core_1.signal)(stateValue);
        let computeCount = 0;
        const derived = (0, core_1.computed)(() => `${typeof state()}:${++computeCount}`);
    ...
    it('should invoke custom equality function even if old / new references are the same', () => {
        const state = { value: 0 };
        const stateSignal = (0, core_1.signal)(state, { equal: (a, b) => false });
        let computeCount = 0;
        const derived = (0, core_1.computed)(() => `${stateSignal().value}:${++computeCount}`);
    ...
    it('should allow converting writable signals to their readonly counterpart', () => {
        ...
        const double = (0, core_1.computed)(() => readOnlyCounter() * 2);
    ...
    it('should set debugName when a debugName is provided', () => {
        const node = (0, core_1.signal)(false, { debugName: 'falseSignal' })[signals_1.SIGNAL];
    ...
    describe('optimizations', () => {
        it('should not repeatedly poll status of a non-live node if no signals have changed', () => {
            const unrelated = (0, core_1.signal)(0);
            const source = (0, core_1.signal)(1);
            let computations = 0;
            const derived = (0, core_1.computed)(() => {
                computations++;
                return source() * 2;
            });
            expect(derived()).toBe(2);
            expect(computations).toBe(1);
            const sourceNode = source[signals_1.SIGNAL];
    ...
    describe('post-signal-set functions', () => {
        ...
        beforeEach(() => {
            log = 0;
            prevPostSignalSetFn = (0, signals_1.setPostSignalSetFn)(() => log++);
        });
        afterEach(() => {
            (0, signals_1.setPostSignalSetFn)(prevPostSignalSetFn);
        });
        ...
        it('should pass post-signal-set fn the node that was updated', () => {
            const counter = (0, core_1.signal)(0, { debugName: 'test-signal' });
            let node = null;
            (0, signals_1.setPostSignalSetFn)((n) => {
                node = n;
            });
            counter.set(1);
...
describe('createSignalTuple', () => {
    it('get returns the signal value', () => {
        const [get] = (0, signals_1.createSignalTuple)(0);
        expect(get()).toBe(0);
    });
    it('set sets the signal value', () => {
        const [get, set] = (0, signals_1.createSignalTuple)(0);
        set(1);
        expect(get()).toBe(1);
    });
    it('update updates the values based on the previous value', () => {
        const [get, , update] = (0, signals_1.createSignalTuple)(0);
        update((prev) => prev + 2);
        expect(get()).toBe(2);
    });
});
