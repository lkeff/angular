...
describe('control flow - if', () => {
    it('should add and remove views based on conditions change', () => {
        let TestComponent = (() => {
            let _classDecorators = [(0, core_1.Component)({ template: '@if (show) {Something} @else {Nothing}' })];
            let _classDescriptor;
            let _classExtraInitializers = [];
            let _classThis;
    ...
    describe('content projection', () => {
      ...
      `,
        ...
        it('should project @if an @else content into separate slots', () => {
            let TestComponent = (() => {
                let _classDecorators = [(0, core_1.Component)({
                        selector: 'test',
                        template: 'if: (<ng-content select="[if_case]"/>),  else: (<ng-content select="[else_case]"/>)',
                    })];
                let _classDescriptor;
                let _classExtraInitializers = [];
                let _classThis;
      ...
      `,
        ...
        it('should project an @if with a single root template node into the root node slot', () => {
            ...
            let App = (() => {
                let _classDecorators = [(0, core_1.Component)({
                        imports: [TestComponent, common_1.NgFor],
