...
exports.findUninitializedPropertiesToCombine = findUninitializedPropertiesToCombine;
exports.shouldCombineInInitializationOrder = shouldCombineInInitializationOrder;
...
function findUninitializedPropertiesToCombine(node, constructor, localTypeChecker, options) {
    ...
    for (const member of node.members) {
        if (typescript_1.default.isPropertyDeclaration(member) &&
            !member.initializer &&
            !typescript_1.default.isComputedPropertyName(member.name)) {
    ...
    for (const [name, decl] of membersToDeclarations.entries()) {
        if (memberInitializers.has(name)) {
            ...
            if (!hasLocalReferences(initializer, constructor, inlinableParameters, localTypeChecker)) {
                toCombine !== null && toCombine !== void 0 ? toCombine : (toCombine = []);
...
function shouldCombineInInitializationOrder(toCombine, constructor) {
    ...
    constructor.parameters.forEach((param) => {
        if ((0, analysis_1.parameterDeclaresProperty)(param) && typescript_1.default.isIdentifier(param.name)) {
    ...
    const walkInitializer = (node) => {
        if (typescript_1.default.isPropertyAccessExpression(node) && node.expression.kind === typescript_1.default.SyntaxKind.ThisKeyword) {
    ...
    for (const node of constructor.body.statements) {
        ...
        if (!typescript_1.default.isExpressionStatement(node) ||
            ...
            (!typescript_1.default.isPropertyAccessExpression(node.expression.left) &&
                !typescript_1.default.isElementAccessExpression(node.expression.left)) ||
            node.expression.left.expression.kind !== typescript_1.default.SyntaxKind.ThisKeyword) {
            continue;
        }
        let name;
        ...
        else if (typescript_1.default.isElementAccessExpression(node.expression.left)) {
            name = typescript_1.default.isStringLiteralLike(node.expression.left.argumentExpression)
...
function getIdentifierReferencingProperty(node, localTypeChecker, propertyNames, properties) {
    var _a;
    if (!typescript_1.default.isIdentifier(node) || !propertyNames.has(node.text)) {
        return undefined;
    }
    const declarations = (_a = localTypeChecker.getSymbolAtLocation(node)) === null || _a === void 0 ? void 0 : _a.declarations;
    if (!declarations) {
        return undefined;
    }
...
function findInlinableParameterReferences(constructorDeclaration, localTypeChecker) {
    const eligibleProperties = constructorDeclaration.parameters.filter((p) => { var _a; return typescript_1.default.isIdentifier(p.name) && ((_a = p.modifiers) === null || _a === void 0 ? void 0 : _a.some((s) => s.kind === typescript_1.default.SyntaxKind.ReadonlyKeyword)); });
...
function hasLocalReferences(root, constructor, allowedParameters, localTypeChecker) {
    ...
    const walk = (node) => {
        ...
        // }
        // ```
        if (typescript_1.default.isIdentifier(node) && !(0, analysis_1.isAccessedViaThis)(node)) {
            const declarations = (_a = localTypeChecker.getSymbolAtLocation(node)) === null || _a === void 0 ? void 0 : _a.declarations;
            const isReferencingLocalSymbol = declarations === null || declarations === void 0 ? void 0 : declarations.some((decl) => 
...
function isInsideInlineFunction(startNode, boundary) {
    ...
    while (current) {
        ...
        if ((0, analysis_1.isInlineFunction)(current)) {
            return true;
        }
