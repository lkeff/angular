...
exports.nakedngif = exports.boundngif = exports.ngif = void 0;
exports.migrateIf = migrateIf;
const compiler_1 = require("@angular/compiler");
const types_1 = require("./types");
const util_1 = require("./util");
exports.ngif = '*ngIf';
exports.boundngif = '[ngIf]';
exports.nakedngif = 'ngIf';
const ifs = [exports.ngif, exports.nakedngif, exports.boundngif];
/**
...
function migrateIf(template) {
    ...
    let result = template;
    const visitor = new types_1.ElementCollector(ifs);
    ...
    for (const el of visitor.elements) {
        ...
        try {
            ...
            errors.push({ type: exports.ngif, error });
        }
...
function buildIfBlock(etm, tmpl, offset) {
    ...
    const startBlock = `${types_1.startMarker}@if (${condition}) {${lbString}${start}`;
    const endBlock = `${end}${lbString}}${types_1.endMarker}`;
...
function buildStandardIfElseBlock(etm, tmpl, elseString, offset) {
    ...
    const condition = etm
        ...
        // replace 'let' with 'as' whatever spaces are between ; and 'let'
        .replace(/;\s*let/g, '; as');
    const elsePlaceholder = (0, util_1.getPlaceholder)(etm.getTemplateName(elseString));
...
function buildBoundIfElseBlock(etm, tmpl, offset) {
    ...
    const elsePlaceholder = (0, util_1.getPlaceholder)(etm.elseAttr.value.trim());
    if (etm.thenAttr !== undefined) {
        const thenPlaceholder = (0, util_1.getPlaceholder)(etm.thenAttr.value.trim());
...
function buildIfElseBlock(etm, tmpl, condition, elsePlaceholder, offset) {
    ...
    const postBlock = elseBlock + elsePlaceholder + `${lbString}}${types_1.endMarker}`;
...
function buildStandardIfThenElseBlock(etm, tmpl, thenString, elseString, offset) {
    ...
    const condition = etm
        ...
        // replace 'let' with 'as' whatever spaces are between ; and 'let'
        .replace(/;\s*let/g, '; as');
    const thenPlaceholder = (0, util_1.getPlaceholder)(etm.getTemplateName(thenString, elseString));
    const elsePlaceholder = (0, util_1.getPlaceholder)(etm.getTemplateName(elseString));
    return buildIfThenElseBlock(etm, tmpl, condition, thenPlaceholder, elsePlaceholder, offset);
}
function buildStandardIfThenBlock(etm, tmpl, thenString, offset) {
    ...
    const condition = etm
        ...
        // replace 'let' with 'as' whatever spaces are between ; and 'let'
        .replace(/;\s*let/g, '; as');
    const thenPlaceholder = (0, util_1.getPlaceholder)(etm.getTemplateName(thenString));
...
function buildIfThenElseBlock(etm, tmpl, condition, thenPlaceholder, elsePlaceholder, offset) {
    const lbString = etm.hasLineBreaks ? '\n' : '';
    const originals = (0, util_1.getOriginals)(etm, tmpl, offset);
    const startBlock = `${types_1.startMarker}@if (${condition}) {${lbString}`;
    const elseBlock = `${lbString}} @else {${lbString}`;
    const postBlock = thenPlaceholder + elseBlock + elsePlaceholder + `${lbString}}${types_1.endMarker}`;
...
function buildIfThenBlock(etm, tmpl, condition, thenPlaceholder, offset) {
    ...
    const originals = (0, util_1.getOriginals)(etm, tmpl, offset);
    const startBlock = `${types_1.startMarker}@if (${condition}) {${lbString}`;
    const postBlock = thenPlaceholder + `${lbString}}${types_1.endMarker}`;
