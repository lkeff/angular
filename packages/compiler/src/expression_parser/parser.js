...
exports.Parser = exports.TemplateBindingParseResult = exports.SplitInterpolation = void 0;
...
class Parser {
    ...
    parseAction(input, location, absoluteOffset, interpolationConfig = defaults_1.DEFAULT_INTERPOLATION_CONFIG) {
    ...
    parseBinding(input, location, absoluteOffset, interpolationConfig = defaults_1.DEFAULT_INTERPOLATION_CONFIG) {
    ...
    parseSimpleBinding(input, location, absoluteOffset, interpolationConfig = defaults_1.DEFAULT_INTERPOLATION_CONFIG) {
        const ast = this._parseBindingAst(input, location, absoluteOffset, interpolationConfig);
    ...
    parseInterpolation(input, location, absoluteOffset, interpolatedTokens, interpolationConfig = defaults_1.DEFAULT_INTERPOLATION_CONFIG) {
    ...
    createInterpolationAst(strings, expressions, input, location, absoluteOffset) {
        const span = new ast_1.ParseSpan(0, input.length);
        const interpolation = new ast_1.Interpolation(span, span.toAbsolute(absoluteOffset), strings, expressions);
     ...
     */
    splitInterpolation(input, location, interpolatedTokens, interpolationConfig = defaults_1.DEFAULT_INTERPOLATION_CONFIG) {
        var _a;
        const strings = [];
        const expressions = [];
        const offsets = [];
    ...
    wrapLiteralPrimitive(input, location, absoluteOffset) {
        const span = new ast_1.ParseSpan(0, input == null ? 0 : input.length);
...
class _ParseAST {
                ...
                if (this.supportsDirectPipeReferences) {
                    ...
                    type =
                        charCode === chars.$_ || (charCode >= chars.$A && charCode <= chars.$Z)
                            ? ast_1.BindingPipeType.ReferencedDirectly
                            : ast_1.BindingPipeType.ReferencedByName;
                }
                else {
                    type = ast_1.BindingPipeType.ReferencedByName;
                }
                result = new ast_1.BindingPipe(this.span(start), this.sourceSpan(start, fullSpanEnd), result, nameId, args, type, nameSpan);
    ...
    parseRelational() {
        ...
        while (this.next.type == lexer_1.TokenType.Operator || this.next.isKeywordIn) {
            const operator = this.next.strValue;
            switch (operator) {
                case '<':
                case '>':
    ...
    parseExponentiation() {
        ...
        while (this.next.type == lexer_1.TokenType.Operator && this.next.strValue === '**') {
            ...
            if (result instanceof ast_1.Unary ||
                result instanceof ast_1.PrefixNot ||
                result instanceof ast_1.TypeofExpression ||
                result instanceof ast_1.VoidExpression) {
    ...
    parsePrefix() {
        if (this.next.type == lexer_1.TokenType.Operator) {
            ...
            const start = this.inputIndex;
            let result = this.parsePrefix();
            return new ast_1.TypeofExpression(this.span(start), this.sourceSpan(start), result);
        }
        else if (this.next.isKeywordVoid()) {
            this.advance();
            const start = this.inputIndex;
            let result = this.parsePrefix();
            return new ast_1.VoidExpression(this.span(start), this.sourceSpan(start), result);
        }
    ...
    parsePrimary() {
        ...
        if (this.consumeOptionalCharacter(chars.$LPAREN)) {
            ...
            return new ast_1.ParenthesizedExpression(this.span(start), this.sourceSpan(start), result);
        }
            ...
            this.advance();
            return new ast_1.LiteralPrimitive(this.span(start), this.sourceSpan(start), 'in');
        }
        ...
        else if (this.next.isString() && this.next.kind === lexer_1.StringTokenKind.Plain) {
    ...
    parseAccessMember(readReceiver, start, isSafe) {
        ...
        if (isSafe) {
            ...
            if (this.consumeOptionalOperator('=')) {
                ...
                receiver = new ast_1.PropertyRead(this.span(start), this.sourceSpan(start), nameSpan, readReceiver, id);
            }
        }
        return receiver;
    }
    parseCall(receiver, start, isSafe) {
        ...
        return isSafe
            ? new ast_1.SafeCall(span, sourceSpan, receiver, args, argumentSpan)
        ...
        return this.withContext(ParseContextFlags.Writable, () => {
            ...
            if (this.consumeOptionalOperator('=')) {
                if (isSafe) {
                    ...
                    const value = this.parseConditional();
                    return new ast_1.KeyedWrite(this.span(start), this.sourceSpan(start), receiver, key, value);
                }
            }
            else {
                return isSafe
                    ? new ast_1.SafeKeyedRead(this.span(start), this.sourceSpan(start), receiver, key)
                    : new ast_1.KeyedRead(this.span(start), this.sourceSpan(start), receiver, key);
            }
            return new ast_1.EmptyExpr(this.span(start), this.sourceSpan(start));
        });
    }
    ...
    parseDirectiveKeywordBindings(key) {
        ...
        const sourceSpan = new ast_1.AbsoluteSourceSpan(key.span.start, spanEnd);
    ...
    parseAsBinding(value) {
        ...
        const key = this.expectTemplateBindingKey();
        this.consumeStatementTerminator();
        const sourceSpan = new ast_1.AbsoluteSourceSpan(value.span.start, this.currentAbsoluteOffset);
    ...
    parseLetBinding() {
        ...
        this.consumeStatementTerminator();
        const sourceSpan = new ast_1.AbsoluteSourceSpan(spanStart, this.currentAbsoluteOffset);
        return new ast_1.VariableBinding(sourceSpan, key, value);
    }
    parseNoInterpolationTaggedTemplateLiteral(tag, start) {
        const template = this.parseNoInterpolationTemplateLiteral();
        return new ast_1.TaggedTemplateLiteral(this.span(start), this.sourceSpan(start), tag, template);
    }
    parseNoInterpolationTemplateLiteral() {
        ...
        const span = this.span(start);
        const sourceSpan = this.sourceSpan(start);
        return new ast_1.TemplateLiteral(span, sourceSpan, [new ast_1.TemplateLiteralElement(span, sourceSpan, text)], []);
    }
    ...
    parseTemplateLiteral() {
        ...
        const expressions = [];
        const start = this.inputIndex;
        while (this.next !== lexer_1.EOF) {
            const token = this.next;
            if (token.isTemplateLiteralPart() || token.isTemplateLiteralEnd()) {
                const partStart = this.inputIndex;
                this.advance();
                elements.push(new ast_1.TemplateLiteralElement(this.span(partStart), this.sourceSpan(partStart), token.strValue));
                ...
                if (expression instanceof ast_1.EmptyExpr) {
        ...
        return new ast_1.TemplateLiteral(this.span(start), this.sourceSpan(start), elements, expressions);
    }
    /**
...
class SimpleExpressionChecker extends ast_1.RecursiveAstVisitor {
    constructor() {
        super(...arguments);
        this.errors = [];
    }
