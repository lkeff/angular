...
function orderOps(job) {
    for (const unit of job.units) {
        ...
        // Update mode:
        const ordering = unit.job.kind === compilation_1.CompilationJobKind.Host ? UPDATE_HOST_ORDERING : UPDATE_ORDERING;
...
function orderWithin(opList, ordering) {
    ...
    for (const op of opList) {
        ...
        if (handledOpKinds.has(op.kind)) {
            opsToOrder.push(op);
            ir.OpList.remove(op);
            firstTargetInGroup = currentTarget !== null && currentTarget !== void 0 ? currentTarget : firstTargetInGroup;
        }
    }
