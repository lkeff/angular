...
const style_url_resolver_1 = require("../style_url_resolver");
const ingest_1 = require("../template/pipeline/src/ingest");
const template_preparser_1 = require("../template_parser/template_preparser");
const t = __importStar(require("./r3_ast"));
const r3_control_flow_1 = require("./r3_control_flow");
const r3_deferred_blocks_1 = require("./r3_deferred_blocks");
const util_1 = require("./view/i18n/util");
...
class HtmlAstToIvyAst {
    ...
    visitElement(element) {
        const isI18nRootElement = (0, ingest_1.isI18nRootNode)(element.i18n);
        ...
        const preparsedElement = (0, template_preparser_1.preparseElement)(element);
        ...
        const isTemplateElement = (0, tags_1.isNgTemplate)(element.name);
        ...
        if (preparsedElement.type === template_preparser_1.PreparsedElementType.NG_CONTENT) {
            ...
            parsedElement = new t.Content(selector, attrs, children, element.sourceSpan, element.startSourceSpan, element.endSourceSpan, element.i18n);
            this.ngContentSelectors.push(selector);
        }
        else if (isTemplateElement) {
            // `<ng-template>`
            const attrs = this.categorizePropertyAttributes(element.name, parsedProperties, i18nAttrsMeta);
            parsedElement = new t.Template(element.name, attributes, attrs.bound, boundEvents, directives, [
            /* no template attributes */
            ], children, references, variables, element.sourceSpan, element.startSourceSpan, element.endSourceSpan, element.i18n);
        }
        else {
            const attrs = this.categorizePropertyAttributes(element.name, parsedProperties, i18nAttrsMeta);
            parsedElement = new t.Element(element.name, attributes, attrs.bound, boundEvents, directives, children, references, element.sourceSpan, element.startSourceSpan, element.endSourceSpan, element.i18n);
        }
    ...
    visitExpansion(expansion) {
        ...
        if (!(0, ingest_1.isI18nRootNode)(expansion.i18n)) {
    ...
    visitLetDeclaration(decl, context) {
        ...
        if (value.errors.length === 0 && value.ast instanceof ast_1.EmptyExpr) {
    ...
    visitComponent(component) {
        const isI18nRootElement = (0, ingest_1.isI18nRootNode)(component.i18n);
        if (isI18nRootElement) {
            if (this.inI18nBlock) {
        ...
        let node = new t.Component(component.componentName, component.tagName, component.fullName, attributes, attrs.bound, boundEvents, directives, children, references, component.sourceSpan, component.startSourceSpan, component.endSourceSpan, component.i18n);
        if (elementHasInlineTemplate) {
    ...
    visitBlock(block, context) {
        ...
        switch (block.name) {
            case 'defer':
                result = (0, r3_deferred_blocks_1.createDeferredBlock)(block, this.findConnectedBlocks(index, context, r3_deferred_blocks_1.isConnectedDeferLoopBlock), this, this.bindingParser);
                break;
            case 'switch':
                result = (0, r3_control_flow_1.createSwitchBlock)(block, this, this.bindingParser);
                break;
            case 'for':
                result = (0, r3_control_flow_1.createForLoop)(block, this.findConnectedBlocks(index, context, r3_control_flow_1.isConnectedForLoopBlock), this, this.bindingParser);
                break;
            case 'if':
                result = (0, r3_control_flow_1.createIfBlock)(block, this.findConnectedBlocks(index, context, r3_control_flow_1.isConnectedIfLoopBlock), this, this.bindingParser);
                break;
            default:
                let errorMessage;
                if ((0, r3_deferred_blocks_1.isConnectedDeferLoopBlock)(block.name)) {
                    errorMessage = `@${block.name} block can only be used after an @defer block.`;
                    this.processedNodes.add(block);
                }
                else if ((0, r3_control_flow_1.isConnectedForLoopBlock)(block.name)) {
                    errorMessage = `@${block.name} block can only be used after an @for block.`;
                    this.processedNodes.add(block);
                }
                else if ((0, r3_control_flow_1.isConnectedIfLoopBlock)(block.name)) {
                ...
                result = {
                    ...
                    errors: [new parse_util_1.ParseError(block.sourceSpan, errorMessage)],
                };
                break;
        }
    ...
    extractDirectives(node) {
        ...
        for (const directive of node.directives) {
            ...
            for (const input of inputs) {
                if (input.type !== ast_1.BindingType.Property && input.type !== ast_1.BindingType.TwoWay) {
                    invalid = true;
            ...
            directives.push(new t.Directive(directive.name, attributes, inputs, boundEvents, references, directive.sourceSpan, directive.startSourceSpan, directive.endSourceSpan, undefined));
        }
        return directives;
    }
    wrapInTemplate(node, templateProperties, templateVariables, i18nAttrsMeta, isTemplateElement, isI18nRootElement) {
        ...
        return new t.Template(name, hoistedAttrs.attributes, hoistedAttrs.inputs, hoistedAttrs.outputs, [
    ...
    _visitTextWithInterpolation(value, sourceSpan, interpolatedTokens, i18n) {
        const valueNoNgsp = (0, html_whitespaces_1.replaceNgsp)(value);
    ...
    reportError(message, sourceSpan, level = parse_util_1.ParseErrorLevel.ERROR) {
        this.errors.push(new parse_util_1.ParseError(sourceSpan, message, level));
    }
}
class NonBindableVisitor {
    visitElement(ast) {
        const preparsedElement = (0, template_preparser_1.preparseElement)(ast);
        if (preparsedElement.type === template_preparser_1.PreparsedElementType.SCRIPT ||
    ...
    visitComponent(ast, context) {
        const children = html.visitAll(this, ast.children, null);
        return new t.Element(ast.fullName, html.visitAll(this, ast.attrs), 
        /* inputs */ [], 
        /* outputs */ [], 
        /* directives */ [], children, 
        /* references */ [], ast.sourceSpan, ast.startSourceSpan, ast.endSourceSpan);
