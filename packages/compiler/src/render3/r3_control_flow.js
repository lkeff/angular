...
exports.isConnectedForLoopBlock = isConnectedForLoopBlock;
exports.isConnectedIfLoopBlock = isConnectedIfLoopBlock;
...
function createIfBlock(ast, connectedBlocks, visitor, bindingParser) {
    ...
    if (lastBranch !== undefined) {
        wholeSourceSpan = new parse_util_1.ParseSourceSpan(ifBlockStartSourceSpan.start, lastBranch.sourceSpan.end);
    }
    return {
...
function createForLoop(ast, connectedBlocks, visitor, bindingParser) {
    ...
    for (const block of connectedBlocks) {
        if (block.name === 'empty') {
            ...
            errors.push(new parse_util_1.ParseError(block.sourceSpan, `Unrecognized @for loop block "${block.name}"`));
        }
    }
    if (params !== null) {
        if (params.trackBy === null) {
            ...
            // service.
            errors.push(new parse_util_1.ParseError(ast.startSourceSpan, '@for loop must have a "track" expression'));
        }
        else {
            // The `for` block has a main span that includes the `empty` branch. For only the span of the
            // main `for` body, use `mainSourceSpan`.
            const endSpan = (_a = empty === null || empty === void 0 ? void 0 : empty.endSourceSpan) !== null && _a !== void 0 ? _a : ast.endSourceSpan;
            const sourceSpan = new parse_util_1.ParseSourceSpan(ast.sourceSpan.start, (_b = endSpan === null || endSpan === void 0 ? void 0 : endSpan.end) !== null && _b !== void 0 ? _b : ast.sourceSpan.end);
...
function parseForLoopParameters(block, errors, bindingParser) {
    ...
    if (block.parameters.length === 0) {
        errors.push(new parse_util_1.ParseError(block.startSourceSpan, '@for loop does not have an expression'));
        return null;
    }
    const [expressionParam, ...secondaryParams] = block.parameters;
    const match = (_a = stripOptionalParentheses(expressionParam, errors)) === null || _a === void 0 ? void 0 : _a.match(FOR_LOOP_EXPRESSION_PATTERN);
    ...
    if (ALLOWED_FOR_LOOP_LET_VARIABLES.has(itemName)) {
        errors.push(new parse_util_1.ParseError(expressionParam.sourceSpan, `@for loop item name cannot be one of ${Array.from(ALLOWED_FOR_LOOP_LET_VARIABLES).join(', ')}.`));
    }
    ...
    const variableSpan = new parse_util_1.ParseSourceSpan(expressionParam.sourceSpan.start, expressionParam.sourceSpan.start.moveBy(variableName.length));
    const result = {
        ...
        context: Array.from(ALLOWED_FOR_LOOP_LET_VARIABLES, (variableName) => {
            ...
            const emptySpanAfterForBlockStart = new parse_util_1.ParseSourceSpan(block.startSourceSpan.end, block.startSourceSpan.end);
    ...
    for (const param of secondaryParams) {
        ...
        if (letMatch !== null) {
            const variablesSpan = new parse_util_1.ParseSourceSpan(param.sourceSpan.start.moveBy(letMatch[0].length - letMatch[1].length), param.sourceSpan.end);
        ...
        if (trackMatch !== null) {
            if (result.trackBy !== null) {
                ...
                if (expression.ast instanceof ast_1.EmptyExpr) {
                    errors.push(new parse_util_1.ParseError(block.startSourceSpan, '@for loop must have a "track" expression'));
                }
                const keywordSpan = new parse_util_1.ParseSourceSpan(param.sourceSpan.start, param.sourceSpan.start.moveBy('track'.length));
                result.trackBy = { expression, keywordSpan };
            }
            continue;
        }
        errors.push(new parse_util_1.ParseError(param.sourceSpan, `Unrecognized @for loop parameter "${param.expression}"`));
    }
    return result;
}
...
function parseLetParameter(sourceSpan, expression, span, loopItemName, context, errors) {
    ...
    for (const part of parts) {
        ...
        if (name.length === 0 || variableName.length === 0) {
            ...
            errors.push(new parse_util_1.ParseError(sourceSpan, `Unknown "let" parameter variable "${variableName}". The allowed variables are: ${Array.from(ALLOWED_FOR_LOOP_LET_VARIABLES).join(', ')}`));
        }
        else if (name === loopItemName) {
            errors.push(new parse_util_1.ParseError(sourceSpan, `Invalid @for loop "let" parameter. Variable cannot be called "${loopItemName}"`));
        }
        else if (context.some((v) => v.name === name)) {
            errors.push(new parse_util_1.ParseError(sourceSpan, `Duplicate "let" parameter variable "${variableName}"`));
        }
        else {
            const [, keyLeadingWhitespace, keyName] = (_a = expressionParts[0].match(CHARACTERS_IN_SURROUNDING_WHITESPACE_PATTERN)) !== null && _a !== void 0 ? _a : [];
            const keySpan = keyLeadingWhitespace !== undefined && expressionParts.length === 2
                ? new parse_util_1.ParseSourceSpan(
            ...
            if (expressionParts.length === 2) {
                const [, valueLeadingWhitespace, implicit] = (_b = expressionParts[1].match(CHARACTERS_IN_SURROUNDING_WHITESPACE_PATTERN)) !== null && _b !== void 0 ? _b : [];
                valueSpan =
                    valueLeadingWhitespace !== undefined
                        ? new parse_util_1.ParseSourceSpan(startSpan.moveBy(expressionParts[0].length + 1 + valueLeadingWhitespace.length), startSpan.moveBy(expressionParts[0].length + 1 + valueLeadingWhitespace.length + implicit.length))
                        : undefined;
            }
            const sourceSpan = new parse_util_1.ParseSourceSpan(keySpan.start, (_c = valueSpan === null || valueSpan === void 0 ? void 0 : valueSpan.end) !== null && _c !== void 0 ? _c : keySpan.end);
...
function validateIfConnectedBlocks(connectedBlocks) {
    ...
    for (let i = 0; i < connectedBlocks.length; i++) {
        ...
        if (block.name === 'else') {
            if (hasElse) {
                errors.push(new parse_util_1.ParseError(block.startSourceSpan, 'Conditional can only have one @else block'));
            }
            else if (connectedBlocks.length > 1 && i < connectedBlocks.length - 1) {
                errors.push(new parse_util_1.ParseError(block.startSourceSpan, '@else block must be last inside the conditional'));
            }
            else if (block.parameters.length > 0) {
                errors.push(new parse_util_1.ParseError(block.startSourceSpan, '@else block cannot have parameters'));
            }
            hasElse = true;
        }
        else if (!ELSE_IF_PATTERN.test(block.name)) {
            errors.push(new parse_util_1.ParseError(block.startSourceSpan, `Unrecognized conditional block @${block.name}`));
        }
    }
    return errors;
}
...
function validateSwitchBlock(ast) {
    ...
    if (ast.parameters.length !== 1) {
        errors.push(new parse_util_1.ParseError(ast.startSourceSpan, '@switch block must have exactly one parameter'));
        return errors;
    }
    for (const node of ast.children) {
        ...
        if (!(node instanceof html.Block) || (node.name !== 'case' && node.name !== 'default')) {
            errors.push(new parse_util_1.ParseError(node.sourceSpan, '@switch block can only contain @case and @default blocks'));
            continue;
        }
        if (node.name === 'default') {
            if (hasDefault) {
                errors.push(new parse_util_1.ParseError(node.startSourceSpan, '@switch block can only have one @default block'));
            }
            else if (node.parameters.length > 0) {
                errors.push(new parse_util_1.ParseError(node.startSourceSpan, '@default block cannot have parameters'));
            }
            hasDefault = true;
        }
        else if (node.name === 'case' && node.parameters.length !== 1) {
            errors.push(new parse_util_1.ParseError(node.startSourceSpan, '@case block must have exactly one parameter'));
        }
    }
    return errors;
}
...
function parseConditionalBlockParameters(block, errors, bindingParser) {
    if (block.parameters.length === 0) {
        errors.push(new parse_util_1.ParseError(block.startSourceSpan, 'Conditional block does not have an expression'));
        return null;
    }
    ...
    for (let i = 1; i < block.parameters.length; i++) {
        ...
        if (aliasMatch === null) {
            errors.push(new parse_util_1.ParseError(param.sourceSpan, `Unrecognized conditional parameter "${param.expression}"`));
        }
            ...
            if (IDENTIFIER_PATTERN.test(name)) {
                ...
                const variableSpan = new parse_util_1.ParseSourceSpan(variableStart, variableStart.moveBy(name.length));
                expressionAlias = new t.Variable(name, name, variableSpan, variableSpan);
            }
            else {
                errors.push(new parse_util_1.ParseError(param.sourceSpan, '"as" expression must be a valid JavaScript identifier'));
            }
        }
    }
