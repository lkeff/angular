...
exports.R3TargetBinder = void 0;
exports.findMatchingDirectivesAndPipes = findMatchingDirectivesAndPipes;
...
const combined_visitor_1 = require("../../combined_visitor");
/**
...
function findMatchingDirectivesAndPipes(template, directiveSelectors) {
    ...
    for (const selector of directiveSelectors) {
        ...
        matcher.addSelectables(directive_matching_1.CssSelector.parse(selector), [fakeDirective]);
    }
    const parsedTemplate = (0, template_1.parseTemplate)(template, '' /* templateUrl */);
...
class Scope {
    constructor(parentScope, rootNode) {
         ...
         */
        this.elementLikeInScope = new Set();
        /**
         * Child `Scope`s for immediately nested `ScopedNode`s.
         */
        this.childScopes = new Map();
        this.isDeferred =
            parentScope !== null && parentScope.isDeferred ? true : rootNode instanceof r3_ast_1.DeferredBlock;
    }
    static newRootScope() {
     ...
     */
    ingest(nodeOrNodes) {
        if (nodeOrNodes instanceof r3_ast_1.Template) {
            // Variables on an <ng-template> are defined in the inner scope.
            nodeOrNodes.variables.forEach((node) => this.visitVariable(node));
            // Process the nodes of the template.
            nodeOrNodes.children.forEach((node) => node.visit(this));
        }
        else if (nodeOrNodes instanceof r3_ast_1.IfBlockBranch) {
            if (nodeOrNodes.expressionAlias !== null) {
                this.visitVariable(nodeOrNodes.expressionAlias);
            }
            nodeOrNodes.children.forEach((node) => node.visit(this));
        }
        else if (nodeOrNodes instanceof r3_ast_1.ForLoopBlock) {
            this.visitVariable(nodeOrNodes.item);
            nodeOrNodes.contextVariables.forEach((v) => this.visitVariable(v));
            nodeOrNodes.children.forEach((node) => node.visit(this));
        }
        else if (nodeOrNodes instanceof r3_ast_1.SwitchBlockCase ||
            nodeOrNodes instanceof r3_ast_1.ForLoopBlockEmpty ||
            nodeOrNodes instanceof r3_ast_1.DeferredBlock ||
            nodeOrNodes instanceof r3_ast_1.DeferredBlockError ||
            nodeOrNodes instanceof r3_ast_1.DeferredBlockPlaceholder ||
            nodeOrNodes instanceof r3_ast_1.DeferredBlockLoading ||
            nodeOrNodes instanceof r3_ast_1.Content) {
            nodeOrNodes.children.forEach((node) => node.visit(this));
        }
        else if (!(nodeOrNodes instanceof r3_ast_1.HostElement)) {
...
class DirectiveBinder {
    ...
    visitComponent(node) {
        if (this.directiveMatcher instanceof directive_matching_1.SelectorlessMatcher) {
    ...
    visitElementOrTemplate(node) {
        if (this.directiveMatcher instanceof directive_matching_1.SelectorMatcher) {
            const directives = [];
            const cssSelector = (0, util_1.createCssSelectorFromNode)(node);
 ...
 */
class TemplateBinder extends combined_visitor_1.CombinedRecursiveAstVisitor {
    ...
    static applyWithScope(nodeOrNodes, scope, expressions, symbols, nestingLevel, usedPipes, eagerPipes, deferBlocks) {
        const template = nodeOrNodes instanceof r3_ast_1.Template ? nodeOrNodes : null;
    ...
    ingest(nodeOrNodes) {
        ...
        else if (nodeOrNodes instanceof r3_ast_1.DeferredBlock) {
    ...
    visitDeferredBlock(deferred) {
        var _a, _b, _c, _d;
        this.ingestScopedNode(deferred);
        (_a = deferred.triggers.when) === null || _a === void 0 ? void 0 : _a.value.visit(this);
        (_b = deferred.prefetchTriggers.when) === null || _b === void 0 ? void 0 : _b.value.visit(this);
        (_c = deferred.hydrateTriggers.when) === null || _c === void 0 ? void 0 : _c.value.visit(this);
        (_d = deferred.hydrateTriggers.never) === null || _d === void 0 ? void 0 : _d.visit(this);
    ...
    maybeMap(ast, name) {
        ...
        if (!(ast.receiver instanceof ast_1.ImplicitReceiver) || ast.receiver instanceof ast_1.ThisReceiver) {
            return;
        }
...
class R3BoundTarget {
    ...
    getEntitiesInScope(node) {
        var _a;
        return (_a = this.scopedNodeEntities.get(node)) !== null && _a !== void 0 ? _a : new Set();
    }
    ...
    getDeferredTriggerTarget(block, trigger) {
        ...
        if (!(trigger instanceof r3_ast_1.InteractionDeferredTrigger) &&
            !(trigger instanceof r3_ast_1.ViewportDeferredTrigger) &&
            !(trigger instanceof r3_ast_1.HoverDeferredTrigger)) {
            return null;
        }
        ...
        if (name === null) {
            ...
            if (block.placeholder !== null) {
                for (const child of block.placeholder.children) {
                    ...
                    if (child instanceof r3_ast_1.Comment) {
                        continue;
                    }
                    ...
                    if (child instanceof r3_ast_1.Element) {
                        trigger = child;
                    }
                }
            }
        ...
        if (outsideRef instanceof r3_ast_1.Reference && this.getDefinitionNodeOfSymbol(outsideRef) !== block) {
        ...
        if (block.placeholder !== null) {
            ...
            const targetInPlaceholder = refInPlaceholder instanceof r3_ast_1.Reference ? this.getReferenceTarget(refInPlaceholder) : null;
    ...
    referenceTargetToElement(target) {
        if (target instanceof r3_ast_1.Element) {
            return target;
        }
        if (target instanceof r3_ast_1.Template ||
            target.node instanceof r3_ast_1.Component ||
            target.node instanceof r3_ast_1.Directive) {
            return null;
        }
