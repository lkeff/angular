...
function compileDeclareDirectiveFromMetadata(meta) {
    ...
    const expression = o.importExpr(r3_identifiers_1.Identifiers.declareDirective).callFn([definitionMap.toLiteralMap()]);
    const type = (0, compiler_1.createDirectiveType)(meta);
...
function compileQuery(query) {
    const meta = new util_2.DefinitionMap();
    meta.set('propertyName', o.literal(query.propertyName));
    if (query.first) {
        meta.set('first', o.literal(true));
    }
    meta.set('predicate', Array.isArray(query.predicate)
        ? (0, util_2.asLiteral)(query.predicate)
...
function compileHostMetadata(meta) {
    const hostMetadata = new util_2.DefinitionMap();
    hostMetadata.set('attributes', (0, util_3.toOptionalLiteralMap)(meta.attributes, (expression) => expression));
        ...
        const keys = [
            {
                ...
                value: current.isForwardReference
                    ? (0, util_1.generateForwardRef)(current.directive.type)
                    : current.directive.type,
                quoted: false,
            },
        ];
        const inputsLiteral = current.inputs ? (0, compiler_1.createHostDirectivesMappingArray)(current.inputs) : null;
        const outputsLiteral = current.outputs
            ? (0, compiler_1.createHostDirectivesMappingArray)(current.outputs)
            : null;
        if (inputsLiteral) {
...
function createInputsPartialMetadata(inputs) {
    ...
    return o.literalMap(keys.map((declaredName) => {
        ...
        return {
            ...
            quoted: util_2.UNSAFE_OBJECT_KEY_NAME_REGEXP.test(declaredName),
            value: o.literalMap([
                { key: 'classPropertyName', quoted: false, value: (0, util_2.asLiteral)(value.classPropertyName) },
                { key: 'publicName', quoted: false, value: (0, util_2.asLiteral)(value.bindingPropertyName) },
                { key: 'isSignal', quoted: false, value: (0, util_2.asLiteral)(value.isSignal) },
                { key: 'isRequired', quoted: false, value: (0, util_2.asLiteral)(value.required) },
                { key: 'transformFunction', quoted: false, value: (_a = value.transformFunction) !== null && _a !== void 0 ? _a : o.NULL_EXPR },
            ]),
        };
    }));
}
...
function legacyInputsPartialMetadata(inputs) {
    ...
    return o.literalMap(keys.map((declaredName) => {
        ...
        if (differentDeclaringName || value.transformFunction !== null) {
            const values = [(0, util_2.asLiteral)(publicName), (0, util_2.asLiteral)(declaredName)];
            ...
            result = (0, util_2.asLiteral)(publicName);
        }
        return {
            key: declaredName,
            // put quotes around keys that contain potentially unsafe characters
