...
class Parser {
    ...
    parse(source, url, options) {
        const tokenizeResult = (0, lexer_1.tokenize)(source, url, this.getTagDefinition, options);
...
class _TreeBuilder {
    ...
    _consumeExpansion(token) {
        ...
        const sourceSpan = new parse_util_1.ParseSourceSpan(token.sourceSpan.start, this._peek.sourceSpan.end, token.sourceSpan.fullStart);
    ...
    _parseExpansionCase() {
        ...
        const sourceSpan = new parse_util_1.ParseSourceSpan(value.sourceSpan.start, end.sourceSpan.end, value.sourceSpan.fullStart);
        const expSourceSpan = new parse_util_1.ParseSourceSpan(start.sourceSpan.start, end.sourceSpan.end, start.sourceSpan.fullStart);
    ...
    _consumeText(token) {
        ...
        if (text.length > 0 && text[0] === '\n') {
            ...
            if (parent != null &&
                ...
                ((_a = this._getTagDefinition(parent)) === null || _a === void 0 ? void 0 : _a.ignoreFirstLf)) {
    ...
    _closeVoidElement() {
        ...
        if (el !== null && ((_a = this._getTagDefinition(el)) === null || _a === void 0 ? void 0 : _a.isVoid)) {
    ...
    _consumeElementStartTag(startTagToken) {
        ...
        if (this._peek.type === 2 /* TokenType.TAG_OPEN_END_VOID */) {
            ...
            if (!((tagDef === null || tagDef === void 0 ? void 0 : tagDef.canSelfClose) || (0, tags_1.getNsPrefix)(fullName) !== null || (tagDef === null || tagDef === void 0 ? void 0 : tagDef.isVoid))) {
        ...
        const span = new parse_util_1.ParseSourceSpan(startTagToken.sourceSpan.start, end, startTagToken.sourceSpan.fullStart);
        // Create a separate `startSpan` because `span` will be modified when there is an `end` span.
        const startSpan = new parse_util_1.ParseSourceSpan(startTagToken.sourceSpan.start, end, startTagToken.sourceSpan.fullStart);
        const el = new html.Element(fullName, attrs, directives, [], span, startSpan, undefined);
        const parent = this._getContainer();
        const isClosedByChild = parent !== null && !!((_a = this._getTagDefinition(parent)) === null || _a === void 0 ? void 0 : _a.isClosedByChild(el.name));
    ...
    _consumeComponentStartTag(startToken) {
        ...
        this._advance();
        const end = this._peek.sourceSpan.fullStart;
        const span = new parse_util_1.ParseSourceSpan(startToken.sourceSpan.start, end, startToken.sourceSpan.fullStart);
        const startSpan = new parse_util_1.ParseSourceSpan(startToken.sourceSpan.start, end, startToken.sourceSpan.fullStart);
        ...
        const isClosedByChild = parent !== null &&
            node.tagName !== null &&
            !!((_a = this._getTagDefinition(parent)) === null || _a === void 0 ? void 0 : _a.isClosedByChild(node.tagName));
    ...
    _consumeElementEndTag(endTagToken) {
        ...
        if ((_a = this._getTagDefinition(fullName)) === null || _a === void 0 ? void 0 : _a.isVoid) {
    ...
    _popContainer(expectedName, expectedType, endSourceSpan) {
        ...
        for (let stackIndex = this._containerStack.length - 1; stackIndex >= 0; stackIndex--) {
            ...
            if (node instanceof html.Block || !((_a = this._getTagDefinition(node)) === null || _a === void 0 ? void 0 : _a.closedByParent)) {
    ...
    _consumeAttr(attrName) {
        const fullName = (0, tags_1.mergeNsAndName)(attrName.parts[0], attrName.parts[1]);
    ...
    _consumeDirective(nameToken) {
        ...
        const startSourceSpan = new parse_util_1.ParseSourceSpan(nameToken.sourceSpan.start, startSourceSpanEnd, nameToken.sourceSpan.fullStart);
        const sourceSpan = new parse_util_1.ParseSourceSpan(startSourceSpan.start, endSourceSpan === null ? nameToken.sourceSpan.end : endSourceSpan.end, startSourceSpan.fullStart);
    ...
    _consumeBlockOpen(token) {
        ...
        const end = this._peek.sourceSpan.fullStart;
        const span = new parse_util_1.ParseSourceSpan(token.sourceSpan.start, end, token.sourceSpan.fullStart);
        // Create a separate `startSpan` because `span` will be modified when there is an `end` span.
        const startSpan = new parse_util_1.ParseSourceSpan(token.sourceSpan.start, end, token.sourceSpan.fullStart);
    ...
    _consumeLet(startToken) {
        ...
        const nameSpan = new parse_util_1.ParseSourceSpan(nameStart, startToken.sourceSpan.end);
    ...
    _consumeIncompleteLet(token) {
        ...
        const name = (_a = token.parts[0]) !== null && _a !== void 0 ? _a : '';
        const nameString = name ? ` "${name}"` : '';
        // If there's at least a name, we can salvage an AST node that can be used for completions.
        if (name.length > 0) {
            const startOffset = token.sourceSpan.toString().lastIndexOf(name);
            const nameStart = token.sourceSpan.start.moveBy(startOffset);
            const nameSpan = new parse_util_1.ParseSourceSpan(nameStart, token.sourceSpan.end);
            const valueSpan = new parse_util_1.ParseSourceSpan(token.sourceSpan.start, token.sourceSpan.start.moveBy(0));
    ...
    _getElementFullName(token, parent) {
        ...
        return (0, tags_1.mergeNsAndName)(prefix, token.parts[1]);
    }
    ...
    _getComponentTagName(token, parent) {
        ...
        if (!prefix && !tagName) {
            ...
            return (0, tags_1.mergeNsAndName)(prefix, tagName || 'ng-component');
        }
    }
    _getPrefix(token, parent) {
        ...
        prefix = prefix || ((_a = this._getTagDefinition(tagName)) === null || _a === void 0 ? void 0 : _a.implicitNamespacePrefix) || '';
        if (!prefix && parent) {
            const parentName = parent instanceof html.Element ? parent.name : parent.tagName;
            if (parentName !== null) {
                const parentTagName = (0, tags_1.splitNsName)(parentName)[1];
                ...
                if (parentTagDefinition !== null && !parentTagDefinition.preventNamespaceInheritance) {
                    prefix = (0, tags_1.getNsPrefix)(parentName);
                }
            }
        }
        return prefix;
