...
exports.CssRule = exports.ShadowCss = void 0;
...
class ShadowCss {
    ...
    _convertColonHostContextInSelectorPart(cssText) {
        return cssText.replace(_cssColonHostContextReGlobal, (selectorText, pseudoPrefix) => {
            ...
            while ((match = _cssColonHostContextRe.exec(selectorText))) {
                ...
                const newContextSelectors = ((_a = match[1]) !== null && _a !== void 0 ? _a : '')
                    .trim()
                    .split(',')
                    .map((m) => m.trim())
                    .filter((m) => m !== '');
    ...
    _applySelectorScope({ selector, scopeSelector, hostSelector, isParentSelector, }) {
        ...
        const _pseudoFunctionAwareScopeSelectorPart = (selectorPart) => {
            ...
            if (pseudoSelectorParts.join('') === selectorPart) {
                scopedPart = pseudoSelectorParts
                    .map((selectorPart) => {
                    var _a;
                    const [cssPseudoSelectorFunction] = (_a = selectorPart.match(_cssPrefixWithPseudoSelectorFunction)) !== null && _a !== void 0 ? _a : [];
                    // Unwrap the pseudo selector to scope its contents.
                    // For example,
                    // - `:where(selectorToScope)` -> `selectorToScope`;
                    // - `:is(.foo, .bar)` -> `.foo, .bar`.
                    const selectorToScope = selectorPart.slice(cssPseudoSelectorFunction === null || cssPseudoSelectorFunction === void 0 ? void 0 : cssPseudoSelectorFunction.length, -1);
        ...
        while ((res = sep.exec(selector)) !== null) {
            ...
            if (part.match(/__esc-ph-(\d+)__/) && ((_a = selector[res.index + 1]) === null || _a === void 0 ? void 0 : _a.match(/[a-fA-F\d]/))) {
                continue;
            }
