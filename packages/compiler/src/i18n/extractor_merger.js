...
const i18n_parser_1 = require("./i18n_parser");
...
class _Visitor {
    ...
    _init(mode, interpolationConfig) {
        ...
        this._createI18nMessage = (0, i18n_parser_1.createI18nMessageFactory)(interpolationConfig, defaults_1.DEFAULT_CONTAINER_BLOCKS, 
    ...
    _visitElementLike(node, context) {
        ...
        if (this._mode === _VisitorMode.Merge) {
            if (node instanceof html.Element) {
                return new html.Element(node.name, this._translateAttributes(node), this._translateDirectives(node), childNodes, node.sourceSpan, node.startSourceSpan, node.endSourceSpan);
            }
            else {
                return new html.Component(node.componentName, node.tagName, node.fullName, this._translateAttributes(node), this._translateDirectives(node), childNodes, node.sourceSpan, node.startSourceSpan, node.endSourceSpan);
            }
        }
        return null;
    }
    ...
    _visitAttributesOf(el) {
        ...
        const implicitAttrNames = this._implicitAttrs[el instanceof html.Component ? el.tagName || '' : el.name] || [];
        el.attrs
    ...
    _isPlaceholderOnlyAttributeValue(ast) {
        ...
        if (!isAttrNode(ast))
            return false;
        const tokens = (_a = ast[0].valueTokens) !== null && _a !== void 0 ? _a : [];
    ...
    _translateDirectives(node) {
        return node.directives.map((dir) => new html.Directive(dir.name, this._translateAttributes(dir), dir.sourceSpan, dir.startSourceSpan, dir.endSourceSpan));
    }
    /**
