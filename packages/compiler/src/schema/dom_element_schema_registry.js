...
const dom_security_schema_1 = require("./dom_security_schema");
const element_schema_registry_1 = require("./element_schema_registry");
...
class DomElementSchemaRegistry extends element_schema_registry_1.ElementSchemaRegistry {
    ...
    hasProperty(tagName, propName, schemaMetas) {
        if (schemaMetas.some((schema) => schema.name === core_1.NO_ERRORS_SCHEMA.name)) {
            return true;
        }
        if (tagName.indexOf('-') > -1) {
            ...
            if (schemaMetas.some((schema) => schema.name === core_1.CUSTOM_ELEMENTS_SCHEMA.name)) {
    ...
    securityContext(tagName, propName, isAttribute) {
        ...
        let ctx = (0, dom_security_schema_1.SECURITY_SCHEMA)()[tagName + '|' + propName];
        if (ctx) {
            return ctx;
        }
