...
describe('ShadowCss, keyframes and animations', () => {
    it('should scope keyframes rules', () => {
        ...
        expect((0, utils_1.shim)(css, 'host-a')).toEqual(expected);
    });
    it('should scope -webkit-keyframes rules', () => {
        const css = '@-webkit-keyframes foo {0% {-webkit-transform:translate(-50%) scaleX(0);}} ';
        const expected = '@-webkit-keyframes host-a_foo {0% {-webkit-transform:translate(-50%) scaleX(0);}}';
        expect((0, utils_1.shim)(css, 'host-a')).toEqual(expected);
    });
    ...
    it('should handle (scope or not) multiple animation-names defined over multiple lines', () => {
        ...
        @keyframes grault {}`;
        const result = (0, utils_1.shim)(css, 'host-a');
        ['foo', 'baz', 'quux', 'grault'].forEach((scoped) => expect(result).toContain(`host-a_${scoped}`));
    ...
    `;
        const result = (0, utils_1.shim)(css, CONTENT_ATTR, HOST_ATTR);
        const animationLineMatch = result.match(/animation:[^;]+;/);
        let animationLine = '';
        if (animationLineMatch) {
            animationLine = animationLineMatch[0];
        }
        ['my-anim', 'my-anim2'].forEach((scoped) => expect(animationLine).toContain(`_ngcontent-%COMP%_${scoped}`));
    ...
    it('should handle the usage of commas in multiple animation definitions in a single declaration', () => {
       ...
       `;
        ...
        expect(result).toContain(`animation: 3s "host-a_bar, baz", 3s 'foo, bar' 1s, 3s "host-a_qux quux";`);
    });
    it('should handle the usage of double quotes escaping in multiple animation definitions in a single declaration', () => {
        ...
        expect(result).toContain('animation: 3s "host-a_foo\\"", 3.5s "host-a_bar", 3.7s "host-a_ba\\"r";');
    ...
    it('should handle the usage of mixed single and double quotes escaping in multiple animation definitions in a single declaration', () => {
        ...
        expect(result).toContain(`animation: 3s 'host-a_foo\\\\', 3.5s "host-a_b\\\\\\"ar", 3.7s 'host-a_ba\\'\\"\\'r';`);
        expect(result).toContain(`animation: 4s 'host-a_fo\\'o', 4.5s 'host-a_b\\"ar\\"', 4.7s "baz\\'";`);
    });
    it('should handle the usage of commas inside quotes', () => {
        ...
        expect(result).toContain(`animation-name: "host-a_bar,, baz", host-a_foo,'host-a_ease, linear , inherit', bar;`);
    });
    ...
    it('should handle css functions correctly', () => {
        ...
        expect(result).toContain('animation: host-a_foo 0.5s alternate infinite cubic-bezier(.17, .67, .83, .67);');
