...
const dom_element_schema_registry_1 = require("../../src/schema/dom_element_schema_registry");
const core_1 = require("@angular/core");
const html_parser_1 = require("../../src/ml_parser/html_parser");
const schema_extractor_1 = require("./schema_extractor");
describe('DOMElementSchema', () => {
    let registry;
    beforeEach(() => {
        registry = new dom_element_schema_registry_1.DomElementSchemaRegistry();
    });
    ...
    it('should return true for custom-like elements if the CUSTOM_ELEMENTS_SCHEMA was used', () => {
        expect(registry.hasProperty('custom-like', 'unknown', [core_1.CUSTOM_ELEMENTS_SCHEMA])).toBeTruthy();
        expect(registry.hasElement('custom-like', [core_1.CUSTOM_ELEMENTS_SCHEMA])).toBeTruthy();
    });
    it('should return true for all elements if the NO_ERRORS_SCHEMA was used', () => {
        expect(registry.hasProperty('custom-like', 'unknown', [core_1.NO_ERRORS_SCHEMA])).toBeTruthy();
        expect(registry.hasProperty('a', 'unknown', [core_1.NO_ERRORS_SCHEMA])).toBeTruthy();
        expect(registry.hasElement('custom-like', [core_1.NO_ERRORS_SCHEMA])).toBeTruthy();
        expect(registry.hasElement('unknown', [core_1.NO_ERRORS_SCHEMA])).toBeTruthy();
    });
    ...
    it('should return an error message when asserting event properties', () => {
        ...
        expect(report.msg).toEqual(`Binding to event property 'onClick' is disallowed for security reasons, please use (Click)=...
If 'onClick' is a directive input, make sure the directive is imported by the current module.`);
        ...
        expect(report.error).toBeTruthy();
        expect(report.msg).toEqual(`Binding to event property 'onAnything' is disallowed for security reasons, please use (Anything)=...
If 'onAnything' is a directive input, make sure the directive is imported by the current module.`);
    ...
    it('should return an error message when asserting event attributes', () => {
        ...
        expect(report.error).toBeTruthy();
        expect(report.msg).toEqual(`Binding to event attribute 'onClick' is disallowed for security reasons, please use (Click)=...`);
        report = registry.validateAttribute('onAnything');
        expect(report.error).toBeTruthy();
        expect(report.msg).toEqual(`Binding to event attribute 'onAnything' is disallowed for security reasons, please use (Anything)=...`);
    });
    ...
    it('should return security contexts for elements', () => {
        expect(registry.securityContext('iframe', 'srcdoc', false)).toBe(core_1.SecurityContext.HTML);
        expect(registry.securityContext('p', 'innerHTML', false)).toBe(core_1.SecurityContext.HTML);
        expect(registry.securityContext('a', 'href', false)).toBe(core_1.SecurityContext.URL);
        expect(registry.securityContext('a', 'style', false)).toBe(core_1.SecurityContext.STYLE);
        expect(registry.securityContext('ins', 'cite', false)).toBe(core_1.SecurityContext.URL);
        expect(registry.securityContext('base', 'href', false)).toBe(core_1.SecurityContext.RESOURCE_URL);
    });
    it('should detect properties on namespaced elements', () => {
        const htmlAst = new html_parser_1.HtmlParser().parse('<svg:style>', 'TestComp');
        const nodeName = htmlAst.rootNodes[0].name;
        expect(registry.hasProperty(nodeName, 'type', [])).toBeTruthy();
    });
    it('should check security contexts case insensitive', () => {
        expect(registry.securityContext('p', 'iNnErHtMl', false)).toBe(core_1.SecurityContext.HTML);
        expect(registry.securityContext('p', 'formaction', false)).toBe(core_1.SecurityContext.URL);
        expect(registry.securityContext('p', 'formAction', false)).toBe(core_1.SecurityContext.URL);
    });
    it('should check security contexts for attributes', () => {
        expect(registry.securityContext('p', 'innerHtml', true)).toBe(core_1.SecurityContext.HTML);
        expect(registry.securityContext('p', 'formaction', true)).toBe(core_1.SecurityContext.URL);
    });
    ...
    if (!isNode) {
        it('generate a new schema', () => {
            let schema = '\n';
            (0, schema_extractor_1.extractSchema)().forEach((props, name) => {
    ...
    describe('normalizeAnimationStyleValue', () => {
        it('should normalize the given dimensional CSS style value to contain a PX value when numeric', () => {
            expect(registry.normalizeAnimationStyleValue('borderRadius', 'border-radius', 10)['value']).toBe('10px');
        });
        ...
        it("should retain the given dimensional CSS style value's unit if it already exists", () => {
            expect(registry.normalizeAnimationStyleValue('borderRadius', 'border-radius', '10em')['value']).toBe('10em');
        });
        it('should trim the provided CSS style value', () => {
            expect(registry.normalizeAnimationStyleValue('color', 'color', '   red ')['value']).toBe('red');
        });
