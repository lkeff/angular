...
const HTMLELEMENT_TAGS = 'abbr,address,article,aside,b,bdi,bdo,cite,content,code,dd,dfn,dt,em,figcaption,figure,footer,header,hgroup,i,kbd,main,mark,nav,noscript,rb,rp,rt,rtc,ruby,s,samp,search,section,small,strong,sub,sup,u,var,wbr';
const ALL_HTML_TAGS = 
...
const ALL_MATH_TAGS = 'math,maction,menclose,merror,mfenced,mfrac,mi,mmultiscripts,mn,mo,mover,mpadded,mphantom,mroot,mrow,ms,mspace,msqrt,mstyle,msub,msubsup,msup,mtable,mtd,mtext,mtr,munder,munderover,semantics';
// Elements missing from Chrome (HtmlUnknownElement), to be manually added
const MISSING_FROM_CHROME = {
...
const document = typeof _G['document'] == 'object' ? _G['document'] : null;
function extractSchema() {
    ...
    types.sort();
    types.forEach((type) => {
        extractRecursiveProperties(visited, descMap, window[type]);
    });
    ...
    if (typeof MathMLElement !== 'undefined') {
        ...
        ALL_MATH_TAGS.split(',').forEach((tag) => descMap.set(`${MATH_PREFIX}${tag}^${MATH_PREFIX}`, []));
    }
...
function assertNoMissingTags(descMap) {
    const extractedTags = [];
    Array.from(descMap.keys()).forEach((key) => {
...
function extractProperties(type, instance, visited, descMap, name, superName) {
    ...
    const props = descMap.has(fullName) ? descMap.get(fullName) : [];
