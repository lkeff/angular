...
describe('lexer', () => {
    describe('token', () => {
        ...
        it('should throw an invalid character error when a hash character is discovered but ' +
            ...
            expectErrorToken(lex('#')[0], 0, 1, `Lexer Error: Invalid character [#] at column 0 in expression [#]`);
            expectErrorToken(lex('#0')[0], 0, 1, `Lexer Error: Invalid character [#] at column 0 in expression [#0]`);
        });
        ...
        it('should tokenize simple quoted strings', () => {
            expectStringToken(lex('"a"')[0], 0, 3, 'a', lexer_1.StringTokenKind.Plain);
        });
        it('should tokenize quoted strings with escaped quotes', () => {
            expectStringToken(lex('"a\\""')[0], 0, 5, 'a"', lexer_1.StringTokenKind.Plain);
        });
        it('should tokenize a string', () => {
            const tokens = lex('j-a.bc[22]+1.3|f:\'a\\\'c\':"d\\"e"');
            expectIdentifierToken(tokens[0], 0, 1, 'j');
            expectOperatorToken(tokens[1], 1, 2, '-');
            ...
            expectStringToken(tokens[13], 17, 23, "a'c", lexer_1.StringTokenKind.Plain);
            expectCharacterToken(tokens[14], 23, 24, ':');
            expectStringToken(tokens[15], 24, 30, 'd"e', lexer_1.StringTokenKind.Plain);
        });
        ...
        it('should tokenize quoted string', () => {
            ...
            expectStringToken(tokens[1], 1, 5, "'", lexer_1.StringTokenKind.Plain);
            expectStringToken(tokens[3], 7, 11, '"', lexer_1.StringTokenKind.Plain);
        });
        ...
        it('should return exception for invalid exponent', () => {
            expectErrorToken(lex('0.5E-')[0], 4, 5, 'Lexer Error: Invalid exponent at column 4 in expression [0.5E-]');
            expectErrorToken(lex('0.5E-A')[0], 4, 5, 'Lexer Error: Invalid exponent at column 4 in expression [0.5E-A]');
        });
        it('should tokenize number starting with a dot', () => {
            expectNumberToken(lex('.5')[0], 0, 2, 0.5);
        });
        it('should throw error on invalid unicode', () => {
            expectErrorToken(lex("'\\u1''bla'")[0], 2, 2, "Lexer Error: Invalid unicode escape [\\u1''b] at column 2 in expression ['\\u1''bla']");
        });
        ...
        it('should tokenize number with separator', () => {
            expectNumberToken(lex('123_456')[0], 0, 7, 123456);
            expectNumberToken(lex('1_000_000_000')[0], 0, 13, 1000000000);
            expectNumberToken(lex('123_456.78')[0], 0, 10, 123456.78);
            expectNumberToken(lex('123_456_789.123_456_789')[0], 0, 23, 123456789.12345679);
            expectNumberToken(lex('1_2_3_4')[0], 0, 7, 1234);
            expectNumberToken(lex('1_2_3_4.5_6_7_8')[0], 0, 15, 1234.5678);
        });
        ...
        it('should throw error for invalid number separators', () => {
            expectErrorToken(lex('123_')[0], 3, 3, 'Lexer Error: Invalid numeric separator at column 3 in expression [123_]');
            expectErrorToken(lex('12__3')[0], 2, 2, 'Lexer Error: Invalid numeric separator at column 2 in expression [12__3]');
            expectErrorToken(lex('1_2_3_.456')[0], 5, 5, 'Lexer Error: Invalid numeric separator at column 5 in expression [1_2_3_.456]');
            expectErrorToken(lex('1_2_3._456')[0], 6, 6, 'Lexer Error: Invalid numeric separator at column 6 in expression [1_2_3._456]');
        });
        describe('template literals', () => {
            it('should tokenize template literal with no interpolations', () => {
                const tokens = lex('`hello world`');
                expect(tokens.length).toBe(1);
                expectStringToken(tokens[0], 0, 13, 'hello world', lexer_1.StringTokenKind.TemplateLiteralEnd);
            });
            it('should tokenize template literal containing strings', () => {
                expectStringToken(lex('`a "b" c`')[0], 0, 9, `a "b" c`, lexer_1.StringTokenKind.TemplateLiteralEnd);
                expectStringToken(lex("`a 'b' c`")[0], 0, 9, `a 'b' c`, lexer_1.StringTokenKind.TemplateLiteralEnd);
                expectStringToken(lex('`a \\`b\\` c`')[0], 0, 11, 'a `b` c', lexer_1.StringTokenKind.TemplateLiteralEnd);
                expectStringToken(lex('`a "\'\\`b\\`\'" c`')[0], 0, 15, `a "'\`b\`'" c`, lexer_1.StringTokenKind.TemplateLiteralEnd);
            });
            ...
            it('should tokenize template literal with an interpolation in the end', () => {
                ...
                expect(tokens.length).toBe(5);
                expectStringToken(tokens[0], 0, 7, 'hello ', lexer_1.StringTokenKind.TemplateLiteralPart);
                expectOperatorToken(tokens[1], 7, 9, '${');
                expectIdentifierToken(tokens[2], 9, 13, 'name');
                expectOperatorToken(tokens[3], 13, 14, '}');
                expectStringToken(tokens[4], 14, 15, '', lexer_1.StringTokenKind.TemplateLiteralEnd);
            });
            it('should tokenize template literal with an interpolation in the beginning', () => {
                const tokens = lex('`${name} Johnson`');
                expect(tokens.length).toBe(5);
                expectStringToken(tokens[0], 0, 1, '', lexer_1.StringTokenKind.TemplateLiteralPart);
                expectOperatorToken(tokens[1], 1, 3, '${');
                expectIdentifierToken(tokens[2], 3, 7, 'name');
                expectOperatorToken(tokens[3], 7, 8, '}');
                expectStringToken(tokens[4], 8, 17, ' Johnson', lexer_1.StringTokenKind.TemplateLiteralEnd);
            });
            it('should tokenize template literal with an interpolation in the middle', () => {
                const tokens = lex('`foo${bar}baz`');
                expect(tokens.length).toBe(5);
                expectStringToken(tokens[0], 0, 4, 'foo', lexer_1.StringTokenKind.TemplateLiteralPart);
                expectOperatorToken(tokens[1], 4, 6, '${');
                expectIdentifierToken(tokens[2], 6, 9, 'bar');
                expectOperatorToken(tokens[3], 9, 10, '}');
                expectStringToken(tokens[4], 10, 14, 'baz', lexer_1.StringTokenKind.TemplateLiteralEnd);
            });
            it('should be able to use interpolation characters inside template string', () => {
                expectStringToken(lex('`foo $`')[0], 0, 7, 'foo $', lexer_1.StringTokenKind.TemplateLiteralEnd);
                expectStringToken(lex('`foo }`')[0], 0, 7, 'foo }', lexer_1.StringTokenKind.TemplateLiteralEnd);
                expectStringToken(lex('`foo $ {}`')[0], 0, 10, 'foo $ {}', lexer_1.StringTokenKind.TemplateLiteralEnd);
                expectStringToken(lex('`foo \\${bar}`')[0], 0, 13, 'foo ${bar}', lexer_1.StringTokenKind.TemplateLiteralEnd);
            });
            it('should tokenize template literal with several interpolations', () => {
                ...
                expectStringToken(tokens[4], 5, 8, ' - ', lexer_1.StringTokenKind.TemplateLiteralPart);
                expectOperatorToken(tokens[5], 8, 10, '${');
                expectIdentifierToken(tokens[6], 10, 11, 'b');
                expectOperatorToken(tokens[7], 11, 12, '}');
                expectStringToken(tokens[8], 12, 15, ' - ', lexer_1.StringTokenKind.TemplateLiteralPart);
            ...
            it('should tokenize template literal with an object literal inside the interpolation', () => {
                const tokens = lex('`foo ${{$: true}} baz`');
                expect(tokens.length).toBe(9);
                expectStringToken(tokens[0], 0, 5, 'foo ', lexer_1.StringTokenKind.TemplateLiteralPart);
                ...
                expectStringToken(tokens[8], 17, 22, ' baz', lexer_1.StringTokenKind.TemplateLiteralEnd);
            });
            it('should tokenize template literal with template literals inside the interpolation', () => {
                const tokens = lex('`foo ${`hello ${`${a} - b`}`} baz`');
                expect(tokens.length).toBe(13);
                expectStringToken(tokens[0], 0, 5, 'foo ', lexer_1.StringTokenKind.TemplateLiteralPart);
                expectOperatorToken(tokens[1], 5, 7, '${');
                expectStringToken(tokens[2], 7, 14, 'hello ', lexer_1.StringTokenKind.TemplateLiteralPart);
                expectOperatorToken(tokens[3], 14, 16, '${');
                expectStringToken(tokens[4], 16, 17, '', lexer_1.StringTokenKind.TemplateLiteralPart);
                expectOperatorToken(tokens[5], 17, 19, '${');
                expectIdentifierToken(tokens[6], 19, 20, 'a');
                expectOperatorToken(tokens[7], 20, 21, '}');
                expectStringToken(tokens[8], 21, 26, ' - b', lexer_1.StringTokenKind.TemplateLiteralEnd);
                expectOperatorToken(tokens[9], 26, 27, '}');
                expectStringToken(tokens[10], 27, 28, '', lexer_1.StringTokenKind.TemplateLiteralEnd);
                expectOperatorToken(tokens[11], 28, 29, '}');
                expectStringToken(tokens[12], 29, 34, ' baz', lexer_1.StringTokenKind.TemplateLiteralEnd);
            });
            it('should tokenize two template literal right after each other', () => {
                const tokens = lex('`hello ${name}``see ${name} later`');
                ...
                expectOperatorToken(tokens[3], 13, 14, '}');
                expectStringToken(tokens[4], 14, 15, '', lexer_1.StringTokenKind.TemplateLiteralEnd);
                expectStringToken(tokens[5], 15, 20, 'see ', lexer_1.StringTokenKind.TemplateLiteralPart);
                expectOperatorToken(tokens[6], 20, 22, '${');
                expectIdentifierToken(tokens[7], 22, 26, 'name');
                expectOperatorToken(tokens[8], 26, 27, '}');
                expectStringToken(tokens[9], 27, 34, ' later', lexer_1.StringTokenKind.TemplateLiteralEnd);
            });
            it('should tokenize a concatenated template literal', () => {
                const tokens = lex('`hello ${name}` + 123');
            ...
            it('should tokenize a template literal with a pipe inside an interpolation', () => {
                const tokens = lex('`hello ${name | capitalize}!!!`');
                expect(tokens.length).toBe(7);
                expectStringToken(tokens[0], 0, 7, 'hello ', lexer_1.StringTokenKind.TemplateLiteralPart);
                expectOperatorToken(tokens[1], 7, 9, '${');
                expectIdentifierToken(tokens[2], 9, 13, 'name');
                ...
                expectStringToken(tokens[6], 27, 31, '!!!', lexer_1.StringTokenKind.TemplateLiteralEnd);
            });
            it('should tokenize a template literal with a pipe inside a parenthesized interpolation', () => {
                ...
                expectStringToken(tokens[8], 29, 33, '!!!', lexer_1.StringTokenKind.TemplateLiteralEnd);
            });
            it('should produce an error if a template literal is not terminated', () => {
                expectErrorToken(lex('`hello')[0], 6, 6, 'Lexer Error: Unterminated template literal at column 6 in expression [`hello]');
            });
            it('should produce an error for an unterminated template literal with an interpolation', () => {
                ...
                expectIdentifierToken(tokens[2], 9, 13, 'name');
                expectOperatorToken(tokens[3], 13, 14, '}');
                expectErrorToken(tokens[4], 15, 15, 'Lexer Error: Unterminated template literal at column 15 in expression [`hello ${name}!]');
            });
            ...
            it('should tokenize tagged template literal with no interpolations', () => {
                const tokens = lex('tag`hello world`');
                expect(tokens.length).toBe(2);
                expectIdentifierToken(tokens[0], 0, 3, 'tag');
                expectStringToken(tokens[1], 3, 16, 'hello world', lexer_1.StringTokenKind.TemplateLiteralEnd);
            });
            it('should tokenize nested tagged template literals', () => {
                const tokens = lex('tag`hello ${tag`world`}`');
                expect(tokens.length).toBe(7);
                expectIdentifierToken(tokens[0], 0, 3, 'tag');
                expectStringToken(tokens[1], 3, 10, 'hello ', lexer_1.StringTokenKind.TemplateLiteralPart);
                expectOperatorToken(tokens[2], 10, 12, '${');
                expectIdentifierToken(tokens[3], 12, 15, 'tag');
                expectStringToken(tokens[4], 15, 22, 'world', lexer_1.StringTokenKind.TemplateLiteralEnd);
                expectOperatorToken(tokens[5], 22, 23, '}');
                expectStringToken(tokens[6], 23, 24, '', lexer_1.StringTokenKind.TemplateLiteralEnd);
            });
        });
    });
});
