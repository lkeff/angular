...
const placeholder_1 = require("../../../src/i18n/serializers/placeholder");
describe('PlaceholderRegistry', () => {
    let reg;
    beforeEach(() => {
        reg = new placeholder_1.PlaceholderRegistry();
    });
    describe('tag placeholder', () => {
        ...
        it('should generate the same name for the same tag with the same attributes', () => {
            expect(reg.getStartTagPlaceholderName('p', { foo: 'a', bar: 'b' }, false)).toEqual('START_PARAGRAPH');
            expect(reg.getStartTagPlaceholderName('p', { foo: 'a', bar: 'b' }, false)).toEqual('START_PARAGRAPH');
            expect(reg.getStartTagPlaceholderName('p', { bar: 'b', foo: 'a' }, false)).toEqual('START_PARAGRAPH');
        });
        ...
        it('should be case sensitive for attributes', () => {
            expect(reg.getStartTagPlaceholderName('p', { foo: 'a', bar: 'b' }, false)).toEqual('START_PARAGRAPH');
            expect(reg.getStartTagPlaceholderName('p', { fOo: 'a', bar: 'b' }, false)).toEqual('START_PARAGRAPH_1');
            expect(reg.getStartTagPlaceholderName('p', { fOo: 'a', bAr: 'b' }, false)).toEqual('START_PARAGRAPH_2');
        });
