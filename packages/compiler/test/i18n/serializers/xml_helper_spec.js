...
const xml = __importStar(require("../../../src/i18n/serializers/xml_helper"));
describe('XML helper', () => {
    it('should serialize XML declaration', () => {
        expect(xml.serialize([new xml.Declaration({ version: '1.0' })])).toEqual('<?xml version="1.0" ?>');
    });
    ...
    it('should serialize node lists', () => {
        expect(xml.serialize([new xml.Tag('el', { order: '0' }, []), new xml.Tag('el', { order: '1' }, [])])).toEqual('<el order="0"/><el order="1"/>');
    });
    it('should escape attribute values', () => {
        expect(xml.serialize([new xml.Tag('el', { foo: '<">' }, [])])).toEqual('<el foo="&lt;&quot;&gt;"/>');
    });
});
