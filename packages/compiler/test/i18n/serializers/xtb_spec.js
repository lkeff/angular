...
const xtb_1 = require("../../../src/i18n/serializers/xtb");
describe('XTB serializer', () => {
    const serializer = new xtb_1.Xtb();
    function loadAsMap(xtb) {
        const { i18nNodesByMsgId } = serializer.load(xtb, 'url');
        const msgMap = {};
        Object.keys(i18nNodesByMsgId).forEach((id) => {
            msgMap[id] = (0, digest_1.serializeNodes)(i18nNodesByMsgId[id]).join('');
        });
        return msgMap;
    }
    describe('load', () => {
...
<translationbundle>
  ...
  <translation id="non angular">is <invalid>less</invalid> {count, plural, =0 {{GREAT}}}</translation>
            ...
            let i18nNodesByMsgId = undefined;
            expect(() => {
                i18nNodesByMsgId = serializer.load(XTB, 'url').i18nNodesByMsgId;
            }).not.toThrow();
            expect(Object.keys(i18nNodesByMsgId).length).toEqual(2);
            expect((0, digest_1.serializeNodes)(i18nNodesByMsgId['angular']).join('')).toEqual('is great');
  ...
  <translation id="1186013544048295927"><ph /></translation>
        ...
        it('should throw on unknown xtb tags', () => {
            ...
            expect(() => {
                loadAsMap(XTB);
            }).toThrowError(new RegExp((0, util_1.escapeRegExp)(`Unexpected tag ("[ERROR ->]<what></what>")`)));
        });
