...
const extractor_merger_1 = require("../../src/i18n/extractor_merger");
...
describe('Extractor', () => {
    describe('elements', () => {
        ...
        it('should extract from attributes', () => {
            expect(extract('<div i18n="m1|d1"><span i18n-title="m2|d2" title="single child">nested</span></div>')).toEqual([
                [['<ph tag name="START_TAG_SPAN">nested</ph name="CLOSE_TAG_SPAN">'], 'm1', 'd1', ''],
                [['single child'], 'm2', 'd2', ''],
            ]);
        });
        it('should extract from attributes with id', () => {
            expect(extract('<div i18n="m1|d1@@i1"><span i18n-title="m2|d2@@i2" title="single child">nested</span></div>')).toEqual([
        ...
        it('should extract from attributes without meaning and with id', () => {
            expect(extract('<div i18n="d1@@i1"><span i18n-title="d2@@i2" title="single child">nested</span></div>')).toEqual([
                [['<ph tag name="START_TAG_SPAN">nested</ph name="CLOSE_TAG_SPAN">'], '', 'd1', 'i1'],
                [['single child'], '', 'd2', 'i2'],
            ]);
        });
        it('should extract from attributes with id only', () => {
            expect(extract('<div i18n="@@i1"><span i18n-title="@@i2" title="single child">nested</span></div>')).toEqual([
                [['<ph tag name="START_TAG_SPAN">nested</ph name="CLOSE_TAG_SPAN">'], '', '', 'i1'],
                [['single child'], '', '', 'i2'],
            ]);
        });
        it('should extract from ICU messages', () => {
            expect(extract('<div i18n="m|d">{count, plural, =0 { <p i18n-title i18n-desc title="title" desc="desc"></p>}}</div>')).toEqual([
                [
    ...
    describe('blocks', () => {
        ...
        it('should handle blocks inside of translated elements', () => {
            expect(extract('<span i18n="a|b|c">@if (cond) {main content} @else {else content}</span>`')).toEqual([
                [
                    [
    ...
    describe('i18n comment blocks', () => {
        it('should extract from blocks', () => {
         ...
         <!-- i18n: @@id5 -->message5<!-- /i18n -->`)).toEqual([
        ...
        it('should extract siblings', () => {
            expect(extract(`<!-- i18n -->text<p>html<b>nested</b></p>{count, plural, =0 {<span>html</span>}}{{interp}}<!-- /i18n -->`)).toEqual([
                [
        ...
        it('should ignore other comments', () => {
            expect(extract(`<!-- i18n: meaning1|desc1@@id1 --><!-- other -->message1<!-- /i18n -->`)).toEqual([[['message1'], 'meaning1', 'desc1', 'id1']]);
        });
    ...
    describe('ICU messages', () => {
        ...
        it('should extract ICU messages from translatable block', () => {
            ...
            expect(extract('<!-- i18n:m|d -->before{count, plural, =0 {text}}after<!-- /i18n -->')).toEqual([
                [['{count, plural, =0 {[text]}}'], '', '', ''],
        ...
        it('should ignore nested ICU messages', () => {
            expect(extract('<div i18n="m|d">{count, plural, =0 { {sex, select, male {m}} }}</div>')).toEqual([[['{count, plural, =0 {[{sex, select, male {[m]}},  ]}}'], 'm', 'd', '']]);
        });
    ...
    describe('attributes', () => {
        ...
        it('should extract from attributes in translatable blocks', () => {
            expect(extract('<!-- i18n --><p><b i18n-title="m|d" title="msg"></b></p><!-- /i18n -->')).toEqual([
                [['msg'], 'm', 'd', ''],
                [
                    [
                        '<ph tag name="START_PARAGRAPH"><ph tag name="START_BOLD_TEXT"></ph' +
    ...
    describe('implicit attributes', () => {
        it('should extract implicit attributes', () => {
            expect(extract('<b title="bb">bold</b><i title="ii">italic</i>', [], { 'b': ['title'] })).toEqual([[['bb'], '', '', '']]);
        });
    });
    describe('errors', () => {
        describe('elements', () => {
...
describe('Merger', () => {
    describe('elements', () => {
        ...
        it('should merge empty messages', () => {
            ...
            const messages = (0, extractor_merger_1.extractMessages)(htmlNodes, index_1.DEFAULT_INTERPOLATION_CONFIG, [], {}, 
            /* preserveSignificantWhitespace */ true).messages;
            expect(messages.length).toEqual(1);
            const i18nMsgMap = {};
            i18nMsgMap[(0, digest_1.digest)(messages[0])] = [];
            const translations = new translation_bundle_1.TranslationBundle(i18nMsgMap, null, digest_1.digest);
            const output = (0, extractor_merger_1.mergeTranslations)(htmlNodes, translations, index_1.DEFAULT_INTERPOLATION_CONFIG, [], {});
            expect(output.errors).toEqual([]);
            expect((0, util_1.serializeNodes)(output.rootNodes).join('')).toEqual(`<div></div>`);
        });
    });
    describe('i18n comment blocks', () => {
        ...
        it('should merge blocks', () => {
            ...
            expect(fakeTranslate(HTML)).toEqual('before**[ph tag name="START_PARAGRAPH">foo[/ph name="CLOSE_PARAGRAPH">[ph tag' +
        ...
        it('should merge nested blocks', () => {
            ...
            expect(fakeTranslate(HTML)).toEqual('<div>before**[ph tag name="START_PARAGRAPH">foo[/ph name="CLOSE_PARAGRAPH">[ph' +
    ...
    describe('attributes', () => {
        ...
        it('should merge nested attributes', () => {
            ...
            expect(fakeTranslate(HTML)).toEqual('<div>{count, plural, =0 {<p title="**foo**"></p>}}</div>');
        });
        ...
        it('should merge empty attributes', () => {
            ...
            const output = (0, extractor_merger_1.mergeTranslations)(htmlNodes, translations, index_1.DEFAULT_INTERPOLATION_CONFIG, [], {});
            expect(output.errors).toEqual([]);
            expect((0, util_1.serializeNodes)(output.rootNodes).join('')).toEqual(`<div title="">some element</div>`);
        });
    });
    describe('no translations', () => {
        ...
        it('should remove nested i18n markup', () => {
            ...
            expect(fakeNoTranslate(HTML)).toEqual('<span someAttr="ok">foo</span><div>{count, plural, =0 {<p title="foo"></p>}}</div>');
        });
    });
});
...
function fakeTranslate(content, implicitTags = [], implicitAttrs = {}) {
    const htmlNodes = parseHtml(content);
    const messages = (0, extractor_merger_1.extractMessages)(htmlNodes, index_1.DEFAULT_INTERPOLATION_CONFIG, implicitTags, implicitAttrs, 
    /* preserveSignificantWhitespace */ true).messages;
    const i18nMsgMap = {};
    messages.forEach((message) => {
        const id = (0, digest_1.digest)(message);
        const text = (0, digest_1.serializeNodes)(message.nodes).join('').replace(/</g, '[');
        i18nMsgMap[id] = [new i18n.Text(`**${text}**`, null)];
    });
    const translationBundle = new translation_bundle_1.TranslationBundle(i18nMsgMap, null, digest_1.digest);
    const output = (0, extractor_merger_1.mergeTranslations)(htmlNodes, translationBundle, index_1.DEFAULT_INTERPOLATION_CONFIG, implicitTags, implicitAttrs);
    expect(output.errors).toEqual([]);
    return (0, util_1.serializeNodes)(output.rootNodes).join('');
}
function fakeNoTranslate(content, implicitTags = [], implicitAttrs = {}) {
    const htmlNodes = parseHtml(content);
    const translationBundle = new translation_bundle_1.TranslationBundle({}, null, digest_1.digest, undefined, core_1.MissingTranslationStrategy.Ignore, console);
    const output = (0, extractor_merger_1.mergeTranslations)(htmlNodes, translationBundle, index_1.DEFAULT_INTERPOLATION_CONFIG, implicitTags, implicitAttrs);
    expect(output.errors).toEqual([]);
    return (0, util_1.serializeNodes)(output.rootNodes).join('');
}
function extract(html, implicitTags = [], implicitAttrs = {}) {
    const result = (0, extractor_merger_1.extractMessages)(parseHtml(html), index_1.DEFAULT_INTERPOLATION_CONFIG, implicitTags, implicitAttrs, 
    /* preserveSignificantWhitespace */ true);
    if (result.errors.length > 0) {
        throw new Error(`unexpected errors: ${result.errors.join('\n')}`);
    }
    return result.messages.map((message) => [
        (0, digest_1.serializeNodes)(message.nodes),
        message.meaning,
        message.description,
        message.id,
    ]);
...
function extractErrors(html, implicitTags = [], implicitAttrs = {}) {
    const errors = (0, extractor_merger_1.extractMessages)(parseHtml(html), index_1.DEFAULT_INTERPOLATION_CONFIG, implicitTags, implicitAttrs, 
    /* preserveSignificantWhitespace */ true).errors;
    return errors.map((e) => [e.msg, e.span.toString()]);
}
