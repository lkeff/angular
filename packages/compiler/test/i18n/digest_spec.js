...
describe('digest', () => {
    ...
    describe('sha1', () => {
        it('should work on empty strings', () => {
            expect((0, digest_1.sha1)('')).toEqual('da39a3ee5e6b4b0d3255bfef95601890afd80709');
        });
        it('should returns the sha1 of "hello world"', () => {
            expect((0, digest_1.sha1)('abc')).toEqual('a9993e364706816aba3e25717850c26c9cd0d89d');
        });
        it('should returns the sha1 of unicode strings', () => {
            expect((0, digest_1.sha1)('你好，世界')).toEqual('3becb03b015ed48050611c8d7afe4b88f70d5a20');
        });
        it('should support arbitrary string size', () => {
            ...
            const prefix = `你好，世界`;
            let result = (0, digest_1.sha1)(prefix);
            for (let size = prefix.length; size < 5000; size += 101) {
                result = prefix + (0, digest_1.sha1)(result);
                while (result.length < size) {
                    result += result;
                }
                result = result.slice(-size);
            }
            expect((0, digest_1.sha1)(result)).toEqual('24c2dae5c1ac6f604dbe670a60290d7ce6320b45');
        });
    });
    describe('decimal fingerprint', () => {
        it('should work on well known inputs w/o meaning', () => {
            ...
            Object.keys(fixtures).forEach((msg) => {
                expect((0, digest_1.computeMsgId)(msg, '')).toEqual(fixtures[msg]);
            });
        });
        it('should work on well known inputs with meaning', () => {
            ...
            Object.keys(fixtures).forEach((id) => {
                expect((0, digest_1.computeMsgId)(fixtures[id][0], fixtures[id][1])).toEqual(id);
            });
        });
        it('should support arbitrary string size', () => {
            const prefix = `你好，世界`;
            let result = (0, digest_1.computeMsgId)(prefix, '');
            for (let size = prefix.length; size < 5000; size += 101) {
                result = prefix + (0, digest_1.computeMsgId)(result, '');
                while (result.length < size) {
                    result += result;
                }
                result = result.slice(-size);
            }
            expect((0, digest_1.computeMsgId)(result, '')).toEqual('2122606631351252558');
        });
    });
});
