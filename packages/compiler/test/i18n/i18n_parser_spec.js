...
describe('I18nParser', () => {
    ...
    describe('attributes', () => {
        ...
        it('should extract from attributes in translatable element', () => {
            expect(_humanizeMessages('<div i18n><p><b i18n-title="m|d" title="msg"></b></p></div>')).toEqual([
                [
                    [
        ...
        it('should extract from attributes in translatable block', () => {
            expect(_humanizeMessages('<!-- i18n --><p><b i18n-title="m|d" title="msg"></b></p><!-- /i18n -->')).toEqual([
                [['msg'], 'm', 'd', ''],
                [
                    [
                        '<ph tag name="START_PARAGRAPH"><ph tag name="START_BOLD_TEXT"></ph name="CLOSE_BOLD_TEXT"></ph name="CLOSE_PARAGRAPH">',
        ...
        it('should extract from attributes in translatable ICU', () => {
            expect(_humanizeMessages('<!-- i18n -->{count, plural, =0 {<p><b i18n-title="m|d" title="msg"></b></p>}}<!-- /i18n -->')).toEqual([
                [['msg'], 'm', 'd', ''],
                [
                    [
        ...
        it('should extract from attributes in non translatable ICU', () => {
            expect(_humanizeMessages('{count, plural, =0 {<p><b i18n-title="m|d" title="msg"></b></p>}}')).toEqual([[['msg'], 'm', 'd', '']]);
        });
    ...
    describe('interpolation', () => {
        ...
        it('should support named interpolation', () => {
            expect(_humanizeMessages('<div i18n="m|d">before{{ exp //i18n(ph="teSt") }}after</div>')).toEqual([
    ...
    describe('ICU messages', () => {
        ...
        it('should extract as ICU when single child of a block', () => {
            expect(_humanizeMessages('<!-- i18n:m|d -->{count, plural, =0 {zero}}<!-- /i18n -->')).toEqual([[['{count, plural, =0 {[zero]}}'], 'm', 'd', '']]);
        });
        it('should extract as ICU + ph when not single child of a block', () => {
            expect(_humanizeMessages('<!-- i18n:m|d -->b{count, plural, =0 {zero}}a<!-- /i18n -->')).toEqual([
                [['{count, plural, =0 {[zero]}}'], '', '', ''],
                [['b', '<ph icu name="ICU">{count, plural, =0 {[zero]}}</ph>', 'a'], 'm', 'd', ''],
            ]);
        });
        it('should not extract nested ICU messages', () => {
            expect(_humanizeMessages('<div i18n="m|d">b{count, plural, =0 {{sex, select, male {m}}}}a</div>')).toEqual([
                [
        ...
        it('should preserve whitespace when preserving significant whitespace', () => {
            ...
            expect(_humanizeMessages(html, 
            /* implicitTags */ undefined, 
            /* implicitAttrs */ undefined, 
            /* preserveSignificantWhitespace */ true)).toEqual([
        ...
        it('should normalize whitespace when not preserving significant whitespace', () => {
            ...
            /* implicitTags */ undefined, 
            /* implicitAttrs */ undefined, 
            /* preserveSignificantWhitespace */ false)).toEqual([
                [['{count, plural, =0 {[[, <ph name="INTERPOLATION">foo</ph>, ]]}}'], 'm', 'd', ''],
            ]);
        });
    });
    ...
    describe('implicit attributes', () => {
        it('should extract implicit attributes', () => {
            expect(_humanizeMessages('<b title="bb">bold</b><i title="ii">italic</i>', [], { 'b': ['title'] })).toEqual([[['bb'], '', '', '']]);
        });
    });
    describe('placeholders', () => {
        ...
        it('should reuse the same placeholder name for icu messages', () => {
            const html = '<div i18n="m|d">{count, plural, =0 {0}}{count, plural, =0 {0}}{count, plural, =1 {1}}</div>';
            expect(_humanizeMessages(html)).toEqual([
                [
                    [
        ...
        it('should preserve whitespace when preserving significant whitespace', () => {
            ...
            /* implicitTags */ undefined, 
            /* implicitAttrs */ undefined, 
            /* preserveSignificantWhitespace */ true)).toEqual([[['[hello , <ph name="INTERPOLATION">   foo   </ph>]'], 'm', 'd', '']]);
        });
        it('should normalize whitespace when not preserving significant whitespace', () => {
            const html = '<div i18n="m|d">hello {{   foo   }}</div>';
            expect(_humanizeMessages(html, 
            /* implicitTags */ undefined, 
            /* implicitAttrs */ undefined, 
            /* preserveSignificantWhitespace */ false)).toEqual([[['[hello , <ph name="INTERPOLATION">foo</ph>, ]'], 'm', 'd', '']]);
        });
    });
});
function _humanizeMessages(html, implicitTags = [], implicitAttrs = {}, preserveSignificantWhitespace = true) {
    return _extractMessages(html, implicitTags, implicitAttrs, preserveSignificantWhitespace).map((message) => [(0, digest_1.serializeNodes)(message.nodes), message.meaning, message.description, message.id]);
}
function _humanizePlaceholders(html, implicitTags = [], implicitAttrs = {}, preserveSignificantWhitespace = true) {
    return _extractMessages(html, implicitTags, implicitAttrs, preserveSignificantWhitespace).map((msg) => Object.keys(msg.placeholders)
        .map((name) => `${name}=${msg.placeholders[name].text}`)
        .join(', '));
}
function _humanizePlaceholdersToMessage(html, implicitTags = [], implicitAttrs = {}, preserveSignificantWhitespace = true) {
    return _extractMessages(html, implicitTags, implicitAttrs, preserveSignificantWhitespace).map((msg) => Object.keys(msg.placeholderToMessage)
        .map((k) => `${k}=${(0, digest_1.digest)(msg.placeholderToMessage[k])}`)
        .join(', '));
}
function _extractMessages(html, implicitTags = [], implicitAttrs = {}, preserveSignificantWhitespace = true) {
    ...
    return (0, extractor_merger_1.extractMessages)(parseResult.rootNodes, defaults_1.DEFAULT_INTERPOLATION_CONFIG, implicitTags, implicitAttrs, preserveSignificantWhitespace).messages;
}
