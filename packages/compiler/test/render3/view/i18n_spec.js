...
const get_msg_utils_1 = require("../../../src/render3/view/i18n/get_msg_utils");
const icu_serializer_1 = require("../../../src/render3/view/i18n/icu_serializer");
const localize_utils_1 = require("../../../src/render3/view/i18n/localize_utils");
const meta_1 = require("../../../src/render3/view/i18n/meta");
const util_1 = require("../../../src/render3/view/i18n/util");
...
describe('Utils', () => {
    it('formatI18nPlaceholderName', () => {
        ...
        cases.forEach(([input, output]) => {
            expect((0, util_1.formatI18nPlaceholderName)(input)).toEqual(output);
        });
    });
    describe('metadata serialization', () => {
        it('parseI18nMeta()', () => {
            expect((0, meta_1.parseI18nMeta)('')).toEqual(meta());
            expect((0, meta_1.parseI18nMeta)('desc')).toEqual(meta('', '', 'desc'));
            expect((0, meta_1.parseI18nMeta)('desc@@id')).toEqual(meta('id', '', 'desc'));
            expect((0, meta_1.parseI18nMeta)('meaning|desc')).toEqual(meta('', 'meaning', 'desc'));
            expect((0, meta_1.parseI18nMeta)('meaning|desc@@id')).toEqual(meta('id', 'meaning', 'desc'));
            expect((0, meta_1.parseI18nMeta)('@@id')).toEqual(meta('id', '', ''));
            expect((0, meta_1.parseI18nMeta)('\n   ')).toEqual(meta());
            expect((0, meta_1.parseI18nMeta)('\n   desc\n   ')).toEqual(meta('', '', 'desc'));
        ...
        it('serializeI18nHead()', () => {
            expect(o.localizedString(meta(), [literal('')], [], []).serializeI18nHead()).toEqual({
                cooked: '',
                raw: '',
                range: jasmine.any(parse_util_1.ParseSourceSpan),
            });
            expect(o.localizedString(meta('', '', 'desc'), [literal('')], [], []).serializeI18nHead()).toEqual({ cooked: ':desc:', raw: ':desc:', range: jasmine.any(parse_util_1.ParseSourceSpan) });
            expect(o.localizedString(meta('id', '', 'desc'), [literal('')], [], []).serializeI18nHead()).toEqual({ cooked: ':desc@@id:', raw: ':desc@@id:', range: jasmine.any(parse_util_1.ParseSourceSpan) });
            expect(o.localizedString(meta('', 'meaning', 'desc'), [literal('')], [], []).serializeI18nHead()).toEqual({
                cooked: ':meaning|desc:',
                raw: ':meaning|desc:',
                range: jasmine.any(parse_util_1.ParseSourceSpan),
            });
            expect(o.localizedString(meta('id', 'meaning', 'desc'), [literal('')], [], []).serializeI18nHead()).toEqual({
                cooked: ':meaning|desc@@id:',
                raw: ':meaning|desc@@id:',
                range: jasmine.any(parse_util_1.ParseSourceSpan),
            });
            expect(o.localizedString(meta('id', '', ''), [literal('')], [], []).serializeI18nHead()).toEqual({ cooked: ':@@id:', raw: ':@@id:', range: jasmine.any(parse_util_1.ParseSourceSpan) });
            // Escaping colons (block markers)
            expect(o
                .localizedString(meta('id:sub_id', 'meaning', 'desc'), [literal('')], [], [])
                .serializeI18nHead()).toEqual({
        ...
        it('serializeI18nPlaceholderBlock()', () => {
            expect(o
                ...
                .serializeI18nTemplatePart(1)).toEqual({ cooked: '', raw: '', range: jasmine.any(parse_util_1.ParseSourceSpan) });
            expect(o
                .localizedString(meta('', '', ''), [literal(''), literal('')], [new o.LiteralPiece('abc', {})], [])
                .serializeI18nTemplatePart(1)).toEqual({ cooked: ':abc:', raw: ':abc:', range: jasmine.any(parse_util_1.ParseSourceSpan) });
            expect(o
                .localizedString(meta('', '', ''), [literal(''), literal('message')], [literal('')], [])
                .serializeI18nTemplatePart(1)).toEqual({ cooked: 'message', raw: 'message', range: jasmine.any(parse_util_1.ParseSourceSpan) });
            expect(o
                .localizedString(meta('', '', ''), [literal(''), literal('message')], [literal('abc')], [])
                .serializeI18nTemplatePart(1)).toEqual({ cooked: ':abc:message', raw: ':abc:message', range: jasmine.any(parse_util_1.ParseSourceSpan) });
            expect(o
                .localizedString(meta('', '', ''), [literal(''), literal(':message')], [literal('')], [])
                .serializeI18nTemplatePart(1)).toEqual({ cooked: ':message', raw: '\\:message', range: jasmine.any(parse_util_1.ParseSourceSpan) });
            expect(o
                .localizedString(meta('', '', ''), [literal(''), literal(':message')], [literal('abc')], [])
                .serializeI18nTemplatePart(1)).toEqual({
                cooked: ':abc::message',
                raw: ':abc::message',
                range: jasmine.any(parse_util_1.ParseSourceSpan),
            });
    ...
    describe('jsdoc generation', () => {
        it('generates with description', () => {
            const docComment = (0, meta_1.i18nMetaToJSDoc)(meta('', '', 'desc'));
            expect(docComment.tags.length).toBe(1);
            expect(docComment.tags[0]).toEqual({ tagName: "desc" /* o.JSDocTagName.Desc */, text: 'desc' });
        });
        it('generates with no description suppressed', () => {
            const docComment = (0, meta_1.i18nMetaToJSDoc)(meta('', '', ''));
            expect(docComment.tags.length).toBe(1);
        ...
        it('generates with description and meaning', () => {
            const docComment = (0, meta_1.i18nMetaToJSDoc)(meta('', 'meaning', ''));
            expect(docComment.tags).toContain({ tagName: "meaning" /* o.JSDocTagName.Meaning */, text: 'meaning' });
        });
    });
    function meta(customId, meaning, description) {
        return { customId, meaning, description };
    }
});
describe('serializeI18nMessageForGetMsg', () => {
    const serialize = (input) => {
        const tree = (0, util_2.parseR3)(`<div i18n>${input}</div>`);
        const root = tree.nodes[0];
        return (0, get_msg_utils_1.serializeI18nMessageForGetMsg)(root.i18n);
    };
    ...
    it('should serialize text with interpolation for `GetMsg()`', () => {
        expect(serialize('Some text {{ valueA }} and {{ valueB + valueC }}')).toEqual('Some text {$interpolation} and {$interpolation_1}');
    });
    it('should serialize interpolation with named placeholder for `GetMsg()`', () => {
        expect(serialize('{{ valueB + valueC // i18n(ph="PLACEHOLDER NAME") }}')).toEqual('{$placeholderName}');
    });
    it('should serialize content with HTML tags for `GetMsg()`', () => {
        expect(serialize('A <span>B<div>C</div></span> D')).toEqual('A {$startTagSpan}B{$startTagDiv}C{$closeTagDiv}{$closeTagSpan} D');
    });
    it('should serialize simple ICU for `GetMsg()`', () => {
        expect(serialize('{age, plural, 10 {ten} other {other}}')).toEqual('{VAR_PLURAL, plural, 10 {ten} other {other}}');
    });
    it('should serialize nested ICUs for `GetMsg()`', () => {
        expect(serialize('{age, plural, 10 {ten {size, select, 1 {one} 2 {two} other {2+}}} other {other}}')).toEqual('{VAR_PLURAL, plural, 10 {ten {VAR_SELECT, select, 1 {one} 2 {two} other {2+}}} other {other}}');
    });
    it('should serialize ICU with nested HTML for `GetMsg()`', () => {
        expect(serialize('{age, plural, 10 {<b>ten</b>} other {<div class="A">other</div>}}')).toEqual('{VAR_PLURAL, plural, 10 {{START_BOLD_TEXT}ten{CLOSE_BOLD_TEXT}} other {{START_TAG_DIV}other{CLOSE_TAG_DIV}}}');
    });
...
describe('serializeI18nMessageForLocalize', () => {
    const serialize = (input) => {
        const tree = (0, util_2.parseR3)(`<div i18n>${input}</div>`, { leadingTriviaChars: template_1.LEADING_TRIVIA_CHARS });
        const root = tree.nodes[0];
        return (0, localize_utils_1.serializeI18nMessageForLocalize)(root.i18n);
    };
    ...
    it('should compute source-spans when serializing text with interpolation for `$localize()`', () => {
        const { messageParts, placeHolders } = serialize('Some text {{ valueA }} and {{ valueB + valueC }} done');
    ...
    it('should serialize nested ICUs for `$localize()`', () => {
        expect(serialize('{age, plural, 10 {ten {size, select, 1 {one} 2 {two} other {2+}}} other {other}}')).toEqual({
            messageParts: [
                literal('{VAR_PLURAL, plural, 10 {ten {VAR_SELECT, select, 1 {one} 2 {two} other {2+}}} other {other}}'),
            ],
            placeHolders: [],
        });
    });
    it('should serialize ICU with embedded HTML for `$localize()`', () => {
        expect(serialize('{age, plural, 10 {<b>ten</b>} other {<div class="A">other</div>}}')).toEqual({
            messageParts: [
                literal('{VAR_PLURAL, plural, 10 {{START_BOLD_TEXT}ten{CLOSE_BOLD_TEXT}} other {{START_TAG_DIV}other{CLOSE_TAG_DIV}}}'),
            ],
            placeHolders: [],
        });
    });
    it('should serialize ICU with embedded interpolation for `$localize()`', () => {
        expect(serialize('{age, plural, 10 {<b>ten</b>} other {{{age}} years old}}')).toEqual({
            messageParts: [
                literal('{VAR_PLURAL, plural, 10 {{START_BOLD_TEXT}ten{CLOSE_BOLD_TEXT}} other {{INTERPOLATION} years old}}'),
            ],
            placeHolders: [],
        });
    });
    it('should serialize ICU with nested HTML containing further ICUs for `$localize()`', () => {
        ...
        icu.associatedMessage = jasmine.any(i18n.Message);
        expect(serialize('{gender, select, male {male} female {female} other {other}}<div>{gender, select, male {male} female {female} other {other}}</div>')).toEqual({
    ...
    it('should serialize nested ICUs with embedded interpolation for `$localize()`', () => {
        expect(serialize('{age, plural, 10 {ten {size, select, 1 {{{ varOne }}} 2 {{{ varTwo }}} other {2+}}} other {other}}')).toEqual({
            messageParts: [
                literal('{VAR_PLURAL, plural, 10 {ten {VAR_SELECT, select, 1 {{INTERPOLATION}} 2 {{INTERPOLATION_1}} other {2+}}} other {other}}'),
            ],
            placeHolders: [],
        });
    });
...
describe('serializeIcuNode', () => {
    const serialize = (input) => {
        const tree = (0, util_2.parseR3)(`<div i18n>${input}</div>`);
        const rooti18n = tree.nodes[0].i18n;
        return (0, icu_serializer_1.serializeIcuNode)(rooti18n.nodes[0]);
    };
    ...
    it('should serialize an ICU with embedded interpolations', () => {
        expect(serialize('{age, select, 10 {ten} other {{{age}} years old}}')).toEqual('{VAR_SELECT, select, 10 {ten} other {{INTERPOLATION} years old}}');
    });
});
function literal(text, span = jasmine.any(parse_util_1.ParseSourceSpan)) {
    return new o.LiteralPiece(text, span);
}
function placeholder(name, span = jasmine.any(parse_util_1.ParseSourceSpan)) {
    return new o.PlaceholderPiece(name, span);
}
function humanizeSourceSpan(span) {
