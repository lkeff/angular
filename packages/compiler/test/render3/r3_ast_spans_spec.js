...
function humanizeSpan(span) {
    if (span === null || span === undefined) {
        return `<empty>`;
    }
    return span.toString();
}
function expectFromHtml(html, selectorlessEnabled = false) {
    return expectFromR3Nodes((0, util_1.parseR3)(html, { selectorlessEnabled }).nodes);
}
...
describe('R3 AST source spans', () => {
    ...
    describe('ICU expressions', () => {
        it('is correct for variables and placeholders', () => {
            expectFromHtml('<span i18n>{item.var, plural, other { {{item.placeholder}} items } }</span>').toEqual([
                [
                    'Element',
        ...
        it('is correct for nested ICUs', () => {
            expectFromHtml('<span i18n>{item.var, plural, other { {{item.placeholder}} {nestedVar, plural, other { {{nestedPlaceholder}} }}} }</span>').toEqual([
                [
                    'Element',
    ...
    describe('deferred blocks', () => {
        it('is correct for deferred blocks', () => {
            const html = '@defer (when isVisible() && foo; on hover(button), timer(10s), idle, immediate, ' +
    ...
    describe('for loop blocks', () => {
        it('is correct for loop blocks', () => {
            const html = `@for (item of items.foo.bar; track item.id; let i = $index, _o_d_d_ = $odd) {<h1>{{ item }}</h1>}` +
    ...
    describe('component tags', () => {
        ...
        it('is correct for a component with attributes and directives', () => {
            expectFromHtml('<MyComp before="foo" @Dir middle @OtherDir([a]="a" (b)="b()") after="123">Hello</MyComp>', true).toEqual([
                [
                    'Component',
        ...
        it('is correct for a component nested inside other markup', () => {
            expectFromHtml('@if (expr) {<div>Hello: <MyComp><span><OtherComp/></span></MyComp></div>}', true).toEqual([
                [
                    'IfBlock',
    ...
    describe('directives', () => {
        ...
        it('is correct for directives mixed with other attributes', () => {
            expectFromHtml('<div before="foo" @Dir middle @OtherDir([a]="a" (b)="b()") after="123"></div>', true).toEqual([
                [
                    'Element',
