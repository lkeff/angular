...
class R3AstHumanizer {
    ...
    visitBoundAttribute(attribute) {
        this.result.push(['BoundAttribute', attribute.type, attribute.name, (0, unparser_1.unparse)(attribute.value)]);
    }
    visitBoundEvent(event) {
        this.result.push(['BoundEvent', event.type, event.name, event.target, (0, unparser_1.unparse)(event.handler)]);
    }
    visitText(text) {
        this.result.push(['Text', text.value]);
    }
    visitBoundText(text) {
        this.result.push(['BoundText', (0, unparser_1.unparse)(text.value)]);
    }
    visitIcu(icu) {
        return null;
    }
    ...
    visitSwitchBlock(block) {
        this.result.push(['SwitchBlock', (0, unparser_1.unparse)(block.expression)]);
        this.visitAll([block.cases]);
    }
    visitSwitchBlockCase(block) {
        this.result.push([
            'SwitchBlockCase',
            block.expression === null ? null : (0, unparser_1.unparse)(block.expression),
        ]);
        this.visitAll([block.children]);
    }
    visitForLoopBlock(block) {
        var _a;
        const result = ['ForLoopBlock', (0, unparser_1.unparse)(block.expression), (0, unparser_1.unparse)(block.trackBy)];
    ...
    visitDeferredTrigger(trigger) {
        if (trigger instanceof t.BoundDeferredTrigger) {
            this.result.push(['BoundDeferredTrigger', (0, unparser_1.unparse)(trigger.value)]);
        }
    ...
    visitLetDeclaration(decl) {
        this.result.push(['LetDeclaration', decl.name, (0, unparser_1.unparse)(decl.value)]);
    }
...
function expectFromHtml(html, ignoreError = false, selectorlessEnabled = false) {
    const res = (0, util_1.parseR3)(html, { ignoreError, selectorlessEnabled });
...
function expectSpanFromHtml(html) {
    const { nodes } = (0, util_1.parseR3)(html);
...
describe('R3 template transform', () => {
    ...
    describe('Nodes without binding', () => {
        ...
        it('should parse ngContent', () => {
            const res = (0, util_1.parseR3)('<ng-content select="a"></ng-content>');
    ...
    describe('Bound attributes', () => {
        it('should parse mixed case bound properties', () => {
            expectFromHtml('<div [someProp]="v"></div>').toEqual([
                ['Element', 'div'],
                ['BoundAttribute', ast_1.BindingType.Property, 'someProp', 'v'],
            ]);
        });
        it('should parse bound properties via bind- ', () => {
            expectFromHtml('<div bind-prop="v"></div>').toEqual([
                ['Element', 'div'],
                ['BoundAttribute', ast_1.BindingType.Property, 'prop', 'v'],
            ]);
        });
        it('should report missing property names in bind- syntax', () => {
            expect(() => (0, util_1.parseR3)('<div bind-></div>')).toThrowError(/Property name is missing in binding/);
        });
    ...
    describe('templates', () => {
        ...
        it('should support <ng-template> with structural directive', () => {
            ...
            const res = (0, util_1.parseR3)('<ng-template *ngIf="true"></ng-template>', { ignoreError: false });
            expect(res.nodes[0].tagName).toEqual(null);
            expect(res.nodes[0].children[0].tagName).toEqual('ng-template');
        });
        ...
        it('should report an error if a reference is used multiple times on the same template', () => {
            expect(() => (0, util_1.parseR3)('<ng-template #a #a></ng-template>')).toThrowError(/Reference "#a" is defined more than once/);
        });
        ...
        it('should parse bound attributes', () => {
            expectFromHtml('<ng-template [k1]="v1" [k2]="v2"></ng-template>').toEqual([
                ['Template'],
                ['BoundAttribute', ast_1.BindingType.Property, 'k1', 'v1'],
                ['BoundAttribute', ast_1.BindingType.Property, 'k2', 'v2'],
            ]);
        });
    });
    describe('inline templates', () => {
        it('should support attribute and bound attributes', () => {
            ...
            expectFromHtml('<div *ngFor="let item of items"></div>').toEqual([
                ['Template'],
                ['TextAttribute', 'ngFor', ''],
                ['BoundAttribute', ast_1.BindingType.Property, 'ngForOf', 'items'],
            ...
            expectFromHtml('<div *ngFor="item of items"></div>').toEqual([
                ['Template'],
                ['BoundAttribute', ast_1.BindingType.Property, 'ngFor', 'item'],
                ['BoundAttribute', ast_1.BindingType.Property, 'ngForOf', 'items'],
                ['Element', 'div'],
            ]);
        });
        ...
        it('should parse variables via as ...', () => {
            expectFromHtml('<div *ngIf="expr as local"></div>').toEqual([
                ['Template'],
                ['BoundAttribute', ast_1.BindingType.Property, 'ngIf', 'expr'],
                ['Variable', 'local', 'ngIf'],
                ['Element', 'div'],
            ]);
        });
    ...
    describe('events', () => {
        it('should parse bound events with a target', () => {
            expectFromHtml('<div (window:event)="v"></div>').toEqual([
                ['Element', 'div'],
                ['BoundEvent', ast_1.ParsedEventType.Regular, 'event', 'window', 'v'],
            ]);
        });
        it('should parse event names case sensitive', () => {
            expectFromHtml('<div (some-event)="v"></div>').toEqual([
                ['Element', 'div'],
                ['BoundEvent', ast_1.ParsedEventType.Regular, 'some-event', null, 'v'],
            ]);
            expectFromHtml('<div (someEvent)="v"></div>').toEqual([
                ['Element', 'div'],
                ['BoundEvent', ast_1.ParsedEventType.Regular, 'someEvent', null, 'v'],
            ]);
        });
        it('should parse bound events via on-', () => {
            expectFromHtml('<div on-event="v"></div>').toEqual([
                ['Element', 'div'],
                ['BoundEvent', ast_1.ParsedEventType.Regular, 'event', null, 'v'],
            ]);
        });
        it('should report missing event names in on- syntax', () => {
            expect(() => (0, util_1.parseR3)('<div on-></div>')).toThrowError(/Event name is missing in binding/);
        });
        it('should parse bound events and properties via [(...)]', () => {
            expectFromHtml('<div [(prop)]="v"></div>').toEqual([
                ['Element', 'div'],
                ['BoundAttribute', ast_1.BindingType.TwoWay, 'prop', 'v'],
                ['BoundEvent', ast_1.ParsedEventType.TwoWay, 'propChange', null, 'v'],
            ]);
        });
        it('should parse $any in a two-way binding', () => {
            expectFromHtml('<div [(prop)]="$any(v)"></div>').toEqual([
                ['Element', 'div'],
                ['BoundAttribute', ast_1.BindingType.TwoWay, 'prop', '$any(v)'],
                ['BoundEvent', ast_1.ParsedEventType.TwoWay, 'propChange', null, '$any(v)'],
            ]);
        });
        ...
        it('should parse property reads bound via [(...)]', () => {
            expectFromHtml('<div [(prop)]="a.b.c"></div>').toEqual([
                ['Element', 'div'],
                ['BoundAttribute', ast_1.BindingType.TwoWay, 'prop', 'a.b.c'],
                ['BoundEvent', ast_1.ParsedEventType.TwoWay, 'propChange', null, 'a.b.c'],
            ]);
        });
        ...
        it('should report assignments in two-way bindings', () => {
            expect(() => (0, util_1.parseR3)(`<div [(prop)]="v = 1"></div>`)).toThrowError(/Bindings cannot contain assignments/);
        });
        it('should report pipes in two-way bindings', () => {
            expect(() => (0, util_1.parseR3)(`<div [(prop)]="v | pipe"></div>`)).toThrowError(/Cannot have a pipe in an action expression/);
        });
        it('should report unsupported expressions in two-way bindings', () => {
            ...
            for (const expression of unsupportedExpressions) {
                expect(() => (0, util_1.parseR3)(`<div [(prop)]="${expression}"></div>`))
        ...
        it('should report an error for assignments into non-null asserted expressions', () => {
            ...
            expect(() => (0, util_1.parseR3)('<div (prop)="v! = $event"></div>')).toThrowError(/Unexpected token '=' at column 4/);
        });
        it('should report missing property names in bindon- syntax', () => {
            expect(() => (0, util_1.parseR3)('<div bindon-></div>')).toThrowError(/Property name is missing in binding/);
        });
        it('should report an error on empty expression', () => {
            expect(() => (0, util_1.parseR3)('<div (event)="">')).toThrowError(/Empty expressions are not allowed/);
            expect(() => (0, util_1.parseR3)('<div (event)="   ">')).toThrowError(/Empty expressions are not allowed/);
        });
        it('should parse bound animation events when event name is empty', () => {
            expectFromHtml('<div (@)="onAnimationEvent($event)"></div>', true).toEqual([
                ['Element', 'div'],
                ['BoundEvent', ast_1.ParsedEventType.Animation, '', null, 'onAnimationEvent($event)'],
            ]);
            expect(() => (0, util_1.parseR3)('<div (@)></div>')).toThrowError(/Animation event name is missing in binding/);
        });
        it('should report invalid phase value of animation event', () => {
            expect(() => (0, util_1.parseR3)('<div (@event.invalidPhase)></div>')).toThrowError(/The provided animation output phase value "invalidphase" for "@event" is not supported \(use start or done\)/);
            expect(() => (0, util_1.parseR3)('<div (@event.)></div>')).toThrowError(/The animation trigger output event \(@event\) is missing its phase value name \(start or done are currently supported\)/);
            expect(() => (0, util_1.parseR3)('<div (@event)></div>')).toThrowError(/The animation trigger output event \(@event\) is missing its phase value name \(start or done are currently supported\)/);
        });
    });
    describe('variables', () => {
        it('should report variables not on template elements', () => {
            expect(() => (0, util_1.parseR3)('<div let-a-name="b"></div>')).toThrowError(/"let-" is only supported on ng-template elements./);
        });
        it('should report missing variable names', () => {
            expect(() => (0, util_1.parseR3)('<ng-template let-><ng-template>')).toThrowError(/Variable does not have a name/);
        });
    });
    describe('references', () => {
        ...
        it('should report invalid reference names', () => {
            expect(() => (0, util_1.parseR3)('<div #a-b></div>')).toThrowError(/"-" is not allowed in reference names/);
        });
        it('should report missing reference names', () => {
            expect(() => (0, util_1.parseR3)('<div #></div>')).toThrowError(/Reference does not have a name/);
        });
        it('should report an error if a reference is used multiple times on the same element', () => {
            expect(() => (0, util_1.parseR3)('<div #a #a></div>')).toThrowError(/Reference "#a" is defined more than once/);
        });
    });
    describe('literal attribute', () => {
        it('should report missing animation trigger in @ syntax', () => {
            expect(() => (0, util_1.parseR3)('<div @></div>')).toThrowError(/Animation trigger is missing/);
        });
    });
    describe('ng-content', () => {
        it('should parse ngContent without selector', () => {
            const res = (0, util_1.parseR3)('<ng-content></ng-content>');
            expectFromR3Nodes(res.nodes).toEqual([['Content', '*']]);
        });
        it('should parse ngContent with a specific selector', () => {
            const res = (0, util_1.parseR3)('<ng-content select="tag[attribute]"></ng-content>');
        ...
        it('should parse ngContent with a selector', () => {
            const res = (0, util_1.parseR3)('<ng-content select="a"></ng-content><ng-content></ng-content><ng-content select="b"></ng-content>');
            const selectors = ['*', 'a', 'b'];
            expectFromR3Nodes(res.nodes).toEqual([
                ['Content', selectors[1]],
                ['TextAttribute', 'select', selectors[1]],
        ...
        it('should parse ngProjectAs as an attribute', () => {
            const res = (0, util_1.parseR3)('<ng-content ngProjectAs="a"></ng-content>');
            expectFromR3Nodes(res.nodes).toEqual([
                ['Content', '*'],
        ...
        it('should parse ngContent with children', () => {
            const res = (0, util_1.parseR3)('<ng-content><section>Root <div>Parent <span>Child</span></div></section></ng-content>');
            expectFromR3Nodes(res.nodes).toEqual([
                ['Content', '*'],
                ['Element', 'section'],
                ['Text', 'Root '],
    ...
    describe('deferred blocks', () => {
        ...
        it('should parse a deferred block with connected blocks', () => {
            expectFromHtml('@defer {<calendar-cmp [date]="current"/>}' +
                '@loading {Loading...}' +
                '@placeholder {Placeholder content!}' +
                '@error {Loading failed :(}').toEqual([
                ['DeferredBlock'],
        ...
        it('should parse a deferred block with comments between the connected blocks', () => {
            expectFromHtml('@defer {<calendar-cmp [date]="current"/>}' +
                ...
                '<!-- Show this on error --> @error {Loading failed :(}').toEqual([
                ['DeferredBlock'],
                ['Element', 'calendar-cmp'],
                ['BoundAttribute', 0, 'date', 'current'],
                ['DeferredBlockPlaceholder'],
        ...
        it('should parse a deferred block with connected blocks that have an arbitrary ' +
            ...
            expectFromR3Nodes((0, util_1.parseR3)(template, { preserveWhitespaces: true }).nodes).toEqual([
        ...
        it('should parse a loading block with parameters', () => {
            expectFromHtml('@defer{<calendar-cmp [date]="current"/>}' +
                '@loading (after 100ms; minimum 1.5s){Loading...}').toEqual([
                ['DeferredBlock'],
                ['Element', 'calendar-cmp'],
                ['BoundAttribute', 0, 'date', 'current'],
        ...
        it('should parse a placeholder block with parameters', () => {
            expectFromHtml('@defer {<calendar-cmp [date]="current"/>}' + '@placeholder (minimum 1.5s){Placeholder...}').toEqual([
                ['DeferredBlock'],
                ['Element', 'calendar-cmp'],
                ['BoundAttribute', 0, 'date', 'current'],
        ...
        it('should parse a deferred block with prefetch triggers', () => {
            const html = '@defer (on idle; prefetch on viewport(button), hover(button); prefetch when shouldPrefetch()){hello}';
        ...
        it('should parse a deferred block with hydrate triggers', () => {
            const html = '@defer (on idle; hydrate on viewport, hover, timer(500); hydrate when shouldHydrate()){hello}';
            expectFromHtml(html).toEqual([
                ['DeferredBlock'],
        ...
        it('should allow arbitrary number of spaces after the `hydrate` keyword', () => {
            const html = '@defer (on idle; hydrate         on viewport, hover; hydrate    when shouldHydrate()){hello}';
            expectFromHtml(html).toEqual([
                ['DeferredBlock'],
                ['ViewportDeferredTrigger', null],
                ['HoverDeferredTrigger', null],
        ...
        it('should parse a complete example', () => {
            expectFromHtml('@defer (when isVisible() && foo; on hover(button), timer(10s), idle, immediate, ' +
        ...
        it('should treat blocks as plain text inside ngNonBindable', () => {
            expectFromHtml('<div ngNonBindable>' +
        ...
        it('should parse triggers with implied target elements', () => {
            expectFromHtml('@defer (on hover, interaction, viewport; prefetch on hover, interaction, viewport) {hello}' +
                '@placeholder {<implied-trigger/>}').toEqual([
                ['DeferredBlock'],
                ['HoverDeferredTrigger', null],
        ...
        describe('block validations', () => {
            it('should report syntax error in `when` trigger', () => {
                expect(() => (0, util_1.parseR3)('@defer (when isVisible#){hello}')).toThrowError(/Invalid character \[#\]/);
            });
            it('should report unrecognized trigger', () => {
                expect(() => (0, util_1.parseR3)('@defer (unknown visible()){hello}')).toThrowError(/Unrecognized trigger/);
            });
            it('should report content before a connected block', () => {
                expect(() => (0, util_1.parseR3)('@defer {hello} <br> @placeholder {placeholder}')).toThrowError(/@placeholder block can only be used after an @defer block/);
            });
            it('should report connected defer blocks used without a defer block', () => {
                expect(() => (0, util_1.parseR3)('@placeholder {placeholder}')).toThrowError(/@placeholder block can only be used after an @defer block/);
                expect(() => (0, util_1.parseR3)('@loading {loading}')).toThrowError(/@loading block can only be used after an @defer block/);
                expect(() => (0, util_1.parseR3)('@error {error}')).toThrowError(/@error block can only be used after an @defer block/);
            });
            it('should report multiple placeholder blocks', () => {
                expect(() => (0, util_1.parseR3)('@defer {hello} @placeholder {p1} @placeholder {p2}')).toThrowError(/@defer block can only have one @placeholder block/);
            });
            it('should report multiple loading blocks', () => {
                expect(() => (0, util_1.parseR3)('@defer {hello} @loading {l1} @loading {l2}')).toThrowError(/@defer block can only have one @loading block/);
            });
            it('should report multiple error blocks', () => {
                expect(() => (0, util_1.parseR3)('@defer {hello} @error {e1} @error {e2}')).toThrowError(/@defer block can only have one @error block/);
            });
            it('should report unrecognized parameter in placeholder block', () => {
                expect(() => (0, util_1.parseR3)('@defer {hello} @placeholder (unknown 100ms) {hi}')).toThrowError(/Unrecognized parameter in @placeholder block: "unknown 100ms"/);
            });
            it('should report unrecognized parameter in loading block', () => {
                expect(() => (0, util_1.parseR3)('@defer {hello} @loading (unknown 100ms) {hi}')).toThrowError(/Unrecognized parameter in @loading block: "unknown 100ms"/);
            });
            it('should report any parameter usage in error block', () => {
                expect(() => (0, util_1.parseR3)('@defer {hello} @error (foo) {hi}')).toThrowError(/@error block cannot have parameters/);
            });
            it('should report if minimum placeholder time cannot be parsed', () => {
                expect(() => (0, util_1.parseR3)('@defer {hello} @placeholder (minimum 123abc) {hi}')).toThrowError(/Could not parse time value of parameter "minimum"/);
            });
            it('should report if minimum loading time cannot be parsed', () => {
                expect(() => (0, util_1.parseR3)('@defer {hello} @loading (minimum 123abc) {hi}')).toThrowError(/Could not parse time value of parameter "minimum"/);
            });
            it('should report if after loading time cannot be parsed', () => {
                expect(() => (0, util_1.parseR3)('@defer {hello} @loading (after 123abc) {hi}')).toThrowError(/Could not parse time value of parameter "after"/);
            });
            it('should report unrecognized `on` trigger', () => {
                expect(() => (0, util_1.parseR3)('@defer (on foo) {hello}')).toThrowError(/Unrecognized trigger type "foo"/);
            });
            it('should report missing comma after unparametarized `on` trigger', () => {
                expect(() => (0, util_1.parseR3)('@defer (on hover idle) {hello}')).toThrowError(/Unexpected token/);
            });
            it('should report missing comma after parametarized `on` trigger', () => {
                expect(() => (0, util_1.parseR3)('@defer (on viewport(button) idle) {hello}')).toThrowError(/Unexpected token/);
            });
            it('should report mutliple commas after between `on` triggers', () => {
                expect(() => (0, util_1.parseR3)('@defer (on viewport(button), , idle) {hello}')).toThrowError(/Unexpected token/);
            });
            it('should report unclosed parenthesis in `on` trigger', () => {
                expect(() => (0, util_1.parseR3)('@defer (on viewport(button) {hello}')).toThrowError(/Incomplete block "defer"/);
            });
            it('should report incorrect closing parenthesis in `on` trigger', () => {
                expect(() => (0, util_1.parseR3)('@defer (on viewport(but)ton) {hello}')).toThrowError(/Unexpected token/);
            });
            it('should report stray closing parenthesis in `on` trigger', () => {
                expect(() => (0, util_1.parseR3)('@defer (on idle)) {hello}')).toThrowError(/Unexpected character "EOF"/);
            });
            it('should report non-identifier token usage in `on` trigger', () => {
                expect(() => (0, util_1.parseR3)('@defer (on 123) {hello}')).toThrowError(/Unexpected token/);
            });
            it('should report if identifier is not followed by an opening parenthesis', () => {
                expect(() => (0, util_1.parseR3)('@defer (on viewport[]) {hello}')).toThrowError(/Unexpected token/);
            });
            it('should report if parameters are passed to `idle` trigger', () => {
                expect(() => (0, util_1.parseR3)('@defer (on idle(1)) {hello}')).toThrowError(/"idle" trigger cannot have parameters/);
            });
            it('should report if no parameters are passed into `timer` trigger', () => {
                expect(() => (0, util_1.parseR3)('@defer (on timer) {hello}')).toThrowError(/"timer" trigger must have exactly one parameter/);
            });
            it('should report if `timer` trigger value cannot be parsed', () => {
                expect(() => (0, util_1.parseR3)('@defer (on timer(123abc)) {hello}')).toThrowError(/Could not parse time value of trigger "timer"/);
            });
            it('should report if `interaction` trigger has more than one parameter', () => {
                expect(() => (0, util_1.parseR3)('@defer (on interaction(a, b)) {hello}')).toThrowError(/"interaction" trigger can only have zero or one parameters/);
            });
            it('should report if parameters are passed to `immediate` trigger', () => {
                expect(() => (0, util_1.parseR3)('@defer (on immediate(1)) {hello}')).toThrowError(/"immediate" trigger cannot have parameters/);
            });
            it('should report if `hover` trigger has more than one parameter', () => {
                expect(() => (0, util_1.parseR3)('@defer (on hover(a, b)) {hello}')).toThrowError(/"hover" trigger can only have zero or one parameters/);
            });
            it('should report if `viewport` trigger has more than one parameter', () => {
                expect(() => (0, util_1.parseR3)('@defer (on viewport(a, b)) {hello}')).toThrowError(/"viewport" trigger can only have zero or one parameters/);
            });
            it('should report duplicate when triggers', () => {
                expect(() => (0, util_1.parseR3)('@defer (when isVisible(); when somethingElse()) {hello}')).toThrowError(/Duplicate "when" trigger is not allowed/);
            });
            it('should report duplicate on triggers', () => {
                expect(() => (0, util_1.parseR3)('@defer (on idle; when isVisible(); on timer(10), idle) {hello}')).toThrowError(/Duplicate "idle" trigger is not allowed/);
            });
            it('should report duplicate prefetch when triggers', () => {
                expect(() => (0, util_1.parseR3)('@defer (prefetch when isVisible(); prefetch when somethingElse()) {hello}')).toThrowError(/Duplicate "when" trigger is not allowed/);
            });
            it('should report duplicate prefetch on triggers', () => {
                expect(() => (0, util_1.parseR3)('@defer (prefetch on idle; prefetch when isVisible(); prefetch on timer(10), idle) {hello}')).toThrowError(/Duplicate "idle" trigger is not allowed/);
            });
            it('should report multiple minimum parameters on a placeholder block', () => {
                expect(() => (0, util_1.parseR3)('@defer {hello} @placeholder (minimum 1s; minimum 500ms) {placeholder}')).toThrowError(/@placeholder block can only have one "minimum" parameter/);
            });
            it('should report multiple minimum parameters on a loading block', () => {
                expect(() => (0, util_1.parseR3)('@defer {hello} @loading (minimum 1s; minimum 500ms) {loading}')).toThrowError(/@loading block can only have one "minimum" parameter/);
            });
            it('should report multiple after parameters on a loading block', () => {
                expect(() => (0, util_1.parseR3)('@defer {hello} @loading (after 1s; after 500ms) {loading}')).toThrowError(/@loading block can only have one "after" parameter/);
            });
            it('should report if reference-based trigger has no reference and there is no placeholder block', () => {
                expect(() => (0, util_1.parseR3)('@defer (on viewport) {hello}')).toThrowError(/"viewport" trigger with no parameters can only be placed on an @defer that has a @placeholder block/);
            });
            it('should report if reference-based trigger has no reference and the placeholder is empty', () => {
                expect(() => (0, util_1.parseR3)('@defer (on viewport) {hello} @placeholder {}')).toThrowError(/"viewport" trigger with no parameters can only be placed on an @defer that has a @placeholder block with exactly one root element node/);
            });
            it('should report if reference-based trigger has no reference and the placeholder with text at the root', () => {
                expect(() => (0, util_1.parseR3)('@defer (on viewport) {hello} @placeholder {placeholder}')).toThrowError(/"viewport" trigger with no parameters can only be placed on an @defer that has a @placeholder block with exactly one root element node/);
            });
            it('should report if reference-based trigger has no reference and the placeholder has multiple root elements', () => {
                expect(() => (0, util_1.parseR3)('@defer (on viewport) {hello} @placeholder {<div></div><span></span>}')).toThrowError(/"viewport" trigger with no parameters can only be placed on an @defer that has a @placeholder block with exactly one root element node/);
            });
            it('should report parameter passed to hydrate trigger with reference-based equivalent', () => {
                expect(() => (0, util_1.parseR3)('@defer (on interaction(button); hydrate on interaction(button)) {hello}')).toThrowError(/Hydration trigger "interaction" cannot have parameters/);
            });
            it('should not report missing reference on hydrate trigger', () => {
                expect(() => (0, util_1.parseR3)('@defer (on immediate; hydrate on viewport) {hello}')).not.toThrow();
            });
            it('should report if reference-based trigger has no reference and there is no placeholder block but a hydrate trigger exists', () => {
                expect(() => (0, util_1.parseR3)('@defer (on viewport; hydrate on immediate) {hello}')).toThrowError(/"viewport" trigger with no parameters can only be placed on an @defer that has a @placeholder block/);
            });
            it('should report if reference-based trigger has no reference and there is no placeholder block but a hydrate trigger exists and it is also viewport', () => {
                expect(() => (0, util_1.parseR3)('@defer (on viewport; hydrate on viewport) {hello}')).toThrowError(/"viewport" trigger with no parameters can only be placed on an @defer that has a @placeholder block/);
            });
            it('should report never trigger used without `hydrate`', () => {
                expect(() => (0, util_1.parseR3)('@defer (on immediate; never) {hello}')).toThrowError(/Unrecognized trigger/);
                expect(() => (0, util_1.parseR3)('@defer (on immediate; prefetch never) {hello}')).toThrowError(/Unrecognized trigger/);
            });
            it('should report `hydrate never` used with additonal characters', () => {
                expect(() => (0, util_1.parseR3)('@defer (hydrate never, and thank you) {hello}')).toThrowError(/Unrecognized trigger/);
            });
            it('should not report an error when `hydrate never` is used with additonal blocks', () => {
                expect(() => (0, util_1.parseR3)('@defer (hydrate never; on idle;) {hello}')).not.toThrowError(/Unrecognized trigger/);
            });
            ...
            it('should not report an error when `hydrate never` is used after another block', () => {
                expect(() => (0, util_1.parseR3)(`@defer(
        on idle;
        hydrate never) {hello}`)).not.toThrowError(/Unrecognized trigger/);
            });
            it('should report when `hydrate never` is used together with another `hydrate` trigger', () => {
                // Extra trigger after `hydrate never`.
                expect(() => (0, util_1.parseR3)('@defer (hydrate never; hydrate when shouldHydrate()) {hello}')).toThrowError(/Cannot specify additional `hydrate` triggers if `hydrate never` is present/);
                // Extra trigger before `hydrate never`.
                expect(() => (0, util_1.parseR3)('@defer (hydrate when shouldHydrate(); hydrate never) {hello}')).toThrowError(/Cannot specify additional `hydrate` triggers if `hydrate never` is present/);
            });
        });
    });
      ...
      `;
        ...
        `)).toThrowError(/Parser Error: Unexpected token \./);
            ...
            it('should report if a block different from "case" and "default" is used in a switch', () => {
                const result = (0, util_1.parseR3)(`
              @switch (cond) {
                @case (x()) {X case}
                @foo {Foo}
              }
            `, { ignoreError: true });
                const switchNode = result.nodes[0];
            ...
            it('should report if @case or @default is used outside of a switch block', () => {
                expect(() => (0, util_1.parseR3)(`@case (foo) {}`)).toThrowError(/Unrecognized block @case/);
                expect(() => (0, util_1.parseR3)(`@default {}`)).toThrowError(/Unrecognized block @default/);
            });
      ...
      `).toEqual([
        ...
        describe('validations', () => {
            it('should report if for loop does not have an expression', () => {
                expect(() => (0, util_1.parseR3)(`@for {hello}`)).toThrowError(/@for loop does not have an expression/);
            });
            it('should report if for loop does not have a tracking expression', () => {
                expect(() => (0, util_1.parseR3)(`@for (a of b) {hello}`)).toThrowError(/@for loop must have a "track" expression/);
                expect(() => (0, util_1.parseR3)(`@for (a of b; track      ) {hello}`)).toThrowError(/@for loop must have a "track" expression/);
            });
            it('should report mismatching optional parentheses around for loop expression', () => {
                expect(() => (0, util_1.parseR3)(`@for ((a of b; track c) {hello}`)).toThrowError(/Unclosed parentheses in expression/);
                expect(() => (0, util_1.parseR3)(`@for ((a of b(); track c) {hello}`)).toThrowError(/Unexpected end of expression: b\(/);
                expect(() => (0, util_1.parseR3)(`@for (a of b); track c) {hello}`)).toThrowError(/Unexpected character "EOF"/);
            });
            it('should report unrecognized for loop parameters', () => {
                expect(() => (0, util_1.parseR3)(`@for (a of b; foo bar) {hello}`)).toThrowError(/Unrecognized @for loop parameter "foo bar"/);
            });
            it('should report multiple `track` parameters', () => {
                expect(() => (0, util_1.parseR3)(`@for (a of b; track c; track d) {hello}`)).toThrowError(/@for loop can only have one "track" expression/);
            });
            it('should report invalid for loop expression', () => {
                const errorPattern = /Cannot parse expression\. @for loop expression must match the pattern "<identifier> of <expression>"/;
                expect(() => (0, util_1.parseR3)(`@for (//invalid of items) {hello}`)).toThrowError(errorPattern);
                expect(() => (0, util_1.parseR3)(`@for (item) {hello}`)).toThrowError(errorPattern);
                expect(() => (0, util_1.parseR3)(`@for (item in items) {hello}`)).toThrowError(errorPattern);
                expect(() => (0, util_1.parseR3)(`@for (item of    ) {hello}`)).toThrowError(errorPattern);
            });
            it('should report syntax error in for loop expression', () => {
                expect(() => (0, util_1.parseR3)(`@for (item of items..foo) {hello}`)).toThrowError(/Unexpected token \./);
            });
        ...
        `)).toThrowError(/@empty block can only be used after an @for block/);
            ...
            it('should report an empty block used without a @for loop block', () => {
                expect(() => (0, util_1.parseR3)(`@empty {hello}`)).toThrowError(/@empty block can only be used after an @for block/);
            });
            it('should report an empty `let` parameter', () => {
                expect(() => (0, util_1.parseR3)(`@for (item of items.foo.bar; track item.id; let ) {}`)).toThrowError(/Invalid @for loop "let" parameter. Parameter should match the pattern "<name> = <variable name>"/);
            });
            it('should report an invalid `let` parameter', () => {
                expect(() => (0, util_1.parseR3)(`@for (item of items.foo.bar; track item.id; let i = $index, $odd) {}`)).toThrowError(/Invalid @for loop "let" parameter\. Parameter should match the pattern "<name> = <variable name>"/);
            });
            it('should an unknown variable in a `let` parameter', () => {
                expect(() => (0, util_1.parseR3)(`@for (item of items.foo.bar; track item.id; let foo = $foo) {}`)).toThrowError(/Unknown "let" parameter variable "\$foo"\. The allowed variables are:/);
            });
            it('should report duplicate `let` parameter variables', () => {
                expect(() => (0, util_1.parseR3)(`@for (item of items.foo.bar; track item.id; let i = $index, f = $first, i = $index) {}`)).toThrowError(/Duplicate "let" parameter variable "\$index"/);
                expect(() => (0, util_1.parseR3)(`@for (item of items.foo.bar; track item.id; let $index = $index) {}`)).toThrowError(/Duplicate "let" parameter variable "\$index"/);
            });
            it('should report an item name that conflicts with the implicit context variables', () => {
                ['$index', '$count', '$first', '$last', '$even', '$odd'].forEach((varName) => {
                    expect(() => (0, util_1.parseR3)(`@for (${varName} of items; track $index) {}`)).toThrowError(/@for loop item name cannot be one of \$index, \$first, \$last, \$even, \$odd, \$count/);
                });
            });
            it('should report a context variable alias that is the same as the variable name', () => {
                expect(() => (0, util_1.parseR3)(`@for (item of items; let item = $index; track $index) {}`)).toThrowError(/Invalid @for loop "let" parameter. Variable cannot be called "item"/);
            });
        });
    });
    describe('if blocks', () => {
        ...
        `)).toThrowError(/"as" expression is only allowed on the primary @if block/);
            ...
            it('should report an @else if block used without an @if block', () => {
                expect(() => (0, util_1.parseR3)(`@else if (foo) {hello}`)).toThrowError(/@else if block can only be used after an @if or @else if block/);
            });
            it('should report an @else block used without an @if block', () => {
                expect(() => (0, util_1.parseR3)(`@else (foo) {hello}`)).toThrowError(/@else block can only be used after an @if or @else if block/);
            });
            it('should report content between an @if and @else if block', () => {
                expect(() => (0, util_1.parseR3)(`@if (foo) {hello} <div></div> @else if (bar) {goodbye}`)).toThrowError(/@else if block can only be used after an @if or @else if block/);
            });
            it('should report content between an @if and @else block', () => {
                expect(() => (0, util_1.parseR3)(`@if (foo) {hello} <div></div> @else {goodbye}`)).toThrowError(/@else block can only be used after an @if or @else if block/);
            });
    ...
    describe('@let declarations', () => {
        ...
        it('should report syntax errors in the let declaration value', () => {
            expect(() => (0, util_1.parseR3)('@let foo = {one: 1;')).toThrowError(/Parser Error: Missing expected } at the end of the expression \[\{one: 1]/);
        });
        it('should report a let declaration with no value', () => {
            expect(() => (0, util_1.parseR3)('@let foo =  ;')).toThrowError(/@let declaration value cannot be empty/);
        });
    ...
    describe('component nodes', () => {
        function expectSelectorless(html, ignoreError) {
            return expectFromHtml(html, ignoreError, true);
        }
        function parseSelectorless(html) {
            return (0, util_1.parseR3)(html, { selectorlessEnabled: true });
        }
        ...
        it('should parse a component tag nested within other markup', () => {
            expectSelectorless('@if (expr) {<div>Hello: <MyComp><span><OtherComp/></span></MyComp></div>}').toEqual([
                ['IfBlock'],
                ['IfBlockBranch', 'expr'],
                ['Element', 'div'],
                ['Text', 'Hello: '],
        ...
        it('should parse a component node with attributes and directives', () => {
            expectSelectorless('<MyComp before="foo" @Dir middle @OtherDir([a]="a" (b)="b()") after="123">Hello</MyComp>').toEqual([
                ['Component', 'MyComp', null, 'MyComp'],
        ...
        it('should not pick up attributes from directives when using * syntax', () => {
            expectSelectorless('<MyComp *ngIf="true" @Dir(static="1" [bound]="expr" (event)="fn()")/>').toEqual([
                ['Template'],
                ['BoundAttribute', 0, 'ngIf', 'true'],
                ['Component', 'MyComp', null, 'MyComp'],
                ['Directive', 'Dir'],
        ...
        it('should treat components as elements inside ngNonBindable', () => {
            expectSelectorless('<div ngNonBindable><MyComp foo="bar" @Dir(some="attr")></MyComp></div>').toEqual([
                ['Element', 'div'],
                ['TextAttribute', 'ngNonBindable', ''],
                ['Element', 'MyComp'],
        ...
        it('should not allow a selectorless component with an unsupported tag name', () => {
            ...
            for (const name of unsupportedTags) {
                expect(() => parseSelectorless(`<MyComp:${name}></MyComp:${name}>`)).toThrowError(new RegExp(`Tag name "${name}" cannot be used as a component tag`));
            }
        });
    });
    describe('directives', () => {
        ...
        it('should parse a directive mixed with other attributes', () => {
            expectSelectorless('<div before="foo" @Dir middle @OtherDir([a]="a" (b)="b()") after="123"></div>').toEqual([
                ['Element', 'div'],
                ['TextAttribute', 'before', 'foo'],
                ['TextAttribute', 'middle', ''],
                ['TextAttribute', 'after', '123'],
        ...
        it('should remove directives inside ngNonBindable', () => {
            expectSelectorless('<div ngNonBindable><span @EmptyDir @WithAttrs(foo="123" [bar]="321")></span></div>').toEqual([
                ['Element', 'div'],
                ['TextAttribute', 'ngNonBindable', ''],
                ['Element', 'span'],
            ]);
        ...
        it('should pick up attributes from selectorless directives when using * syntax', () => {
            expectSelectorless('<div *ngIf="true" @Dir(static="1" [bound]="expr" (event)="fn()")></div>').toEqual([
                ['Template'],
                ['BoundAttribute', 0, 'ngIf', 'true'],
                ['Element', 'div'],
                ['Directive', 'Dir'],
        ...
        describe('validations', () => {
            it('should not allow * syntax inside directives', () => {
                expect(() => parseSelectorless(`<div @Dir(*ngIf="true")></div>`)).toThrowError(/Shorthand template syntax "\*ngIf" is not supported inside a directive context/);
            });
            it('should not allow ngProjectAs inside directive syntax', () => {
                expect(() => parseSelectorless(`<div @Dir(ngProjectAs="foo")></div>`)).toThrowError(/Attribute "ngProjectAs" is not supported in a directive context/);
            });
            it('should not allow ngNonBindable inside directive syntax', () => {
                expect(() => parseSelectorless(`<div @Dir(ngNonBindable)></div>`)).toThrowError(/Attribute "ngNonBindable" is not supported in a directive context/);
            });
            it('should not allow the same directive to be applied multiple times', () => {
                expect(() => parseSelectorless(`<div @One @Two @One(input="123")></div>`)).toThrowError(/Cannot apply directive "One" multiple times on the same element/);
            });
            it('should not allow class bindings inside directives', () => {
                expect(() => parseSelectorless(`<div @Dir([class.foo]="expr")></div>`)).toThrowError(/Binding is not supported in a directive context/);
            });
            it('should not allow style bindings inside directives', () => {
                expect(() => parseSelectorless(`<div @Dir([style.foo]="expr")></div>`)).toThrowError(/Binding is not supported in a directive context/);
            });
            it('should not allow attribute bindings inside directives', () => {
                expect(() => parseSelectorless(`<div @Dir([attr.foo]="expr")></div>`)).toThrowError(/Binding is not supported in a directive context/);
            });
            it('should not allow animation bindings inside directives', () => {
                expect(() => parseSelectorless(`<div @Dir([@animation]="expr")></div>`)).toThrowError(/Binding is not supported in a directive context/);
            });
