...
describe('HttpClient', () => {
    ...
    describe('makes a basic request', () => {
        ...
        it('with an array of params of different types', (done) => {
            client.get('/test', { params: { 'test': [true, 'a', 2] } }).subscribe(() => done());
    ...
    describe('makes a request for an error response', () => {
        it('with a JSON body', (done) => {
            ...
            backend
                .expectOne('/test')
                .flush({ 'data': 'hello world' }, { status: response_1.HttpStatusCode.InternalServerError, statusText: 'Server error' });
        });
    });
    describe('throws an error', () => {
        it('for a request with nullish header', () => {
            client.request('GET', '/test', { headers: { foo: null } }).subscribe();
            expect(() => backend.expectOne('/test').request.headers.has('random-header')).toThrowError('Unexpected value of the `foo` header provided. ' +
