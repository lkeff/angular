...
const tokens_1 = require("../..//src/directives/ng_optimized_image/tokens");
const image_loader_1 = require("../../src/directives/ng_optimized_image/image_loaders/image_loader");
const ng_optimized_image_1 = require("../../src/directives/ng_optimized_image/ng_optimized_image");
const preconnect_link_checker_1 = require("../../src/directives/ng_optimized_image/preconnect_link_checker");
describe('Image directive', () => {
    ...
    describe('preload <link> element on a server', () => {
            ...
            it('should create `<link>` element when the image priority attr is true', () => {
                ...
                setupTestingModule({
                    extraProviders: [
                        ...
                        {
                            provide: image_loader_1.IMAGE_LOADER,
                            useValue: (config) => config.width
                ...
                const _document = testing_1.TestBed.inject(index_1.DOCUMENT);
                const _window = _document.defaultView;
                const setAttributeSpy = spyOn(_window.HTMLLinkElement.prototype, 'setAttribute').and.callThrough();
                ...
                (0, matchers_1.expect)(preloadLink).toBeTruthy();
                const [name, value] = setAttributeSpy.calls.argsFor(0);
                (0, matchers_1.expect)(name).toEqual('as');
                (0, matchers_1.expect)(value).toEqual('image');
                (0, matchers_1.expect)(preloadLink.getAttribute('rel')).toEqual('preload');
                (0, matchers_1.expect)(preloadLink.getAttribute('as')).toEqual('image');
                (0, matchers_1.expect)(preloadLink.getAttribute('imagesizes')).toEqual('10vw');
                (0, matchers_1.expect)(preloadLink.getAttribute('imagesrcset')).toEqual(`${rewrittenSrc}?width=100 100w`);
                (0, matchers_1.expect)(preloadLink.getAttribute('fetchpriority')).toEqual('high');
                preloadLink.remove();
            });
            it('should not create a preload `<link>` element when src is already preloaded.', () => {
                ...
                setupTestingModule({
                    extraProviders: [
                        ...
                        {
                            provide: image_loader_1.IMAGE_LOADER,
                            useValue: (config) => `https://angular.io/${config.src}`,
                        },
                    ],
                });
                ...
                fixture.detectChanges();
                const head = _document.head;
                const preloadImages = testing_1.TestBed.inject(tokens_1.PRELOADED_IMAGES);
                (0, matchers_1.expect)(preloadImages.has(rewrittenSrc)).toBeTruthy();
                const preloadLinks = head.querySelectorAll(`link[href="${rewrittenSrc}"]`);
                (0, matchers_1.expect)(preloadLinks.length).toEqual(1);
        ...
        `;
                ...
                fixture.detectChanges();
                (0, matchers_1.expect)(consoleWarnSpy.calls.count()).toBe(1);
                (0, matchers_1.expect)(consoleWarnSpy.calls.argsFor(0)[0]).toMatch(/NG02961: The `NgOptimizedImage` directive has detected that more than 5 images were marked as priority/);
            });
        });
        it('should not hit max preload limit when not on the server', () => {
            ...
            (0, matchers_1.expect)(preloadImages.size).toEqual(0);
            (0, matchers_1.expect)(preloadLinks.length).toEqual(0);
        });
    });
    it('should set `loading` and `fetchpriority` attributes before `src`', () => {
        ...
        const _document = testing_1.TestBed.inject(index_1.DOCUMENT);
        const _window = _document.defaultView;
        const setAttributeSpy = spyOn(_window.HTMLImageElement.prototype, 'setAttribute').and.callThrough();
        const fixture = testing_1.TestBed.createComponent(TestComponent);
        fixture.detectChanges();
        const nativeElement = fixture.nativeElement;
        const img = nativeElement.querySelector('img');
        (0, matchers_1.expect)(img.getAttribute('loading')).toBe('eager');
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (attrName == 'fetchpriority')
                _fetchpriorityAttrId = i;
            if (attrName == 'src')
                _srcAttrId = i;
        }
        // Verify that both `loading` and `fetchpriority` are set *before* `src`:
        (0, matchers_1.expect)(_loadingAttrId).toBeGreaterThan(-1); // was actually set
        (0, matchers_1.expect)(_loadingAttrId).toBeLessThan(_srcAttrId); // was set after `src`
        (0, matchers_1.expect)(_fetchpriorityAttrId).toBeGreaterThan(-1); // was actually set
        (0, matchers_1.expect)(_fetchpriorityAttrId).toBeLessThan(_srcAttrId); // was set after `src`
    });
    it('should always reflect the width/height attributes if bound', () => {
        ...
        const nativeElement = fixture.nativeElement;
        const img = nativeElement.querySelector('img');
        (0, matchers_1.expect)(img.getAttribute('width')).toBe('100');
        (0, matchers_1.expect)(img.getAttribute('height')).toBe('50');
    });
    describe('setup error handling', () => {
        it('should throw if both `src` and `ngSrc` are present', () => {
            ...
            (0, matchers_1.expect)(() => {
                const fixture = createTestComponent(template);
                fixture.detectChanges();
            }).toThrowError('NG02950: The NgOptimizedImage directive (activated on an <img> ' +
        ...
        it('should throw if both `ngSrcet` and `srcset` is present', () => {
            setupTestingModule();
            const template = '<img ngSrc="img-100.png" ngSrcset="100w, 200w" srcset="img-100.png 100w, img-200.png 200w" width="100" height="50">';
            (0, matchers_1.expect)(() => {
                const fixture = createTestComponent(template);
                fixture.detectChanges();
            }).toThrowError('NG02951: The NgOptimizedImage directive (activated on an <img> ' +
        ...
        it('should throw if `ngSrc` contains a Base64-encoded image (that starts with `data:`)', () => {
            ...
            (0, matchers_1.expect)(() => {
                ...
                const fixture = createTestComponent(template);
                fixture.detectChanges();
            }).toThrowError('NG02952: The NgOptimizedImage directive has detected that `ngSrc` ' +
        ...
        it('should throw if `ngSrc` contains a `blob:` URL', (done) => {
            ...
            canvas.toBlob(function (blob) {
                ...
                const errorMessageRegExp = /NG02952: The NgOptimizedImage directive (.*?) has detected that `ngSrc` was set to a blob URL \(blob:/;
                (0, matchers_1.expect)(() => {
        ...
        it('should throw if `width` and `height` are not set', () => {
            ...
            (0, matchers_1.expect)(() => {
                const fixture = createTestComponent(template);
                fixture.detectChanges();
            }).toThrowError('NG02954: The NgOptimizedImage directive (activated on an <img> ' +
        ...
        it('should throw if `width` is 0', () => {
            ...
            (0, matchers_1.expect)(() => {
                const fixture = createTestComponent(template);
                fixture.detectChanges();
            }).toThrowError('NG02952: The NgOptimizedImage directive (activated on an <img> ' +
        ...
        it('should throw if `height` has an invalid value', () => {
            ...
            (0, matchers_1.expect)(() => {
                const fixture = createTestComponent(template);
                fixture.detectChanges();
            }).toThrowError('NG02952: The NgOptimizedImage directive (activated on an <img> element ' +
        ...
        describe('invalid `ngSrcset` values', () => {
            const mockDirectiveInstance = { ngSrc: 'img.png' };
          ...
          `;
                (0, matchers_1.expect)(() => {
                    const fixture = createTestComponent(template);
                    fixture.detectChanges();
                }).toThrowError(`NG0${2952 /* RuntimeErrorCode.INVALID_INPUT */}: The NgOptimizedImage directive (activated on an <img> element with the \`ngSrc="img"\`) ` +
                    ...
                    `${ng_optimized_image_1.RECOMMENDED_SRCSET_DENSITY_CAP}x but supports image densities up to ` +
                    `${ng_optimized_image_1.ABSOLUTE_SRCSET_DENSITY_CAP}x. The human eye cannot distinguish between image densities ` +
                    `greater than ${ng_optimized_image_1.RECOMMENDED_SRCSET_DENSITY_CAP}x - which makes them unnecessary for ` +
                    `most use cases. Images that will be pinch-zoomed are typically the primary use case for ` +
                    `${ng_optimized_image_1.ABSOLUTE_SRCSET_DENSITY_CAP}x images. Please remove the high density descriptor and try again.`);
            });
          ...
          `;
            ...
            it('should throw if width srcset is missing a comma', () => {
                (0, matchers_1.expect)(() => {
                    (0, ng_optimized_image_1.assertValidNgSrcset)(mockDirectiveInstance, '100w 200w');
                }).toThrowError();
            });
            it('should throw if density srcset is missing a comma', () => {
                (0, matchers_1.expect)(() => {
                    (0, ng_optimized_image_1.assertValidNgSrcset)(mockDirectiveInstance, '1x 2x');
                }).toThrowError();
            });
            it('should throw if density srcset has too many digits', () => {
                (0, matchers_1.expect)(() => {
                    (0, ng_optimized_image_1.assertValidNgSrcset)(mockDirectiveInstance, '100x, 2x');
                }).toThrowError();
            });
            it('should throw if width srcset includes a file name', () => {
                (0, matchers_1.expect)(() => {
                    (0, ng_optimized_image_1.assertValidNgSrcset)(mockDirectiveInstance, 'a.png 100w, b.png 200w');
                }).toThrowError();
            });
            it('should throw if density srcset includes a file name', () => {
                (0, matchers_1.expect)(() => {
                    (0, ng_optimized_image_1.assertValidNgSrcset)(mockDirectiveInstance, 'a.png 1x, b.png 2x');
                }).toThrowError();
            });
            it('should throw if srcset starts with a letter', () => {
                (0, matchers_1.expect)(() => {
                    (0, ng_optimized_image_1.assertValidNgSrcset)(mockDirectiveInstance, 'a100w, 200w');
                }).toThrowError();
            });
            ...
            it('should throw if first descriptor in srcset is junk', () => {
                (0, matchers_1.expect)(() => {
                    (0, ng_optimized_image_1.assertValidNgSrcset)(mockDirectiveInstance, 'foo, 1x');
                }).toThrowError();
            });
            it('should throw if later descriptors in srcset are junk', () => {
                (0, matchers_1.expect)(() => {
                    (0, ng_optimized_image_1.assertValidNgSrcset)(mockDirectiveInstance, '100w, foo');
                }).toThrowError();
            });
            it('should throw if srcset has a density descriptor after a width descriptor', () => {
                (0, matchers_1.expect)(() => {
                    (0, ng_optimized_image_1.assertValidNgSrcset)(mockDirectiveInstance, '100w, 1x');
                }).toThrowError();
            });
            it('should throw if srcset has a width descriptor after a density descriptor', () => {
                (0, matchers_1.expect)(() => {
                    (0, ng_optimized_image_1.assertValidNgSrcset)(mockDirectiveInstance, '1x, 200w');
                }).toThrowError();
            });
        });
        const inputs = [
        ...
        inputs.forEach(([inputName, value]) => {
          ...
          />`,
                ...
                (0, matchers_1.expect)(() => {
                    ...
                    fixture.componentInstance[inputName] =
                        value;
                    fixture.detectChanges();
        ...
        it('should accept a safeUrl ngSrc value', () => {
            let TestComponent = (() => {
                ...
                var TestComponent = _classThis = class {
                    constructor(sanitizer) {
                        ...
                        this.rawImage = `javascript:alert("Hi there")`;
            ...
            (0, matchers_1.expect)(img.src).toContain(`${IMG_BASE_URL}/javascript:alert`);
        });
    });
    describe('lazy loading', () => {
        ...
        it('should lazily load non-priority images', () => {
            ...
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)(img.getAttribute('loading')).toBe('lazy');
        });
    });
    describe('loading attribute', () => {
        ...
        it('should throw if used with priority images', () => {
            setupTestingModule();
            const template = '<img ngSrc="path/img.png" width="150" height="50" loading="eager" priority>';
            (0, matchers_1.expect)(() => {
                const fixture = createTestComponent(template);
                fixture.detectChanges();
            }).toThrowError('NG02952: The NgOptimizedImage directive (activated on an <img> element ' +
        ...
        it('should support setting loading priority to "auto"', () => {
            ...
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)(img.getAttribute('loading')).toBe('auto');
        });
    ...
    describe('fetch priority', () => {
        it('should be "high" for priority images', () => {
            ...
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)(img.getAttribute('fetchpriority')).toBe('high');
        });
        it('should be "auto" for non-priority images', () => {
            ...
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)(img.getAttribute('fetchpriority')).toBe('auto');
        });
        it('should log a warning if the priority attribute is used too often', (0, testing_2.withHead)('<link rel="preconnect" href="https://angular.io/">', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            (0, ng_optimized_image_1.resetImagePriorityCount)();
            const imageLoader = () => {
      ...
      `;
            ...
            if (isBrowser) {
                (0, matchers_1.expect)(consoleWarnSpy.calls.count()).toBe(1);
                (0, matchers_1.expect)(consoleWarnSpy.calls.argsFor(0)[0]).toMatch(new RegExp(`NG0${2966 /* RuntimeErrorCode.TOO_MANY_PRIORITY_ATTRIBUTES */}`));
            }
            else {
                // The warning is only logged on browsers
                (0, matchers_1.expect)(consoleWarnSpy.calls.count()).toBe(0);
            }
            globalThis['ngServerMode'] = undefined;
        })));
    });
    describe('meta data', () => {
        it('should add a data attribute to the element for identification', () => {
            ...
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)(img.getAttribute('ng-img')).not.toBeNull();
        });
    ...
    describe('fill mode', () => {
        ...
        it('should throw if width is provided for fill mode image', () => {
            ...
            (0, matchers_1.expect)(() => {
                const fixture = createTestComponent(template);
                fixture.detectChanges();
            }).toThrowError('NG02952: The NgOptimizedImage directive (activated on an <img> element with the ' +
        ...
        it('should apply appropriate styles in fill mode', () => {
            ...
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)((_a = img.getAttribute('style')) === null || _a === void 0 ? void 0 : _a.replace(/\s/g, '')).toMatch('position:absolute;width:100%;height:100%;inset:0');
        });
        it('should augment existing styles in fill mode', () => {
            ...
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)((_a = img.getAttribute('style')) === null || _a === void 0 ? void 0 : _a.replace(/\s/g, '')).toMatch('border-radius:5px;padding:10px;position:absolute;width:100%;height:100%;inset:0');
        });
        it('should not add fill styles if not in fill mode', () => {
            var _a;
            setupTestingModule();
            const template = '<img ngSrc="path/img.png" width="400" height="300" style="position: relative; border-radius: 5px">';
            const fixture = createTestComponent(template);
            fixture.detectChanges();
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)((_a = img.getAttribute('style')) === null || _a === void 0 ? void 0 : _a.replace(/\s/g, '')).toBe('position:relative;border-radius:5px;');
        });
        it('should add default sizes value in fill mode', () => {
            ...
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)(img.getAttribute('sizes')).toBe('100vw');
        });
        it('should add auto sizes to default in fill mode when lazy', () => {
            ...
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)(img.getAttribute('sizes')).toBe('auto, 100vw');
        });
        it('should not overwrite sizes value in fill mode', () => {
            ...
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)(img.getAttribute('sizes')).toBe('50vw');
        });
        it('should prepend "auto" to sizes in fill mode when lazy', () => {
            ...
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)(img.getAttribute('sizes')).toBe('auto, 50vw');
        });
        it('should cause responsive srcset to be generated in fill mode', () => {
            ...
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)(img.getAttribute('srcset')).toBe(`${IMG_BASE_URL}/path/img.png 640w, ${IMG_BASE_URL}/path/img.png 750w, ${IMG_BASE_URL}/path/img.png 828w, ` +
    ...
    describe('placeholder attribute', () => {
        ...
        it('should add background size, position, and repeat styling if the placeholder attribute is present', () => {
            setupTestingModule();
            const template = '<img ngSrc="path/img.png" width="400" height="300" placeholder="path/img.png" />';
            const fixture = createTestComponent(template);
            fixture.detectChanges();
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            const styles = parseInlineStyles(img);
            (0, matchers_1.expect)(styles.get('background-size')).toBe('cover');
            (0, matchers_1.expect)(styles.get('background-position')).toBe('50%50%');
            (0, matchers_1.expect)(styles.get('background-repeat')).toBe('no-repeat');
        });
        it('should not add background styles if the placeholder attribute is not present', () => {
            ...
            const img = nativeElement.querySelector('img');
            const styles = parseInlineStyles(img);
            (0, matchers_1.expect)(styles.get('background-size')).toBeUndefined();
            (0, matchers_1.expect)(styles.get('background-position')).toBeUndefined();
            (0, matchers_1.expect)(styles.get('background-repeat')).toBeUndefined();
            (0, matchers_1.expect)(styles.get('background-image')).toBeUndefined();
        });
        it('should add a background-image tag when placeholder is provided as a boolean', () => {
            ...
            (0, matchers_1.expect)((_a = styles.get('background-image')) === null || _a === void 0 ? void 0 : _a.replace(/"/g, '')).toBe(`url(${IMG_BASE_URL}/path/img.png?w=30&ph=true)`);
        });
        if (!isNode) {
            ...
            it('should add a background-image tag when placeholder is provided as a data URL', () => {
                var _a;
                setupTestingModule({ imageLoader });
                const template = '<img ngSrc="path/img.png" width="400" height="300" placeholder="data:image/png;base64,iVBORw0KGgoAAAANSUhEU" />';
                const fixture = createTestComponent(template);
                fixture.detectChanges();
                const nativeElement = fixture.nativeElement;
                const img = nativeElement.querySelector('img');
                // Double quotes removed to account for different browser behavior.
                (0, matchers_1.expect)((_a = img.getAttribute('style')) === null || _a === void 0 ? void 0 : _a.replace(/"/g, '').replace(/\s/g, '')).toBe(`background-size:cover;background-position:50%50%;background-repeat:no-repeat;background-image:url(data:image/png;base64,iVBORw0KGgoAAAANSUhEU);filter:blur(${PLACEHOLDER_BLUR_AMOUNT}px);`);
            });
        }
        it('should add a background-image tag when placeholder is provided as a URL', () => {
            var _a;
            setupTestingModule({ imageLoader });
            const template = '<img ngSrc="path/img.png" width="400" height="300" placeholder="https://mysite.com/assets/my-image.png" />';
            const fixture = createTestComponent(template);
            fixture.detectChanges();
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            // Double quotes removed to account for different browser behavior.
            (0, matchers_1.expect)((_a = img.getAttribute('style')) === null || _a === void 0 ? void 0 : _a.replace(/"/g, '').replace(/\s/g, '')).toBe(`background-size:cover;background-position:50%50%;background-repeat:no-repeat;background-image:url(https://mysite.com/assets/my-image.png);filter:blur(${PLACEHOLDER_BLUR_AMOUNT}px);`);
        });
        // DataURLs get stripped from background-image attribute in Node, but not browsers.
        it('should add a background-image tag when placeholder is provided as relative URL', () => {
            var _a;
            setupTestingModule({ imageLoader });
            const template = '<img ngSrc="path/img.png" width="400" height="300" placeholder="../../assets/my-image.png" />';
            const fixture = createTestComponent(template);
            fixture.detectChanges();
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            // Double quotes removed to account for different browser behavior.
            (0, matchers_1.expect)((_a = img.getAttribute('style')) === null || _a === void 0 ? void 0 : _a.replace(/"/g, '').replace(/\s/g, '')).toBe(`background-size:cover;background-position:50%50%;background-repeat:no-repeat;background-image:url(../../assets/my-image.png);filter:blur(${PLACEHOLDER_BLUR_AMOUNT}px);`);
        });
        ...
        it('should replace the placeholder with the actual image on load', () => {
            setupTestingModule();
            const template = '<img ngSrc="path/img.png" width="400" height="300" placeholder="true" />';
            const fixture = createTestComponent(template, core_1.ChangeDetectionStrategy.OnPush);
            fixture.detectChanges();
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)(parseInlineStyles(img).has('background-image')).toBe(true);
            img.dispatchEvent(new Event('load'));
            fixture.detectChanges();
            (0, matchers_1.expect)(parseInlineStyles(img).has('background-image')).toBe(false);
        });
        ...
        it('should apply a background blur to images with a placeholder', () => {
            ...
            const img = nativeElement.querySelector('img');
            const styles = parseInlineStyles(img);
            (0, matchers_1.expect)(styles.get('filter')).toBe(`blur(${PLACEHOLDER_BLUR_AMOUNT}px)`);
        });
        it('should not apply a background blur to placeholders with blur=false', () => {
            setupTestingModule({ imageLoader });
            const template = '<img ngSrc="path/img.png" width="400" height="300" placeholder="data:image/png;base64,iVBORw0KGgoAAAANSUhEU" [placeholderConfig]="{blur: false}" />';
            const fixture = createTestComponent(template);
            fixture.detectChanges();
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            const styles = parseInlineStyles(img);
            (0, matchers_1.expect)(styles.get('filter')).toBeUndefined();
        });
        it('should throw if placeholderConfig is provided without placeholder', () => {
            setupTestingModule();
            const template = '<img ngSrc="path/img.png" width="400" height="300" [placeholderConfig]="{blur: false}">';
            (0, matchers_1.expect)(() => {
                const fixture = createTestComponent(template);
                fixture.detectChanges();
            }).toThrowError(`NG02952: The NgOptimizedImage directive has detected that \`placeholderConfig\` options were ` +
        ...
        it('should throw if placeholderConfig with blur=true is provided without placeholder', () => {
            setupTestingModule();
            const template = '<img ngSrc="path/img.png" width="400" height="300" [placeholderConfig]="{blur: true}">';
            (0, matchers_1.expect)(() => {
                const fixture = createTestComponent(template);
                fixture.detectChanges();
            }).toThrowError(`NG02952: The NgOptimizedImage directive has detected that \`placeholderConfig\` options were ` +
        ...
        it('should throw if there is no image loader but `ngSrcset` is present', () => {
            ...
            (0, matchers_1.expect)(() => {
                const fixture = createTestComponent(template);
                fixture.detectChanges();
            }).toThrowError(`NG0${2963 /* RuntimeErrorCode.MISSING_NECESSARY_LOADER */}: The NgOptimizedImage directive (activated on an <img> element with the \`ngSrc="img.png"\`) ` +
        ...
        it('should throw if a very large inline placeholder is supplied', () => {
            setupTestingModule();
            const template = `<img ngSrc="path/img.png" width="400" height="300" placeholder="data:image/png;base64,${'a'.repeat(ng_optimized_image_1.DATA_URL_ERROR_LIMIT + 100)}">`;
            (0, matchers_1.expect)(() => {
                const fixture = createTestComponent(template);
                fixture.detectChanges();
            }).toThrowError(`NG0${2965 /* RuntimeErrorCode.OVERSIZED_PLACEHOLDER */}: The NgOptimizedImage directive (activated on an <img> element with the \`ngSrc="path/img.png"\`) ` +
                `has detected that the \`placeholder\` attribute is set to a data URL which is longer than ${ng_optimized_image_1.DATA_URL_ERROR_LIMIT} characters. This is strongly ` +
        ...
        it('should warn if a large inline placeholder is supplied', () => {
            setUpModuleNoLoader();
            const template = `<img ngSrc="path/img.png" width="400" height="300" placeholder="data:image/png;base64,${'a'.repeat(ng_optimized_image_1.DATA_URL_WARN_LIMIT + 100)}">`;
            const fixture = createTestComponent(template);
            const consoleWarnSpy = spyOn(console, 'warn');
            fixture.detectChanges();
            (0, matchers_1.expect)(consoleWarnSpy.calls.count()).toBe(1);
            (0, matchers_1.expect)(consoleWarnSpy.calls.argsFor(0)[0]).toBe(`NG0${2965 /* RuntimeErrorCode.OVERSIZED_PLACEHOLDER */}: The NgOptimizedImage directive (activated on an <img> element with the \`ngSrc="path/img.png"\`) ` +
                `has detected that the \`placeholder\` attribute is set to a data URL which is longer than ${ng_optimized_image_1.DATA_URL_WARN_LIMIT} characters. This is discouraged, ` +
        ...
        if (isBrowser) {
            describe('browser', () => {
                ...
                it('should throw if the placeholder height exceeds the threshold', () => {
                    setUpModuleNoLoader();
                    const template = `<img ngSrc="path/img.png" width="100" height="100" style="width:1001px; height: 300px" placeholder="data:image/png;base64,${'a'.repeat(100)}">`;
                    const consoleWarnSpy = spyOn(console, 'warn');
                    const fixture = createTestComponent(template);
                    fixture.detectChanges();
                    (0, matchers_1.expect)(consoleWarnSpy.calls.count()).toBe(1);
                    (0, matchers_1.expect)(consoleWarnSpy.calls.argsFor(0)[0]).toMatch(new RegExp(`NG0${2967 /* RuntimeErrorCode.PLACEHOLDER_DIMENSION_LIMIT_EXCEEDED */}:`));
                });
                it('should throw if the placeholder width exceeds the threshold', () => {
                    setUpModuleNoLoader();
                    const template = `<img ngSrc="path/img.png" width="100" height="100" style="height:1001px; width: 300px" placeholder="data:image/png;base64,${'a'.repeat(100)}">`;
                    const consoleWarnSpy = spyOn(console, 'warn');
                    const fixture = createTestComponent(template);
                    fixture.detectChanges();
                    (0, matchers_1.expect)(consoleWarnSpy.calls.count()).toBe(1);
    ...
    describe('preconnect detector', () => {
        ...
        it('should log a warning if there is no preconnect link for a priority image', (0, testing_2.withHead)('', () => {
            ...
            fixture.detectChanges();
            (0, matchers_1.expect)(consoleWarnSpy.calls.count()).toBe(1);
            (0, matchers_1.expect)(consoleWarnSpy.calls.argsFor(0)[0]).toBe('NG02956: The NgOptimizedImage directive (activated on an <img> element ' +
        ...
        it('should not log a warning if there is no preconnect link, but the image is not set as a priority', (0, testing_2.withHead)('', () => {
            setupTestingModule({ imageLoader });
            const consoleWarnSpy = spyOn(console, 'warn');
            const template = '<img ngSrc="a.png" width="100" height="50">';
            const fixture = createTestComponent(template);
        ...
        it("should log a warning if there is a preconnect, but it doesn't match the priority image", (0, testing_2.withHead)('<link rel="preconnect" href="http://angular.io">', () => {
            // The warning is only logged on the client
            if (!isBrowser)
                return;
            setupTestingModule({ imageLoader });
        ...
        it('should log a warning if there is no matching preconnect link for a priority image, but there is a preload tag', (0, testing_2.withHead)('<link rel="preload" href="https://angular.io/assets/images/logos/angular/angular.svg" as="image">', () => {
            // The warning is only logged on the client
            if (!isBrowser)
                return;
            setupTestingModule({ imageLoader });
        ...
        it('should not log a warning if there is a matching preconnect link for a priority image (with an extra `/` at the end)', (0, testing_2.withHead)('<link rel="preconnect" href="https://angular.io/">', () => {
            setupTestingModule({ imageLoader });
            const consoleWarnSpy = spyOn(console, 'warn');
            const template = '<img ngSrc="a.png" width="100" height="50" priority>';
            const fixture = createTestComponent(template);
        ...
        ['localhost', '127.0.0.1', '0.0.0.0'].forEach((blocklistedHostname) => {
            it(`should not log a warning if an origin domain is blocklisted ` +
                `(checking ${blocklistedHostname})`, (0, testing_2.withHead)('', () => {
                const imageLoader = () => {
        ...
        describe('PRECONNECT_CHECK_BLOCKLIST token', () => {
            it(`should allow passing host names`, (0, testing_2.withHead)('', () => {
                const providers = [{ provide: preconnect_link_checker_1.PRECONNECT_CHECK_BLOCKLIST, useValue: 'angular.io' }];
            ...
            it(`should allow passing origins`, (0, testing_2.withHead)('', () => {
                const providers = [{ provide: preconnect_link_checker_1.PRECONNECT_CHECK_BLOCKLIST, useValue: 'https://angular.io' }];
                setupTestingModule({ imageLoader, extraProviders: providers });
                const consoleWarnSpy = spyOn(console, 'warn');
                const template = '<img ngSrc="a.png" width="100" height="50" priority>';
                const fixture = createTestComponent(template);
            ...
            it(`should allow passing arrays of host names`, (0, testing_2.withHead)('', () => {
                const providers = [
                    { provide: preconnect_link_checker_1.PRECONNECT_CHECK_BLOCKLIST, useValue: ['https://angular.io'] },
                ];
                setupTestingModule({ imageLoader, extraProviders: providers });
                const consoleWarnSpy = spyOn(console, 'warn');
                const template = '<img ngSrc="a.png" width="100" height="50" priority>';
            ...
            it(`should allow passing nested arrays of host names`, (0, testing_2.withHead)('', () => {
                const providers = [
                    { provide: preconnect_link_checker_1.PRECONNECT_CHECK_BLOCKLIST, useValue: [['https://angular.io']] },
                ];
                setupTestingModule({ imageLoader, extraProviders: providers });
    ...
    describe('loaders', () => {
        const imageLoaderWithData = (config) => {
            let paramsString = '';
        ...
        // }
        const nestedImageLoader = (config) => {
            var _a, _b;
            return `${config.src}/${(_a = config.loaderParams) === null || _a === void 0 ? void 0 : _a['transforms1'].example1}/${(_b = config.loaderParams) === null || _b === void 0 ? void 0 : _b['transforms2'].example2}`;
        };
        it('should set `src` to match `ngSrc` if image loader is not provided', () => {
            ...
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)(img.src).toBe(`${IMG_BASE_URL}/img.png`);
        });
        it('should warn if there is no image loader but using Imgix URL', () => {
            ...
            fixture.detectChanges();
            (0, matchers_1.expect)(consoleWarnSpy.calls.count()).toBe(1);
            (0, matchers_1.expect)(consoleWarnSpy.calls.argsFor(0)[0]).toMatch(/your images may be hosted on the Imgix CDN/);
        });
        it('should warn if there is no image loader but using ImageKit URL', () => {
            ...
            fixture.detectChanges();
            (0, matchers_1.expect)(consoleWarnSpy.calls.count()).toBe(1);
            (0, matchers_1.expect)(consoleWarnSpy.calls.argsFor(0)[0]).toMatch(/your images may be hosted on the ImageKit CDN/);
        });
        it('should warn if there is no image loader but using Cloudinary URL', () => {
            ...
            fixture.detectChanges();
            (0, matchers_1.expect)(consoleWarnSpy.calls.count()).toBe(1);
            (0, matchers_1.expect)(consoleWarnSpy.calls.argsFor(0)[0]).toMatch(/your images may be hosted on the Cloudinary CDN/);
        });
        it('should warn if there is no image loader but using Netlify URL', () => {
            ...
            fixture.detectChanges();
            (0, matchers_1.expect)(consoleWarnSpy.calls.count()).toBe(1);
            (0, matchers_1.expect)(consoleWarnSpy.calls.argsFor(0)[0]).toMatch(/your images may be hosted on the Netlify CDN/);
        });
        ...
        it('should warn if there is no image loader but `ngSrcset` is present', () => {
            ...
            fixture.detectChanges();
            (0, matchers_1.expect)(consoleWarnSpy.calls.count()).toBe(1);
            (0, matchers_1.expect)(consoleWarnSpy.calls.argsFor(0)[0]).toBe(`NG0${2963 /* RuntimeErrorCode.MISSING_NECESSARY_LOADER */}: The NgOptimizedImage directive (activated on an <img> element ` +
        ...
        it('should set `src` using the image loader provided via the `IMAGE_LOADER` token to compose src URL', () => {
            const imageLoader = (config) => `${IMG_BASE_URL}/${config.src}`;
            setupTestingModule({ imageLoader });
            const template = `
       ...
       `;
            ...
            (0, matchers_1.expect)(imgs[0].src.trim()).toBe(`${IMG_BASE_URL}/img.png`);
            (0, matchers_1.expect)(imgs[1].src.trim()).toBe(`${IMG_BASE_URL}/img-2.png`);
        });
        it('should use the image loader to update `src` if `ngSrc` updated', () => {
            ...
            (0, matchers_1.expect)(imgs[0].src).toBe(`${IMG_BASE_URL}/img.png`);
            fixture.componentInstance.ngSrc = 'updatedImg.png';
            fixture.detectChanges();
            (0, matchers_1.expect)(imgs[0].src).toBe(`${IMG_BASE_URL}/updatedImg.png`);
        });
        ...
        it('should pass absolute URLs defined in the `ngSrc` to custom image loaders provided via the `IMAGE_LOADER` token', () => {
          ...
          `;
            ...
            const nativeElement = fixture.nativeElement;
            const imgs = nativeElement.querySelectorAll('img');
            (0, matchers_1.expect)(imgs[0].src.trim()).toBe(`${IMG_BASE_URL}/img.png?rewritten=true`);
        });
      ...
      `;
            ...
            const nativeElement = fixture.nativeElement;
            const imgs = nativeElement.querySelectorAll('img');
            (0, matchers_1.expect)(imgs[0].src).toBe(`${IMG_BASE_URL}/img.png?testProp1=testValue1&testProp2=testValue2`);
        });
        ...
        />`,
                ...
                var TestComponent = _classThis = class {
                    constructor() {
                        this.ngSrc = `${IMG_BASE_URL}/img.png`;
                        this.width = 300;
                        this.height = 300;
                        this.params = { transforms1: { example1: 'foo' }, transforms2: { example2: 'bar' } };
                    }
                };
                __setFunctionName(_classThis, "TestComponent");
                (() => {
            ...
            const nativeElement = fixture.nativeElement;
            const imgs = nativeElement.querySelectorAll('img');
            (0, matchers_1.expect)(imgs[0].src).toBe(`${IMG_BASE_URL}/img.png/foo/bar`);
        });
      ...
      `;
            ...
            const nativeElement = fixture.nativeElement;
            const imgs = nativeElement.querySelectorAll('img');
            (0, matchers_1.expect)(imgs[0].srcset).toBe(`${IMG_BASE_URL}/img.png?w=150&testProp1=testValue1&testProp2=testValue2 1x, ${IMG_BASE_URL}/img.png?w=300&testProp1=testValue1&testProp2=testValue2 2x`);
        });
      ...
      `;
        ...
        it(`should allow providing image loaders via Component providers`, (0, testing_2.withHead)('', () => {
            const createImgUrl = (path, config) => `${path}/${config.src}`;
            const loaderWithPath = (0, image_loader_1.createImageLoader)(createImgUrl);
            let TestComponent = (() => {
                let _classDecorators = [(0, core_1.Component)({
                        selector: 'test-cmp',
            ...
            const fixture = testing_1.TestBed.createComponent(TestComponent);
            fixture.detectChanges();
            const defaultLoader = testing_1.TestBed.inject(image_loader_1.IMAGE_LOADER);
            const nativeElement = fixture.nativeElement;
            const img = nativeElement.querySelector('img');
            (0, matchers_1.expect)(defaultLoader({ src: 'a.png' })).toBe('https://default.io/a.png');
            (0, matchers_1.expect)(img.src).toBe('https://component.io/a.png');
        }));
        describe('`ngSrcset` values', () => {
         ...
         `;
                ...
                const img = nativeElement.querySelector('img');
                (0, matchers_1.expect)(img.src).toBe(`${IMG_BASE_URL}/img.png`);
                (0, matchers_1.expect)(img.srcset).toBe(`${IMG_BASE_URL}/img.png?w=100 100w, ${IMG_BASE_URL}/img.png?w=200 200w`);
            });
         ...
         `;
                ...
                const img = nativeElement.querySelector('img');
                (0, matchers_1.expect)(img.src).toBe(`${IMG_BASE_URL}/img.png`);
                (0, matchers_1.expect)(img.srcset).toBe(`${IMG_BASE_URL}/img.png?w=100 1x, ${IMG_BASE_URL}/img.png?w=200 2x`);
            });
         ...
         `;
                ...
                const nativeElement = fixture.nativeElement;
                const img = nativeElement.querySelector('img');
                (0, matchers_1.expect)(img.src.trim()).toBe(`${IMG_BASE_URL}/img.png`);
                (0, matchers_1.expect)(img.srcset.trim()).toBe(`${IMG_BASE_URL}/img.png?w=100 100w`);
            });
         ...
         `;
                ...
                const img = nativeElement.querySelector('img');
                (0, matchers_1.expect)(img.src).toBe(`${IMG_BASE_URL}/img.png`);
                (0, matchers_1.expect)(img.srcset).toBe(`${IMG_BASE_URL}/img.png?w=100 100w, ` +
         ...
         `;
                ...
                const img = nativeElement.querySelector('img');
                (0, matchers_1.expect)(img.src).toBe(`${IMG_BASE_URL}/img.png`);
                (0, matchers_1.expect)(img.srcset).toBe(`${IMG_BASE_URL}/img.png?w=175 1.75x, ` +
         ...
         `;
                ...
                const img = nativeElement.querySelector('img');
                (0, matchers_1.expect)(img.src).toBe(`${IMG_BASE_URL}/img.png`);
                (0, matchers_1.expect)(img.sizes).toBe(`auto, 100vw`);
            });
         ...
         `;
                ...
                const img = nativeElement.querySelector('img');
                (0, matchers_1.expect)(img.src).toBe(`${IMG_BASE_URL}/img.png`);
                (0, matchers_1.expect)(img.sizes).toBe('');
            });
        });
        describe('sizes attribute', () => {
            it('should pass through the sizes attribute', () => {
                ...
                const nativeElement = fixture.nativeElement;
                const img = nativeElement.querySelector('img');
                (0, matchers_1.expect)(img.getAttribute('sizes')).toBe('(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw');
            });
            it('should prepend sizes="auto" to a lazy-loaded image', () => {
                ...
                const nativeElement = fixture.nativeElement;
                const img = nativeElement.querySelector('img');
                (0, matchers_1.expect)(img.getAttribute('sizes')).toBe('auto, (max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw');
            });
            it('should throw if a complex `sizes` is used', () => {
                setupTestingModule();
                const template = '<img ngSrc="path/img.png" width="100" height="50" sizes="(min-width: 768px) 500px, 100vw">';
                (0, matchers_1.expect)(() => {
                    const fixture = createTestComponent(template);
                    fixture.detectChanges();
                }).toThrowError('NG02952: The NgOptimizedImage directive has detected that `sizes` was set to a string including pixel values. ' +
            ...
            it('should throw if a complex `sizes` is used with srcset', () => {
                setupTestingModule();
                const template = '<img ngSrc="path/img.png" width="100" height="50" sizes="(min-width: 768px) 500px, 100vw" srcset="www.example.com/img.png?w=500 768w, www.example.com/img.png?w=2000" >';
                (0, matchers_1.expect)(() => {
                    const fixture = createTestComponent(template);
                    fixture.detectChanges();
                }).toThrowError('NG02952: The NgOptimizedImage directive has detected that `sizes` was set to a string including pixel values. ' +
            ...
            it('should not throw if a complex `sizes` is used with ngSrcset', () => {
                setupTestingModule();
                const template = '<img ngSrc="path/img.png" width="100" height="50" sizes="(min-width: 768px) 500px, 100vw" ngSrcset="100w, 200w">';
                (0, matchers_1.expect)(() => {
                    const fixture = createTestComponent(template);
                    fixture.detectChanges();
                }).not.toThrow();
        ...
        `;
                ...
                const nativeElement = fixture.nativeElement;
                const img = nativeElement.querySelector('img');
                (0, matchers_1.expect)(img.getAttribute('srcset')).toBeNull();
            });
    ...
    `;
                ...
                const nativeElement = fixture.nativeElement;
                const img = nativeElement.querySelector('img');
                (0, matchers_1.expect)(img.getAttribute('srcset')).toBe(`${IMG_BASE_URL}/img?w=640 640w, ${IMG_BASE_URL}/img?w=750 750w, ${IMG_BASE_URL}/img?w=828 828w, ${IMG_BASE_URL}/img?w=1080 1080w, ${IMG_BASE_URL}/img?w=1200 1200w, ${IMG_BASE_URL}/img?w=1920 1920w, ${IMG_BASE_URL}/img?w=2048 2048w, ${IMG_BASE_URL}/img?w=3840 3840w`);
            });
    ...
    `;
                ...
                const nativeElement = fixture.nativeElement;
                const img = nativeElement.querySelector('img');
                (0, matchers_1.expect)(img.getAttribute('srcset')).toBe(`${IMG_BASE_URL}/img?w=16 16w, ${IMG_BASE_URL}/img?w=32 32w, ${IMG_BASE_URL}/img?w=48 48w, ${IMG_BASE_URL}/img?w=64 64w, ${IMG_BASE_URL}/img?w=96 96w, ${IMG_BASE_URL}/img?w=128 128w, ${IMG_BASE_URL}/img?w=256 256w, ${IMG_BASE_URL}/img?w=384 384w, ${IMG_BASE_URL}/img?w=640 640w, ${IMG_BASE_URL}/img?w=750 750w, ${IMG_BASE_URL}/img?w=828 828w, ${IMG_BASE_URL}/img?w=1080 1080w, ${IMG_BASE_URL}/img?w=1200 1200w, ${IMG_BASE_URL}/img?w=1920 1920w, ${IMG_BASE_URL}/img?w=2048 2048w, ${IMG_BASE_URL}/img?w=3840 3840w`);
            });
    ...
    `;
                ...
                const nativeElement = fixture.nativeElement;
                const img = nativeElement.querySelector('img');
                (0, matchers_1.expect)(img.getAttribute('srcset')).toBe(`${IMG_BASE_URL}/img?w=100 1x, ${IMG_BASE_URL}/img?w=200 2x`);
            });
    ...
    `;
                ...
                const nativeElement = fixture.nativeElement;
                const img = nativeElement.querySelector('img');
                (0, matchers_1.expect)(img.getAttribute('srcset')).toBe(`${IMG_BASE_URL}/img?w=16 16w, ${IMG_BASE_URL}/img?w=48 48w, ${IMG_BASE_URL}/img?w=640 640w, ${IMG_BASE_URL}/img?w=1280 1280w, ${IMG_BASE_URL}/img?w=3840 3840w`);
            });
...
function setupTestingModule(config) {
    ...
    const loader = (config === null || config === void 0 ? void 0 : config.imageLoader) || defaultLoader;
    const extraProviders = (config === null || config === void 0 ? void 0 : config.extraProviders) || [];
    const providers = [
        { provide: index_1.DOCUMENT, useValue: window.document },
        ...((config === null || config === void 0 ? void 0 : config.noLoader) ? [] : [{ provide: image_loader_1.IMAGE_LOADER, useValue: loader }]),
        ...extraProviders,
    ];
    if (config === null || config === void 0 ? void 0 : config.imageConfig) {
        providers.push({ provide: index_1.IMAGE_CONFIG, useValue: config.imageConfig });
    }
    testing_1.TestBed.configureTestingModule({
        declarations: [(_a = config === null || config === void 0 ? void 0 : config.component) !== null && _a !== void 0 ? _a : TestComponent],
        // Note: the `NgOptimizedImage` directive is experimental and is not a part of the
        // `CommonModule` yet, so it's imported separately.
        imports: [index_1.CommonModule, ng_optimized_image_1.NgOptimizedImage],
        providers,
    });
}
...
function setUpModuleNoLoader() {
    testing_1.TestBed.configureTestingModule({
        ...
        imports: [index_1.CommonModule, ng_optimized_image_1.NgOptimizedImage],
        providers: [{ provide: index_1.DOCUMENT, useValue: window.document }],
    });
}
function createTestComponent(template, changeDetection = core_1.ChangeDetectionStrategy.OnPush) {
    return testing_1.TestBed.overrideComponent(TestComponent, {
        set: { template, changeDetection },
    }).createComponent(TestComponent);
}
function parseInlineStyles(img) {
    var _a;
    const styles = new Map();
    const rawStyles = (_a = img.getAttribute('style')) === null || _a === void 0 ? void 0 : _a.replace(/\s/g, '');
    if (rawStyles) {
