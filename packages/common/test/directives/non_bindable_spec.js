...
describe('non-bindable', () => {
    ...
    it('should not interpolate children', (0, testing_1.waitForAsync)(() => {
        const template = '<div>{{text}}<span ngNonBindable>{{text}}</span></div>';
        const fixture = createTestComponent(template);
        fixture.detectChanges();
        (0, matchers_1.expect)(fixture.nativeElement).toHaveText('foo{{text}}');
    }));
    it('should ignore directives on child nodes', (0, testing_1.waitForAsync)(() => {
        ...
        (0, matchers_1.expect)((0, browser_util_1.hasClass)(span, 'compiled')).toBeFalsy();
    }));
    it('should trigger directives on the same node', (0, testing_1.waitForAsync)(() => {
        const template = '<div><span id=child ngNonBindable test-dec>{{text}}</span></div>';
        const fixture = createTestComponent(template);
        fixture.detectChanges();
        const span = fixture.nativeElement.querySelector('#child');
        (0, matchers_1.expect)((0, browser_util_1.hasClass)(span, 'compiled')).toBeTruthy();
    }));
});
let TestDirective = (() => {
