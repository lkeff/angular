...
exports.generateClosureLocaleFile = generateClosureLocaleFile;
 ...
 */
function generateClosureLocaleFile(cldrData, baseCurrencies) {
    const locales = closureLibraryLocales.map((localeName) => {
...
`;
     ...
     */
    function generateLocaleConstants(locale) {
        ...
        const extraLocaleConstantNames = localeNamesToExpose.map((d) => `locale_extra_${formatLocale(d)}`);
...
${aliasExtraDataConstantNames
    ...
    function generateLocaleConstant(locale, constantName) {
        return (0, locale_file_1.generateLocale)(locale.canonicalLocaleName, locale.data, baseCurrencies)
            .replace(`${file_header_1.fileHeader}\n`, '')
    ...
    function generateLocaleExtraDataConstant(locale, constantName) {
        return `export const ${constantName} = ${(0, locale_extra_file_1.generateLocaleExtraDataArrayCode)(locale.canonicalLocaleName, locale.data)};`;
    }
    /** Generates a TypeScript `switch` case for the specified locale. */
    function generateCase(locale) {
...
function computeEquivalentLocaleNames(localeName) {
    var _a;
    const equivalents = new Set([localeName, formatLocale(localeName)]);
    (_a = closureLibraryAliases[localeName]) === null || _a === void 0 ? void 0 : _a.forEach((aliasName) => {
