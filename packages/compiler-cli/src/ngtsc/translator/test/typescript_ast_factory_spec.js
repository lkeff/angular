...
const typescript_ast_factory_1 = require("../src/typescript_ast_factory");
describe('TypeScriptAstFactory', () => {
    let factory;
    beforeEach(() => (factory = new typescript_ast_factory_1.TypeScriptAstFactory(/* annotateForClosureCompiler */ false)));
    describe('attachComments()', () => {
        it('should add the comments to the given statement', () => {
            ...
            factory.attachComments(stmt, [
                (0, compiler_1.leadingComment)('comment 1', true),
                (0, compiler_1.leadingComment)('comment 2', false),
            ]);
    ...
    describe('createBinaryExpression()', () => {
        ...
        it('should create a binary operation node for exponentiation', () => {
            const { items: [left, right], generate, } = setupExpressions(`2`, `3`);
            const assignment = factory.createBinaryExpression(left, '**', right);
    ...
    describe('createCallExpression()', () => {
        ...
        it('should create a call marked with a closure-style pure comment if `pure` is true', () => {
            factory = new typescript_ast_factory_1.TypeScriptAstFactory(/* annotateForClosureCompiler */ true);
            const { items: [callee, arg1, arg2], generate, } = setupExpressions(`foo`, `42`, `"moo"`);
            const call = factory.createCallExpression(callee, [arg1, arg2], true);
    ...
    describe('createConditional()', () => {
        it('should create a condition expression', () => {
            const { items: [test, thenExpr, elseExpr], generate, } = setupExpressions(`!test`, `42`, `"moo"`);
    ...
    describe('createExpressionStatement()', () => {
        it('should create a statement node from the given expression', () => {
            ...
            expect(typescript_1.default.isExpressionStatement(stmt)).toBe(true);
    ...
    describe('createFunctionExpression()', () => {
        it('should create a function expression node with the given name, parameters and body statements', () => {
            ...
            expect(typescript_1.default.isExpressionStatement(fn)).toBe(false);
            expect(generate(fn)).toEqual('function foo(arg1, arg2) { x = 10; y = 20; }');
        });
    ...
    describe('createIdentifier()', () => {
        it('should create an identifier with the given name', () => {
            const id = factory.createIdentifier('someId');
            expect(typescript_1.default.isIdentifier(id)).toBe(true);
            expect(id.text).toEqual('someId');
        });
    });
    describe('createIfStatement()', () => {
        it('should create an if-else statement', () => {
            const { items: [testStmt, thenStmt, elseStmt], generate, } = setupStatements('!test;x = 10;x = 42;');
            const test = testStmt.expression;
    ...
    describe('createLiteral()', () => {
        it('should create a string literal', () => {
            ...
            expect(typescript_1.default.isStringLiteral(literal)).toBe(true);
            expect(generate(literal)).toEqual('"moo"');
        });
        it('should create a number literal', () => {
            const { generate } = setupStatements();
            const literal = factory.createLiteral(42);
            expect(typescript_1.default.isNumericLiteral(literal)).toBe(true);
            expect(generate(literal)).toEqual('42');
        });
        it('should create a negative number literal', () => {
            const { generate } = setupStatements();
            const literal = factory.createLiteral(-42);
            expect(typescript_1.default.isPrefixUnaryExpression(literal)).toBe(true);
            expect(generate(literal)).toEqual('-42');
        });
    ...
    describe('setSourceMapRange()', () => {
        it('should attach the `sourceMapRange` to the given `node`', () => {
            ...
            const range = typescript_1.default.getSourceMapRange(expr);
            expect(range.pos).toEqual(1);
            expect(range.end).toEqual(15);
            expect((_a = range.source) === null || _a === void 0 ? void 0 : _a.getLineAndCharacterOfPosition(range.pos)).toEqual({
                line: 0,
                character: 1,
            });
            expect((_b = range.source) === null || _b === void 0 ? void 0 : _b.getLineAndCharacterOfPosition(range.end)).toEqual({
                line: 2,
                character: 3,
            });
        });
 ...
 */
function setupStatements(stmts = '') {
    const printer = typescript_1.default.createPrinter({ newLine: typescript_1.default.NewLineKind.LineFeed });
    const sf = typescript_1.default.createSourceFile('test.ts', stmts, typescript_1.default.ScriptTarget.ES2015, true);
    return {
        items: Array.from(sf.statements),
        generate: (node) => printer.printNode(typescript_1.default.EmitHint.Unspecified, node, sf),
    };
}
/**
 * Setup some statements to use in a test, along with a generate function to print the created nodes
 ...
 */
function setupExpressions(...exprs) {
    const { items: [arrayStmt], generate, } = setupStatements(`[${exprs.join(',')}];`);
    const expressions = Array.from(arrayStmt.expression.elements);
