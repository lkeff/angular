...
const import_manager_1 = require("../src/import_manager/import_manager");
describe('import manager', () => {
    it('should be possible to import a symbol', () => {
        const { testFile, emit } = createTestProgram('');
        const manager = new import_manager_1.ImportManager();
        const ref = manager.addImport({
            exportModuleSpecifier: '@angular/core',
            exportSymbolName: 'input',
            requestedFile: testFile,
        });
        const res = emit(manager, [typescript_1.default.factory.createExpressionStatement(ref)]);
    ...
    it('should be possible to import multiple symbols', () => {
        ...
        const res = emit(manager, [
            typescript_1.default.factory.createExpressionStatement(inputRef),
            typescript_1.default.factory.createExpressionStatement(outputRef),
        ]);
        expect(res).toBe(omitLeadingWhitespace(`
    ...
    it('should be possible to import multiple namespaces', () => {
        ...
        const res = emit(manager, [
            typescript_1.default.factory.createExpressionStatement(coreNamespace),
            typescript_1.default.factory.createExpressionStatement(interopNamespace),
        ]);
        expect(res).toBe(omitLeadingWhitespace(`
      import * as i0 from "@angular/core";
    ...
    `);
        ...
        const res = emit(manager, [typescript_1.default.factory.createExpressionStatement(inputRef)]);
        expect(res).toBe(omitLeadingWhitespace(`
      import * as existingImport from '@angular/core';
    ...
    `);
        ...
        const res = emit(manager, [typescript_1.default.factory.createExpressionStatement(coreRef)]);
        expect(res).toBe(omitLeadingWhitespace(`
      import * as existingImport from '@angular/core';
      existingImport;
    `));
    ...
    `);
        ...
        const res = emit(manager, [typescript_1.default.factory.createExpressionStatement(outputRef)]);
        expect(res).toBe(omitLeadingWhitespace(`
      import { input, output } from '@angular/core';
      output;
      const x = input();
    ...
    it('should be able to always prefer namespace imports for new imports, ' +
       ...
       `);
        ...
        const res = emit(manager, [typescript_1.default.factory.createExpressionStatement(blaRef)]);
        expect(res).toBe(omitLeadingWhitespace(`
        import { Dir } from 'bla';
        Dir;
        const x = new Dir();
    ...
    it('should avoid an import specifier alias if similar import is generated in different', () => {
        ...
        manager.addImport({
            exportModuleSpecifier: '@angular/core',
            exportSymbolName: 'input',
            requestedFile: typescript_1.default.createSourceFile('other_file', '', typescript_1.default.ScriptTarget.Latest),
        });
        const inputRef = manager.addImport({
            exportModuleSpecifier: '@angular/core',
            exportSymbolName: 'input',
    ...
    it('should avoid collisions with generated identifiers', () => {
        ...
        const res = emit(manager, [
            typescript_1.default.factory.createExpressionStatement(inputRef),
            typescript_1.default.factory.createExpressionStatement(inputRef2),
        ]);
        expect(res).toBe(omitLeadingWhitespace(`
      import { input } from "@angular/core";
    ...
    `);
        ...
        const res = emit(manager, [typescript_1.default.factory.createExpressionStatement(fooRef)]);
        expect(res).toBe(omitLeadingWhitespace(`
    ...
    it('should reuse import if both the name and alias are the same when added through `addImport`', () => {
        ...
        const res = emit(manager, [
            typescript_1.default.factory.createExpressionStatement(firstRef),
            typescript_1.default.factory.createExpressionStatement(secondRef),
        ]);
        expect(res).toBe(omitLeadingWhitespace(`
        import { foo as bar } from "@angular/core";
        bar;
    ...
    it('should not reuse import if symbol is imported under a different alias', () => {
        ...
        const res = emit(manager, [
            typescript_1.default.factory.createExpressionStatement(barRef),
            typescript_1.default.factory.createExpressionStatement(bazRef),
        ]);
        expect(res).toBe(omitLeadingWhitespace(`
    ...
    it('should not attempt to de-duplicate imports with an explicit alias', () => {
        ...
        const res = emit(manager, [
            typescript_1.default.factory.createExpressionStatement(fooRef),
            typescript_1.default.factory.createExpressionStatement(barRef),
        ]);
        expect(res).toBe(omitLeadingWhitespace(`
    ...
    `);
        ...
        manager.removeImport(testFile, 'output', '@angular/core');
        expect(emit(manager, [])).toBe(omitLeadingWhitespace(`
      input();
      output();
      export {};
    `));
    ...
    it('should work when using an isolated transform', () => {
        ...
        const extraStatements = [typescript_1.default.factory.createExpressionStatement(ref)];
        const transformer = manager.toTsTransform(new Map([[testFile.fileName, extraStatements]]));
        const result = typescript_1.default.transform(testFile, [transformer]);
        ...
        expect(output).toBe(omitLeadingWhitespace(`
      import { input } from "@angular/core";
      input;
    `));
    });
});
function createTestProgram(text) {
    ...
    fs.writeFile((0, file_system_1.absoluteFrom)('/test.ts'), text);
    const program = typescript_1.default.createProgram({
        rootNames: ['/test.ts'],
        options,
        host: new testing_2.NgtscTestCompilerHost(fs, options),
    });
    const testFile = program.getSourceFile('/test.ts');
    if (testFile === undefined) {
        throw new Error('Could not get test source file from program.');
    }
    const emit = (manager, newStatements) => {
