...
function canEmitType(type, canEmit) {
    ...
    function visitNode(node) {
        ...
        // the walk.
        if (typescript_1.default.isTypeReferenceNode(node) && !canEmitTypeReference(node)) {
            return INELIGIBLE;
        }
        else {
            return typescript_1.default.forEachChild(node, visitNode);
        }
    }
...
class TypeEmitter {
    ...
    emitType(type) {
        const typeReferenceTransformer = (context) => {
            const visitNode = (node) => {
                ...
                if (typescript_1.default.isTypeReferenceNode(node)) {
                    ...
                    if (typescript_1.default.isStringLiteral(node)) {
                        clone = typescript_1.default.factory.createStringLiteral(node.text);
                    }
                    else if (typescript_1.default.isNumericLiteral(node)) {
                        clone = typescript_1.default.factory.createNumericLiteral(node.text);
                    }
                    else if (typescript_1.default.isBigIntLiteral(node)) {
                        clone = typescript_1.default.factory.createBigIntLiteral(node.text);
                    }
                    else if (typescript_1.default.isNoSubstitutionTemplateLiteral(node)) {
                        clone = typescript_1.default.factory.createNoSubstitutionTemplateLiteral(node.text, node.rawText);
                    }
                    else if (typescript_1.default.isRegularExpressionLiteral(node)) {
                        clone = typescript_1.default.factory.createRegularExpressionLiteral(node.text);
                    }
                    else {
                        throw new Error(`Unsupported literal kind ${typescript_1.default.SyntaxKind[node.kind]}`);
                    }
                    typescript_1.default.setTextRange(clone, { pos: -1, end: -1 });
                    return clone;
                }
                else {
            ...
            return (node) => typescript_1.default.visitNode(node, visitNode, typescript_1.default.isTypeNode);
        };
        return typescript_1.default.transform(type, [typeReferenceTransformer]).transformed[0];
    }
    emitTypeReference(type) {
        ...
        if (type.typeArguments !== undefined) {
            typeArguments = typescript_1.default.factory.createNodeArray(type.typeArguments.map((typeArg) => this.emitType(typeArg)));
        }
        return typescript_1.default.factory.updateTypeReferenceNode(type, translatedType.typeName, typeArguments);
    }
}
