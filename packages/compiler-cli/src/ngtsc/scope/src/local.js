...
class LocalModuleScopeRegistry {
    ...
    getScopeOfModuleReference(ref) {
        ...
        for (const decl of ngModule.imports) {
            ...
            if (directive !== null) {
                if (directive.isStandalone) {
                    ...
                    diagnostics.push((0, util_1.makeNotStandaloneDiagnostic)(this, decl, ngModule.rawImports, directive.isComponent ? 'component' : 'directive'));
                    isPoisoned = true;
                }
                continue;
            }
            ...
            if (pipe !== null) {
                if (pipe.isStandalone) {
                    ...
                    diagnostics.push((0, util_1.makeNotStandaloneDiagnostic)(this, decl, ngModule.rawImports, 'pipe'));
                    isPoisoned = true;
                }
                continue;
            }
        ...
        for (const decl of ngModule.declarations) {
            ...
            if (directive !== null) {
                if (directive.isStandalone) {
                    ...
                    diagnostics.push((0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.NGMODULE_DECLARATION_IS_STANDALONE, decl.getOriginForDiagnostics(ngModule.rawDeclarations), `${refType} ${decl.node.name.text} is standalone, and cannot be declared in an NgModule. Did you mean to import it instead?`));
                    isPoisoned = true;
                    continue;
                }
                ...
                if (pipe.isStandalone) {
                    diagnostics.push((0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.NGMODULE_DECLARATION_IS_STANDALONE, decl.getOriginForDiagnostics(ngModule.rawDeclarations), `Pipe ${decl.node.name.text} is standalone, and cannot be declared in an NgModule. Did you mean to import it instead?`));
                    isPoisoned = true;
                    continue;
                }
        ...
        const scope = {
            kind: api_1.ComponentScopeKind.NgModule,
    ...
    getExportedScope(ref, diagnostics, ownerForErrors, type) {
        if (ref.node.getSourceFile().isDeclarationFile) {
            ...
            if (!typescript_1.default.isClassDeclaration(ref.node)) {
                ...
                const code = type === 'import' ? diagnostics_1.ErrorCode.NGMODULE_INVALID_IMPORT : diagnostics_1.ErrorCode.NGMODULE_INVALID_EXPORT;
                diagnostics.push((0, diagnostics_1.makeDiagnostic)(code, (0, typescript_2.identifierOfNode)(ref.node) || ref.node, `Appears in the NgModule.${type}s of ${(0, typescript_2.nodeNameForError)(ownerForErrors)}, but could not be resolved to an NgModule`));
                return 'invalid';
            }
            ...
            if (this.cache.get(ref.node) === IN_PROGRESS_RESOLUTION) {
                diagnostics.push((0, diagnostics_1.makeDiagnostic)(type === 'import'
                    ? diagnostics_1.ErrorCode.NGMODULE_INVALID_IMPORT
                    : diagnostics_1.ErrorCode.NGMODULE_INVALID_EXPORT, (0, typescript_2.identifierOfNode)(ref.node) || ref.node, `NgModule "${type}" field contains a cycle`));
                return 'cycle';
            }
        ...
        const addReexport = (exportRef) => {
            ...
            if (!reexportMap.has(exportName)) {
                if (exportRef.alias && exportRef.alias instanceof compiler_1.ExternalExpr) {
                    ...
                    (0, imports_1.assertSuccessfulReferenceEmit)(emittedRef, ngModuleRef.node.name, 'class');
...
function invalidRef(decl, rawExpr, type) {
    ...
    return (0, diagnostics_1.makeDiagnostic)(code, (0, util_1.getDiagnosticNode)(decl, rawExpr), message, [
        (0, diagnostics_1.makeRelatedInformation)(decl.node.name, relatedMessage),
    ]);
}
/**
...
function invalidTransitiveNgModuleRef(decl, rawExpr, type) {
    const code = type === 'import' ? diagnostics_1.ErrorCode.NGMODULE_INVALID_IMPORT : diagnostics_1.ErrorCode.NGMODULE_INVALID_EXPORT;
    return (0, diagnostics_1.makeDiagnostic)(code, (0, util_1.getDiagnosticNode)(decl, rawExpr), `This ${type} contains errors, which may affect components that depend on this NgModule.`);
}
/**
...
function invalidReexport(decl, rawExpr, isStandalone) {
    ...
    return (0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.NGMODULE_INVALID_REEXPORT, (0, util_1.getDiagnosticNode)(decl, rawExpr), message);
}
/**
