...
class TemplateVisitor extends compiler_1.CombinedRecursiveAstVisitor {
    ...
    constructor(boundTemplate) {
        ...
        this.errors = [];
        this.currentAstWithSource = null;
        ...
        this.directiveHostIdentifierCache = new Map();
    }
    /**
    ...
    visitBoundAttribute(attribute) {
        ...
        this.currentAstWithSource = {
            source: ((_a = attribute.valueSpan) === null || _a === void 0 ? void 0 : _a.toString()) || null,
            absoluteOffset: attribute.valueSpan ? attribute.valueSpan.start.offset : -1,
        };
        this.visit(attribute.value instanceof compiler_1.ASTWithSource ? attribute.value.ast : attribute.value);
    ...
    directiveHostToIdentifier(node) {
        ...
        if (node instanceof compiler_1.TmplAstTemplate) {
            name = (_a = node.tagName) !== null && _a !== void 0 ? _a : 'ng-template';
            ...
            name = node.fullName;
            kind = api_1.IdentifierKind.Component;
        }
        else {
            name = node.name;
            kind = api_1.IdentifierKind.Directive;
        }
        ...
        if ((node instanceof compiler_1.TmplAstTemplate || node instanceof compiler_1.TmplAstElement) &&
            name.startsWith(':')) {
            name = name.split(':').pop();
        }
        ...
        const absoluteSpan = new api_1.AbsoluteSourceSpan(start, start + name.length);
    ...
    targetToIdentifier(node) {
        ...
        const span = new api_1.AbsoluteSourceSpan(start, start + name.length);
        let identifier;
        if (node instanceof compiler_1.TmplAstReference) {
            ...
            if (refTarget) {
                let node = null;
                let directive = null;
                if (refTarget instanceof compiler_1.TmplAstElement ||
                    refTarget instanceof compiler_1.TmplAstTemplate ||
                    refTarget instanceof compiler_1.TmplAstComponent ||
                    refTarget instanceof compiler_1.TmplAstDirective) {
            ...
            identifier = {
                name,
                span,
                kind: api_1.IdentifierKind.LetDeclaration,
            };
        }
    ...
    visitIdentifier(ast, kind) {
        ...
        if (ast instanceof compiler_1.PropertyRead || ast instanceof compiler_1.PropertyWrite) {
        ...
        const span = new api_1.AbsoluteSourceSpan(absoluteStart, absoluteStart + ast.name.length);
...
function getTemplateIdentifiers(boundTemplate) {
    ...
    if (boundTemplate.target.template !== undefined) {
        (0, compiler_1.tmplAstVisitAll)(visitor, boundTemplate.target.template);
    }
