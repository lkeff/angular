...
class ImportGraph {
    ...
    scanImports(sf) {
        return this.perf.inPhase(perf_1.PerfPhase.CycleDetection, () => {
            const imports = new Set();
            // Look through the source file for import and export statements.
            for (const stmt of sf.statements) {
                if ((!typescript_1.default.isImportDeclaration(stmt) && !typescript_1.default.isExportDeclaration(stmt)) ||
                ...
                if (typescript_1.default.isSourceFile(moduleFile) && isLocalFile(moduleFile)) {
...
function isTypeOnlyImportClause(node) {
    ...
    if (node.namedBindings !== undefined &&
        typescript_1.default.isNamedImports(node.namedBindings) &&
