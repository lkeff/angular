...
class AdapterResourceLoader {
    ...
    getRootedCandidateLocations(url) {
        ...
        const segment = ('.' + url);
        return this.adapter.rootDirs.map((rootDir) => (0, file_system_1.join)(rootDir, segment));
    }
    /**
    ...
    getResolvedCandidateLocations(url, fromFile) {
        const failedLookup = typescript_1.default.resolveModuleName(url + RESOURCE_MARKER, fromFile, this.options, this.lookupResolutionHost);
