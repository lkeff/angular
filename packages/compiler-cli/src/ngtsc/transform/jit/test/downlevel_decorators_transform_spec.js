...
describe('downlevel decorator transform', () => {
    ...
    function transform(contents, compilerOptions = {}, preTransformers = []) {
        context.writeFile(TEST_FILE_INPUT, contents);
        const program = typescript_1.default.createProgram([TEST_FILE_INPUT, '/dom_globals.d.ts'], Object.assign({ module: typescript_1.default.ModuleKind.CommonJS, importHelpers: true, lib: ['dom', 'es2015'], target: typescript_1.default.ScriptTarget.ES2017, declaration: true, experimentalDecorators: true, emitDecoratorMetadata: false }, compilerOptions), host);
        ...
        const transformers = {
            before: [
                ...preTransformers,
                (0, index_1.getDownlevelDecoratorsTransform)(program.getTypeChecker(), reflectionHost, diagnostics, 
    ...
    it('should not retain unused type imports due to decorator downleveling with ' +
     ...
     `, { module: typescript_1.default.ModuleKind.ES2015, emitDecoratorMetadata: true });
        expect(diagnostics.length).toBe(0);
    ...
    it('should not retain unused type imports due to decorator downleveling with ' +
     ...
     `, { module: typescript_1.default.ModuleKind.ES2015, emitDecoratorMetadata: false });
        expect(diagnostics.length).toBe(0);
        expect(output).not.toContain('Directive');
        expect(output).not.toContain('ErrorHandler');
    });
    ...
    it('should create diagnostic if property name is non-serializable', () => {
     ...
     `);
        ...
        expect(diagnostics[0].messageText).toBe(`Cannot process decorators for class element with non-analyzable name.`);
    });
    ...
    it('should allow preceding custom transformers to strip decorators', () => {
        const stripAllDecoratorsTransform = (context) => {
            return (sourceFile) => {
                const visitNode = (node) => {
                    if (typescript_1.default.isClassDeclaration(node)) {
                        return typescript_1.default.factory.createClassDeclaration(typescript_1.default.getModifiers(node), node.name, node.typeParameters, node.heritageClauses, node.members);
                    }
    ...
    it('should work using an isolated transform operation', () => {
       ...
       `);
        ...
        const result = typescript_1.default.transform(program.getSourceFile('/foo.ts'), [
            ...((_a = transformers.before) !== null && _a !== void 0 ? _a : []),
            ...((_b = transformers.after) !== null && _b !== void 0 ? _b : []),
        ]);
        expect((_d = (_c = result.diagnostics) === null || _c === void 0 ? void 0 : _c.length) !== null && _d !== void 0 ? _d : 0).toBe(0);
    ...
    function createProgramWithTransform(files) {
