...
exports.queryFunctionsTransforms = void 0;
...
const queryFunctionsTransforms = (member, sourceFile, host, factory, importTracker, importManager, classDecorator, isCore) => {
    ...
    const queryDecorators = decorators && (0, annotations_1.getAngularDecorators)(decorators, annotations_1.queryDecoratorNames, isCore);
    if (queryDecorators !== null && queryDecorators.length > 0) {
        return member.node;
    }
    const queryDefinition = (0, annotations_1.tryParseSignalQueryFromInitializer)(member, host, importTracker);
    if (queryDefinition === null) {
        return member.node;
    }
    const callArgs = queryDefinition.call.arguments;
    const newDecorator = factory.createDecorator(factory.createCallExpression((0, transform_api_1.createSyntheticAngularCoreDecoratorAccess)(factory, importManager, classDecorator, sourceFile, queryFunctionToDecorator[queryDefinition.name]), undefined, 
    ...
    [
        ...
        (0, transform_api_1.castAsAny)(factory, factory.createObjectLiteralExpression([
            ...(callArgs.length > 1 ? [factory.createSpreadAssignment(callArgs[1])] : []),
            factory.createPropertyAssignment('isSignal', factory.createTrue()),
        ])),
    ]));
    return factory.updatePropertyDeclaration(member.node, [newDecorator, ...((_a = member.node.modifiers) !== null && _a !== void 0 ? _a : [])], member.node.name, member.node.questionToken, member.node.type, member.node.initializer);
};
exports.queryFunctionsTransforms = queryFunctionsTransforms;
