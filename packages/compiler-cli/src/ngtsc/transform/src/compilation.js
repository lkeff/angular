...
class TraitCompiler {
    ...
    analyze(sf, preanalyze) {
        ...
        const priorWork = this.compilationMode !== api_1.CompilationMode.LOCAL
    ...
    adopt(priorRecord) {
        ...
        for (const priorTrait of priorRecord.traits) {
            ...
            let trait = trait_1.Trait.pending(handler, priorTrait.detected);
    ...
    detectTraits(clazz, decorators) {
        ...
        const nonNgDecoratorsInLocalMode = this.compilationMode === api_1.CompilationMode.LOCAL ? new Set(decorators) : null;
        for (const handler of this.handlers) {
            ...
            const isPrimaryHandler = handler.precedence === api_1.HandlerPrecedence.PRIMARY;
            const isWeakHandler = handler.precedence === api_1.HandlerPrecedence.WEAK;
            const trait = trait_1.Trait.pending(handler, result);
            ...
            if (record === null) {
                ...
                if (!isWeakHandler && record.hasWeakHandlers) {
                    ...
                    // Remove them.
                    record.traits = record.traits.filter((field) => field.handler.precedence !== api_1.HandlerPrecedence.WEAK);
                    ...
                    record.metaDiagnostics = [
                        {
                            ...
                            file: (0, typescript_2.getSourceFile)(clazz),
        ...
        if (nonNgDecoratorsInLocalMode !== null &&
            ...
            record.metaDiagnostics = [...nonNgDecoratorsInLocalMode].map((decorator) => ({
                category: typescript_1.default.DiagnosticCategory.Error,
                code: Number('-99' + diagnostics_1.ErrorCode.DECORATOR_UNEXPECTED),
                file: (0, typescript_2.getSourceFile)(clazz),
    ...
    makeSymbolForTrait(handler, decl, analysis) {
        ...
        if (symbol !== null && this.semanticDepGraphUpdater !== null) {
            const isPrimary = handler.precedence === api_1.HandlerPrecedence.PRIMARY;
            if (!isPrimary) {
     ...
     */
    typeCheck(sf, ctx) {
        if (!this.fileToClasses.has(sf) || this.compilationMode === api_1.CompilationMode.LOCAL) {
            return;
        }
    ...
    runAdditionalChecks(sf, check) {
        if (this.compilationMode === api_1.CompilationMode.LOCAL) {
            return [];
        }
    ...
    compile(clazz, constantPool) {
        ...
        for (const trait of record.traits) {
            ...
            if (this.compilationMode === api_1.CompilationMode.LOCAL) {
                ...
                // `Readonly<unknown>` is nullable (as `unknown` itself is nullable) due to the way that
                // `Readonly` works.
                if (this.compilationMode === api_1.CompilationMode.PARTIAL &&
    ...
    decoratorsFor(node) {
        const original = typescript_1.default.getOriginalNode(node);
        ...
        for (const trait of record.traits) {
            ...
            if (this.compilationMode !== api_1.CompilationMode.LOCAL && trait.state !== trait_1.TraitState.Resolved) {
                continue;
            }
            if (trait.detected.trigger !== null && typescript_1.default.isDecorator(trait.detected.trigger)) {
    ...
    get diagnostics() {
        ...
        for (const clazz of this.classes.keys()) {
            ...
            for (const trait of record.traits) {
                ...
                if (trait.state === trait_1.TraitState.Resolved) {
                    diagnostics.push(...((_a = trait.resolveDiagnostics) !== null && _a !== void 0 ? _a : []));
                }
            }
        }
        return diagnostics;
...
function containsErrors(diagnostics) {
    return (diagnostics !== null &&
        diagnostics.some((diag) => diag.category === typescript_1.default.DiagnosticCategory.Error));
}
