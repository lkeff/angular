...
const BINARY_OPERATORS = new Map([
    [typescript_1.default.SyntaxKind.PlusToken, literalBinaryOp((a, b) => a + b)],
    [typescript_1.default.SyntaxKind.MinusToken, literalBinaryOp((a, b) => a - b)],
    [typescript_1.default.SyntaxKind.AsteriskToken, literalBinaryOp((a, b) => a * b)],
    [typescript_1.default.SyntaxKind.SlashToken, literalBinaryOp((a, b) => a / b)],
    [typescript_1.default.SyntaxKind.PercentToken, literalBinaryOp((a, b) => a % b)],
    [typescript_1.default.SyntaxKind.AmpersandToken, literalBinaryOp((a, b) => a & b)],
    [typescript_1.default.SyntaxKind.BarToken, literalBinaryOp((a, b) => a | b)],
    [typescript_1.default.SyntaxKind.CaretToken, literalBinaryOp((a, b) => a ^ b)],
    [typescript_1.default.SyntaxKind.LessThanToken, literalBinaryOp((a, b) => a < b)],
    [typescript_1.default.SyntaxKind.LessThanEqualsToken, literalBinaryOp((a, b) => a <= b)],
    [typescript_1.default.SyntaxKind.GreaterThanToken, literalBinaryOp((a, b) => a > b)],
    [typescript_1.default.SyntaxKind.GreaterThanEqualsToken, literalBinaryOp((a, b) => a >= b)],
    [typescript_1.default.SyntaxKind.EqualsEqualsToken, literalBinaryOp((a, b) => a == b)],
    [typescript_1.default.SyntaxKind.EqualsEqualsEqualsToken, literalBinaryOp((a, b) => a === b)],
    [typescript_1.default.SyntaxKind.ExclamationEqualsToken, literalBinaryOp((a, b) => a != b)],
    [typescript_1.default.SyntaxKind.ExclamationEqualsEqualsToken, literalBinaryOp((a, b) => a !== b)],
    [typescript_1.default.SyntaxKind.LessThanLessThanToken, literalBinaryOp((a, b) => a << b)],
    [typescript_1.default.SyntaxKind.GreaterThanGreaterThanToken, literalBinaryOp((a, b) => a >> b)],
    [typescript_1.default.SyntaxKind.GreaterThanGreaterThanGreaterThanToken, literalBinaryOp((a, b) => a >>> b)],
    [typescript_1.default.SyntaxKind.AsteriskAsteriskToken, literalBinaryOp((a, b) => Math.pow(a, b))],
    [typescript_1.default.SyntaxKind.AmpersandAmpersandToken, referenceBinaryOp((a, b) => a && b)],
    [typescript_1.default.SyntaxKind.BarBarToken, referenceBinaryOp((a, b) => a || b)],
]);
const UNARY_OPERATORS = new Map([
    [typescript_1.default.SyntaxKind.TildeToken, (a) => ~a],
    [typescript_1.default.SyntaxKind.MinusToken, (a) => -a],
    [typescript_1.default.SyntaxKind.PlusToken, (a) => +a],
    [typescript_1.default.SyntaxKind.ExclamationToken, (a) => !a],
]);
class StaticInterpreter {
    ...
    visitTemplateExpression(node, context) {
        ...
        for (let i = 0; i < node.templateSpans.length; i++) {
            ...
            const value = literal(this.visit(span.expression, context), () => dynamic_1.DynamicValue.fromDynamicString(span.expression));
    ...
    visitIdentifier(node, context) {
        ...
        if (decl === null) {
            if (typescript_1.default.identifierToKeywordKind(node) === typescript_1.default.SyntaxKind.UndefinedKeyword) {
                return undefined;
            }
            else {
    ...
    visitDeclaration(node, context) {
        ...
        else if (typescript_1.default.isParameter(node) && context.scope.has(node)) {
    ...
    visitSourceFile(node, context) {
        ...
        return new result_1.ResolvedModule(declarations, (decl) => {
            const declContext = Object.assign(Object.assign({}, context), joinModuleContext(context, node, decl));
    ...
    accessHelper(node, lhs, rhs, context) {
        ...
        if (lhs instanceof Map) {
            ...
            return new builtin_1.StringConcatBuiltinFn(lhs);
        }
            ...
            else if ((0, typescript_2.isDeclaration)(ref)) {
        ...
        else if (lhs instanceof synthetic_1.SyntheticValue) {
            return dynamic_1.DynamicValue.fromSyntheticInput(node, lhs);
        }
        return dynamic_1.DynamicValue.fromUnknown(node);
    }
    visitCallExpression(node, context) {
        ...
        const resolveFfrExpr = (expr) => {
            ...
            return this.visitFfrExpression(expr, Object.assign(Object.assign({}, context), contextExtension));
        };
        // If the function is foreign (declared through a d.ts file), attempt to resolve it with the
        // foreignFunctionResolver, if one is specified.
        if (fn.body === null && context.foreignFunctionResolver !== undefined) {
            const unresolvable = dynamic_1.DynamicValue.fromDynamicInput(node, dynamic_1.DynamicValue.fromExternalReference(node.expression, lhs));
        ...
        if (res instanceof dynamic_1.DynamicValue && context.foreignFunctionResolver !== undefined) {
            const unresolvable = dynamic_1.DynamicValue.fromComplexFunctionCall(node, fn);
            return context.foreignFunctionResolver(lhs, node, resolveFfrExpr, unresolvable);
        }
        return res;
    }
    ...
    visitFunctionBody(node, fn, context) {
        ...
        else if (fn.body.length !== 1 || !typescript_1.default.isReturnStatement(fn.body[0])) {
    ...
    visitBinaryExpression(node, context) {
        ...
        let lhs, rhs;
        if (opRecord.literal) {
            lhs = literal(this.visitExpression(node.left, context), (value) => dynamic_1.DynamicValue.fromInvalidExpressionType(node.left, value));
            rhs = literal(this.visitExpression(node.right, context), (value) => dynamic_1.DynamicValue.fromInvalidExpressionType(node.right, value));
        }
        else {
    ...
    visitBindingElement(node, context) {
        ...
        while (typescript_1.default.isBindingElement(closestDeclaration) ||
            typescript_1.default.isArrayBindingPattern(closestDeclaration) ||
            typescript_1.default.isObjectBindingPattern(closestDeclaration)) {
            if (typescript_1.default.isBindingElement(closestDeclaration)) {
                path.unshift(closestDeclaration);
            }
            closestDeclaration = closestDeclaration.parent;
        }
        if (!typescript_1.default.isVariableDeclaration(closestDeclaration) ||
        ...
        for (const element of path) {
            let key;
            if (typescript_1.default.isArrayBindingPattern(element.parent)) {
    ...
    stringNameFromPropertyName(node, context) {
        ...
        else if (typescript_1.default.isComputedPropertyName(node)) {
    ...
    visitType(node, context) {
        ...
        else if (typescript_1.default.isNamedTupleMember(node)) {
            return this.visitType(node.type, context);
        }
        else if (typescript_1.default.isTypeOperatorNode(node) && node.operator === typescript_1.default.SyntaxKind.ReadonlyKeyword) {
            return this.visitType(node.type, context);
        }
    ...
    visitTypeQuery(node, context) {
        ...
        if (decl === null) {
            return dynamic_1.DynamicValue.fromUnknownIdentifier(node.exprName);
        }
        const declContext = Object.assign(Object.assign({}, context), joinModuleContext(context, node, decl));
        return this.visitDeclaration(decl.node, declContext);
    }
...
function isFunctionOrMethodReference(ref) {
    return (typescript_1.default.isFunctionDeclaration(ref.node) ||
        typescript_1.default.isMethodDeclaration(ref.node) ||
        typescript_1.default.isFunctionExpression(ref.node));
}
...
function isVariableDeclarationDeclared(node) {
    if (node.parent === undefined || !typescript_1.default.isVariableDeclarationList(node.parent)) {
        return false;
    }
    const declList = node.parent;
    if (declList.parent === undefined || !typescript_1.default.isVariableStatement(declList.parent)) {
        return false;
    }
    const varStmt = declList.parent;
    const modifiers = typescript_1.default.getModifiers(varStmt);
    return (modifiers !== undefined && modifiers.some((mod) => mod.kind === typescript_1.default.SyntaxKind.DeclareKeyword));
}
const EMPTY = {};
