...
const semantic_graph_1 = require("../semantic_graph");
const dependency_tracking_1 = require("./dependency_tracking");
...
class IncrementalCompilation {
    ...
    static incremental(program, newVersions, oldProgram, oldState, modifiedResourceFiles, perf) {
        return perf.inPhase(perf_1.PerfPhase.Reconciliation, () => {
            ...
            const deletedTsFiles = new Set(oldFilesArray.map((sf) => (0, file_system_1.absoluteFromSourceFile)(sf)));
            ...
            // files.
            const depGraph = new dependency_tracking_1.FileDependencyGraph();
...
function toOriginalSourceFile(sf) {
    const unredirectedSf = (0, typescript_1.toUnredirectedSourceFile)(sf);
    const originalFile = unredirectedSf[program_driver_1.NgOriginalFile];
