...
exports.queryDecoratorNames = void 0;
...
exports.parseFieldStringArrayValue = parseFieldStringArrayValue;
...
const EMPTY_OBJECT = {};
exports.queryDecoratorNames = [
    'ViewChild',
    'ViewChildren',
    'ContentChild',
    'ContentChildren',
];
const QUERY_TYPES = new Set(exports.queryDecoratorNames);
/**
...
function extractDirectiveMetadata(clazz, decorator, reflector, importTracker, evaluator, refEmitter, referencesRegistry, isCore, annotateForClosureCompiler, compilationMode, defaultSelector, strictStandalone, implicitStandaloneValue) {
    ...
    const inputs = metadata_1.ClassPropertyMapping.fromMappedObject(Object.assign(Object.assign({}, inputsFromMeta), inputsFromFields));
    // And outputs.
    ...
    const outputs = metadata_1.ClassPropertyMapping.fromMappedObject(Object.assign(Object.assign({}, outputsFromMeta), outputsFromFields));
    ...
    if (directive.has('queries')) {
        ...
        const checkAndUnwrapQuery = (q) => {
            if (signalQueryFields.has(q.metadata.propertyName)) {
                throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.INITIALIZER_API_DECORATOR_METADATA_COLLISION, q.expr, `Query is declared multiple times. "@${decorator.name}" declares a query for the same property.`);
            }
            return q.metadata;
        };
    ...
    if (directive.has('selector')) {
        ...
        (0, common_1.assertLocalCompilationUnresolvedConst)(compilationMode, resolved, null, 'Unresolved identifier found for @Component.selector field! Did you ' +
        ...
        if (typeof resolved !== 'string') {
            throw (0, common_1.createValueHasWrongTypeError)(expr, resolved, `selector must be a string`);
        }
    ...
    const providers = directive.has('providers')
        ? new compiler_1.WrappedNodeExpr(annotateForClosureCompiler
            ? (0, common_1.wrapFunctionExpressionsInParens)(directive.get('providers'))
            : directive.get('providers'))
        : null;
    // Determine if `ngOnChanges` is a lifecycle hook defined on the component.
    const usesOnChanges = members.some((member) => !member.isStatic && member.kind === reflection_1.ClassMemberKind.Method && member.name === 'ngOnChanges');
    // Parse exportAs.
    let exportAs = null;
    if (directive.has('exportAs')) {
        const expr = directive.get('exportAs');
        const resolved = evaluator.evaluate(expr);
        (0, common_1.assertLocalCompilationUnresolvedConst)(compilationMode, resolved, null, 'Unresolved identifier found for exportAs field! Did you import this ' +
        ...
        if (typeof resolved !== 'string') {
            throw (0, common_1.createValueHasWrongTypeError)(expr, resolved, `exportAs must be a string`);
        }
        exportAs = resolved.split(',').map((part) => part.trim());
    }
    const rawCtorDeps = (0, common_1.getConstructorDependencies)(clazz, reflector, isCore);
    // Non-abstract directives (those with a selector) require valid constructor dependencies, whereas
    // abstract directives are allowed to have invalid dependencies, given that a subclass may call
    // the constructor explicitly.
    const ctorDeps = selector !== null
        ? (0, common_1.validateConstructorDependencies)(clazz, rawCtorDeps)
        : (0, common_1.unwrapConstructorDependencies)(rawCtorDeps);
    // Structural directives must have a `TemplateRef` dependency.
    const isStructural = ctorDeps !== null &&
        ctorDeps !== 'invalid' &&
        ctorDeps.some((dep) => dep.token instanceof compiler_1.ExternalExpr &&
    ...
    if (directive.has('standalone')) {
        ...
        if (!isStandalone && strictStandalone) {
            throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.NON_STANDALONE_NOT_ALLOWED, expr, `Only standalone components/directives are allowed when 'strictStandalone' is enabled.`);
        }
    }
    let isSignal = false;
    if (directive.has('signals')) {
        ...
        const resolved = evaluator.evaluate(expr);
        if (typeof resolved !== 'boolean') {
            throw (0, common_1.createValueHasWrongTypeError)(expr, resolved, `signals flag must be a boolean`);
        }
        isSignal = resolved;
    }
    ...
    const type = (0, common_1.wrapTypeReference)(reflector, clazz);
    const rawHostDirectives = directive.get('hostDirectives') || null;
    const hostDirectives = rawHostDirectives === null
        ? null
        : extractHostDirectives(rawHostDirectives, evaluator, compilationMode, (0, common_1.createForwardRefResolver)(isCore));
    if (compilationMode !== transform_1.CompilationMode.LOCAL && hostDirectives !== null) {
        ...
        referencesRegistry.add(clazz, ...hostDirectives.map((hostDir) => {
            if (!(0, metadata_1.isHostDirectiveMetaForGlobalMode)(hostDir)) {
    ...
    const metadata = {
        ...
        typeSourceSpan: (0, common_1.createSourceSpan)(clazz.name),
        usesInheritance,
        exportAs,
        providers,
        isStandalone,
        isSignal,
        hostDirectives: (hostDirectives === null || hostDirectives === void 0 ? void 0 : hostDirectives.map((hostDir) => toHostDirectiveMetadata(hostDir, sourceFile, refEmitter))) ||
            null,
    };
    return {
        jitForced: false,
...
function extractDecoratorQueryMetadata(exprNode, name, args, propertyName, reflector, evaluator) {
    ...
    const forwardReferenceTarget = (0, common_1.tryUnwrapForwardRef)(args[0], reflector);
    ...
    if (arg instanceof imports_1.Reference || arg instanceof partial_evaluator_1.DynamicValue) {
        ...
        predicate = (0, compiler_1.createMayBeForwardRefExpression)(new compiler_1.WrappedNodeExpr(node), forwardReferenceTarget !== null ? compiler_1.ForwardRefHandling.Unwrapped : compiler_1.ForwardRefHandling.None);
    }
        ...
        throw (0, common_1.createValueHasWrongTypeError)(node, arg, `@${name} predicate cannot be interpreted`);
    }
    // Extract the read and descendants options.
    let read = null;
    // The default value for descendants is true for every decorator except @ContentChildren.
    let descendants = name !== 'ContentChildren';
    let emitDistinctChangesOnly = compiler_1.emitDistinctChangesOnlyDefaultValue;
    if (args.length === 2) {
        const optionsExpr = (0, common_1.unwrapExpression)(args[1]);
        if (!typescript_1.default.isObjectLiteralExpression(optionsExpr)) {
        ...
        const options = (0, reflection_1.reflectObjectLiteral)(optionsExpr);
        if (options.has('read')) {
        ...
        if (options.has('descendants')) {
            ...
            if (typeof descendantsValue !== 'boolean') {
                throw (0, common_1.createValueHasWrongTypeError)(descendantsExpr, descendantsValue, `@${name} options.descendants must be a boolean`);
            }
        ...
        if (options.has('emitDistinctChangesOnly')) {
            ...
            if (typeof emitDistinctChangesOnlyValue !== 'boolean') {
                throw (0, common_1.createValueHasWrongTypeError)(emitDistinctChangesOnlyExpr, emitDistinctChangesOnlyValue, `@${name} options.emitDistinctChangesOnly must be a boolean`);
            }
        ...
        if (options.has('static')) {
            ...
            if (typeof staticValue !== 'boolean') {
                throw (0, common_1.createValueHasWrongTypeError)(node, staticValue, `@${name} options.static must be a boolean`);
            }
...
function extractHostBindings(members, evaluator, coreModule, compilationMode, hostBindingNodes, metadata) {
    ...
    if (metadata && metadata.has('host')) {
        ...
        if (typescript_1.default.isObjectLiteralExpression(hostExpression)) {
    ...
    (0, reflection_1.filterToMembersWithDecorator)(members, 'HostBinding', coreModule).forEach(({ member, decorators }) => {
        decorators.forEach((decorator) => {
            let hostPropertyName = member.name;
            if (decorator.args !== null && decorator.args.length > 0) {
                if (decorator.args.length !== 1) {
                    throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.DECORATOR_ARITY_WRONG, decorator.node, `@HostBinding can have at most one argument, got ${decorator.args.length} argument(s)`);
                }
                const resolved = evaluator.evaluate(decorator.args[0]);
                // Specific error for local compilation mode if the argument cannot be resolved
                (0, common_1.assertLocalCompilationUnresolvedConst)(compilationMode, resolved, null, "Unresolved identifier found for @HostBinding's argument! Did " +
                ...
                if (typeof resolved !== 'string') {
                    throw (0, common_1.createValueHasWrongTypeError)(decorator.node, resolved, `@HostBinding's argument must be a string`);
                }
                hostPropertyName = resolved;
            }
            if (typescript_1.default.isDecorator(decorator.node)) {
    ...
    (0, reflection_1.filterToMembersWithDecorator)(members, 'HostListener', coreModule).forEach(({ member, decorators }) => {
        decorators.forEach((decorator) => {
            ...
            if (decorator.args !== null && decorator.args.length > 0) {
                ...
                (0, common_1.assertLocalCompilationUnresolvedConst)(compilationMode, resolved, null, "Unresolved identifier found for @HostListener's event name " +
                ...
                if (typeof resolved !== 'string') {
                    throw (0, common_1.createValueHasWrongTypeError)(decorator.args[0], resolved, `@HostListener's event name argument must be a string`);
                }
                eventName = resolved;
                if (decorator.args.length === 2) {
                    ...
                    if (!isStringArrayOrDie(resolvedArgs, '@HostListener.args', expression)) {
                        throw (0, common_1.createValueHasWrongTypeError)(decorator.args[1], resolvedArgs, `@HostListener's second argument must be a string array`);
                    }
                    args = resolvedArgs;
                }
            }
...
function extractQueriesFromDecorator(queryData, reflector, evaluator, isCore) {
    const content = [];
    const view = [];
    if (!typescript_1.default.isObjectLiteralExpression(queryData)) {
    ...
    (0, reflection_1.reflectObjectLiteral)(queryData).forEach((queryExpr, propertyName) => {
        queryExpr = (0, common_1.unwrapExpression)(queryExpr);
        if (!typescript_1.default.isNewExpression(queryExpr)) {
        ...
        const queryType = typescript_1.default.isPropertyAccessExpression(queryExpr.expression)
            ? queryExpr.expression.name
            : queryExpr.expression;
        if (!typescript_1.default.isIdentifier(queryType)) {
            throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, queryData, 'Decorator query metadata must be an instance of a query type');
        }
...
function parseDirectiveStyles(directive, evaluator, compilationMode) {
    ...
    if (compilationMode === transform_1.CompilationMode.LOCAL) {
        ...
        if (Array.isArray(value)) {
            const entry = value.find((e) => e instanceof partial_evaluator_1.DynamicValue && e.isFromUnknownIdentifier());
            unresolvedNode = (_a = entry === null || entry === void 0 ? void 0 : entry.node) !== null && _a !== void 0 ? _a : null;
        }
        else if (value instanceof partial_evaluator_1.DynamicValue && value.isFromUnknownIdentifier()) {
            unresolvedNode = value.node;
        }
        if (unresolvedNode !== null) {
            throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.LOCAL_COMPILATION_UNRESOLVED_CONST, unresolvedNode, 'Unresolved identifier found for @Component.styles field! Did you import ' +
    ...
    if (!isStringArrayOrDie(value, 'styles', expression)) {
        throw (0, common_1.createValueHasWrongTypeError)(expression, value, `Failed to resolve @Component.styles to a string or an array of strings`);
    }
    return value;
}
function parseFieldStringArrayValue(directive, field, evaluator) {
    ...
    if (!isStringArrayOrDie(value, field, expression)) {
        throw (0, common_1.createValueHasWrongTypeError)(expression, value, `Failed to resolve @Directive.${field} to a string array`);
    }
    return value;
}
function isStringArrayOrDie(value, name, node) {
    ...
    for (let i = 0; i < value.length; i++) {
        if (typeof value[i] !== 'string') {
            throw (0, common_1.createValueHasWrongTypeError)(node, value[i], `Failed to resolve ${name} at position ${i} to a string`);
        }
    }
    return true;
}
function tryGetQueryFromFieldDecorator(member, reflector, evaluator, isCore) {
    ...
    const queryDecorators = (0, common_1.getAngularDecorators)(decorators, exports.queryDecoratorNames, isCore);
    if (queryDecorators.length === 0) {
        return null;
    }
    if (queryDecorators.length !== 1) {
        throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.DECORATOR_COLLISION, (_a = member.node) !== null && _a !== void 0 ? _a : queryDecorators[0].node, 'Cannot combine multiple query decorators.');
    }
    ...
    const name = ((_c = (_b = decorator.import) === null || _b === void 0 ? void 0 : _b.name) !== null && _c !== void 0 ? _c : decorator.name);
    return {
        name,
        decorator,
...
function isPropertyTypeMember(member) {
    return (member.kind === reflection_1.ClassMemberKind.Getter ||
        member.kind === reflection_1.ClassMemberKind.Setter ||
...
function parseInputsArray(clazz, decoratorMetadata, evaluator, reflector, refEmitter, compilationMode) {
    ...
    if (!Array.isArray(inputsArray)) {
        throw (0, common_1.createValueHasWrongTypeError)(inputsField, inputsArray, `Failed to resolve @Directive.inputs to an array`);
    }
    for (let i = 0; i < inputsArray.length; i++) {
        ...
        if (typeof value === 'string') {
            ...
            if (typeof name !== 'string') {
                throw (0, common_1.createValueHasWrongTypeError)(inputsField, name, `Value at position ${i} of @Directive.inputs array must have a "name" property`);
            }
            if (value.has('transform')) {
                ...
                if (!(transformValue instanceof partial_evaluator_1.DynamicValue) && !(transformValue instanceof imports_1.Reference)) {
                    throw (0, common_1.createValueHasWrongTypeError)(inputsField, transformValue, `Transform of value at position ${i} of @Directive.inputs array must be a function`);
                }
            ...
            throw (0, common_1.createValueHasWrongTypeError)(inputsField, value, `@Directive.inputs array can only contain strings or object literals`);
        }
    }
    return inputs;
}
...
function tryGetDecoratorOnMember(member, decoratorName, isCore) {
    ...
    for (const decorator of member.decorators) {
        if ((0, common_1.isAngularDecorator)(decorator, decoratorName, isCore)) {
            return decorator;
        }
    }
    return null;
...
function tryParseInputFieldMapping(clazz, member, evaluator, reflector, importTracker, isCore, refEmitter, compilationMode) {
    ...
    const signalInputMapping = (0, input_function_1.tryParseSignalInputMapping)(member, reflector, importTracker);
    const modelInputMapping = (0, model_function_1.tryParseSignalModelMapping)(member, reflector, importTracker);
    if (decorator !== null && signalInputMapping !== null) {
        throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.INITIALIZER_API_WITH_DISALLOWED_DECORATOR, decorator.node, `Using @Input with a signal input is not allowed.`);
    }
    if (decorator !== null && modelInputMapping !== null) {
        throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.INITIALIZER_API_WITH_DISALLOWED_DECORATOR, decorator.node, `Using @Input with a model input is not allowed.`);
    }
    // Check `@Input` case.
    if (decorator !== null) {
        if (decorator.args !== null && decorator.args.length > 1) {
            throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.DECORATOR_ARITY_WRONG, decorator.node, `@${decorator.name} can have at most one argument, got ${decorator.args.length} argument(s)`);
        }
        ...
        if (options !== null && typeof options !== 'string' && !(options instanceof Map)) {
            throw (0, common_1.createValueHasWrongTypeError)(decorator.node, options, `@${decorator.name} decorator argument must resolve to a string or an object literal`);
        }
        let alias = null;
        ...
        const publicInputName = alias !== null && alias !== void 0 ? alias : classPropertyName;
        let transform = null;
        if (options instanceof Map && options.has('transform')) {
            const transformValue = options.get('transform');
            if (!(transformValue instanceof partial_evaluator_1.DynamicValue) && !(transformValue instanceof imports_1.Reference)) {
                throw (0, common_1.createValueHasWrongTypeError)(optionsNode, transformValue, `Input transform must be a function`);
            }
...
function parseInputFields(clazz, members, evaluator, reflector, importTracker, refEmitter, isCore, compilationMode, inputsFromClassDecorator, classDecorator) {
    ...
    for (const member of members) {
        ...
        if (member.isStatic) {
            throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.INCORRECTLY_DECLARED_ON_STATIC_MEMBER, (_a = member.node) !== null && _a !== void 0 ? _a : clazz, `Input "${member.name}" is incorrectly declared as static member of "${clazz.name.text}".`);
        }
        ...
        if (inputMapping.isSignal && inputsFromClassDecorator.hasOwnProperty(classPropertyName)) {
            throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.INITIALIZER_API_DECORATOR_METADATA_COLLISION, (_b = member.node) !== null && _b !== void 0 ? _b : clazz, `Input "${member.name}" is also declared as non-signal in @${classDecorator.name}.`);
        }
...
function parseDecoratorInputTransformFunction(clazz, classPropertyName, value, reflector, refEmitter, compilationMode) {
    ...
    // all the diagnostics.
    if (compilationMode === transform_1.CompilationMode.LOCAL) {
        const node = value instanceof imports_1.Reference ? value.getIdentityIn(clazz.getSourceFile()) : value.node;
        // This should never be null since we know the reference originates
        // from the same file, but we null check it just in case.
        if (node === null) {
            throw (0, common_1.createValueHasWrongTypeError)(value.node, value, 'Input transform function could not be referenced');
        }
        return {
            node,
            type: new imports_1.Reference(typescript_1.default.factory.createKeywordTypeNode(typescript_1.default.SyntaxKind.UnknownKeyword)),
        };
    }
    const definition = reflector.getDefinitionOfFunction(value.node);
    if (definition === null) {
        throw (0, common_1.createValueHasWrongTypeError)(value.node, value, 'Input transform must be a function');
    }
    if (definition.typeParameters !== null && definition.typeParameters.length > 0) {
        throw (0, common_1.createValueHasWrongTypeError)(value.node, value, 'Input transform function cannot be generic');
    }
    if (definition.signatureCount > 1) {
        throw (0, common_1.createValueHasWrongTypeError)(value.node, value, 'Input transform function cannot have multiple signatures');
    }
    const members = reflector.getMembersOfClass(clazz);
    for (const member of members) {
        const conflictingName = `ngAcceptInputType_${classPropertyName}`;
        if (member.name === conflictingName && member.isStatic) {
            throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.CONFLICTING_INPUT_TRANSFORM, value.node, `Class cannot have both a transform function on Input ${classPropertyName} and a static member called ${conflictingName}`);
        }
    }
    const node = value instanceof imports_1.Reference ? value.getIdentityIn(clazz.getSourceFile()) : value.node;
    // This should never be null since we know the reference originates
    ...
    const firstParam = ((_a = definition.parameters[0]) === null || _a === void 0 ? void 0 : _a.name) === 'this' ? definition.parameters[1] : definition.parameters[0];
    ...
    if (!firstParam.type) {
        throw (0, common_1.createValueHasWrongTypeError)(value.node, value, 'Input transform function first parameter must have a type');
    }
    if (firstParam.node.dotDotDotToken) {
        throw (0, common_1.createValueHasWrongTypeError)(value.node, value, 'Input transform function first parameter cannot be a spread parameter');
    }
    assertEmittableInputType(firstParam.type, clazz.getSourceFile(), reflector, refEmitter);
    const viaModule = value instanceof imports_1.Reference ? value.bestGuessOwningModule : null;
    return { node, type: new imports_1.Reference(firstParam.type, viaModule) };
}
/**
        ...
        if (typescript_1.default.isTypeReferenceNode(node) && typescript_1.default.isIdentifier(node.typeName)) {
            ...
            if (declaration !== null) {
                ...
                if (declaration.node.getSourceFile() !== contextFile) {
                    const emittedType = refEmitter.emit(new imports_1.Reference(declaration.node, declaration.viaModule === reflection_1.AmbientImport ? reflection_1.AmbientImport : null), contextFile, imports_1.ImportFlags.NoAliasing |
                        imports_1.ImportFlags.AllowTypeImports |
                        imports_1.ImportFlags.AllowRelativeDtsImports |
                        imports_1.ImportFlags.AllowAmbientReferences);
                    (0, imports_1.assertSuccessfulReferenceEmit)(emittedType, node, 'type');
                }
                else if (!reflector.isStaticallyExported(declaration.node)) {
                    throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.SYMBOL_NOT_EXPORTED, type, `Symbol must be exported in order to be used as the type of an Input transform function`, [(0, diagnostics_1.makeRelatedInformation)(declaration.node, `The symbol is declared here.`)]);
                }
            }
        }
...
function parseQueriesOfClassFields(members, reflector, importTracker, evaluator, isCore) {
    ...
    for (const member of members) {
        ...
        const signalQuery = (0, query_functions_1.tryParseSignalQueryFromInitializer)(member, reflector, importTracker);
        if (decoratorQuery !== null && signalQuery !== null) {
            throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.INITIALIZER_API_WITH_DISALLOWED_DECORATOR, decoratorQuery.decorator.node, `Using @${decoratorQuery.name} with a signal-based query is not allowed.`);
        }
        const queryNode = (_a = decoratorQuery === null || decoratorQuery === void 0 ? void 0 : decoratorQuery.decorator.node) !== null && _a !== void 0 ? _a : signalQuery === null || signalQuery === void 0 ? void 0 : signalQuery.call;
        if (queryNode !== undefined && member.isStatic) {
            throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.INCORRECTLY_DECLARED_ON_STATIC_MEMBER, queryNode, `Query is incorrectly declared on a static class member.`);
        }
...
function parseOutputFields(clazz, classDecorator, members, isCore, reflector, importTracker, evaluator, outputsFromMeta) {
    ...
    for (const member of members) {
        ...
        const initializerOutput = (0, output_function_1.tryParseInitializerBasedOutput)(member, reflector, importTracker);
        const modelMapping = (0, model_function_1.tryParseSignalModelMapping)(member, reflector, importTracker);
        if (decoratorOutput !== null && initializerOutput !== null) {
            throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.INITIALIZER_API_WITH_DISALLOWED_DECORATOR, decoratorOutput.decorator.node, `Using "@Output" with "output()" is not allowed.`);
        }
        if (decoratorOutput !== null && modelMapping !== null) {
            throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.INITIALIZER_API_WITH_DISALLOWED_DECORATOR, decoratorOutput.decorator.node, `Using @Output with a model input is not allowed.`);
        }
        const queryNode = (_b = (_a = decoratorOutput === null || decoratorOutput === void 0 ? void 0 : decoratorOutput.decorator.node) !== null && _a !== void 0 ? _a : initializerOutput === null || initializerOutput === void 0 ? void 0 : initializerOutput.call) !== null && _b !== void 0 ? _b : modelMapping === null || modelMapping === void 0 ? void 0 : modelMapping.call;
        if (queryNode !== undefined && member.isStatic) {
            throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.INCORRECTLY_DECLARED_ON_STATIC_MEMBER, queryNode, `Output is incorrectly declared on a static class member.`);
        }
        ...
        if ((initializerOutput !== null || modelMapping !== null) &&
            ...
            throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.INITIALIZER_API_DECORATOR_METADATA_COLLISION, (_c = member.node) !== null && _c !== void 0 ? _c : clazz, `Output "${member.name}" is unexpectedly declared in @${classDecorator.name} as well.`);
        }
...
function tryParseDecoratorOutput(member, evaluator, isCore) {
    ...
    if (decorator.args !== null && decorator.args.length > 1) {
        throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.DECORATOR_ARITY_WRONG, decorator.node, `@Output can have at most one argument, got ${decorator.args.length} argument(s)`);
    }
    const classPropertyName = member.name;
    let alias = null;
    if (((_a = decorator.args) === null || _a === void 0 ? void 0 : _a.length) === 1) {
        const resolvedAlias = evaluator.evaluate(decorator.args[0]);
        if (typeof resolvedAlias !== 'string') {
            throw (0, common_1.createValueHasWrongTypeError)(decorator.node, resolvedAlias, `@Output decorator argument must resolve to a string`);
        }
        alias = resolvedAlias;
    }
    return {
        ...
        metadata: {
            isSignal: false,
            classPropertyName,
            bindingPropertyName: alias !== null && alias !== void 0 ? alias : classPropertyName,
        },
    };
}
function evaluateHostExpressionBindings(hostExpr, evaluator) {
    ...
    if (!(hostMetaMap instanceof Map)) {
        throw (0, common_1.createValueHasWrongTypeError)(hostExpr, hostMetaMap, `Decorator host metadata must be an object`);
    }
    const hostMetadata = {};
    hostMetaMap.forEach((value, key) => {
        ...
        if (typeof key !== 'string') {
            throw (0, common_1.createValueHasWrongTypeError)(hostExpr, key, `Decorator host metadata must be a string -> string object, but found unparseable key`);
        }
        if (typeof value == 'string') {
            ...
            throw (0, common_1.createValueHasWrongTypeError)(hostExpr, value, `Decorator host metadata must be a string -> string object, but found unparseable value`);
        }
    });
    const bindings = (0, compiler_1.parseHostBindings)(hostMetadata);
    const errors = (0, compiler_1.verifyHostBindings)(bindings, (0, common_1.createSourceSpan)(hostExpr));
    if (errors.length > 0) {
        throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.HOST_BINDING_PARSE_ERROR, getHostBindingErrorNode(errors[0], hostExpr), errors.map((error) => error.msg).join('\n'));
    }
    return bindings;
}
/**
...
function getHostBindingErrorNode(error, hostExpr) {
    ...
    if (typescript_1.default.isObjectLiteralExpression(hostExpr) && error.relatedError instanceof compiler_1.ParserError) {
        for (const prop of hostExpr.properties) {
            if (typescript_1.default.isPropertyAssignment(prop) &&
                typescript_1.default.isStringLiteralLike(prop.initializer) &&
...
function extractHostDirectives(rawHostDirectives, evaluator, compilationMode, forwardRefResolver) {
    ...
    if (!Array.isArray(resolved)) {
        throw (0, common_1.createValueHasWrongTypeError)(rawHostDirectives, resolved, 'hostDirectives must be an array');
    }
    return resolved.map((value) => {
        const hostReference = value instanceof Map ? value.get('directive') : value;
        // Diagnostics
        if (compilationMode !== transform_1.CompilationMode.LOCAL) {
            if (!(hostReference instanceof imports_1.Reference)) {
                throw (0, common_1.createValueHasWrongTypeError)(rawHostDirectives, hostReference, 'Host directive must be a reference');
            }
            if (!(0, reflection_1.isNamedClassDeclaration)(hostReference.node)) {
                throw (0, common_1.createValueHasWrongTypeError)(rawHostDirectives, hostReference, 'Host directive reference must be a class');
            }
        }
        let directive;
        let nameForErrors = (fieldName) => '@Directive.hostDirectives';
        if (compilationMode === transform_1.CompilationMode.LOCAL && hostReference instanceof partial_evaluator_1.DynamicValue) {
            ...
            if (!typescript_1.default.isIdentifier(hostReference.node) &&
                !typescript_1.default.isPropertyAccessExpression(hostReference.node)) {
                throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.LOCAL_COMPILATION_UNSUPPORTED_EXPRESSION, hostReference.node, `In local compilation mode, host directive cannot be an expression. Use an identifier instead`);
            }
            directive = new compiler_1.WrappedNodeExpr(hostReference.node);
        }
        else if (hostReference instanceof imports_1.Reference) {
        ...
        const meta = {
            directive,
            isForwardReference: hostReference instanceof imports_1.Reference && hostReference.synthetic,
...
function toHostDirectiveMetadata(hostDirective, context, refEmitter) {
    let directive;
    if (hostDirective.directive instanceof imports_1.Reference) {
        directive = (0, common_1.toR3Reference)(hostDirective.directive.node, hostDirective.directive, context, refEmitter);
    }
    else {
        directive = {
...
function toR3InputMetadata(mapping) {
    return {
        ...
        transformFunction: mapping.transform !== null ? new compiler_1.WrappedNodeExpr(mapping.transform.node) : null,
