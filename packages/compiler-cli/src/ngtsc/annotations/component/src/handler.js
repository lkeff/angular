...
const selectorless_1 = require("./selectorless");
const EMPTY_ARRAY = [];
const isUsedDirective = (decl) => decl.kind === compiler_1.R3TemplateDependencyKind.Directive;
const isUsedPipe = (decl) => decl.kind === compiler_1.R3TemplateDependencyKind.Pipe;
/**
...
class ComponentDecoratorHandler {
    ...
    detect(node, decorators) {
        ...
        const decorator = (0, common_1.findAngularDecorator)(decorators, 'Component', this.isCore);
    ...
    preanalyze(node, decorator) {
        ...
        const meta = (0, common_1.resolveLiteral)(decorator, this.literalCache);
        const component = (0, reflection_1.reflectObjectLiteral)(meta);
        ...
        const templateAndTemplateStyleResources = (0, resources_1.preloadAndParseTemplate)(this.evaluator, this.resourceLoader, this.depTracker, this.preanalyzeTemplateCache, node, decorator, component, containingFile, this.defaultPreserveWhitespaces, this.extractTemplateOptions, this.compilationMode).then((template) => {
            if (template === null) {
        ...
        const componentStyleUrls = (0, resources_1.extractComponentStyleUrls)(this.evaluator, component);
        return templateAndTemplateStyleResources.then((templateInfo) => __awaiter(this, void 0, void 0, function* () {
            ...
            let orderOffset = 0;
            const rawStyles = (0, directive_1.parseDirectiveStyles)(component, this.evaluator, this.compilationMode);
            if (rawStyles === null || rawStyles === void 0 ? void 0 : rawStyles.length) {
                styles = yield Promise.all(rawStyles.map((style) => this.resourceLoader.preprocessInline(style, {
                    type: 'style',
                    containingFile,
                    order: orderOffset++,
                    className: node.name.text,
            ...
            if (templateInfo.templateStyles) {
                ...
                styles.push(...(yield Promise.all(templateInfo.templateStyles.map((style) => {
                    var _a;
                    return this.resourceLoader.preprocessInline(style, {
                        type: 'style',
                        containingFile: (_a = templateInfo.templateUrl) !== null && _a !== void 0 ? _a : containingFile,
                        order: orderOffset++,
                        className: node.name.text,
                    });
                }))));
    ...
    analyze(node, decorator) {
        ...
        const encapsulation = (_a = (this.compilationMode !== transform_1.CompilationMode.LOCAL
            ? (0, common_1.resolveEnumValue)(this.evaluator, component, 'encapsulation', 'ViewEncapsulation', this.isCore)
            : (0, common_1.resolveEncapsulationEnumValueLocally)(component.get('encapsulation')))) !== null && _a !== void 0 ? _a : compiler_1.ViewEncapsulation.Emulated;
        let changeDetection = null;
        if (this.compilationMode !== transform_1.CompilationMode.LOCAL) {
            changeDetection = (0, common_1.resolveEnumValue)(this.evaluator, component, 'changeDetection', 'ChangeDetectionStrategy', this.isCore);
        }
        else if (component.has('changeDetection')) {
            changeDetection = new compiler_1.outputAst.WrappedNodeExpr(component.get('changeDetection'));
        }
        let animations = null;
        let animationTriggerNames = null;
        if (component.has('animations')) {
            const animationExpression = component.get('animations');
            animations = new compiler_1.outputAst.WrappedNodeExpr(animationExpression);
            const animationsValue = this.evaluator.evaluate(animationExpression, util_2.animationTriggerResolver);
            animationTriggerNames = { includesDynamicAnimations: false, staticTriggerNames: [] };
            (0, util_2.collectAnimationNames)(animationsValue, animationTriggerNames);
        }
        // Go through the root directories for this project, and select the one with the smallest
        // relative path representation.
        const relativeContextFilePath = this.rootDirs.reduce((previous, rootDir) => {
            const candidate = (0, file_system_1.relative)((0, file_system_1.absoluteFrom)(rootDir), (0, file_system_1.absoluteFrom)(containingFile));
        ...
        if (component.has('viewProviders')) {
            ...
            viewProvidersRequiringFactory = (0, common_1.resolveProvidersRequiringFactory)(viewProviders, this.reflector, this.evaluator);
            wrappedViewProviders = new compiler_1.outputAst.WrappedNodeExpr(this.annotateForClosureCompiler
                ? (0, common_1.wrapFunctionExpressionsInParens)(viewProviders)
                : viewProviders);
        }
        if (component.has('providers')) {
            providersRequiringFactory = (0, common_1.resolveProvidersRequiringFactory)(component.get('providers'), this.reflector, this.evaluator);
        }
        let resolvedImports = null;
        let resolvedDeferredImports = null;
        let rawImports = (_b = component.get('imports')) !== null && _b !== void 0 ? _b : null;
        let rawDeferredImports = (_c = component.get('deferredImports')) !== null && _c !== void 0 ? _c : null;
        if ((rawImports || rawDeferredImports) && !metadata.isStandalone) {
            if (diagnostics === undefined) {
                diagnostics = [];
            }
            const importsField = rawImports ? 'imports' : 'deferredImports';
            diagnostics.push((0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.COMPONENT_NOT_STANDALONE, component.get(importsField), `'${importsField}' is only valid on a component that is standalone.`, [
                (0, diagnostics_1.makeRelatedInformation)(node.name, `Did you forget to add 'standalone: true' to this @Component?`),
            ]));
            // Poison the component so that we don't spam further template type-checking errors that
            // result from misconfigured imports.
            isPoisoned = true;
        }
        else if (this.compilationMode !== transform_1.CompilationMode.LOCAL &&
            (rawImports || rawDeferredImports)) {
            const importResolvers = (0, common_1.combineResolvers)([
                (0, ng_module_1.createModuleWithProvidersResolver)(this.reflector, this.isCore),
                (0, common_1.createForwardRefResolver)(this.isCore),
            ]);
            const importDiagnostics = [];
            if (rawImports) {
                const expr = rawImports;
                const imported = this.evaluator.evaluate(expr, importResolvers);
                const { imports: flattened, diagnostics } = (0, util_2.validateAndFlattenComponentImports)(imported, expr, false /* isDeferred */);
            ...
            if (rawDeferredImports) {
                ...
                const imported = this.evaluator.evaluate(expr, importResolvers);
                const { imports: flattened, diagnostics } = (0, util_2.validateAndFlattenComponentImports)(imported, expr, true /* isDeferred */);
                importDiagnostics.push(...diagnostics);
        ...
        if (component.has('schemas') && !metadata.isStandalone) {
            ...
            diagnostics.push((0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.COMPONENT_NOT_STANDALONE, component.get('schemas'), `'schemas' is only valid on a component that is standalone.`));
        }
        else if (this.compilationMode !== transform_1.CompilationMode.LOCAL && component.has('schemas')) {
            schemas = (0, common_1.extractSchemas)(component.get('schemas'), this.evaluator, 'Component');
        }
        ...
        if (this.preanalyzeTemplateCache.has(node)) {
            ...
            try {
                const templateDecl = (0, resources_1.parseTemplateDeclaration)(node, decorator, component, containingFile, this.evaluator, this.depTracker, this.resourceLoader, this.defaultPreserveWhitespaces);
                template = (0, resources_1.extractTemplate)(node, templateDecl, this.evaluator, this.depTracker, this.resourceLoader, {
                    enableI18nLegacyMessageIdFormat: this.enableI18nLegacyMessageIdFormat,
                    i18nNormalizeLineEndingsInICUs: this.i18nNormalizeLineEndingsInICUs,
                    usePoisonedData: this.usePoisonedData,
                    enableBlockSyntax: this.enableBlockSyntax,
                ...
                if (this.compilationMode === transform_1.CompilationMode.LOCAL &&
                    ...
                    diagnostics.push(...(0, typecheck_1.getTemplateDiagnostics)(template.errors, 
                ...
                if (e instanceof diagnostics_1.FatalDiagnosticError) {
                    diagnostics !== null && diagnostics !== void 0 ? diagnostics : (diagnostics = []);
                    ...
                    template = (0, resources_1.createEmptyTemplate)(node, component, containingFile);
                }
                else {
                    throw e;
                }
        ...
        const relativeTemplatePath = (0, path_1.getProjectRelativePath)((_d = templateResource.path) !== null && _d !== void 0 ? _d : typescript_1.default.getOriginalNode(node).getSourceFile().fileName, this.rootDirs, this.compilerHost);
        let selectorlessEnabled = false;
        let localReferencedSymbols = null;
        if (this.enableSelectorless) {
            const templateAnalysis = (0, selectorless_1.analyzeTemplateForSelectorless)(template.nodes);
        ...
        if (selectorlessEnabled) {
            if (!metadata.isStandalone) {
                isPoisoned = true;
                diagnostics !== null && diagnostics !== void 0 ? diagnostics : (diagnostics = []);
                diagnostics.push((0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.COMPONENT_NOT_STANDALONE, component.get('standalone') || node.name, `Cannot use selectorless with a component that is not standalone`));
            }
            else if (rawImports || rawDeferredImports) {
                isPoisoned = true;
                diagnostics !== null && diagnostics !== void 0 ? diagnostics : (diagnostics = []);
                diagnostics.push((0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.UNSUPPORTED_SELECTORLESS_COMPONENT_FIELD, (rawImports || rawDeferredImports), `Cannot use the "${rawImports === null ? 'deferredImports' : 'imports'}" field in a selectorless components`));
            }
        }
        ...
        const hostBindingResources = (0, directive_1.extractHostBindingResources)(directiveResult.hostBindingNodes);
        const styleResources = (0, resources_1.extractInlineStyleResources)(component);
        const styleUrls = [
            ...(0, resources_1.extractComponentStyleUrls)(this.evaluator, component),
            ...(0, resources_1._extractTemplateStyleUrls)(template),
        ];
        for (const styleUrl of styleUrls) {
            try {
                ...
                if (styleUrl.source === 2 /* ResourceTypeForDiagnostics.StylesheetFromDecorator */ &&
                    typescript_1.default.isStringLiteralLike(styleUrl.expression)) {
                    // Only string literal values from the decorator are considered style resources
                    styleResources.add({
                        path: (0, file_system_1.absoluteFrom)(resourceUrl),
                ...
                diagnostics.push((0, resources_1.makeResourceNotFoundError)(styleUrl.url, styleUrl.expression, resourceType).toDiagnostic());
            }
        }
        if (encapsulation === compiler_1.ViewEncapsulation.ShadowDom && metadata.selector !== null) {
            const selectorError = (0, diagnostics_2.checkCustomElementSelectorForErrors)(metadata.selector);
        ...
        if (this.preanalyzeStylesCache.has(node)) {
            ...
            if (inlineStyles === null || inlineStyles === void 0 ? void 0 : inlineStyles.length) {
                if (this.externalRuntimeStyles) {
            ...
            if (component.has('styles')) {
                const litStyles = (0, directive_1.parseDirectiveStyles)(component, this.evaluator, this.compilationMode);
                if (litStyles !== null) {
        ...
        if (metadata.isStandalone && rawDeferredImports !== null) {
            ...
            for (const [deferredType, importDetails] of deferredTypes) {
                explicitlyDeferredTypes !== null && explicitlyDeferredTypes !== void 0 ? explicitlyDeferredTypes : (explicitlyDeferredTypes = []);
        ...
        const output = {
            analysis: {
                ...
                meta: Object.assign(Object.assign({}, metadata), { template,
                    encapsulation,
                    changeDetection, interpolation: (_e = template.interpolationConfig) !== null && _e !== void 0 ? _e : compiler_1.DEFAULT_INTERPOLATION_CONFIG, styles,
                    externalStyles,
                    // These will be replaced during the compilation step, after all `NgModule`s have been
                    // analyzed and the full compilation scope for the component can be realized.
                    animations, viewProviders: wrappedViewProviders, i18nUseExternalIds: this.i18nUseExternalIds, relativeContextFilePath, rawImports: rawImports !== null ? new compiler_1.outputAst.WrappedNodeExpr(rawImports) : undefined, relativeTemplatePath }),
                typeCheckMeta: (0, metadata_1.extractDirectiveTypeCheckMeta)(node, inputs, this.reflector),
                classMetadata: this.includeClassMetadata
                    ? (0, common_1.extractClassMetadata)(node, this.reflector, this.isCore, this.annotateForClosureCompiler, (dec) => (0, resources_1.transformDecoratorResources)(dec, component, styles, template))
                    : null,
                classDebugInfo: (0, common_1.extractClassDebugInfo)(node, this.reflector, this.compilerHost, this.rootDirs, 
                ...
                schemas,
                decorator: (_f = decorator === null || decorator === void 0 ? void 0 : decorator.node) !== null && _f !== void 0 ? _f : null,
    ...
    symbol(node, analysis) {
        ...
        return new symbol_1.ComponentSymbol(node, analysis.meta.selector, analysis.inputs, analysis.outputs, analysis.meta.exportAs, analysis.typeCheckMeta, typeParameters);
    }
    register(node, analysis) {
        ...
        this.metaRegistry.registerDirectiveMetadata(Object.assign(Object.assign({ kind: metadata_1.MetaKind.Directive, matchSource: metadata_1.MatchSource.Selector, ref, name: node.name.text, selector: analysis.meta.selector, exportAs: analysis.meta.exportAs, inputs: analysis.inputs, inputFieldNamesFromMetadataArray: analysis.inputFieldNamesFromMetadataArray, outputs: analysis.outputs, queries: analysis.meta.queries.map((query) => query.propertyName), isComponent: true, baseClass: analysis.baseClass, hostDirectives: analysis.hostDirectives }, analysis.typeCheckMeta), { isPoisoned: analysis.isPoisoned, isStructural: false, isStandalone: analysis.meta.isStandalone, isSignal: analysis.meta.isSignal, imports: analysis.resolvedImports, rawImports: analysis.rawImports, deferredImports: analysis.resolvedDeferredImports, animationTriggerNames: analysis.animationTriggerNames, schemas: analysis.schemas, decorator: analysis.decorator, assumedToExportProviders: false, ngContentSelectors: analysis.template.ngContentSelectors, preserveWhitespaces: (_a = analysis.template.preserveWhitespaces) !== null && _a !== void 0 ? _a : false, isExplicitlyDeferred: false, selectorlessEnabled: analysis.selectorlessEnabled, localReferencedSymbols: analysis.localReferencedSymbols }));
    ...
    index(context, node, analysis) {
        ...
        let matcher = null;
        if (scope !== null) {
            const isPoisoned = scope.kind === scope_1.ComponentScopeKind.NgModule
                ? scope.compilation.isPoisoned
                : scope.isPoisoned;
            if ((isPoisoned || (scope.kind === scope_1.ComponentScopeKind.NgModule && scope.exported.isPoisoned)) &&
    ...
    typeCheck(ctx, node, meta) {
        ...
        const templateContext = {
            ...
            preserveWhitespaces: (_a = meta.meta.template.preserveWhitespaces) !== null && _a !== void 0 ? _a : false,
        };
        const hostElement = this.typeCheckHostBindings
            ? (0, typecheck_1.createHostElement)('component', meta.meta.selector, node, meta.hostBindingNodes.literal, meta.hostBindingNodes.bindingDecorators, meta.hostBindingNodes.listenerDecorators)
            : null;
        ...
        ctx.addDirective(new imports_1.Reference(node), binder, scope.schemas, templateContext, hostBindingsContext, meta.meta.isStandalone);
    }
    ...
    resolve(node, analysis, symbol) {
        ...
        if (nonRemovableImports.length > 0) {
            for (const importDecl of nonRemovableImports) {
                const diagnostic = (0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.DEFERRED_DEPENDENCY_IMPORTED_EAGERLY, importDecl, `This import contains symbols that are used both inside and outside of the ` +
        ...
        if (this.compilationMode === transform_1.CompilationMode.LOCAL) {
            ...
            data = {
                ...
                declarationListEmitMode: !analysis.meta.isStandalone || analysis.rawImports !== null
                    ? compiler_1.DeclarationListEmitMode.RuntimeResolved
                    : compiler_1.DeclarationListEmitMode.Direct,
                deferPerBlockDependencies: this.locateDeferBlocksWithoutScope(analysis.template),
                deferBlockDepsEmitMode: compiler_1.DeferBlockDepsEmitMode.PerComponent,
                deferrableDeclToImportDecl: new Map(),
                deferPerComponentDependencies: (_a = analysis.explicitlyDeferredTypes) !== null && _a !== void 0 ? _a : [],
            };
            ...
            data = {
                declarations: EMPTY_ARRAY,
                declarationListEmitMode: compiler_1.DeclarationListEmitMode.Direct,
                deferPerBlockDependencies: new Map(),
                deferBlockDepsEmitMode: compiler_1.DeferBlockDepsEmitMode.PerBlock,
                deferrableDeclToImportDecl: new Map(),
    ...
    updateResources(node, analysis) {
        ...
        if (!templateDecl.isInline) {
            analysis.template = (0, resources_1.extractTemplate)(node, templateDecl, this.evaluator, this.depTracker, this.resourceLoader, this.extractTemplateOptions, this.compilationMode);
        }
    ...
    compileFull(node, analysis, resolution, pool) {
        ...
        const meta = Object.assign(Object.assign(Object.assign({}, analysis.meta), resolution), { defer });
        const fac = (0, common_1.compileNgFactoryDefField)((0, common_1.toFactoryMetadata)(meta, compiler_1.FactoryTarget.Component));
        if (perComponentDeferredDeps !== null) {
            removeDeferrableTypesFromComponentDecorator(analysis, perComponentDeferredDeps);
        }
        const def = (0, compiler_1.compileComponentFromMetadata)(meta, pool, this.getNewBindingParser());
        const inputTransformFields = (0, common_1.compileInputTransformFields)(analysis.inputs);
        const classMetadata = analysis.classMetadata !== null
            ? (0, compiler_1.compileComponentClassMetadata)(analysis.classMetadata, perComponentDeferredDeps).toStmt()
            : null;
        const debugInfo = analysis.classDebugInfo !== null
            ? (0, compiler_1.compileClassDebugInfo)(analysis.classDebugInfo).toStmt()
            : null;
        const hmrMeta = this.enableHmr
            ? (0, hmr_1.extractHmrMetatadata)(node, this.reflector, this.evaluator, this.compilerHost, this.rootDirs, def, fac, defer, classMetadata, debugInfo)
            : null;
        const hmrInitializer = hmrMeta ? (0, compiler_1.compileHmrInitializer)(hmrMeta).toStmt() : null;
        const deferrableImports = this.canDeferDeps
            ? this.deferredSymbolTracker.getDeferrableImportDecls()
            : null;
        return (0, common_1.compileResults)(fac, def, classMetadata, 'ɵcmp', inputTransformFields, deferrableImports, debugInfo, hmrInitializer);
    }
    compilePartial(node, analysis, resolution) {
        ...
        const templateInfo = {
            ...
            inlineTemplateLiteralExpression: analysis.template.sourceMapping.type === 'direct'
                ? new compiler_1.outputAst.WrappedNodeExpr(analysis.template.sourceMapping.node)
                : null,
        };
        const perComponentDeferredDeps = this.canDeferDeps
            ? this.resolveAllDeferredDependencies(resolution)
        ...
        const defer = this.compileDeferBlocks(resolution);
        const meta = Object.assign(Object.assign(Object.assign({}, analysis.meta), resolution), { defer });
        const fac = (0, common_1.compileDeclareFactory)((0, common_1.toFactoryMetadata)(meta, compiler_1.FactoryTarget.Component));
        const inputTransformFields = (0, common_1.compileInputTransformFields)(analysis.inputs);
        const def = (0, compiler_1.compileDeclareComponentFromMetadata)(meta, analysis.template, templateInfo);
        const classMetadata = analysis.classMetadata !== null
            ? (0, compiler_1.compileComponentDeclareClassMetadata)(analysis.classMetadata, perComponentDeferredDeps).toStmt()
            : null;
        const hmrMeta = this.enableHmr
            ? (0, hmr_1.extractHmrMetatadata)(node, this.reflector, this.evaluator, this.compilerHost, this.rootDirs, def, fac, defer, classMetadata, null)
            : null;
        const hmrInitializer = hmrMeta ? (0, compiler_1.compileHmrInitializer)(hmrMeta).toStmt() : null;
        const deferrableImports = this.canDeferDeps
            ? this.deferredSymbolTracker.getDeferrableImportDecls()
            : null;
        return (0, common_1.compileResults)(fac, def, classMetadata, 'ɵcmp', inputTransformFields, deferrableImports, null, hmrInitializer);
    }
    compileLocal(node, analysis, resolution, pool) {
        ...
        const inputTransformFields = (0, common_1.compileInputTransformFields)(analysis.inputs);
        const classMetadata = analysis.classMetadata !== null
            ? (0, compiler_1.compileComponentClassMetadata)(analysis.classMetadata, deferrableTypes).toStmt()
            : null;
        const debugInfo = analysis.classDebugInfo !== null
            ? (0, compiler_1.compileClassDebugInfo)(analysis.classDebugInfo).toStmt()
            : null;
    ...
    compileHmrUpdateDeclaration(node, analysis, resolution) {
        ...
        const def = (0, compiler_1.compileComponentFromMetadata)(meta, pool, this.getNewBindingParser());
        const classMetadata = analysis.classMetadata !== null
            ? (0, compiler_1.compileComponentClassMetadata)(analysis.classMetadata, null).toStmt()
            : null;
        const debugInfo = analysis.classDebugInfo !== null
            ? (0, compiler_1.compileClassDebugInfo)(analysis.classDebugInfo).toStmt()
            : null;
        const hmrMeta = this.enableHmr
            ? (0, hmr_1.extractHmrMetatadata)(node, this.reflector, this.evaluator, this.compilerHost, this.rootDirs, def, fac, defer, classMetadata, debugInfo)
            : null;
        const res = (0, common_1.compileResults)(fac, def, classMetadata, 'ɵcmp', null, null, debugInfo, null);
        return hmrMeta === null || res.length === 0
            ? null
            : (0, hmr_1.getHmrUpdateDeclaration)(res, pool.statements, hmrMeta, node);
    }
    /**
    ...
    resolveComponentDependencies(node, context, analysis, scope, metadata, diagnostics) {
        ...
        var _a;
        const isModuleScope = scope.kind === scope_1.ComponentScopeKind.NgModule;
        const isSelectorlessScope = scope.kind === scope_1.ComponentScopeKind.Selectorless;
        const pipes = new Map();
        // Dependencies from the `@Component.deferredImports` field.
        const explicitlyDeferredDependencies = scope.kind === scope_1.ComponentScopeKind.Standalone ? scope.deferredDependencies : null;
        const dependencies = [];
        if (isSelectorlessScope) {
            for (const [localName, dep] of scope.dependencies) {
                ...
                if (dep.kind === metadata_1.MetaKind.Pipe) {
        ...
        if (isModuleScope && context.fileName !== (0, typescript_2.getSourceFile)(scope.ngModule).fileName) {
            (_a = this.localCompilationExtraImportsTracker) === null || _a === void 0 ? void 0 : _a.markFileForExtraImportGeneration(context);
        }
        ...
        const binder = new compiler_1.R3TargetBinder(createMatcherFromScope(scope, this.hostDirectivesResolver));
        ...
        if (explicitlyDeferredDependencies !== null && explicitlyDeferredDependencies.length > 0) {
            ...
            const deferBlockMatcher = new compiler_1.SelectorMatcher();
            for (const dep of allDependencies) {
                if (dep.kind === metadata_1.MetaKind.Pipe) {
                    pipes.set(dep.name, dep);
                }
                else if (dep.kind === metadata_1.MetaKind.Directive && dep.selector !== null) {
                    deferBlockMatcher.addSelectables(compiler_1.CssSelector.parse(dep.selector), [dep]);
                }
            }
            deferBlockBinder = new compiler_1.R3TargetBinder(deferBlockMatcher);
        }
    ...
    componentDependenciesToDeclarations(node, context, allDependencies, wholeTemplateUsed, pipes) {
        ...
        for (const dep of allDependencies) {
            ...
            switch (dep.kind) {
                case metadata_1.MetaKind.Directive:
                    if (!wholeTemplateUsed.has(dep.ref.node) || dep.matchSource !== metadata_1.MatchSource.Selector) {
                        continue;
                    }
                    const dirType = this.refEmitter.emit(dep.ref, context);
                    (0, imports_1.assertSuccessfulReferenceEmit)(dirType, node.name, dep.isComponent ? 'component' : 'directive');
                    ...
                    break;
                case metadata_1.MetaKind.NgModule:
                    const ngModuleType = this.refEmitter.emit(dep.ref, context);
                    (0, imports_1.assertSuccessfulReferenceEmit)(ngModuleType, node.name, 'NgModule');
                    declarations.set(dep.ref.node, {
        ...
        for (const [localName, dep] of pipes) {
            ...
            (0, imports_1.assertSuccessfulReferenceEmit)(pipeType, node.name, 'pipe');
            declarations.set(dep.ref.node, {
    ...
    handleDependencyCycles(node, context, scope, data, analysis, metadata, declarations, eagerlyUsed, symbol) {
        const eagerDeclarations = Array.from(declarations.values()).filter((decl) => {
            return decl.kind === compiler_1.R3TemplateDependencyKind.NgModule || eagerlyUsed.has(decl.ref.node);
        });
        ...
        if (!metadata.isStandalone) {
            for (const usedDep of eagerDeclarations) {
                ...
                if (cycle !== null) {
                    switch (usedDep.kind) {
                        case compiler_1.R3TemplateDependencyKind.Directive:
                            cyclesFromDirectives.set(usedDep, cycle);
                            break;
                        case compiler_1.R3TemplateDependencyKind.Pipe:
        ...
        if (!cycleDetected) {
            ...
            const declarationIsForwardDeclared = eagerDeclarations.some((decl) => (0, common_1.isExpressionForwardReference)(decl.type, node.name, context));
            if (this.compilationMode !== transform_1.CompilationMode.LOCAL &&
                (declarationIsForwardDeclared || standaloneImportMayBeForwardDeclared)) {
                data.declarationListEmitMode = compiler_1.DeclarationListEmitMode.Closure;
            }
            ...
            if (this.compilationMode === transform_1.CompilationMode.LOCAL &&
                ...
                for (const { type } of eagerDeclarations) {
                    if (type instanceof compiler_1.ExternalExpr && type.value.moduleName) {
            ...
            for (const [dir, cycle] of cyclesFromDirectives) {
                relatedMessages.push((0, diagnostics_2.makeCyclicImportInfo)(dir.ref, dir.isComponent ? 'component' : 'directive', cycle));
            }
            for (const [pipe, cycle] of cyclesFromPipes) {
                relatedMessages.push((0, diagnostics_2.makeCyclicImportInfo)(pipe.ref, 'pipe', cycle));
            }
    ...
    getNonLocalDiagnostics(node, analysis) {
        ...
        if (analysis.providersRequiringFactory !== null &&
            analysis.meta.providers instanceof compiler_1.outputAst.WrappedNodeExpr) {
        ...
        if (analysis.viewProvidersRequiringFactory !== null &&
            analysis.meta.viewProviders instanceof compiler_1.outputAst.WrappedNodeExpr) {
            const viewProviderDiagnostics = (0, common_1.getProviderDiagnostics)(analysis.viewProvidersRequiringFactory, analysis.meta.viewProviders.node, this.injectableRegistry);
            diagnostics !== null && diagnostics !== void 0 ? diagnostics : (diagnostics = []);
            diagnostics.push(...viewProviderDiagnostics);
        }
        const directiveDiagnostics = (0, common_1.getDirectiveDiagnostics)(node, this.injectableRegistry, this.evaluator, this.reflector, this.scopeRegistry, this.strictCtorDeps, 'Component');
    ...
    locateDeferBlocksWithoutScope(template) {
        const deferBlocks = new Map();
        const directivelessBinder = new compiler_1.R3TargetBinder(null);
    ...
    resolveAllDeferredDependencies(resolution) {
        ...
        for (const [_, deps] of resolution.deferPerBlockDependencies) {
            for (const deferBlockDep of deps) {
                ...
                const importDecl = (_a = resolution.deferrableDeclToImportDecl.get(node)) !== null && _a !== void 0 ? _a : null;
    ...
    collectExplicitlyDeferredSymbols(rawDeferredImports) {
        ...
        if (!typescript_1.default.isArrayLiteralExpression(rawDeferredImports)) {
            return deferredTypes;
        }
        for (const element of rawDeferredImports.elements) {
            const node = (0, common_1.tryUnwrapForwardRef)(element, this.reflector) || element;
    ...
    _checkForCyclicImport(importedFile, expr, origin) {
        const imported = (0, common_1.resolveImportedFile)(this.moduleResolver, importedFile, expr, origin);
        if (imported === null) {
            return null;
        }
    ...
    resolveDeferBlocks(componentClassDecl, scope, deferBlocks, deferrableDecls, resolutionData, analysisData, eagerlyUsedDecls) {
        ...
        for (const [deferBlock, bound] of deferBlocks) {
            ...
            for (const decl of Array.from(deferrableDecls.values())) {
                if (decl.kind === compiler_1.R3TemplateDependencyKind.NgModule) {
                    continue;
                }
                if (decl.kind === compiler_1.R3TemplateDependencyKind.Directive &&
                    !usedDirectives.has(decl.ref.node)) {
                    continue;
                }
                if (decl.kind === compiler_1.R3TemplateDependencyKind.Pipe && !usedPipes.has(decl.name)) {
                    continue;
                }
        ...
        if (analysisData.meta.isStandalone) {
            ...
            if (analysisData.rawImports !== null &&
                typescript_1.default.isArrayLiteralExpression(analysisData.rawImports)) {
            ...
            if (analysisData.rawDeferredImports !== null &&
                typescript_1.default.isArrayLiteralExpression(analysisData.rawDeferredImports)) {
            ...
            if (scope.kind === scope_1.ComponentScopeKind.Selectorless) {
    ...
    registerDeferrableCandidate(componentClassDecl, element, isDeferredImport, allDeferredDecls, eagerlyUsedDecls, resolutionData) {
        ...
        if (!(0, reflection_1.isNamedClassDeclaration)(decl.node)) {
        ...
        const dirMeta = this.metaReader.getDirectiveMetadata(new imports_1.Reference(decl.node));
        if (dirMeta !== null && !dirMeta.isStandalone) {
            return;
        }
        // Is it a standalone pipe?
        const pipeMeta = this.metaReader.getPipeMetadata(new imports_1.Reference(decl.node));
    ...
    compileDeferBlocks(resolution) {
        ...
        if (mode === compiler_1.DeferBlockDepsEmitMode.PerBlock) {
            if (!perBlockDeps) {
            ...
            for (const [block, dependencies] of perBlockDeps) {
                blocks.set(block, dependencies.length === 0 ? null : (0, compiler_1.compileDeferResolverFunction)({ mode, dependencies }));
            }
            return { mode, blocks };
        }
        if (mode === compiler_1.DeferBlockDepsEmitMode.PerComponent) {
            if (!perComponentDeps) {
            ...
            return {
                ...
                dependenciesFn: perComponentDeps.length === 0
                    ? null
                    : (0, compiler_1.compileDeferResolverFunction)({ mode, dependencies: perComponentDeps }),
            };
        }
    ...
    getNewBindingParser() {
        return (0, compiler_1.makeBindingParser)(undefined, this.enableSelectorless);
    }
}
...
function createMatcherFromScope(scope, hostDirectivesResolver) {
    ...
    const dependencies = scope.kind === scope_1.ComponentScopeKind.NgModule
        ...
        : scope.dependencies;
    for (const dep of dependencies) {
        if (dep.kind === metadata_1.MetaKind.Directive && dep.selector !== null) {
            matcher.addSelectables(compiler_1.CssSelector.parse(dep.selector), [dep]);
        }
    }
    return matcher;
}
...
function removeDeferrableTypesFromComponentDecorator(analysis, deferrableTypes) {
    if (analysis.classMetadata) {
        ...
        const rewrittenDecoratorsNode = (0, common_1.removeIdentifierReferences)(analysis.classMetadata.decorators.node, deferrableSymbols);
        analysis.classMetadata.decorators = new compiler_1.outputAst.WrappedNodeExpr(rewrittenDecoratorsNode);
    }
}
/**
...
function validateNoImportOverlap(eagerDeps, deferredDeps, rawDeferredImports) {
    ...
    for (const deferredDep of deferredDeps) {
        if (eagerDepsSet.has(deferredDep.ref.node)) {
            ...
            diagnostic = (0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.DEFERRED_DEPENDENCY_IMPORTED_EAGERLY, (0, util_1.getDiagnosticNode)(deferredDep.ref, rawDeferredImports), `\`${classInfo}\` is imported via both \`@Component.imports\` and ` +
    ...
    for (const ref of importRefs) {
        ...
        if (dirMeta !== null) {
            if (!dirMeta.isStandalone) {
                ...
                diagnostics.push((0, scope_1.makeNotStandaloneDiagnostic)(scopeReader, ref, importExpr, dirMeta.isComponent ? 'component' : 'directive'));
            }
            continue;
        }
        ...
        if (pipeMeta !== null) {
            if (!pipeMeta.isStandalone) {
                diagnostics.push((0, scope_1.makeNotStandaloneDiagnostic)(scopeReader, ref, importExpr, 'pipe'));
            }
            continue;
        }
        ...
        const error = isDeferredImport
            ? (0, util_1.makeUnknownComponentDeferredImportDiagnostic)(ref, importExpr)
            : (0, scope_1.makeUnknownComponentImportDiagnostic)(ref, importExpr);
