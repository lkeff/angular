...
exports.findInheritedCtor = findInheritedCtor;
...
function createValueHasWrongTypeError(node, value, messageText) {
    ...
    if (value instanceof partial_evaluator_1.DynamicValue) {
        ...
        const referenceNode = (_a = (0, typescript_2.identifierOfNode)(value.node)) !== null && _a !== void 0 ? _a : value.node;
...
function getProviderDiagnostics(providerClasses, providersDeclaration, registry) {
    ...
    for (const provider of providerClasses) {
        ...
        diagnostics.push((0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.UNDECORATED_PROVIDER, contextNode, `The class '${provider.node.name.text}' cannot be created via dependency injection, as it does not have an Angular decorator. This will result in an error at runtime.
...
function validateHostDirectives(origin, hostDirectives, metaReader) {
    ...
    for (const current of hostDirectives) {
        ...
        const hostMeta = (0, metadata_1.flattenInheritedDirectiveMetadata)(metaReader, current.directive);
        if (hostMeta === null) {
            diagnostics.push((0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.HOST_DIRECTIVE_INVALID, current.directive.getOriginForDiagnostics(origin), `${current.directive.debugName} must be a standalone directive to be used as a host directive`));
            continue;
        }
        if (!hostMeta.isStandalone) {
            diagnostics.push((0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.HOST_DIRECTIVE_NOT_STANDALONE, current.directive.getOriginForDiagnostics(origin), `Host directive ${hostMeta.name} must be standalone`));
        }
        if (hostMeta.isComponent) {
            diagnostics.push((0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.HOST_DIRECTIVE_COMPONENT, current.directive.getOriginForDiagnostics(origin), `Host directive ${hostMeta.name} cannot be a component`));
        }
...
function validateHostDirectiveMappings(bindingType, hostDirectiveMeta, meta, origin, diagnostics, requiredBindings) {
    if (!(0, metadata_1.isHostDirectiveMetaForGlobalMode)(hostDirectiveMeta)) {
        throw new Error('Impossible state: diagnostics code path for local compilation');
    }
    ...
    for (const publicName in hostDirectiveMappings) {
        if (hostDirectiveMappings.hasOwnProperty(publicName)) {
            ...
            if (bindings === null) {
                diagnostics.push((0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.HOST_DIRECTIVE_UNDEFINED_BINDING, hostDirectiveMeta.directive.getOriginForDiagnostics(origin), `Directive ${className} does not have an ${bindingType} with a public name of ${publicName}.`));
            }
    ...
    if (requiredBindings !== null && requiredBindings.size !== exposedRequiredBindings.size) {
        ...
        diagnostics.push((0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.HOST_DIRECTIVE_MISSING_REQUIRED_BINDING, hostDirectiveMeta.directive.getOriginForDiagnostics(origin), `Required ${bindingType}${missingBindings.length === 1 ? '' : 's'} ${missingBindings.join(', ')} from host directive ${className} must be exposed.`));
    }
}
...
function checkInheritanceOfInjectable(node, injectableRegistry, reflector, evaluator, strictInjectionParameters, kind) {
    ...
    if ((0, typescript_2.isFromDtsFile)(classWithCtor.ref.node)) {
    ...
    if (!strictInjectionParameters || (0, util_1.isAbstractClassDeclaration)(node)) {
...
function findInheritedCtor(node, injectableRegistry, reflector, evaluator) {
    ...
    let baseClass = (0, util_1.readBaseClass)(node, reflector, evaluator);
    while (baseClass !== null) {
        ...
        baseClass = (0, util_1.readBaseClass)(baseClass.node, reflector, evaluator);
    }
    return null;
}
function getInheritedInvalidCtorDiagnostic(node, baseClass, kind) {
    const baseClassName = baseClass.debugName;
    return (0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.INJECTABLE_INHERITS_INVALID_CONSTRUCTOR, node.name, `The ${kind.toLowerCase()} ${node.name.text} inherits its constructor from ${baseClassName}, ` +
...
function getInheritedUndecoratedCtorDiagnostic(node, baseClass, kind) {
    ...
    return (0, diagnostics_1.makeDiagnostic)(diagnostics_1.ErrorCode.DIRECTIVE_INHERITS_UNDECORATED_CTOR, node.name, `The ${kind.toLowerCase()} ${node.name.text} inherits its constructor from ${baseClassName}, ` +
...
function assertLocalCompilationUnresolvedConst(compilationMode, value, nodeToHighlight, errorMessage) {
    if (compilationMode === transform_1.CompilationMode.LOCAL &&
        value instanceof partial_evaluator_1.DynamicValue &&
        value.isFromUnknownIdentifier()) {
        throw new diagnostics_1.FatalDiagnosticError(diagnostics_1.ErrorCode.LOCAL_COMPILATION_UNRESOLVED_CONST, nodeToHighlight !== null && nodeToHighlight !== void 0 ? nodeToHighlight : value.node, errorMessage);
    }
}
