...
exports.TcbInliningRequirement = void 0;
exports.requiresInlineTypeCheckBlock = requiresInlineTypeCheckBlock;
exports.getSourceMapping = getSourceMapping;
exports.findTypeCheckBlock = findTypeCheckBlock;
exports.findSourceLocation = findSourceLocation;
exports.ensureTypeCheckFilePreparationImports = ensureTypeCheckFilePreparationImports;
exports.checkIfGenericTypeBoundsCanBeEmitted = checkIfGenericTypeBoundsCanBeEmitted;
exports.findNodeInFile = findNodeInFile;
...
const TCB_FILE_IMPORT_GRAPH_PREPARE_IDENTIFIERS = [
    ...
    // or removed.
    compiler_1.R3Identifiers.InputSignalBrandWriteType,
];
/**
...
})(TcbInliningRequirement || (exports.TcbInliningRequirement = TcbInliningRequirement = {}));
...
function getSourceMapping(shimSf, position, resolver, isDiagnosticRequest) {
    const node = (0, typescript_2.getTokenAtPosition)(shimSf, position);
...
function isInHostBindingTcb(node) {
    let current = node;
    while (current && !typescript_1.default.isFunctionDeclaration(current)) {
        if ((0, host_bindings_1.isHostBindingsBlockGuard)(current)) {
            return true;
        }
...
function findTypeCheckBlock(file, id, isDiagnosticRequest) {
    ...
    for (const stmt of file.statements) {
        if (typescript_1.default.isFunctionDeclaration(stmt) && getTypeCheckId(stmt, file, isDiagnosticRequest) === id) {
            return stmt;
        }
    }
    ...
    return findNodeInFile(file, (node) => typescript_1.default.isFunctionDeclaration(node) && getTypeCheckId(node, file, isDiagnosticRequest) === id);
}
/**
...
function findSourceLocation(node, sourceFile, isDiagnosticsRequest) {
    ...
    while (node !== undefined && !typescript_1.default.isFunctionDeclaration(node)) {
        if ((0, comments_1.hasIgnoreForDiagnosticsMarker)(node, sourceFile) && isDiagnosticsRequest) {
            // There's an ignore marker on this node, so the diagnostic should not be reported.
            return null;
        }
        const span = (0, comments_1.readSpanComment)(node, sourceFile);
        if (span !== null) {
...
function getTypeCheckId(node, sourceFile, isDiagnosticRequest) {
    ...
    while (!typescript_1.default.isFunctionDeclaration(node)) {
    ...
    return (typescript_1.default.forEachLeadingCommentRange(sourceFile.text, start, (pos, end, kind) => {
...
function checkIfGenericTypeBoundsCanBeEmitted(node, reflector, env) {
    ...
    const emitter = new type_parameter_emitter_1.TypeParameterEmitter(node.typeParameters, reflector);
...
function findNodeInFile(file, predicate) {
    ...
    const visit = (node) => {
        ...
        return (_a = typescript_1.default.forEachChild(node, visit)) !== null && _a !== void 0 ? _a : null;
    };
    return (_a = typescript_1.default.forEachChild(file, visit)) !== null && _a !== void 0 ? _a : null;
}
