...
exports.createHostBindingsBlockGuard = createHostBindingsBlockGuard;
exports.isHostBindingsBlockGuard = isHostBindingsBlockGuard;
...
function createHostElement(type, selector, sourceNode, literal, bindingDecorators, listenerDecorators) {
    ...
    if (literal !== null) {
        for (const prop of literal.properties) {
            ...
            if (typescript_1.default.isPropertyAssignment(prop) &&
                ...
                parser !== null && parser !== void 0 ? parser : (parser = (0, compiler_1.makeBindingParser)());
    ...
    const tagNames = [];
    if (selector !== null) {
        const parts = compiler_1.CssSelector.parse(selector);
    ...
    return new compiler_1.TmplAstHostElement(tagNames, bindings, listeners, (0, common_1.createSourceSpan)(sourceNode.name));
}
/**
...
function createHostBindingsBlockGuard() {
    ...
    // `true /*hostBindings*/`.
    const trueExpr = typescript_1.default.addSyntheticTrailingComment(typescript_1.default.factory.createTrue(), typescript_1.default.SyntaxKind.MultiLineCommentTrivia, GUARD_COMMENT_TEXT);
    // Wrap the expression in parentheses to ensure that the comment is attached to the correct node.
    return typescript_1.default.factory.createParenthesizedExpression(trueExpr);
}
/**
...
function isHostBindingsBlockGuard(node) {
    ...
    if (!typescript_1.default.isParenthesizedExpression(expr) || expr.expression.kind !== typescript_1.default.SyntaxKind.TrueKeyword) {
        return false;
    }
    const text = expr.getSourceFile().text;
    return (typescript_1.default.forEachTrailingCommentRange(text, expr.expression.getEnd(), (pos, end, kind) => kind === typescript_1.default.SyntaxKind.MultiLineCommentTrivia &&
...
function createNodeFromHostLiteralProperty(property, parser, bindings, listeners) {
    ...
    if (name.text.startsWith('[') && name.text.endsWith(']')) {
        ...
        bindings.push(new compiler_1.TmplAstBoundAttribute(attrName, type, 0, ast, null, (0, common_1.createSourceSpan)(property), createStaticExpressionSpan(name), valueSpan, undefined));
    }
    else if (name.text.startsWith('(') && name.text.endsWith(')')) {
        const events = [];
        parser.parseEvent(name.text.slice(1, -1), initializer.text, false, (0, common_1.createSourceSpan)(property), createStaticExpressionSpan(initializer), [], events, createStaticExpressionSpan(name));
        if (events.length === 0 || events[0].handler.errors.length > 0) {
            return; // See TODO above.
        }
        fixupSpans(events[0].handler, initializer);
        listeners.push(compiler_1.TmplAstBoundEvent.fromParsedEvent(events[0]));
    }
}
/**
...
function createNodeFromBindingDecorator(decorator, bindings) {
    ...
    if (property && typescript_1.default.isPropertyDeclaration(property) && isStaticName(property.name)) {
    ...
    else if (typescript_1.default.isStringLiteralLike(args[0])) {
        nameNode = args[0];
    }
    else {
        return;
    ...
    const span = new compiler_1.ParseSpan(-1, -1);
    const propertyStart = property.getStart();
    const receiver = new compiler_1.ThisReceiver(span, new compiler_1.AbsoluteSourceSpan(propertyStart, propertyStart));
    const nameSpan = new compiler_1.AbsoluteSourceSpan(propertyName.getStart(), propertyName.getEnd());
    const read = typescript_1.default.isIdentifier(propertyName)
        ? new compiler_1.PropertyRead(span, nameSpan, nameSpan, receiver, propertyName.text)
        : new compiler_1.KeyedRead(span, nameSpan, receiver, new compiler_1.LiteralPrimitive(span, nameSpan, propertyName.text));
    const { attrName, type } = inferBoundAttribute(nameNode.text);
    bindings.push(new compiler_1.TmplAstBoundAttribute(attrName, type, 0, read, null, (0, common_1.createSourceSpan)(decorator), createStaticExpressionSpan(nameNode), (0, common_1.createSourceSpan)(decorator), undefined));
}
/**
...
function createNodeFromListenerDecorator(decorator, parser, listeners) {
    ...
    if (!typescript_1.default.isCallExpression(decorator.expression) || decorator.expression.arguments.length === 0) {
        return;
    }
    const args = decorator.expression.arguments;
    ...
    if (!method ||
        !typescript_1.default.isMethodDeclaration(method) ||
        !isStaticName(method.name) ||
        !typescript_1.default.isStringLiteralLike(args[0])) {
        return;
    }
    // We can't synthesize a fake expression here and pass it through the binding parser, because
    // it constructs all the spans based on the source code origin and they aren't easily mappable
    ...
    const methodReceiver = new compiler_1.ThisReceiver(span, new compiler_1.AbsoluteSourceSpan(methodStart, methodStart));
    const nameSpan = new compiler_1.AbsoluteSourceSpan(method.name.getStart(), method.name.getEnd());
    const receiver = typescript_1.default.isIdentifier(method.name)
        ? new compiler_1.PropertyRead(span, nameSpan, nameSpan, methodReceiver, method.name.text)
        : new compiler_1.KeyedRead(span, nameSpan, methodReceiver, new compiler_1.LiteralPrimitive(span, nameSpan, method.name.text));
    if (args.length > 1 && typescript_1.default.isArrayLiteralExpression(args[1])) {
        for (const expr of args[1].elements) {
            ...
            if (typescript_1.default.isStringLiteralLike(expr)) {
                ...
                const expressionSpan = new compiler_1.AbsoluteSourceSpan(expr.getStart(), expr.getEnd());
                const anyRead = new compiler_1.PropertyRead(span, expressionSpan, expressionSpan, new compiler_1.ImplicitReceiver(span, expressionSpan), '$any');
                const anyCall = new compiler_1.Call(span, expressionSpan, anyRead, [new compiler_1.LiteralPrimitive(span, expressionSpan, 0)], expressionSpan);
                argNodes.push(anyCall);
            }
        }
    }
    const callNode = new compiler_1.Call(span, nameSpan, receiver, argNodes, span);
    ...
    if (eventNameNode.text.startsWith('@')) {
        ...
        type = compiler_1.ParsedEventType.Animation;
        ...
        type = compiler_1.ParsedEventType.Regular;
        eventName = parsedName.eventName;
        target = parsedName.target;
        phase = null;
    }
    listeners.push(new compiler_1.TmplAstBoundEvent(eventName, type, callNode, target, phase, (0, common_1.createSourceSpan)(decorator), (0, common_1.createSourceSpan)(decorator), createStaticExpressionSpan(eventNameNode)));
}
/**
...
function fixupSpans(ast, initializer) {
    ...
    if (escapeIndex > -1) {
        const newSpan = new compiler_1.ParseSpan(0, initializer.getWidth());
        const newSourceSpan = new compiler_1.AbsoluteSourceSpan(initializer.getStart(), initializer.getEnd());
 ...
 */
class ReplaceSpanVisitor extends compiler_1.RecursiveAstVisitor {
    ...
    visit(ast) {
        ...
        if (ast.span.start >= this.afterIndex || ast.span.end >= this.afterIndex) {
            ...
            if (ast instanceof compiler_1.Call || ast instanceof compiler_1.SafeCall) {
