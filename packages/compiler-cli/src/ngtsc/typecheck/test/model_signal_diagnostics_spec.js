    ...
    describe('model inputs type-check diagnostics', () => {
        const bindingCases = [
            ...
            },
            {
                ...
                expected: [
                    jasmine.stringContaining(`Object literal may only specify known properties, and '"extraField"' does not exist in type '{ works: boolean; }'.`),
                ],
            },
            {
        ...
        `,
            ...
            },
            {
                ...
                component: `val!: (v: string) => number;`,
                expected: [
                    jasmine.stringContaining(`TestComponent.html(1, 12): Type '(v: string) => number' is not assignable to type '(v: number) => number`),
                    jasmine.stringContaining(`TestComponent.html(1, 10): Type '(v: number) => number' is not assignable to type '(v: string) => number`),
                ],
            },
        ];
        (0, test_case_helper_1.generateDiagnoseJasmineSpecs)(bindingCases);
    });
});
