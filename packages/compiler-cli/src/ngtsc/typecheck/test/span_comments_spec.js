...
describe('type check blocks diagnostics', () => {
    ...
    describe('parse spans', () => {
        ...
        it('should annotate binary ops', () => {
            expect(tcbWithSpans('{{ a + b }}')).toContain('(((this).a /*3,4*/) /*3,4*/) + (((this).b /*7,8*/) /*7,8*/) /*3,8*/');
        });
        it('should annotate conditions', () => {
            expect(tcbWithSpans('{{ a ? b : c }}')).toContain('(((this).a /*3,4*/) /*3,4*/ ? ((this).b /*7,8*/) /*7,8*/ : (((this).c /*11,12*/) /*11,12*/)) /*3,12*/');
        });
        it('should annotate interpolations', () => {
            expect(tcbWithSpans('{{ hello }} {{ world }}')).toContain('"" + (((this).hello /*3,8*/) /*3,8*/) + (((this).world /*15,20*/) /*15,20*/)');
        });
        it('should annotate literal map expressions', () => {
            ...
            expect(tcbWithSpans(TEMPLATE)).toContain('(this).m /*3,4*/({ "foo": ((this).a /*11,12*/) /*11,12*/, "bar": ((this).b /*19,20*/) /*19,20*/ } /*5,21*/) /*3,22*/');
        });
        it('should annotate literal map expressions with shorthand declarations', () => {
            // The additional method call is present to avoid that the object literal is emitted as
            // statement, which would wrap it into parenthesis that clutter the expected output.
            const TEMPLATE = '{{ m({a, b}) }}';
            expect(tcbWithSpans(TEMPLATE)).toContain('((this).m /*3,4*/({ "a": ((this).a /*6,7*/) /*6,7*/, "b": ((this).b /*9,10*/) /*9,10*/ } /*5,11*/) /*3,12*/)');
        });
        ...
        it('should annotate method calls', () => {
            ...
            expect(tcbWithSpans(TEMPLATE)).toContain('(this).method /*3,9*/(((this).a /*10,11*/) /*10,11*/, ((this).b /*13,14*/) /*13,14*/) /*3,15*/');
        });
        it('should annotate safe calls', () => {
            const TEMPLATE = `{{ method?.(a, b) }}`;
            expect(tcbWithSpans(TEMPLATE)).toContain('((0 as any ? (((this).method /*3,9*/) /*3,9*/)!(((this).a /*12,13*/) /*12,13*/, ((this).b /*15,16*/) /*15,16*/) : undefined) /*3,17*/)');
        });
        it('should annotate method calls of variables', () => {
            const TEMPLATE = `<ng-template let-method>{{ method(a, b) }}</ng-template>`;
            expect(tcbWithSpans(TEMPLATE)).toContain('_t2 /*27,33*/(((this).a /*34,35*/) /*34,35*/, ((this).b /*37,38*/) /*37,38*/) /*27,39*/');
        });
        it('should annotate function calls', () => {
            const TEMPLATE = `{{ method(a)(b, c) }}`;
            expect(tcbWithSpans(TEMPLATE)).toContain('(this).method /*3,9*/(((this).a /*10,11*/) /*10,11*/) /*3,12*/(((this).b /*13,14*/) /*13,14*/, ((this).c /*16,17*/) /*16,17*/) /*3,18*/');
        });
        it('should annotate property access', () => {
            const TEMPLATE = `{{ a.b.c }}`;
            expect(tcbWithSpans(TEMPLATE)).toContain('((((((this).a /*3,4*/) /*3,4*/).b /*5,6*/) /*3,6*/).c /*7,8*/) /*3,8*/');
        });
        it('should annotate property writes', () => {
            const TEMPLATE = `<div (click)='a.b.c = d'></div>`;
            expect(tcbWithSpans(TEMPLATE)).toContain('(((((((this).a /*14,15*/) /*14,15*/).b /*16,17*/) /*14,17*/).c /*18,19*/) /*14,23*/ = (((this).d /*22,23*/) /*22,23*/)) /*14,23*/');
        });
        it('should $event property writes', () => {
            const TEMPLATE = `<div (click)='a = $event'></div>`;
            expect(tcbWithSpans(TEMPLATE)).toContain('(((this).a /*14,15*/) /*14,24*/ = ($event /*18,24*/)) /*14,24*/;');
        });
        it('should annotate keyed property access', () => {
            const TEMPLATE = `{{ a[b] }}`;
            expect(tcbWithSpans(TEMPLATE)).toContain('(((this).a /*3,4*/) /*3,4*/)[((this).b /*5,6*/) /*5,6*/] /*3,7*/');
        });
        it('should annotate keyed property writes', () => {
            const TEMPLATE = `<div (click)="a[b] = c"></div>`;
            expect(tcbWithSpans(TEMPLATE)).toContain('((((this).a /*14,15*/) /*14,15*/)[((this).b /*16,17*/) /*16,17*/] = (((this).c /*21,22*/) /*21,22*/)) /*14,22*/');
        });
        it('should annotate safe property access', () => {
            const TEMPLATE = `{{ a?.b }}`;
            expect(tcbWithSpans(TEMPLATE)).toContain('(0 as any ? (((this).a /*3,4*/) /*3,4*/)!.b /*6,7*/ : undefined) /*3,7*/');
        });
        it('should annotate safe method calls', () => {
            const TEMPLATE = `{{ a?.method(b) }}`;
            expect(tcbWithSpans(TEMPLATE)).toContain('((0 as any ? (0 as any ? (((this).a /*3,4*/) /*3,4*/)!.method /*6,12*/ : undefined) /*3,12*/!(((this).b /*13,14*/) /*13,14*/) : undefined) /*3,15*/)');
        });
        it('should annotate safe keyed reads', () => {
            const TEMPLATE = `{{ a?.[0] }}`;
            expect(tcbWithSpans(TEMPLATE)).toContain('(0 as any ? (((this).a /*3,4*/) /*3,4*/)![0 /*7,8*/] /*3,9*/ : undefined) /*3,9*/');
        });
        ...
        it('should annotate chained expressions', () => {
            const TEMPLATE = `<div (click)='a; b; c'></div>`;
            expect(tcbWithSpans(TEMPLATE)).toContain('(((this).a /*14,15*/) /*14,15*/, ((this).b /*17,18*/) /*17,18*/, ((this).c /*20,21*/) /*20,21*/) /*14,21*/');
        });
        it('should annotate pipe usages', () => {
            ...
            expect(block).toContain('(_pipe1.transform /*7,11*/(((this).a /*3,4*/) /*3,4*/, ((this).b /*12,13*/) /*12,13*/) /*3,13*/);');
        });
        describe('attaching multiple comments for multiple references', () => {
            ...
            it('should be correct for directive refs', () => {
                ...
                expect(tcbWithSpans(TEMPLATE, DIRECTIVES)).toContain('((_t1 /*23,24*/) || (_t1 /*28,29*/) /*23,29*/);');
            });
        });
        describe('attaching comments for generic directive inputs', () => {
            it('should be correct for directive refs', () => {
                ...
                expect(tcbWithSpans(TEMPLATE, DIRECTIVES)).toContain('_t1.inputA /*9,15*/ = ("" /*18,20*/) /*8,21*/;');
            });
        });
        describe('control flow', () => {
            it('@for', () => {
                ...
                // index variable
                expect(tcbWithSpans(template, [])).toContain('var _t2 /*37,38*/ = null! as number /*T:VAE*/ /*37,47*/');
                // track expression
            ...
            it('@for with comma separated variables', () => {
                ...
                expect(tcbWithSpans(template, [])).toContain('var _t2 /*27,28*/ = null! as number /*T:VAE*/ /*27,37*/');
                expect(tcbWithSpans(template, [])).toContain('var _t3 /*39,42*/ = null! as boolean /*T:VAE*/ /*39,54*/');
                expect(tcbWithSpans(template, [])).toContain('var _t4 /*55,62*/ = null! as boolean /*T:VAE*/ /*55,68*/');
            });
            it('@if', () => {
...
function tcbWithSpans(template, declarations = []) {
    return (0, testing_2.tcb)(template, declarations, undefined, { emitSpans: true });
}
