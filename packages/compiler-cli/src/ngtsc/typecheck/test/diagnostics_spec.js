...
(0, testing_1.runInEachFileSystem)(() => {
    describe('template diagnostics', () => {
        it('works for directive bindings', () => {
            const messages = (0, testing_2.diagnose)(`<div dir [input]="person.name"></div>`, `
        class Dir {
          input: number;
        }
        class TestComponent {
        ...
        it('infers type of template variables', () => {
            const messages = (0, testing_2.diagnose)(`<div *ngFor="let person of persons; let idx=index">{{ render(idx) }}</div>`, `
        class TestComponent {
          persons: {}[];

          render(input: string): string { return input; }
        }`, [(0, testing_2.ngForDeclaration)()], [(0, testing_2.ngForDts)()]);
            expect(messages).toEqual([
        ...
        it('infers any type when generic type inference fails', () => {
            const messages = (0, testing_2.diagnose)(`<div *ngFor="let person of persons;">{{ render(person.namme) }}</div>`, `
        class TestComponent {
          persons: any;
        ...
        it('infers type of element references', () => {
            const messages = (0, testing_2.diagnose)(`<div dir #el>{{ render(el) }}</div>`, `
        class Dir {
          value: number;
        }
        class TestComponent {
        ...
        it('infers type of directive references', () => {
            const messages = (0, testing_2.diagnose)(`<div dir #dir="dir">{{ render(dir) }}</div>`, `
        class Dir {
          value: number;
        }
        class TestComponent {
        ...
        it('infers TemplateRef<any> for ng-template references', () => {
            const messages = (0, testing_2.diagnose)(`<ng-template #tmpl>{{ render(tmpl) }}</ng-template>`, `
      class TestComponent {
        render(input: string): string { return input; }
      }`);
            expect(messages).toEqual([
        ...
        it('infers type of template context', () => {
            const messages = (0, testing_2.diagnose)(`<div *ngFor="let person of persons">{{ person.namme }}</div>`, `
        class TestComponent {
          persons: {
            name: string;
          }[];
        ...
        it('interprets interpolation as strings', () => {
            const messages = (0, testing_2.diagnose)(`<blockquote title="{{ person }}"></blockquote>`, `
      class TestComponent {
        person: {};
      }`);
            expect(messages).toEqual([]);
        ...
        it('checks bindings to regular element', () => {
            const messages = (0, testing_2.diagnose)(`<img [srcc]="src" [height]="heihgt">`, `
      class TestComponent {
        src: string;
        height: number;
      }`);
        ...
        it('checks text attributes that are consumed by bindings with literal string types', () => {
            const messages = (0, testing_2.diagnose)(`<div dir mode="drak"></div><div dir mode="light"></div>`, `
        class Dir {
          mode: 'dark'|'light';
        }
        class TestComponent {}`, [
        ...
        it('checks expressions in ICUs', () => {
            const messages = (0, testing_2.diagnose)(`<span i18n>{switch, plural, other { {{interpolation}}
            {nestedSwitch, plural, other { {{nestedInterpolation}} }}
          }}</span>`, `class TestComponent {}`);
        ...
        it('produces diagnostics for pipes', () => {
            const messages = (0, testing_2.diagnose)(`<div>{{ person.name | pipe:person.age:1 }}</div>`, `
        class Pipe {
        ...
        it('does not repeat diagnostics for missing pipes in directive inputs', () => {
            ...
            const messages = (0, testing_2.diagnose)('<div *dir="let x of name | uppercase"></div>', `
            class Dir<T> {
              dirOf: T;
            }
        ...
        it('does not repeat diagnostics for errors within LHS of safe-navigation operator', () => {
            const messages = (0, testing_2.diagnose)(`{{ personn?.name }} {{ personn?.getName() }}`, `
         class TestComponent {
           person: {
             name: string;
             getName: () => string;
        ...
        it('does not repeat diagnostics for errors used in template guard expressions', () => {
            const messages = (0, testing_2.diagnose)(`<div *guard="personn.name"></div>`, `
          class GuardDir {
        ...
        it('should retain literal types in object literals together if strictNullInputBindings is disabled', () => {
            const messages = (0, testing_2.diagnose)(`<div dir [ngModelOptions]="{updateOn: 'change'}"></div>`, `
              class Dir {
        ...
        it('should retain literal types in array literals together if strictNullInputBindings is disabled', () => {
            const messages = (0, testing_2.diagnose)(`<div dir [options]="['literal']"></div>`, `
                class Dir {
        ...
        it('does not produce diagnostics for user code', () => {
            const messages = (0, testing_2.diagnose)(`{{ person.name }}`, `
      class TestComponent {
        person: {
          name: string;
        };
        ...
        it('should treat unary operators as literal types', () => {
            const messages = (0, testing_2.diagnose)(`{{ test(-1) + test(+1) + test(-2) }}`, `
      class TestComponent {
        ...
        it('should support type-narrowing for methods with type guards', () => {
            const messages = (0, testing_2.diagnose)(`<div *ngIf="hasSuccess()">{{ success }}</div>`, `
          class TestComponent {
            hasSuccess(): this is { success: boolean };
          }`, [(0, testing_2.ngIfDeclaration)()], [(0, testing_2.ngIfDts)()]);
            expect(messages).toEqual([]);
        });
        describe('outputs', () => {
            it('should produce a diagnostic for directive outputs', () => {
                const messages = (0, testing_2.diagnose)(`<div dir (event)="handleEvent($event)"></div>`, `
          ...
          class TestComponent {
            ...
            it('should produce a diagnostic for animation events', () => {
                const messages = (0, testing_2.diagnose)(`<div dir (@animation.done)="handleEvent($event)"></div>`, `
          class TestComponent {
            handleEvent(event: string): void {}
          }`);
                expect(messages).toEqual([
            ...
            it('should produce a diagnostic for element outputs', () => {
                const messages = (0, testing_2.diagnose)(`<div (click)="handleEvent($event)"></div>`, `
          import {EventEmitter} from '@angular/core';
          class TestComponent {
            handleEvent(event: string): void {}
          }`);
          ...
          class TestComponent {
            ...
            it('should not produce a diagnostic for implicit any return types', () => {
                const messages = (0, testing_2.diagnose)(`<div (click)="state = null"></div>`, `
          class TestComponent {
            state: any;
          }`, 
        ...
        describe('strict null checks', () => {
            it('produces diagnostic for unchecked property access', () => {
                const messages = (0, testing_2.diagnose)(`<div [class.has-street]="person.address.street.length > 0"></div>`, `
        export class TestComponent {
          person: {
            ...
            it('does not produce diagnostic for checked property access', () => {
                const messages = (0, testing_2.diagnose)(`<div [class.has-street]="person.address && person.address.street.length > 0"></div>`, `
        export class TestComponent {
          person: {
            address?: {
              street: string;
            ...
            it('does not produce diagnostic for fallback value using nullish coalescing', () => {
                const messages = (0, testing_2.diagnose)(`<div>{{ greet(name ?? 'Frodo') }}</div>`, `
        export class TestComponent {
          name: string | null;
          ...
          greet(name: string) {
            ...
            it('does not produce diagnostic for safe keyed access', () => {
                const messages = (0, testing_2.diagnose)(`<div [class.red-text]="person.favoriteColors?.[0] === 'red'"></div>`, `
              export class TestComponent {
                person: {
            ...
            it('infers a safe keyed read as undefined', () => {
                const messages = (0, testing_2.diagnose)(`<div (click)="log(person.favoriteColors?.[0])"></div>`, `
          export class TestComponent {
            person: {
              favoriteColors?: string[];
            };
  ...
  Type 'undefined' is not assignable to type 'string'.`,
            ...
            it('does not produce diagnostic for safe calls', () => {
                const messages = (0, testing_2.diagnose)(`<div [class.is-hobbit]="person.getName?.() === 'Bilbo'"></div>`, `
              export class TestComponent {
                person: {
                  getName?: () => string;
                };
            ...
            it('infers a safe call return value as undefined', () => {
                const messages = (0, testing_2.diagnose)(`<div (click)="log(person.getName?.())"></div>`, `
          export class TestComponent {
            person: {
              getName?: () => string;
            };
  ...
  Type 'undefined' is not assignable to type 'string'.`,
        ...
        it('computes line and column offsets', () => {
            const messages = (0, testing_2.diagnose)(`
<div>
  <img [src]="srcc"
       [height]="heihgt">
</div>
...
class TestComponent {
  ...
  height: number;
        ...
        it('works for shorthand property declarations', () => {
            const messages = (0, testing_2.diagnose)(`<div dir [input]="{a, b: 2}"></div>`, `
        class Dir {
        ...
        it('allows access to protected members', () => {
            const messages = (0, testing_2.diagnose)(`<button (click)="doFoo()">{{ message }}</button>`, `
        class TestComponent {
    ...
    describe('method call spans', () => {
        it('reports invalid method name on method name span', () => {
            const messages = (0, testing_2.diagnose)(`{{ person.getNName() }}`, `
        export class TestComponent {
          person: {
            getName(): string;
          };
        ...
        it('reports invalid method call signature on parameter span', () => {
            const messages = (0, testing_2.diagnose)(`{{ person.getName('abcd') }}`, `
        export class TestComponent {
          person: {
            getName(): string;
          };
    ...
    describe('property write spans', () => {
        it('reports invalid receiver property access on property access name span', () => {
            const messages = (0, testing_2.diagnose)(`<div (click)="person.nname = 'jacky'"></div>`, `
        export class TestComponent {
          person: {
            name: string;
          };
        ...
        it('reports unassignable value on property write span', () => {
            const messages = (0, testing_2.diagnose)(`<div (click)="person.name = 2"></div>`, `
        export class TestComponent {
          person: {
            name: string;
          };
    ...
    it('should not fail for components outside of rootDir', () => {
        ...
        const messages = (0, testing_2.diagnose)(`{{invalid}}`, `export class TestComponent {}`, [], [], {}, { rootDir: '/root' });
        expect(messages).toEqual([
    ...
    describe('host directives', () => {
        it('should produce a diagnostic for host directive input bindings', () => {
            const messages = (0, testing_2.diagnose)(`<div dir [input]="person.name" [alias]="person.age"></div>`, `
            class Dir {
            }
            class HostDir {
              input: number;
        ...
        it('should produce a diagnostic for directive outputs', () => {
            const messages = (0, testing_2.diagnose)(`<div
            dir
        ...
        it('should produce a diagnostic for host directive inputs and outputs that have not been exposed', () => {
            const messages = (0, testing_2.diagnose)(`<div dir [input]="person.name" (output)="handleStringEvent($event)"></div>`, `
            class Dir {
            }
            class HostDir {
              input: number;
        ...
        it('should infer the type of host directive references', () => {
            const messages = (0, testing_2.diagnose)(`<div dir #hostDir="hostDir">{{ render(hostDir) }}</div>`, `
            class Dir {}
            class HostDir {
              value: number;
            }
    ...
    describe('required inputs', () => {
        it('should produce a diagnostic when a single required input is missing', () => {
            const messages = (0, testing_2.diagnose)(`<div dir></div>`, `
            class Dir {
              input: any;
            }
            class TestComponent {}
        ...
        it('should produce a diagnostic when a multiple required inputs are missing', () => {
            const messages = (0, testing_2.diagnose)(`<div dir otherDir></div>`, `
            class Dir {
              input: any;
              otherInput: any;
            }
        ...
        it('should not produce a diagnostic if a required input is used in a binding', () => {
            const messages = (0, testing_2.diagnose)(`<div dir [input]="foo"></div>`, `
            class Dir {
              input: any;
            }
            class TestComponent {
        ...
        it('should not produce a diagnostic if a required input is used in a binding through an alias', () => {
            const messages = (0, testing_2.diagnose)(`<div dir [inputAlias]="foo"></div>`, `
                class Dir {
                  input: any;
                }
                class TestComponent {
        ...
        it('should not produce a diagnostic if a required input is used in a static binding', () => {
            const messages = (0, testing_2.diagnose)(`<div dir input="hello"></div>`, `
            class Dir {
              input: any;
            }
            class TestComponent {}
        ...
        it('should not report missing required inputs for an attribute binding with the same name', () => {
            const messages = (0, testing_2.diagnose)(`<div [attr.maxlength]="123"></div>`, `
    ...
    describe('template parse failures', () => {
        afterEach(diagnostics_1.resetParseTemplateAsSourceFileForTest);
        it('baseline test without parse failure', () => {
            const messages = (0, testing_2.diagnose)(`<div (click)="test(name)"></div>`, `
      export class TestComponent {
  ...
  Type 'undefined' is not assignable to type 'string'.`,
        ...
        it('should handle TypeScript parse failures gracefully', () => {
            (0, diagnostics_1.setParseTemplateAsSourceFileForTest)(() => {
      ...
      export class TestComponent {
        ...
        test(n: string): void {}
            ...
            expect(messages.length).toBe(1);
            expect(messages[0]).toContain(`main.ts(2, 20): Argument of type 'string | undefined' is not assignable to parameter of type 'string'.
  Type 'undefined' is not assignable to type 'string'.
