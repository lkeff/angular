...
const testing_2 = require("../../logging/testing");
const source_file_loader_1 = require("../src/source_file_loader");
(0, testing_1.runInEachFileSystem)(() => {
    describe('SourceFileLoader', () => {
        ...
        beforeEach(() => {
            ...
            logger = new testing_2.MockLogger();
            _ = file_system_1.absoluteFrom;
            registry = new source_file_loader_1.SourceFileLoader(fs, logger, { webpack: _('/foo') });
        });
        ...
        it('should log a warning if there is a cyclic dependency in source files loaded from disk', () => {
            ...
            fs.writeFile(aPath, 'a content\n' + convert_source_map_1.default.fromObject(aMap).toComment());
            const bPath = _('/foo/src/b.js');
            fs.writeFile(bPath, 'b content\n' +
                convert_source_map_1.default.fromObject(createRawSourceMap({ file: 'b.js', sources: ['c.js'] })).toComment());
            const cPath = _('/foo/src/c.js');
            fs.writeFile(cPath, 'c content\n' +
                convert_source_map_1.default.fromObject(createRawSourceMap({ file: 'c.js', sources: ['a.js'] })).toComment());
            ...
            expect(sourceFile.sources.length).toEqual(1);
            expect(logger.logs.warn[0][0]).toContain(`Circular source file mapping dependency: ` +
        ...
        it('should not fail if the filename of an inline source looks like a cyclic dependency', () => {
            ...
            const aMap = createRawSourceMap({
                file: 'a.js',
                sources: ['a.js'],
                sourcesContent: ['inline original a.js content'],
            });
            fs.writeFile(aPath, 'a content\n' + convert_source_map_1.default.fromObject(aMap).toComment());
            const sourceFile = registry.loadSourceFile(aPath);
            if (sourceFile === null) {
...
function createRawSourceMap(custom) {
    return Object.assign({ 'version': 3, 'sourceRoot': '', 'sources': [], 'sourcesContent': [], 'names': [], 'mappings': '' }, custom);
}
