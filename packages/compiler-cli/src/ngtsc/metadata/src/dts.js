...
class DtsMetadataReader {
    ...
    getNgModuleMetadata(ref) {
        ...
        ngModuleDef.type === null ||
            !typescript_1.default.isTypeReferenceNode(ngModuleDef.type) ||
        ...
        const declarations = (0, util_1.extractReferencesFromType)(this.checker, declarationMetadata, ref.bestGuessOwningModule);
        const exports = (0, util_1.extractReferencesFromType)(this.checker, exportMetadata, ref.bestGuessOwningModule);
        const imports = (0, util_1.extractReferencesFromType)(this.checker, importMetadata, ref.bestGuessOwningModule);
    ...
    getDirectiveMetadata(ref) {
        ...
        if (def === undefined) {
            ...
            !typescript_1.default.isTypeReferenceNode(def.type) ||
        ...
        const ngContentSelectors = def.type.typeArguments.length > 6 ? (0, util_1.readStringArrayType)(def.type.typeArguments[6]) : null;
        ...
        const isStandalone = def.type.typeArguments.length > 7 && ((_a = (0, util_1.readBooleanType)(def.type.typeArguments[7])) !== null && _a !== void 0 ? _a : false);
        const inputs = property_mapping_1.ClassPropertyMapping.fromMappedObject(readInputsType(def.type.typeArguments[3]));
        const outputs = property_mapping_1.ClassPropertyMapping.fromMappedObject((0, util_1.readMapType)(def.type.typeArguments[4], util_1.readStringType));
        const hostDirectives = def.type.typeArguments.length > 8
            ...
            : null;
        const isSignal = def.type.typeArguments.length > 9 && ((_b = (0, util_1.readBooleanType)(def.type.typeArguments[9])) !== null && _b !== void 0 ? _b : false);
        ...
        const isPoisoned = hostDirectives !== null && (hostDirectives === null || hostDirectives === void 0 ? void 0 : hostDirectives.isIncomplete);
        return Object.assign(Object.assign({ kind: api_1.MetaKind.Directive, matchSource: api_1.MatchSource.Selector, ref, name: clazz.name.text, isComponent, selector: (0, util_1.readStringType)(def.type.typeArguments[1]), exportAs: (0, util_1.readStringArrayType)(def.type.typeArguments[2]), inputs,
            outputs, hostDirectives: (_c = hostDirectives === null || hostDirectives === void 0 ? void 0 : hostDirectives.result) !== null && _c !== void 0 ? _c : null, queries: (0, util_1.readStringArrayType)(def.type.typeArguments[5]) }, (0, util_1.extractDirectiveTypeCheckMeta)(clazz, inputs, this.reflector)), { baseClass: readBaseClass(clazz, this.checker, this.reflector), isPoisoned,
            isStructural, animationTriggerNames: null, ngContentSelectors,
            isStandalone,
            isSignal, 
            ...
            imports: null, rawImports: null, deferredImports: null, 
            // The same goes for schemas.
            schemas: null, decorator: null, 
            ...
            preserveWhitespaces: false, isExplicitlyDeferred: false, 
            // We don't need to know if imported components from .d.ts
            // files are selectorless for type-checking purposes.
            selectorlessEnabled: false, localReferencedSymbols: null });
    }
    /**
    ...
    getPipeMetadata(ref) {
        ...
        const isStandalone = def.type.typeArguments.length > 2 && ((_a = (0, util_1.readBooleanType)(def.type.typeArguments[2])) !== null && _a !== void 0 ? _a : false);
        return {
            kind: api_1.MetaKind.Pipe,
            ref,
            name,
            nameExpr: null,
            isStandalone,
...
function readInputsType(type) {
    ...
    if (typescript_1.default.isTypeLiteralNode(type)) {
        for (const member of type.members) {
            ...
            const stringValue = (0, util_1.readStringType)(member.type);
            ...
            if (stringValue != null) {
                ...
                const config = (0, util_1.readMapType)(member.type, (innerValue) => {
                    var _a;
                    return (_a = (0, util_1.readStringType)(innerValue)) !== null && _a !== void 0 ? _a : (0, util_1.readBooleanType)(innerValue);
                });
                inputsMap[classPropertyName] = {
                    classPropertyName,
    ...
    if (clazz.heritageClauses !== undefined) {
        for (const clause of clazz.heritageClauses) {
            if (clause.token === typescript_1.default.SyntaxKind.ExtendsKeyword) {
                ...
                else if (symbol.flags & typescript_1.default.SymbolFlags.Alias) {
...
function readHostDirectivesType(checker, type, bestGuessOwningModule) {
    if (!typescript_1.default.isTupleTypeNode(type) || type.elements.length === 0) {
        return null;
    }
    const result = [];
    let isIncomplete = false;
    for (const hostDirectiveType of type.elements) {
        const { directive, inputs, outputs } = (0, util_1.readMapType)(hostDirectiveType, (type) => type);
        if (directive) {
            if (!typescript_1.default.isTypeQueryNode(directive)) {
                throw new Error(`Expected TypeQueryNode: ${(0, typescript_2.nodeDebugInfo)(directive)}`);
            }
            const ref = (0, util_1.extraReferenceFromTypeQuery)(checker, directive, type, bestGuessOwningModule);
            if (ref === null) {
                isIncomplete = true;
                continue;
            }
            result.push({
                ...
                inputs: (0, util_1.readMapType)(inputs, util_1.readStringType),
                outputs: (0, util_1.readMapType)(outputs, util_1.readStringType),
            });
        }
    }
