...
exports.isSyntheticAngularConstant = isSyntheticAngularConstant;
...
function extractConstant(declaration, typeChecker) {
    ...
    const rawComment = (0, jsdoc_extractor_1.extractRawJsDoc)(declaration.parent.parent);
    const jsdocTags = (0, jsdoc_extractor_1.extractJsDocTags)(declaration);
    const description = (0, jsdoc_extractor_1.extractJsDocDescription)(declaration);
    ...
    return {
        ...
        entryType: entities_1.EntryType.Constant,
        rawComment,
        description,
        jsdocTags,
    };
...
function extractLiteralPropertiesAsEnumMembers(declaration) {
    ...
    while (initializer &&
        (typescript_1.default.isAsExpression(initializer) || typescript_1.default.isParenthesizedExpression(initializer))) {
        initializer = initializer.expression;
    }
    if (initializer === undefined || !typescript_1.default.isObjectLiteralExpression(initializer)) {
        throw new Error(`Declaration tagged with "${LITERAL_AS_ENUM_TAG}" must be initialized to an object literal, but received ${initializer ? typescript_1.default.SyntaxKind[initializer.kind] : 'undefined'}`);
    }
    return initializer.properties.map((prop) => {
        ...
        if (!typescript_1.default.isNumericLiteral(prop.initializer) && !typescript_1.default.isStringLiteralLike(prop.initializer)) {
        ...
        return {
            ...
            jsdocTags: (0, jsdoc_extractor_1.extractJsDocTags)(prop),
            description: (0, jsdoc_extractor_1.extractJsDocDescription)(prop),
            memberTags: [],
        };
    });
}
