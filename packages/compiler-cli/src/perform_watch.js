...
exports.FileChangeEvent = void 0;
...
function createPerformWatchHost(configFileName, reportDiagnostics, existingOptions, createEmitCallback) {
    return {
        ...
        createCompilerHost: (options) => (0, entry_points_1.createCompilerHost)({ options }),
        readConfiguration: () => (0, perform_compile_1.readConfiguration)(configFileName, existingOptions),
        ...
        onFileChange: (options, listener, ready) => {
            ...
            const watcher = chokidar.watch(options.basePath, {
                ...
                ignored: (path) => /((^[\/\\])\..)|(\.js$)|(\.map$)|(\.metadata\.json|node_modules)/.test(path),
                ignoreInitial: true,
                persistent: true,
            });
        ...
        setTimeout: (typescript_1.default.sys.clearTimeout && typescript_1.default.sys.setTimeout) || setTimeout,
        clearTimeout: (typescript_1.default.sys.setTimeout && typescript_1.default.sys.clearTimeout) || clearTimeout,
    };
}
/**
...
function performWatchCompilation(host) {
    ...
    function doCompilation() {
        ...
        const compileResult = (0, perform_compile_1.performCompilation)({
        ...
        const exitCode = (0, perform_compile_1.exitCodeFromResult)(compileResult.diagnostics);
        if (exitCode == 0) {
            cachedProgram = compileResult.program;
            host.reportDiagnostics([
                (0, util_1.createMessageDiagnostic)('Compilation complete. Watching for file changes.'),
            ]);
        }
        else {
            host.reportDiagnostics([
                (0, util_1.createMessageDiagnostic)('Compilation failed. Watching for file changes.'),
            ]);
        }
    ...
    function recompile() {
        host.reportDiagnostics([
            (0, util_1.createMessageDiagnostic)('File change detected. Starting incremental compilation.'),
        ]);
        doCompilation();
        timerHandleForRecompilation = undefined;
    }
