...
(0, testing_1.runInEachFileSystem)(() => {
    describe('type checking of host bindings', () => {
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe(`Can't bind to 'foo' since it isn't a known property of 'ng-component'.`);
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe(`Can't bind to 'foo' since it isn't a known property of 'input'.`);
            expect(getDiagnosticSourceCode(diags[0])).toBe('[foo]');
        });
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe(`Can't bind to 'value' since it isn't a known property of 'div'.`);
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText.messageText).toBe(`Argument of type 'MouseEvent' is not assignable to parameter of type 'KeyboardEvent'.`);
      ...
      `);
            ...
            expect(diags[0].messageText).toBe(`Property 'doesNotExistTemplate' does not exist on type 'Comp'.`);
            expect(diags[1].messageText).toBe(`Property 'doesNotExistHost' does not exist on type 'Comp'.`);
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe(`Argument of type 'MouseEvent' is not assignable to parameter of type 'string'.`);
            expect(getDiagnosticSourceCode(diags[0])).toBe('$event');
        });
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(2);
            expect(diags[0].messageText).toBe(`Property 'id' is private and only accessible within class 'Comp'.`);
            expect(diags[1].messageText).toBe(`Property 'handleClick' is private and only accessible within class 'Comp'.`);
      ...
      `);
            ...
            expect(diags[0].messageText).toBe(`Property 'literalBindingDoesNotExist' does not exist on type 'SomeDir'.`);
            expect(getDiagnosticSourceCode(diags[0])).toBe('literalBindingDoesNotExist');
            expect(diags[1].messageText).toBe(`Property 'literalListenerDoesNotExist' does not exist on type 'SomeDir'.`);
            ...
            expect(diags[3].messageText).toBe(`Can't bind to 'foo' since it isn't a known property of 'button'.`);
