...
(0, testing_1.runInEachFileSystem)(() => {
    describe('ngtsc compilation of standalone types', () => {
        ...
        describe('component-side', () => {
            ...
            `);
                ...
                expect(dtsCode).toContain('i0.ɵɵDirectiveDeclaration<TestDir, "[dir]", never, {}, {}, never, never, true, never>;');
                expect(dtsCode).toContain('i0.ɵɵComponentDeclaration<TestCmp, "test-cmp", never, {}, {}, never, never, true, never>;');
            });
        ...
        `);
                ...
                expect(moduleJs).toContain('i0.ɵɵsetComponentScope(ModuleCmp, function () { return [i1.StandaloneCmp]; }, []);');
            ...
            `);
                ...
                expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.COMPONENT_NOT_STANDALONE));
                expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toEqual('[TestDir]');
            });
            ...
            `);
                ...
                const jsCodeAoT = jsCode.slice(0, jsCode.indexOf('(() => { (typeof ngDevMode === "undefined" || ngDevMode)'));
            ...
            `);
                ...
                expect(diags.length).toBe(1);
                expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.COMPONENT_NOT_STANDALONE));
                expect(diags[0].messageText).toBe(`'schemas' is only valid on a component that is standalone.`);
                expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toEqual('[NO_ERRORS_SCHEMA]');
            });
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.COMPONENT_IMPORT_NOT_STANDALONE));
                expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toEqual('TestDir');
                ...
                expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0].relatedInformation[0])).toEqual('TestModule');
                expect(diags[0].relatedInformation[0].messageText).toMatch(/It can be imported using its '.*' NgModule instead./);
            });
           ...
           `);
                ...
                expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.COMPONENT_UNKNOWN_IMPORT));
                // The import occurs within the array literal, such that the error can be reported for
                // the specific import that is rejected.
                expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toEqual('moduleWithProviders()');
                expect(diags[1].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.COMPONENT_UNKNOWN_IMPORT));
                // The import occurs in a referenced variable, which reports the error on the full
                // `imports` expression.
                expect((0, testing_2.getSourceCodeForDiagnostic)(diags[1])).toEqual('IMPORTS');
            });
        ...
        `);
                ...
                expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toEqual('[TestModule.forRoot()]');
            });
          ...
          `);
                ...
                expect(diags[0].relatedInformation).not.toBeUndefined();
                expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0].relatedInformation[0])).toEqual('TestModule');
                expect(diags[0].relatedInformation[0].messageText).toEqual(`It's declared in the 'TestModule' NgModule, but is not exported. Consider exporting it and importing the NgModule instead.`);
            });
        ...
        describe('NgModule-side', () => {
            ...
            `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.NGMODULE_DECLARATION_IS_STANDALONE));
                expect(diags[0].messageText).toContain('Component TestCmp is standalone');
                expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toEqual('TestCmp');
            });
        ...
        `);
                ...
                expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toEqual('TestPipe');
            });
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.NGMODULE_INVALID_REEXPORT));
                expect(diags[0].messageText).toContain('it must be imported first');
                expect((0, testing_2.diagnosticToNode)(diags[0], typescript_1.default.isIdentifier).parent.parent.getText()).toEqual('exports: [TestDir]');
            });
        ...
        `);
                ...
                expect((0, testing_2.diagnosticToNode)(diags[0], typescript_1.default.isIdentifier).parent.parent.getText()).toEqual('imports: [TestDir]');
            });
        });
    ...
    describe('strictStandalone flag', () => {
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].code).toEqual((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.NON_STANDALONE_NOT_ALLOWED));
