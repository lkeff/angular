    ...
    describe('ngtsc @defer block', () => {
      ...
      `);
        ...
        `);
            env.driveMain();
            const jsContents = env.getContents('test.js');
            expect(jsContents).toContain('ɵɵdefer(2, 0, TestCmp_Defer_2_DepsFn, 1, null, null, 0, null, i0.ɵɵdeferEnableTimerScheduling)');
        });
        describe('imports', () => {
        ...
        `);
                ...
                const jsContents = env.getContents('test.js');
                expect(jsContents).toContain('ɵɵdefer(1, 0, TestCmp_Defer_1_DepsFn)');
                expect(jsContents).toContain('() => [import("./cmps").then(m => m.CmpA), import("./cmps").then(m => m.CmpB)]');
                expect(jsContents).not.toContain('import { CmpA }');
        ...
        `);
                ...
                const jsContents = env.getContents('test.js');
                expect(jsContents).toContain('ɵɵdefer(1, 0, TestCmp_Defer_1_DepsFn)');
                expect(jsContents).toContain('const TestCmp_Defer_1_DepsFn = () => [import("./cmp-a").then(m => m.default), LocalDep];');
                expect(jsContents).toContain('i0.ɵsetClassMetadataAsync(TestCmp, () => [import("./cmp-a").then(m => m.default)]');
                // The `CmpA` symbol wasn't referenced elsewhere, so it can be defer-loaded
                // via dynamic imports and an original import can be removed.
                expect(jsContents).not.toContain('import CmpA');
            });
      ...
      `);
        ...
        `);
                ...
                expect(jsContents).toContain('const AppCmp_For_1_Conditional_0_Defer_1_DepsFn = () => [' +
                    'import("./deferred-a").then(m => m.DeferredCmpA), ' +
                    'import("./pipe-a").then(m => m.PipeA)];');
                expect(jsContents).toContain('const AppCmp_For_1_Conditional_0_Defer_4_DepsFn = () => [' +
          ...
          `);
                ...
                expect(jsContents).toContain('const AppCmp_Defer_1_DepsFn = () => [' +
                    'import("./deferred-a").then(m => m.DeferredCmpA), ' +
                    'EagerCmpA];');
                expect(jsContents).toContain('const AppCmp_Defer_4_DepsFn = () => [' +
                    'import("./deferred-b").then(m => m.DeferredCmpB), ' +
                    'EagerCmpA];');
                // Make sure there are no eager imports present in the output.
                expect(jsContents).not.toContain(`from './deferred-a'`);
            ...
            `);
                    ...
                    expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.COMPONENT_UNKNOWN_DEFERRED_IMPORT));
                });
            ...
            `);
                    const diags = env.driveDiagnostics();
                    expect(diags.length).toBe(1);
                    expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.DEFERRED_DIRECTIVE_USED_EAGERLY));
                });
            ...
            `);
                    const diags = env.driveDiagnostics();
                    expect(diags.length).toBe(1);
                    expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.DEFERRED_DEPENDENCY_IMPORTED_EAGERLY));
                });
            ...
            `);
                    const diags = env.driveDiagnostics();
                    expect(diags.length).toBe(1);
                    expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.DEFERRED_PIPE_USED_EAGERLY));
                });
