    ...
    describe('ngtsc module scopes', () => {
        ...
        describe('diagnostics', () => {
          ...
          `);
                    ...
                    const node = (0, testing_2.diagnosticToNode)(diags[0], typescript_1.default.isIdentifier);
          ...
          `);
                    const diags = env.driveDiagnostics();
                    expect(diags.length).toBe(1);
                    const node = findContainingClass((0, testing_2.diagnosticToNode)(diags[0], typescript_1.default.isIdentifier));
                    expect(node.name.text).toEqual('TestDir');
                    const relatedNodes = new Set(diags[0].relatedInformation.map((related) => findContainingClass((0, testing_2.diagnosticToNode)(related, typescript_1.default.isIdentifier)).name.text));
        ...
        `);
                    ...
                    expect(jsContents).toContain('function () { (typeof ngJitMode === "undefined" || ngJitMode) && i0.ɵɵsetNgModuleScope(TestModule, { imports: [OtherModule] }); })();');
                    const dtsContents = env.getContents('test.d.ts');
                    expect(dtsContents).toContain('static ɵmod: i0.ɵɵNgModuleDeclaration<TestModule, never, [typeof OtherModule], never>');
                });
        ...
        `);
                    ...
                    expect(error.code).toEqual((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.NGMODULE_INVALID_IMPORT));
                    expect((0, testing_2.diagnosticToNode)(error, typescript_1.default.isIdentifier).parent.parent.getText()).toEqual('imports: [NotAModule]');
                });
        ...
        `);
                    const [error] = env.driveDiagnostics();
                    expect(error).not.toBeUndefined();
                    const messageText = typescript_1.default.flattenDiagnosticMessageText(error.messageText, '\n');
                    expect(messageText).toContain('Value at position 0 in the NgModule.imports of IsAModule is not a class');
                    expect(messageText).toContain("Value is a reference to 'NotAClass'.");
                    expect(error.code).toEqual((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE));
                    expect((0, testing_2.diagnosticToNode)(error, typescript_1.default.isIdentifier).text).toEqual('NotAClass');
                });
            });
            describe('exports', () => {
        ...
        `);
                    ...
                    const jsContents = env.getContents('test.js');
                    expect(jsContents).toContain('i0.ɵɵdefineNgModule({ type: TestModule });');
                    expect(jsContents).toContain('(function () { (typeof ngJitMode === "undefined" || ngJitMode) && i0.ɵɵsetNgModuleScope(TestModule, { exports: [OtherModule] }); })();');
                    const dtsContents = env.getContents('test.d.ts');
                    expect(dtsContents).toContain('static ɵmod: i0.ɵɵNgModuleDeclaration<TestModule, never, never, [typeof OtherModule]>');
                });
        ...
        `);
                    const [error] = env.driveDiagnostics();
                    expect(error).not.toBeUndefined();
                    expect(error.code).toEqual((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.NGMODULE_INVALID_EXPORT));
                    expect((0, testing_2.diagnosticToNode)(error, typescript_1.default.isIdentifier).parent.parent.getText()).toEqual('exports: [NotAModule]');
                });
        ...
        `);
                    ...
                    const error = env
                        .driveDiagnostics()
                        .find((error) => (0, testing_2.diagnosticToNode)(error, typescript_1.default.isIdentifier).text === 'InvalidModule');
                    if (error === undefined) {
                        return fail('Expected to find a diagnostic referencing InvalidModule');
                    }
                    expect(error.code).toEqual((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.NGMODULE_INVALID_EXPORT));
                    expect((0, testing_2.diagnosticToNode)(error, typescript_1.default.isIdentifier).parent.parent.getText()).toEqual('exports: [InvalidModule]');
                });
            });
            describe('re-exports', () => {
        ...
        `);
                    const [error] = env.driveDiagnostics();
                    expect(error).not.toBeUndefined();
                    expect(error.code).toEqual((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.NGMODULE_INVALID_REEXPORT));
                    expect((0, testing_2.diagnosticToNode)(error, typescript_1.default.isIdentifier).parent.parent.getText()).toEqual('exports: [Dir]');
                });
            });
        ...
        `);
                for (const diag of env.driveDiagnostics()) {
                    ...
                    expect(diag.messageText).not.toContain('doesnt-exist', "Template type-checking ran for a component, when it shouldn't have.");
                }
            });
        });
    });
