...
(0, testing_1.runInEachFileSystem)(() => {
    describe('selectorless', () => {
        ...
        `);
            ...
            expect(diags[0].messageText).toBe('Cannot find name "Dep". Selectorless references are only supported to classes or non-type import statements.');
        });
        ...
        `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe('Incorrect reference type. Type must be a standalone @Component.');
        });
        ...
        `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe('Incorrect reference type. Type must be a standalone @Directive.');
        });
        ...
        `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe('Cannot use selectorless with a component that is not standalone');
        });
        ...
        `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe('Cannot use the "imports" field in a selectorless components');
        });
        ...
        `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe('Cannot use the "deferredImports" field in a selectorless components');
        });
        ...
        `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe(`Argument of type 'boolean' is not assignable to parameter of type 'number'.`);
        });
        ...
        `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe('Directive Dep does not have an input named "id". Bindings to directives must target existing inputs or outputs.');
        });
        ...
        `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe(`Argument of type 'MouseEvent' is not assignable to parameter of type 'number'.`);
        });
        ...
        `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe('Directive Dep does not have an output named "click". Bindings to directives must target existing inputs or outputs.');
        });
        ...
        `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe(`Required input 'someInput' from component Dep must be specified.`);
        });
        ...
        `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe(`Required input 'someInput' from directive Dep must be specified.`);
        });
      ...
      `);
        ...
        it('should defer selectorless symbols', () => {
            env.write('dep-comp.ts', `
          import { Component } from '@angular/core';

          @Component({template: ''})
          export class DepComp {}
        `);
            env.write('dep-dir.ts', `
          import { Directive } from '@angular/core';

          @Directive()
          export class DepDir {}
        `);
            env.write('dep-pipe.ts', `
          import { Pipe } from '@angular/core';
        ...
        `);
            ...
            expect(jsContents).toContain('const Comp_Defer_1_DepsFn = () => [import("./dep-comp").then(m => m.DepComp), ' +
