    ...
    describe('initializer-based input() API', () => {
      ...
      `);
            ...
            expect(env.getContents('test.d.ts'))
                .withContext('Expected no coercion member as input signal captures the write type of the transform')
      ...
      `);
        ...
        `);
                ...
                expect(diagnostics[0].messageText).toBe(`Type 'boolean' is not assignable to type 'number'.`);
            });
        ...
        `);
                const diagnostics = env.driveDiagnostics();
                expect(diagnostics.length).toBe(1);
                expect(diagnostics[0].messageText).toBe(`Type 'boolean' is not assignable to type 'string | number'.`);
            });
        ...
        `);
                const diagnostics = env.driveDiagnostics();
                expect(diagnostics.length).toBe(1);
                expect(diagnostics[0].messageText).toBe(`Required input 'data' from directive TestDir must be specified.`);
            });
        });
