...
const program_1 = require("../../src/ngtsc/program");
...
const trim = (input) => input.replace(/\s+/g, ' ').trim();
const varRegExp = (name) => new RegExp(`const \\w+ = \\[\"${name}\"\\];`);
const viewQueryRegExp = (predicate, flags, ref) => {
    const maybeRef = ref ? `, ${ref}` : ``;
    return new RegExp(`i0\\.ɵɵviewQuery\\(${predicate}, ${flags}${maybeRef}\\)`);
};
const contentQueryRegExp = (predicate, flags, ref) => {
    const maybeRef = ref ? `, ${ref}` : ``;
    return new RegExp(`i0\\.ɵɵcontentQuery\\(dirIndex, ${predicate}, ${flags}${maybeRef}\\)`);
};
const setClassMetadataRegExp = (expectedType) => new RegExp(`setClassMetadata(.*?${expectedType}.*?)`);
const testFiles = (0, testing_2.loadStandardTestFiles)();
function getDiagnosticSourceCode(diag) {
...
(0, testing_1.runInEachFileSystem)((os) => {
    ...
    `);
            ...
            expect(jsContents).toContain('Dep.ɵprov =');
            expect(jsContents).toContain('Service.ɵprov =');
            expect(jsContents).toContain('Service.ɵfac = function Service_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || Service)(i0.ɵɵinject(Dep)); };');
    ...
    `);
            ...
            expect(jsContents).toContain('Service_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || Service)(); }');
            expect(jsContents).toContain(", providedIn: 'root' });");
            expect(jsContents).not.toContain('__decorate');
            const dtsContents = env.getContents('test.d.ts');
            expect(dtsContents).toContain('static ɵprov: i0.ɵɵInjectableDeclaration<Service>;');
    ...
    `);
            ...
            const jsContents = env.getContents('test.js');
            expect(jsContents).toContain('Service.ɵprov =');
            expect(jsContents).toContain('factory: function Service_Factory(__ngFactoryType__) { let __ngConditionalFactory__ = null; if (__ngFactoryType__) {');
            expect(jsContents).toContain('return new (__ngFactoryType__ || Service)(i0.ɵɵinject(Dep));');
            expect(jsContents).toContain('__ngConditionalFactory__ = ((dep) => new Service(dep))(i0.ɵɵinject(Dep));');
    ...
    `);
            ...
            expect(jsContents).toContain('factory: function Service_Factory(__ngFactoryType__) { let __ngConditionalFactory__ = null; if (__ngFactoryType__) {');
            expect(jsContents).toContain('return new (__ngFactoryType__ || Service)(i0.ɵɵinject(Dep));');
            expect(jsContents).toContain('__ngConditionalFactory__ = ((dep) => new Service(dep))(i0.ɵɵinject(Dep, 10));');
            expect(jsContents).toContain(`return __ngConditionalFactory__; }, providedIn: 'root' });`);
            expect(jsContents).not.toContain('__decorate');
            const dtsContents = env.getContents('test.d.ts');
            expect(dtsContents).toContain('static ɵprov: i0.ɵɵInjectableDeclaration<Service>;');
    ...
    `);
            env.driveMain();
            const jsContents = env.getContents('test.js');
            expect(jsContents).toContain(`Service.ɵfac = function Service_Factory(__ngFactoryType__) { ` +
      ...
      `);
            ...
            expect(jsContents).not.toContain('__decorate');
            const dtsContents = env.getContents('test.d.ts');
            const expectedDirectiveDeclaration = 'static ɵdir: i0.ɵɵDirectiveDeclaration<TestDir, "[dir]", never, {}, {}, never, never, false, never>';
      ...
      `);
            ...
            expect(jsContents).not.toContain('__decorate');
            const dtsContents = env.getContents('test.d.ts');
            expect(dtsContents).toContain('static ɵdir: i0.ɵɵDirectiveDeclaration<TestDir, never, never, {}, {}, never, never, true, never>');
            expect(dtsContents).toContain('static ɵfac: i0.ɵɵFactoryDeclaration<TestDir, never>');
        });
    ...
    `);
            ...
            expect(jsContents).not.toContain('__decorate');
            const dtsContents = env.getContents('test.d.ts');
            const expectedComponentDeclaration = 'static ɵcmp: i0.ɵɵComponentDeclaration<TestCmp, "test-cmp", never, {}, {}, never, never, false, never>';
    ...
    `);
            ...
            expect(jsContents).not.toContain('__decorate');
            const dtsContents = env.getContents('test.d.ts');
            expect(dtsContents).toContain('static ɵcmp: i0.ɵɵComponentDeclaration' +
    ...
    `);
      ...
      `);
            ...
            if (diags.some((diag) => diag.code === (0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.NGMODULE_INVALID_DECLARATION))) {
        ...
        it('should only list public declarations in NgModule .d.ts when requested', () => {
          ...
          `);
            env.driveMain();
            const dtsContents = env.getContents('test.d.ts');
            expect(dtsContents).toContain('static ɵmod: i0.ɵɵNgModuleDeclaration<Module, ' +
        ...
        it('should not list imports in NgModule .d.ts when requested', () => {
          ...
          `);
            env.driveMain();
            const dtsContents = env.getContents('test.d.ts');
            expect(dtsContents).toContain('static ɵmod: i0.ɵɵNgModuleDeclaration<Module, ' + 'never, never, never>');
        });
        it('should error when supportJitMode is false and forbidOrphanComponents is true', () => {
            ...
            expect(diagnostics).toEqual([
                jasmine.objectContaining({
                    messageText: jasmine.stringMatching(/JIT mode support \("supportJitMode" option\) cannot be disabled when forbidOrphanComponents is set to true/),
                }),
            ]);
        });
      ...
      `);
            env.driveMain();
            expect(trim(env.getContents('test.js'))).toContain(trim(`
      ...
      `);
        ...
        `);
                    ...
                    expect(jsContents).toContain('(ɵDir_BaseFactory || (ɵDir_BaseFactory = i0.ɵɵgetInheritedFactory(Dir)))(__ngFactoryType__ || Dir);');
                });
          ...
          `);
                    ...
                    expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.SCHEMA_INVALID_ELEMENT));
                    expect(typescript_1.default.flattenDiagnosticMessageText(diags[0].messageText, '\n')).toContain('not-a-cmp');
                });
                /**
    ...
    `);
      ...
      `);
            env.driveMain();
            const jsContents = trim(env.getContents('test.js'));
            expect(jsContents).toContain('dependencies: [DirectiveA, ComponentA, DirectiveB, ComponentB]');
        });
        it('should not throw, but issue a diagnostic when an `NgModule` from `d.ts` references ' +
        ...
        `);
            ...
            expect(diagnostics[0].messageText).toBe('This import contains errors, which may affect components that depend on this NgModule.');
        });
    ...
    `);
            env.driveMain();
            const jsContents = env.getContents('test.js');
            expect(jsContents).toContain('i0.ɵɵdefineDirective({ type: TestBase, inputs: { input: "input" } });');
            const dtsContents = env.getContents('test.d.ts');
        ...
        describe('undecorated classes using Angular features', () => {
            it('should error if @Input has been discovered', () => assertErrorUndecoratedClassWithField('Input'));
            it('should error if @Output has been discovered', () => assertErrorUndecoratedClassWithField('Output'));
            it('should error if @ViewChild has been discovered', () => assertErrorUndecoratedClassWithField('ViewChild'));
            it('should error if @ViewChildren has been discovered', () => assertErrorUndecoratedClassWithField('ViewChildren'));
            it('should error if @ContentChild has been discovered', () => assertErrorUndecoratedClassWithField('ContentChildren'));
            it('should error if @HostBinding has been discovered', () => assertErrorUndecoratedClassWithField('HostBinding'));
            it('should error if @HostListener has been discovered', () => assertErrorUndecoratedClassWithField('HostListener'));
            it(`should error if ngOnChanges lifecycle hook has been discovered`, () => assertErrorUndecoratedClassWithLifecycleHook('ngOnChanges'));
            it(`should error if ngOnInit lifecycle hook has been discovered`, () => assertErrorUndecoratedClassWithLifecycleHook('ngOnInit'));
            it(`should error if ngOnDestroy lifecycle hook has been discovered`, () => assertErrorUndecoratedClassWithLifecycleHook('ngOnDestroy'));
            it(`should error if ngDoCheck lifecycle hook has been discovered`, () => assertErrorUndecoratedClassWithLifecycleHook('ngDoCheck'));
            it(`should error if ngAfterViewInit lifecycle hook has been discovered`, () => assertErrorUndecoratedClassWithLifecycleHook('ngAfterViewInit'));
            it(`should error if ngAfterViewChecked lifecycle hook has been discovered`, () => assertErrorUndecoratedClassWithLifecycleHook('ngAfterViewChecked'));
            it(`should error if ngAfterContentInit lifecycle hook has been discovered`, () => assertErrorUndecoratedClassWithLifecycleHook('ngAfterContentInit'));
            it(`should error if ngAfterContentChecked lifecycle hook has been discovered`, () => assertErrorUndecoratedClassWithLifecycleHook('ngAfterContentChecked'));
            function assertErrorUndecoratedClassWithField(fieldDecoratorName) {
                env.write('test.ts', `
        ...
        `);
                ...
                expect(trim(errors[0].messageText)).toContain('Class is using Angular features but is not decorated. Please add an explicit ' +
                    'Angular decorator.');
            }
            function assertErrorUndecoratedClassWithLifecycleHook(lifecycleName) {
                env.write('test.ts', `
            import {Component, NgModule} from '@angular/core';
    ...
    `);
            ...
            expect(jsContents).toContain('function () { (typeof ngJitMode === "undefined" || ngJitMode) && i0.ɵɵsetNgModuleScope(TestModule, { declarations: [TestCmp] }); })();');
            expect(jsContents).toContain('TestModule.ɵfac = function TestModule_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || TestModule)(); }');
            expect(jsContents).toContain('i0.ɵɵdefineInjector({});');
            const dtsContents = env.getContents('test.d.ts');
            expect(dtsContents).toContain('static ɵcmp: i0.ɵɵComponentDeclaration<TestCmp, "test-cmp", never, {}, {}, never, never, false, never>');
            expect(dtsContents).toContain('static ɵmod: i0.ɵɵNgModuleDeclaration<TestModule, [typeof TestCmp], never, never>');
    ...
    `);
       ...
       `);
            ...
            expect(diags[0].code).toEqual((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.WARN_NGMODULE_ID_UNNECESSARY));
    ...
    `);
            ...
            const jsContents = env.getContents('test.js');
            expect(jsContents).toContain('TestModule.ɵfac = function TestModule_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || TestModule)(); }');
            expect(jsContents).toContain('i0.ɵɵdefineInjector({ imports: [OtherModule, RouterModule.forRoot(),' +
    ...
    `);
            ...
            expect(jsContents).toContain('TestModule.ɵfac = function TestModule_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || TestModule)(); }');
            expect(jsContents).toContain('i0.ɵɵdefineNgModule({ type: TestModule });');
            expect(jsContents).toContain(`TestModule.ɵinj = /*@__PURE__*/ i0.ɵɵdefineInjector({ ` +
                `providers: [{ provide: Token, useValue: 'test' }], ` +
                `imports: [OtherModule] });`);
            const dtsContents = env.getContents('test.d.ts');
            expect(dtsContents).toContain('static ɵmod: i0.ɵɵNgModuleDeclaration<TestModule, [typeof TestCmp], [typeof OtherModule], never>');
    ...
    `);
            ...
            const jsContents = env.getContents('test.js');
            const dtsContents = env.getContents('test.d.ts');
            expect(jsContents).toContain('TestPipe.ɵpipe = /*@__PURE__*/ i0.ɵɵdefinePipe({ name: "test-pipe", type: TestPipe, pure: false, standalone: false })');
            expect(jsContents).toContain('TestPipe.ɵfac = function TestPipe_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || TestPipe)(); }');
            expect(dtsContents).toContain('static ɵpipe: i0.ɵɵPipeDeclaration<TestPipe, "test-pipe", false>;');
    ...
    `);
            ...
            const jsContents = env.getContents('test.js');
            const dtsContents = env.getContents('test.d.ts');
            expect(jsContents).toContain('TestPipe.ɵpipe = /*@__PURE__*/ i0.ɵɵdefinePipe({ name: "test-pipe", type: TestPipe, pure: true, standalone: false })');
            expect(jsContents).toContain('TestPipe.ɵfac = function TestPipe_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || TestPipe)(); }');
            expect(dtsContents).toContain('static ɵpipe: i0.ɵɵPipeDeclaration<TestPipe, "test-pipe", false>;');
            expect(dtsContents).toContain('static ɵfac: i0.ɵɵFactoryDeclaration<TestPipe, never>;');
        });
    ...
    `);
            env.driveMain();
            const jsContents = env.getContents('test.js');
            expect(jsContents).toContain('return new (__ngFactoryType__ || TestPipe)(i0.ɵɵdirectiveInject(Dep, 16));');
        });
    ...
    `);
            ...
            const dtsContents = env.getContents('test.d.ts');
            expect(dtsContents).toContain('static ɵpipe: i0.ɵɵPipeDeclaration<TestPipe<any>, "test-pipe", false>;');
    ...
    `);
            ...
            const dtsContents = env.getContents('test.d.ts');
            expect(dtsContents).toContain('i0.ɵɵNgModuleDeclaration<TestModule, [typeof TestPipe, typeof TestCmp], never, never>');
        });
        ...
        `);
            ...
            const message = diags.length
                ? typescript_1.default.flattenDiagnosticMessageText(diags[0].messageText, '\n')
                : '';
            expect(diags.length).toBe(1);
            expect(getDiagnosticSourceCode(diags[0])).toBe('someVar');
            expect(message).toContain(`'imports' must be an array of components, directives, pipes, or NgModules.`);
      ...
      `);
        ...
        `);
                const errors = env.driveDiagnostics();
                expect(trim(errors[0].messageText)).toContain('Directive BaseDir has no selector, please add it!');
            });
      ...
      `);
                const errors = env.driveDiagnostics();
                expect(trim(errors[0].messageText)).toContain('Directive TestDir has no selector, please add it!');
            });
        });
        describe('error handling', () => {
            function verifyThrownError(errorCode, errorMessage) {
                const errors = env.driveDiagnostics();
                expect(errors.length).toBe(1);
                const { code, messageText } = errors[0];
                expect(code).toBe((0, diagnostics_1.ngErrorCode)(errorCode));
                const text = typescript_1.default.flattenDiagnosticMessageText(messageText, '\n');
          ...
          export class MyModule {}
        `);
                verifyThrownError(diagnostics_1.ErrorCode.DECORATOR_ARG_NOT_LITERAL, '@NgModule argument must be an object literal');
            });
        ...
        `);
                verifyThrownError(diagnostics_1.ErrorCode.DECORATOR_COLLISION, 'Cannot combine multiple query decorators.');
            });
            ...
            `);
                    verifyThrownError(diagnostics_1.ErrorCode.DECORATOR_COLLISION, 'Cannot combine @Input decorators with query decorators');
                });
          ...
          `);
                    verifyThrownError(diagnostics_1.ErrorCode.DECORATOR_ARG_NOT_LITERAL, `@${decorator} options must be an object literal`);
                });
          ...
          `);
                    verifyThrownError(diagnostics_1.ErrorCode.DECORATOR_UNEXPECTED, 'Query decorator must go on a property-type member');
                });
          ...
          `);
                    verifyThrownError(diagnostics_1.ErrorCode.DECORATOR_ARITY_WRONG, `@${decorator} has too many arguments`);
                });
          ...
          `);
                    verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, `@${decorator} predicate cannot be interpreted`);
                });
          ...
          `);
                    verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, `Failed to resolve @${decorator} predicate at position 1 to a string`);
                });
            });
          ...
          export class TestDir {}
        `);
                verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, `Failed to resolve @Directive.inputs to an array Value is of type 'string'.`);
            });
        ...
        `);
          ...
          export class TestDir {}
        `);
                verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, `Failed to resolve @Directive.outputs to a string array`);
            });
            ...
            `);
                    verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, '@ContentChild options.descendants must be a boolean');
                });
            });
          ...
          `);
                    verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, `@${decorator} decorator argument must resolve to a string`);
                });
          ...
          `);
                    verifyThrownError(diagnostics_1.ErrorCode.DECORATOR_ARITY_WRONG, `@${decorator} can have at most one argument, got 2 argument(s)`);
                });
            });
        ...
        `);
                verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, `@HostBinding's argument must be a string`);
            });
        ...
        `);
                verifyThrownError(diagnostics_1.ErrorCode.DECORATOR_ARITY_WRONG, '@HostBinding can have at most one argument');
            });
          ...
          export class TestDir {}
        `);
                verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, 'Decorator host metadata must be an object');
            });
            it('should throw error if @Directive.host field is an object with values that have wrong types', () => {
              ...
              export class TestDir {}
            `);
                verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, 'Decorator host metadata must be a string -> string object, but found unparseable value');
            });
          ...
          export class TestDir {}
        `);
                verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, 'Decorator queries metadata must be an object');
            });
          ...
          export class TestDir {}
        `);
                verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, 'Decorator query metadata must be an instance of a query type');
            });
        ...
        `);
                verifyThrownError(diagnostics_1.ErrorCode.DECORATOR_ARG_NOT_LITERAL, '@Injectable argument must be an object literal');
            });
        ...
        `);
                verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, `Input transform must be a function`);
            });
        ...
        `);
                verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, `Transform of value at position 0 of @Directive.inputs array must be a function Value is of type 'null'.`);
            });
            ...
            `);
                verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, `Input transform function first parameter must have a type`);
            });
        ...
        `);
                verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, `Input transform function cannot be generic`);
            });
        ...
        `);
                verifyThrownError(diagnostics_1.ErrorCode.CONFLICTING_INPUT_TRANSFORM, `Class cannot have both a transform function on Input value and a static member called ngAcceptInputType_value`);
            });
          ...
          `);
                verifyThrownError(diagnostics_1.ErrorCode.IMPORT_GENERATION_FAILURE, 'Unable to import type InternalType.');
            });
        ...
        `);
                verifyThrownError(diagnostics_1.ErrorCode.SYMBOL_NOT_EXPORTED, 'Symbol must be exported in order to be used as the type of an Input transform function');
            });
        ...
        `);
                verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, `Input transform function first parameter cannot be a spread parameter`);
            });
        ...
        `);
                verifyThrownError(diagnostics_1.ErrorCode.VALUE_HAS_WRONG_TYPE, `Input transform function cannot have multiple signatures`);
            });
        });
      ...
      `);
                ...
                expect(jsContents).toContain(` { type: String, decorators: [{ type: Inject, args: ['foo',] }] }`);
            });
        });
        describe('compiling invalid @Injectables', () => {
          ...
          `);
                    const errors = env.driveDiagnostics();
                    expect(errors.length).toBe(1);
                    expect(typescript_1.default.flattenDiagnosticMessageText(errors[0].messageText, '\n')).toBe(`No suitable injection token for parameter 'notInjectable' of class 'Test'.\n` +
                        `  Consider using the @Inject decorator to specify an injection token.`);
                    expect(errors[0].relatedInformation.length).toBe(1);
                    expect(errors[0].relatedInformation[0].messageText).toBe('This type is not supported as injection token.');
                });
          ...
          `);
            ...
            `);
                    ...
                    expect(diags[0].relatedInformation[0].messageText).toBe('This type is imported using a type-only import, ' +
                        'which prevents it from being usable as an injection token.');
                    expect(diags[0].relatedInformation[1].messageText).toBe('The type-only import occurs here.');
                });
          ...
          `);
                    ...
                    expect(diags[0].relatedInformation[0].messageText).toBe('This type is not supported as injection token.');
                });
          ...
          `);
                    ...
                    expect(typescript_1.default.flattenDiagnosticMessageText(diags[0].messageText, '\n')).toBe(`No suitable injection token for parameter 'param' of class 'MyService'.\n` +
                        `  Consider using the @Inject decorator to specify an injection token.`);
                    expect(diags[0].relatedInformation.length).toBe(2);
                    expect(diags[0].relatedInformation[0].messageText).toBe('This type does not have a value, so it cannot be used as injection token.');
          ...
          `);
                    const diags = env.driveDiagnostics();
                    expect(diags.length).toBe(1);
                    expect(typescript_1.default.flattenDiagnosticMessageText(diags[0].messageText, '\n')).toBe(`No suitable injection token for parameter 'param' of ` +
                        `class 'MyService'.\n` +
                        `  Consider using the @Inject decorator to specify an injection token.`);
                    expect(diags[0].relatedInformation.length).toBe(1);
                    expect(diags[0].relatedInformation[0].messageText).toBe('This type does not have a value, so it cannot be used as injection token.');
          ...
          `);
             ...
             `);
                    env.driveMain();
                    const jsContents = env.getContents('test.js');
                    expect(jsContents).toMatch(/function Test_Factory\(__ngFactoryType__\) { i0\.ɵɵinvalidFactory\(\)/m);
                });
             ...
             `);
                    ...
                    expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.INJECTABLE_INHERITS_INVALID_CONSTRUCTOR));
                    expect(diags[0].messageText).toEqual(`The directive ConcreteMiddleDir inherits its constructor from ParentDir, but the latter has ` +
                    ...
                    expect(diags[1].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.INJECTABLE_INHERITS_INVALID_CONSTRUCTOR));
                    expect(diags[1].messageText).toEqual(`The directive ConcreteDirWithoutCtor inherits its constructor from ParentDir, but the latter ` +
             ...
             `);
                    ...
                    expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.INJECTABLE_INHERITS_INVALID_CONSTRUCTOR));
                    expect(diags[0].messageText).toEqual(`The injectable ConcreteMiddleService inherits its constructor from ParentService, but the ` +
                    ...
                    expect(diags[1].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.INJECTABLE_INHERITS_INVALID_CONSTRUCTOR));
                    expect(diags[1].messageText).toEqual(`The injectable ConcreteServiceWithoutCtor inherits its constructor from ParentService, but the ` +
                        `latter has a constructor parameter that is not compatible with dependency injection. Either add ` +
                    ...
                    expect(diags[2].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.INJECTABLE_INHERITS_INVALID_CONSTRUCTOR));
                    expect(diags[2].messageText).toEqual(`The injectable ProvidedInRootService inherits its constructor from ParentService, but the ` +
                        `latter has a constructor parameter that is not compatible with dependency injection. Either add ` +
             ...
             `);
                    const diags = env.driveDiagnostics();
                    expect(diags.length).toBe(3);
                    expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.DIRECTIVE_INHERITS_UNDECORATED_CTOR));
                    expect(diags[0].messageText).toEqual(`The directive AbstractMiddleDir inherits its constructor from ParentClass, but the latter ` +
                    ...
                    expect(diags[1].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.DIRECTIVE_INHERITS_UNDECORATED_CTOR));
                    expect(diags[1].messageText).toEqual(`The directive ConcreteMiddleDir inherits its constructor from ParentClass, but the latter ` +
                        `does not have an Angular decorator of its own. Dependency injection will not be able to resolve ` +
                    ...
                    expect(diags[2].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.DIRECTIVE_INHERITS_UNDECORATED_CTOR));
                    expect(diags[2].messageText).toEqual(`The directive ConcreteDirWithoutCtor inherits its constructor from ParentClass, but the latter ` +
                        `does not have an Angular decorator of its own. Dependency injection will not be able to resolve ` +
                        `the parameters of ParentClass's constructor. Either add a @Directive decorator to ParentClass, ` +
          ...
          `);
                    env.driveMain();
                    const jsContents = env.getContents('test.js');
                    expect(jsContents).toContain('Test.ɵfac = function Test_Factory(__ngFactoryType__) { i0.ɵɵinvalidFactory()');
                });
      ...
      `);
        ...
        describe('templateUrl and styleUrls processing', () => {
            const testsForResource = (resource) => [
      ...
      `);
        ...
        describe('unwrapping ModuleWithProviders functions', () => {
          ...
          `);
                ...
                expect(dtsContents).toContain('i0.ɵɵNgModuleDeclaration<TestModule, never, [typeof i1.SomeModule], never>');
            });
    ...
    `);
                ...
                expect(dtsContents).toContain('i0.ɵɵNgModuleDeclaration<TestModule, never, [typeof i1.RouterModule], never>');
            });
        ...
        `);
                const errors = env.driveDiagnostics();
                expect(trim(errors[0].messageText)).toContain(`RouterModule.forRoot returns a ModuleWithProviders type without a generic type argument. ` +
    ...
    `);
                ...
                const dtsContents = env.getContents('test.d.ts');
                expect(dtsContents).toContain(`import * as i1 from "router";`);
                expect(dtsContents).toContain('i0.ɵɵNgModuleDeclaration<TestModule, never, [typeof i1.InternalRouterModule], never>');
            });
            ...
            export declare class Router2Module {
              static ɵmod: ɵɵNgModuleDeclaration<Router2Module, never, never, never>;
                ...
                expect(dtsContents).toContain('i0.ɵɵNgModuleDeclaration<TestModule, never, [typeof i1.Router2Module], never>');
            });
  ...
  `);
    ...
    `);
            env.driveMain();
            const jsContents = env.getContents('test.js');
            expect(jsContents).toContain(`FooCmp.ɵfac = function FooCmp_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || FooCmp)(i0.ɵɵinjectAttribute("test"), i0.ɵɵdirectiveInject(i0.ChangeDetectorRef), i0.ɵɵdirectiveInject(i0.ElementRef), i0.ɵɵdirectiveInject(i0.Injector), i0.ɵɵdirectiveInject(i0.Renderer2), i0.ɵɵdirectiveInject(i0.TemplateRef), i0.ɵɵdirectiveInject(i0.ViewContainerRef)); }`);
        });
    ...
    `);
            env.driveMain();
            const dtsContents = env.getContents('test.d.ts');
            expect(dtsContents).toContain('static ɵfac: i0.ɵɵFactoryDeclaration<WithDecorators, [' +
            ...
            expect(dtsContents).toContain(`static ɵfac: i0.ɵɵFactoryDeclaration<WithoutDecorators, never>`);
            expect(dtsContents).toContain(`static ɵfac: i0.ɵɵFactoryDeclaration<MyCmp, [{ host: true; }]>`);
            expect(dtsContents).toContain(`static ɵfac: i0.ɵɵFactoryDeclaration<MyPipe, [{ host: true; }]>`);
        });
    ...
    `);
            ...
            expect(dtsContents).toContain(`static ɵfac: i0.ɵɵFactoryDeclaration<InjUseExisting, [{ self: true; }]>`);
            expect(dtsContents).toContain(`static ɵfac: i0.ɵɵFactoryDeclaration<InjUseClass, [{ self: true; }]>`);
            expect(dtsContents).toContain(`static ɵfac: i0.ɵɵFactoryDeclaration<InjUseClassWithDeps, [{ self: true; }]>`);
            expect(dtsContents).toContain(`static ɵfac: i0.ɵɵFactoryDeclaration<InjUseFactory, [{ self: true; }]>`);
            expect(dtsContents).toContain(`static ɵfac: i0.ɵɵFactoryDeclaration<InjUseFactoryWithDeps, [{ self: true; }]>`);
            expect(dtsContents).toContain(`static ɵfac: i0.ɵɵFactoryDeclaration<InjUseValue, [{ self: true; }]>`);
        });
    ...
    `);
            env.driveMain();
            const dtsContents = env.getContents('test.d.ts');
            expect(dtsContents).toContain('static ɵcmp: i0.ɵɵComponentDeclaration<TestCmp, "test", never, {}, {}, never, ["*", ".foo"], false, never>');
        });
    ...
    `);
            const errors = env.driveDiagnostics();
            expect(trim(errors[0].messageText)).toContain(`Unexpected global target 'UnknownTarget' defined for 'click' event. Supported list of global targets: window,document,body.`);
        });
      ...
      `);
            const errors = env.driveDiagnostics();
            expect(trim(errors[0].messageText)).toContain('Cannot have a pipe in an action expression');
        });
        it('should throw in case pipes are used in host bindings (defined as `value | pipe`)', () => {
         ...
         `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(trim(diags[0].messageText)).toContain('Host binding expression cannot contain pipes');
    ...
    `);
            const errors = env.driveDiagnostics();
            expect(errors.length).toBe(1);
            expect(errors[0].messageText).toContain(`Can't bind to 'valueOf' since it isn't a known property of 'div'.`);
        });
    ...
    `);
            env.driveMain();
            expect(env.getContents('test.js')).toContain(`ɵɵlistener("click", function TestCmp_Template_div_click_0_listener() { return 123; });`);
        });
    ...
    `);
        ...
        class FooCmp {}`);
            env.driveMain();
            const jsContents = env.getContents('test.js');
            expect(jsContents).toContain('`:\u241F5dbba0a3da8dff890e20cf76eb075d58900fbcd3\u241F8321000940098097247:Some text`');
        });
        ...
        class FooCmp {}`);
            env.driveMain();
            const jsContents = env.getContents('test.js');
            expect(jsContents).toContain(':@@custom\u241F5dbba0a3da8dff890e20cf76eb075d58900fbcd3\u241F8321000940098097247:Some text');
        });
     ...
     class FooCmp {
       age = 1;
            ...
            env.driveMain();
            const jsContents = env.getContents('test.js');
            expect(jsContents).toContain(':\u241F720ba589d043a0497ac721ff972f41db0c919efb\u241F3221232817843005870:{VAR_PLURAL, plural, 10 {ten} other {other}}');
            expect(jsContents).toContain(':@@custom\u241Fdcb6170595f5d548a3d00937e87d11858f51ad04\u241F7419139165339437596:Some text');
        });
    ...
    `);
            const errors = env.driveDiagnostics();
            expect(errors.length).toBe(1);
            const messageText = typescript_1.default.flattenDiagnosticMessageText(errors[0].messageText, '\n');
    ...
    `);
            env.driveMain();
            const jsContents = env.getContents('test.js');
            expect(jsContents).toContain('function Base_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || Base)(i0.ɵɵinject(Dep)); }');
            expect(jsContents).toContain('(() => { let ɵChild_BaseFactory; return function Child_Factory(__ngFactoryType__) { return (ɵChild_BaseFactory || (ɵChild_BaseFactory = i0.ɵɵgetInheritedFactory(Child)))(__ngFactoryType__ || Child); }; })();');
            expect(jsContents).toContain('function GrandChild_Factory(__ngFactoryType__) { return new (__ngFactoryType__ || GrandChild)(); }');
        });
    ...
    `);
            env.driveMain();
            const jsContents = env.getContents('test.js');
            expect(jsContents).toContain('/*@__PURE__*/ (() => { let ɵDir_BaseFactory; return function Dir_Factory(__ngFactoryType__) { return (ɵDir_BaseFactory || (ɵDir_BaseFactory = i0.ɵɵgetInheritedFactory(Dir)))(__ngFactoryType__ || Dir); }; })();');
        });
    ...
    `);
      ...
      `);
            ...
            expect(jsContents).toContain(`(() => { (typeof ngDevMode === "undefined" || ngDevMode) && ` +
      ...
      `));
            expect(jsContents).toContain(trim('MyModule.ɵmod = /*@__PURE__*/ i0.ɵɵdefineNgModule({ type: MyModule });'));
        });
      ...
      `);
        ...
        describe('namespace support', () => {
           ...
           `);
                ...
                const jsContents = trim(env.getContents('test.js'));
                expect(jsContents).toContain(`import * as i1 from "ns";`);
                expect(jsContents).toContain('i0.ɵɵinject(i1.ns.Zero), ' +
       ...
       `);
                ...
                expect(typescript_1.default.flattenDiagnosticMessageText(diags[0].messageText, '\n')).toBe(`No suitable injection token for parameter 'foo' of class 'MyService'.\n` +
                    `  Consider using the @Inject decorator to specify an injection token.`);
                expect(diags[0].relatedInformation.length).toBe(2);
                expect(diags[0].relatedInformation[0].messageText).toBe('This type corresponds with a namespace, which cannot be used as injection token.');
                expect(diags[0].relatedInformation[1].messageText).toBe('The namespace import occurs here.');
            });
        });
  ...
  `);
      ...
      `);
                env.driveMain();
                const jsContents = env.getContents('test.js');
                expect(jsContents).toMatch(/i\d\.ɵɵsetComponentScope\(NormalComponent,\s+\[CyclicComponent\],\s+\[\]\)/);
      ...
      `);
                ...
                expect(error.code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.IMPORT_CYCLE_DETECTED));
                expect(error.messageText).toEqual('One or more import cycles would need to be created to compile this component, ' +
                    'which is not supported by the current compiler configuration.');
                const _abs = file_system_1.absoluteFrom;
                expect((_a = error.relatedInformation) === null || _a === void 0 ? void 0 : _a.map((diag) => diag.messageText)).toEqual([
        ...
        describe('multiple local refs', () => {
            const getComponentScript = (template) => `
      import {Component, Directive, NgModule} from '@angular/core';
    ...
    `;
      ...
      `);
            env.driveMain();
            const dtsContents = env.getContents('test.d.ts');
            expect(dtsContents).toContain('static ɵdir: i0.ɵɵDirectiveDeclaration<TestDir, "[dir]", never, ' +
      ...
      `);
        ...
        describe('NgModule invalid import/export errors', () => {
            function verifyThrownError(errorCode, errorMessage) {
                ...
                expect(code).toBe((0, diagnostics_1.ngErrorCode)(errorCode));
                expect(trim(messageText)).toContain(errorMessage);
            }
        ...
        `);
          ...
          export class Module {}
        `);
                verifyThrownError(diagnostics_1.ErrorCode.NGMODULE_INVALID_IMPORT, 'This likely means that the library (external) which declares NotAModule is not ' +
        ...
        `);
          ...
          export class Module {}
        `);
                verifyThrownError(diagnostics_1.ErrorCode.NGMODULE_INVALID_IMPORT, 'This likely means that the dependency which declares NotAModule is not ' +
                    'compatible with Angular Ivy.');
            });
        ...
        `);
          ...
          export class Module {}
        `);
                verifyThrownError(diagnostics_1.ErrorCode.NGMODULE_INVALID_IMPORT, 'Is it missing an @NgModule annotation?');
            });
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(3);
                expect(diags[0].messageText).toBe('This import contains errors, which may affect components that depend on this NgModule.');
                expect(diags[1].messageText).toBe('NgModule "import" field contains a cycle');
                expect(diags[2].messageText).toBe('This import contains errors, which may affect components that depend on this NgModule.');
            });
        ...
        `);
                ...
                expect(diags.length).toBe(3);
                expect(diags[0].messageText).toBe('This import contains errors, which may affect components that depend on this NgModule.');
                expect(diags[1].messageText).toBe('This import contains errors, which may affect components that depend on this NgModule.');
      ...
      `);
        ...
        ['sync', 'async'].forEach((mode) => {
            describe(`preanalysis phase checks [${mode}]`, () => {
                ...
                it('should throw if @Component is missing a template', () => __awaiter(void 0, void 0, void 0, function* () {
                    env.write('test.ts', `
            import {Component} from '@angular/core';
            ...
            export class TestCmp {}
          `);
                    const diags = yield driveDiagnostics();
                    expect(diags[0].messageText).toBe('component is missing a template');
                    expect(diags[0].file.fileName).toBe((0, file_system_1.absoluteFrom)('/test.ts'));
                }));
                it('should throw if `styleUrls` is defined incorrectly in @Component', () => __awaiter(void 0, void 0, void 0, function* () {
                    env.write('test.ts', `
            import {Component} from '@angular/core';
          ...
          `);
                    const diags = yield driveDiagnostics();
                    expect(diags.length).toBe(1);
                    const messageText = typescript_1.default.flattenDiagnosticMessageText(diags[0].messageText, '\n');
        ...
        describe('flat module indices', () => {
            ...
            it('should determine the flat module entry-point within multiple root files', () => {
                ...
                env.driveMain();
                const jsContents = env.getContents('flat.js');
                expect(jsContents).toContain("export * from './index';", 'Should detect the "index.ts" file as flat module entry-point.');
            });
            ...
            it('should report an error when a flat module index is requested but no entrypoint can be determined', () => {
                ...
                const errors = env.driveDiagnostics();
                expect(errors.length).toBe(1);
                expect(errors[0].messageText).toBe('Angular compiler option "flatModuleOutFile" requires one and only one .ts file in the "files" field.');
            });
      ...
      `);
                const errors = env.driveDiagnostics();
                expect(errors.length).toBe(1);
                expect(errors[0].messageText).toBe('Unsupported private class Dir. This class is visible ' +
                ...
                const id = expectTokenAtPosition(error.file, error.start, typescript_1.default.isIdentifier);
                expect(id.text).toBe('Dir');
                expect(typescript_1.default.isClassDeclaration(id.parent)).toBe(true);
            });
      ...
      `);
                const errors = env.driveDiagnostics();
                expect(errors.length).toBe(1);
                expect(errors[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.SYMBOL_NOT_EXPORTED));
            });
      ...
      `);
                ...
                expect(errors[0].messageText).toBe('Unsupported private class DirModule. This class is ' +
                    'visible to consumers via Module -> DirModule, but is not exported from the top-level ' +
                    'library entrypoint.');
                expect(errors[1].messageText).toBe('Unsupported private class Dir. This class is visible ' +
      ...
      `);
        ...
        `);
                ...
                expect(diag[0].code).toEqual((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.NGMODULE_REEXPORT_NAME_COLLISION));
            });
    ...
    `);
      ...
      `);
                env.driveMain();
                const jsContents = env.getContents('index.js');
                expect(jsContents).not.toMatch(/export\s+\{\s*FooDir\s+as\s+ \u0275ng$root$foo$$FooDir\s*\}\s+from\s+"root\/foo";/);
            });
      ...
      `);
                env.driveMain();
                const jsContents = env.getContents('index.js');
                expect(jsContents).toContain('export { TestDir as \u0275ng$root$other___test$$TestDir } from "root/other._$test";');
            });
        });
        describe('disableTypeScriptVersionCheck', () => {
            afterEach(() => (0, typescript_support_1.restoreTypeScriptVersionForTesting)());
            it('produces an error when not supported and version check is enabled', () => {
                (0, typescript_support_1.setTypeScriptVersionForTesting)('3.4.0');
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toBe(`Required input 'input' from directive Dir must be specified.`);
            });
      ...
      `);
                env.driveMain();
                const jsContents = env.getContents('test.js');
                expect(jsContents).toContain('styles: ["h2[_ngcontent-%COMP%] {width: 10px}", "h1[_ngcontent-%COMP%] {font-size: larger}"]');
            });
      ...
      `);
        ...
        `);
                ...
                // separate var.
                expect(jsContents).toContain('_c0 = "div[_ngcontent-%COMP%] { background: url(/some-very-very-long-path.png); }";');
                expect(jsContents).toContain('styles: [' +
        ...
        `);
                ...
                expect(jsContents).toContain('_c0 = function () {' +
      ...
      `);
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toContain('@Component cannot define both `styleUrl` and `styleUrls`');
            });
        });
      ...
      `);
        ...
        describe('iframe processing', () => {
            it('should generate attribute and property bindings with a validator fn when on <iframe>', () => {
              ...
              `);
                ...
                expect(jsContents).toContain('ɵɵproperty("sandbox", "", i0.ɵɵvalidateIframeAttribute)("title", "Hi!")');
        ...
        `);
          ...
          `);
            ...
            `);
                    ...
                    expect(diags[1].messageText).toContain('Parser Error: Bindings cannot contain assignments at column 5 in [ {{x = 2}}]');
                });
            });
          ...
          `);
                    const diags = env.driveDiagnostics();
                    expect(diags.length).toBe(1);
                    expect(diags[0].messageText).toBe('Selector of a component that uses ViewEncapsulation.ShadowDom must contain a hyphen.');
          ...
          `);
                    const diags = env.driveDiagnostics();
                    expect(diags.length).toBe(1);
                    expect(diags[0].messageText).toBe('Selector of a ShadowDom-encapsulated component must all be in lower case.');
          ...
          `);
                    const diags = env.driveDiagnostics();
                    expect(diags.length).toBe(1);
                    expect(diags[0].messageText).toBe('Selector of a ShadowDom-encapsulated component must start with a lower case letter.');
          ...
          `);
            ...
            `);
                    const diags = env.driveDiagnostics();
                    expect(diags.length).toEqual(1);
                    expect(diags[0].messageText).toEqual('Cannot mark an element as translatable inside of a translatable section.' +
                        ' Please remove the nested i18n marker.');
                    expect((_a = diags[0].file) === null || _a === void 0 ? void 0 : _a.fileName).toEqual((0, file_system_1.absoluteFrom)('/test.ts'));
                    expect((_b = diags[0].file) === null || _b === void 0 ? void 0 : _b.text.slice(diags[0].start, diags[0].start + diags[0].length)).toEqual('<div i18n>Content</div>');
                });
          ...
          `);
                    ...
                    expect(diags[0].messageText).toEqual('Cannot mark an element as translatable inside of a translatable section.' +
                        ' Please remove the nested i18n marker.');
                    expect((_a = diags[0].file) === null || _a === void 0 ? void 0 : _a.fileName).toEqual((0, file_system_1.absoluteFrom)('/test.ts'));
                    expect((_b = diags[0].file) === null || _b === void 0 ? void 0 : _b.text.slice(diags[0].start, diags[0].start + diags[0].length)).toEqual('<ng-container i18n>Content</ng-container>');
                });
            });
        });
        it('reports a COMPONENT_RESOURCE_NOT_FOUND for a component with a templateUrl' +
            ...
            const diags = env.driveDiagnostics();
            expect(diags.length).toEqual(1);
            expect(diags[0].code).toEqual((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.COMPONENT_RESOURCE_NOT_FOUND));
            expect(diags[0].messageText).toEqual(`Could not find template file './non-existent-file.html'.`);
        });
        it(`reports a COMPONENT_RESOURCE_NOT_FOUND when style sheet link in a component's template` +
            ...
            expect(diags.length).toEqual(1);
            expect(diags[0].code).toEqual((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.COMPONENT_RESOURCE_NOT_FOUND));
            expect(diags[0].messageText).toEqual(`Could not find stylesheet file './non-existent-file.css' linked from the template.`);
        });
        it('reports a COMPONENT_RESOURCE_NOT_FOUND for a component with a style url ' +
            ...
            expect(diags.length).toEqual(1);
            expect(diags[0].code).toEqual((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.COMPONENT_RESOURCE_NOT_FOUND));
            expect(diags[0].messageText).toEqual(`Could not find stylesheet file './non-existent-file.css'.`);
        });
      ...
      `);
            ...
            expect(codes).toEqual([(0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.INVALID_BANANA_IN_BOX)]);
        });
      ...
      `);
            const diagnostics = env.driveDiagnostics();
            const codes = diagnostics.map((diag) => diag.code);
            expect(codes).toEqual([(0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.NGMODULE_BOOTSTRAP_IS_STANDALONE)]);
        });
      ...
      `);
        ...
        `);
                ...
                expect(jsContents.replace(/\s/g, '')).toContain(`Module.ɵinj=/*@__PURE__*/i0.ɵɵdefineInjector({imports:[${expectedImports.replace(/\s/g, '')}]});`);
            });
        });
        ...
        `);
          ...
          `);
                ...
                expect(dtsContents).toContain('static ngAcceptInputType_value: boolean | string | i1.GenericWrapper<string>;');
            });
            ...
            `);
                ...
                expect(dtsContents).toContain('static ngAcceptInputType_value: boolean | string | i1.GenericWrapper<i2.GenericClass<string>>;');
            });
        ...
        `);
                ...
                const jsContents = env.getContents('test.js');
                const dtsContents = env.getContents('test.d.ts');
                expect(jsContents).toContain('inputs: { value: [2, "value", "value", (value) => value ? 1 : 0] }');
                expect(dtsContents).toContain('import * as i1 from "external";');
                expect(dtsContents).toContain('static ngAcceptInputType_value: i1.ExternalToNumberType;');
            });
        ...
        `);
                ...
                expect(dtsContents).toContain(`static ngAcceptInputType_value: boolean | "" | "true" | "false";`);
            });
        ...
        `);
                ...
                const jsContents = env.getContents('test.js');
                const dtsContents = env.getContents('test.d.ts');
                expect(jsContents).toContain('inputs: { element: [2, "element", "element", coerceElement] }');
                expect(dtsContents).toContain('static ngAcceptInputType_element: HTMLElement | i0.ElementRef<HTMLElement>;');
            });
        });
        describe('debug info', () => {
      ...
      `);
        ...
        `);
                ...
                const program = new program_1.NgtscProgram(['/test.ts'], options, (0, compiler_host_1.createCompilerHost)({ options }));
                const transformers = program.compiler.prepareEmit().transformers;
                // Add a "fake tsickle" transform before Angular's transform.
                transformers.before.unshift((ctx) => (sf) => {
                    const tsickleFileOverview = ctx.factory.createNotEmittedStatement(sf);
                    typescript_1.default.setSyntheticLeadingComments(tsickleFileOverview, [
                        {
                ...
                `,
                    ...
                    return ctx.factory.updateSourceFile(sf, [tsickleFileOverview, ...sf.statements], sf.isDeclarationFile, sf.referencedFiles, sf.typeReferenceDirectives, sf.hasNoDefaultLib, sf.libReferenceDirectives);
                });
        ...
        describe('standalone by default opt-out', () => {
          ...
          `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(3);
                expect(diags[0].messageText).toContain('Directive TestDir is standalone, and cannot be declared in an NgModule.');
                expect(diags[1].messageText).toContain('Component TestComp is standalone, and cannot be declared in an NgModule.');
                expect(diags[2].messageText).toContain('Pipe TestPipe is standalone, and cannot be declared in an NgModule.');
            });
          ...
          `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(3);
                expect(diags[0].messageText).toContain(`The directive 'TestDir' appears in 'imports', but is not standalone`);
                expect(diags[1].messageText).toContain(`The component 'TestComp' appears in 'imports', but is not standalone`);
                expect(diags[2].messageText).toContain(`The pipe 'TestPipe' appears in 'imports', but is not standalone`);
            });
        });
    });
    function expectTokenAtPosition(sf, pos, guard) {
        // getTokenAtPosition is part of TypeScript's private API.
        const node = typescript_1.default.getTokenAtPosition(sf, pos);
