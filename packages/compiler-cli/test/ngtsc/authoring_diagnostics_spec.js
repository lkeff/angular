...
(0, testing_1.runInEachFileSystem)(() => {
    describe('authoring API diagnostics', () => {
      ...
      `);
            ...
            expect(diags[0].messageText).toContain('Unsupported call to the input function. This function can only be called in the initializer of a class member');
        });
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toContain('Unsupported call to the input.required function. This function can only be called in the initializer of a class member');
        });
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toContain('Unsupported call to the input function. This function can only be used as the initializer of a property on a @Component or @Directive class.');
        });
