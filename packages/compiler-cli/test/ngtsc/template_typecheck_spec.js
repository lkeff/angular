...
const api_1 = require("../../src/ngtsc/core/api");
...
const invalid_banana_in_box_1 = require("../../src/ngtsc/typecheck/extended/checks/invalid_banana_in_box");
...
(0, testing_1.runInEachFileSystem)(() => {
    describe('ngtsc type checking', () => {
      ...
      `);
            ...
            expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toBe('does_not_exist');
        });
      ...
      `);
            ...
            expect(diags[1].messageText).toEqual(`Argument of type 'string' is not assignable to parameter of type 'number'.`);
        });
      ...
      `);
            ...
            expect(diags[0].messageText).toEqual(`Argument of type 'number' is not assignable to parameter of type 'string'.`);
            expect(diags[1].messageText).toEqual(`Property 'updated' does not exist on type 'TestCmp'. Did you mean 'update'?`);
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].code).toBe((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.SPLIT_TWO_WAY_BINDING));
            expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toBe('value');
            expect(diags[0].relatedInformation.length).toBe(2);
            expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0].relatedInformation[0])).toBe('ChildCmp');
            expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0].relatedInformation[1])).toBe('child-cmp');
        });
      ...
      `);
            ...
            expect(diags[0].relatedInformation.length).toBe(2);
            expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0].relatedInformation[0])).toBe('ChildCmp');
            expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0].relatedInformation[1])).toBe('ChildCmpDir');
        });
      ...
      `);
            ...
            expect(diags[1].messageText).toEqual(jasmine.objectContaining({
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toContain(`Argument of type 'string' is not assignable to parameter of type 'number'.`);
        });
      ...
      `);
        ...
        `);
            ...
            it('should check expressions and their type when enabled', () => {
                ...
                expect(diags[1].messageText).toEqual(`Property 'invalid' does not exist on type 'TestCmp'.`);
            });
            ...
            it('should check expressions but not their type when not enabled', () => {
                ...
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toEqual(`Property 'invalid' does not exist on type 'TestCmp'.`);
            });
        });
        ...
        `);
            ...
            it('should check expressions and their nullability when enabled', () => {
                ...
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(2);
                expect(diags[0].messageText.messageText).toEqual(`Type 'boolean | null | undefined' is not assignable to type 'boolean'.`);
                expect(diags[1].messageText).toEqual(`Property 'invalid' does not exist on type 'TestCmp'.`);
            });
        ...
        `);
            ...
            it('should infer result type for safe navigation expressions when enabled', () => {
                ...
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(2);
                expect(diags[0].messageText.messageText).toEqual(`Type 'boolean | undefined' is not assignable to type 'boolean'.`);
                expect(diags[1].messageText).toEqual(`Property 'invalid' does not exist on type 'TestCmp'.`);
            });
        ...
        `);
            ...
            it('should expressions and infer type of $event when enabled', () => {
                ...
                expect(diags.length).toBe(2);
                expect(diags[0].messageText).toEqual(`Property 'invalid' does not exist on type 'TestCmp'.`);
                expect(diags[1].messageText).toEqual(`Argument of type 'number' is not assignable to parameter of type 'string'.`);
            });
        ...
        `);
            ...
            it('should check expressions and let $event be of type AnimationEvent when enabled', () => {
                ...
                expect(diags.length).toBe(2);
                expect(diags[0].messageText).toEqual(`Property 'invalid' does not exist on type 'TestCmp'.`);
                expect(diags[1].messageText).toEqual(`Argument of type 'AnimationEvent' is not assignable to parameter of type 'string'.`);
            });
        ...
        `);
            ...
            it('should infer the type of DOM references when enabled', () => {
                ...
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toEqual(`Property 'does_not_exist' does not exist on type 'HTMLInputElement'.`);
            });
        ...
        `);
            ...
            it('should check expressions and infer type of $event when enabled', () => {
                ...
                expect(diags.length).toBe(2);
                expect(diags[0].messageText).toEqual(`Property 'invalid' does not exist on type 'TestCmp'.`);
                expect(diags[1].messageText).toEqual(`Argument of type 'FocusEvent' is not assignable to parameter of type 'string'.`);
            });
    ...
    `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toEqual(`Property 'does_not_exist' does not exist on type '{ name: string; }'.`);
            expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toBe('does_not_exist');
        });
    ...
    `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText.messageText).toContain(`is not assignable to type 'TrackByFunction<UnrelatedType>'.`);
        });
    ...
    `);
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe(`Property 'nonExistingProp' does not exist on type '{ name: string; }'.`);
        });
      ...
      `);
            ...
            expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toBe('unknownTarget');
        });
      ...
      `);
            ...
            expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toBe('unknown');
        });
    ...
    `);
            ...
            for (const error of allErrors) {
                if (!diags.some((diag) => typescript_1.default.flattenDiagnosticMessageText(diag.messageText, '').indexOf(error) > -1)) {
    ...
    `);
            ...
            expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toEqual('fromAbstract');
            expect(diags[1].messageText).toBe(`Type 'number' is not assignable to type 'string'.`);
            expect((0, testing_2.getSourceCodeForDiagnostic)(diags[1])).toEqual('fromBase');
            expect(diags[2].messageText).toBe(`Type 'number' is not assignable to type 'boolean'.`);
            expect((0, testing_2.getSourceCodeForDiagnostic)(diags[2])).toEqual('fromChild');
        });
      ...
      `);
            ...
            expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toEqual('y = !y');
        });
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toEqual(1);
            expect(diags[0].code).toEqual((0, diagnostics_1.ngErrorCode)(diagnostics_1.ErrorCode.DUPLICATE_VARIABLE_DECLARATION));
            expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toContain('let i = index');
        });
        ...
        `);
            ...
            function getDiagnosticLines(diag) {
                const separator = '~~~~~';
                return typescript_1.default.flattenDiagnosticMessageText(diag.messageText, separator).split(separator);
            }
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toBe(`Type 'boolean' is not assignable to type 'string | number'.`);
            });
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toBe(`Type 'number' is not assignable to type 'string | boolean'.`);
            });
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toBe(`Type '"test"' is not assignable to type 'number | boolean'.`);
            });
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(getDiagnosticLines(diags[0]).join('\n')).toContain(`Type 'HTMLDivElement' is not assignable to type ` +
      ...
      `;
        ...
        `;
            describe('with strictInputAccessModifiers', () => {
                ...
                function expectIllegalAssignmentErrors(diags) {
                    expect(diags.length).toBe(3);
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toBe(`Argument of type 'string' is not assignable to parameter of type 'number'.`);
            });
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toBe(`Argument of type 'number' is not assignable to parameter of type 'string'.`);
            });
        });
...
2. If 'my-foo' is a Web Component then add 'CUSTOM_ELEMENTS_SCHEMA' to the '@Component.schemas' of this component to suppress this message.`);
      ...
      `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toBe(`Can't bind to 'foo' since it isn't a known property of 'div'.`);
            });
...
2. To allow any element add 'NO_ERRORS_SCHEMA' to the '@NgModule.schemas' of this component.`);
        ...
        ['sync', 'async'].forEach((mode) => {
            describe(`error locations [${mode}]`, () => {
                ...
                it('should be correct for direct templates', () => __awaiter(void 0, void 0, void 0, function* () {
                    env.write('test.ts', `
          import {Component, NgModule} from '@angular/core';
          ...
          export class TestCmp {
            user: {name: string}[];
                ...
                it('should be correct for indirect templates', () => __awaiter(void 0, void 0, void 0, function* () {
                    env.write('test.ts', `
          import {Component, NgModule} from '@angular/core';
          ...
          export class TestCmp {
            user: {name: string}[];
                    ...
                    const diags = yield driveDiagnostics();
                    expect(diags.length).toBe(1);
                    expect(diags[0].file.fileName).toBe((0, file_system_1.absoluteFrom)('/test.ts') + ' (TestCmp template)');
                    expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toBe('does_not_exist');
                    expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0].relatedInformation[0])).toBe('TEMPLATE');
                }));
                it('should be correct for external templates', () => __awaiter(void 0, void 0, void 0, function* () {
          ...
          export class TestCmp {
            user: {name: string}[];
                    ...
                    const diags = yield driveDiagnostics();
                    expect(diags.length).toBe(1);
                    expect(diags[0].file.fileName).toBe((0, file_system_1.absoluteFrom)('/template.html'));
                    expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toBe('does_not_exist');
                    expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0].relatedInformation[0])).toBe(`'./template.html'`);
                }));
            });
        });
        describe('option compatibility verification', () => {
            ...
            it('should error if "fullTemplateTypeCheck" is false when "strictTemplates" is true', () => {
                ...
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toContain('Angular compiler option "strictTemplates" is enabled, however "fullTemplateTypeCheck" is disabled.');
            });
            ...
            it('should error if "strictTemplates" is false when "extendedDiagnostics" is configured', () => {
                ...
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toContain('Angular compiler option "extendedDiagnostics" is configured, however "strictTemplates" is disabled.');
            });
            ...
            it('should error if "extendedDiagnostics.defaultCategory" is set to an unknown value', () => {
                ...
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toContain('Angular compiler option "extendedDiagnostics.defaultCategory" has an unknown diagnostic category: "does-not-exist".');
...
suppress
        `.trim());
            ...
            it('should not error if "extendedDiagnostics.defaultCategory" is set to a known value', () => {
                env.tsconfig({
                    extendedDiagnostics: {
                        defaultCategory: api_1.DiagnosticCategoryLabel.Error,
                    },
                });
                const diags = env.driveDiagnostics();
                expect(diags).toEqual([]);
            ...
            it('should error if "extendedDiagnostics.checks" contains an unknown check', () => {
                env.tsconfig({
                    extendedDiagnostics: {
                        checks: {
                            doesNotExist: api_1.DiagnosticCategoryLabel.Error,
                        },
                    },
                });
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toContain('Angular compiler option "extendedDiagnostics.checks" has an unknown check: "doesNotExist".');
            });
            it('should not error if "extendedDiagnostics.checks" contains all known checks', () => {
                env.tsconfig({
                    extendedDiagnostics: {
                        checks: {
                            [invalid_banana_in_box_1.factory.name]: api_1.DiagnosticCategoryLabel.Error,
                        },
                    },
                });
                const diags = env.driveDiagnostics();
            ...
            it('should error if "extendedDiagnostics.checks" contains an unknown diagnostic category', () => {
                env.tsconfig({
                    extendedDiagnostics: {
                        checks: {
                            [invalid_banana_in_box_1.factory.name]: 'does-not-exist',
                        },
                    },
                });
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toContain(`Angular compiler option "extendedDiagnostics.checks['${invalid_banana_in_box_1.factory.name}']" has an unknown diagnostic category: "does-not-exist".`);
                expect(diags[0].messageText).toContain(`
Allowed diagnostic categories are:
warning
error
suppress
        ...
        `);
            ...
            it('should not leave referencedFiles in a tagged state', () => {
                ...
                env.driveMain();
                const sf = (0, file_system_1.getSourceFileOrError)(env.getTsProgram(), (0, file_system_1.absoluteFrom)('/test.ts'));
            ...
            it('should allow for complete program reuse during incremental compilations', () => {
                ...
                env.driveMain();
                (0, testing_2.expectCompleteReuse)(env.getTsProgram());
                (0, testing_2.expectCompleteReuse)(env.getReuseTsProgram());
            });
        });
    ...
    `);
      ...
      export class Main { }
       `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(typescript_1.default.flattenDiagnosticMessageText(diags[0].messageText, '')).toContain('Unable to import symbol HostBindDirective');
            });
            it('should check bindings to inherited host directive inputs', () => {
                env.write('test.ts', `
          import {Component, Directive, NgModule, Input} from '@angular/core';
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.map((d) => typescript_1.default.flattenDiagnosticMessageText(d.messageText, ''))).toEqual([
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(typescript_1.default.flattenDiagnosticMessageText(diags[0].messageText, '')).toContain('Trigger cannot find reference "does_not_exist".');
            });
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(typescript_1.default.flattenDiagnosticMessageText(diags[0].messageText, '')).toContain('Trigger cannot find reference "trigger".');
            });
        });
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toContain('Error: Illegal State: Pipes are not allowed in this context');
            });
        ...
        `);
                ...
                expect(diags.length).toBe(1);
                expect(diags[0]).toContain(`Node matches the "bar, [foo]" slot of the "Comp" component, but will ` +
        ...
        `);
                const diags = env
                    ...
                    .map((d) => typescript_1.default.flattenDiagnosticMessageText(d.messageText, ''));
                expect(diags.length).toBe(1);
                expect(diags[0]).toContain(`Node matches the "[bar]" slot of the "Comp" component, but will ` +
        ...
        `);
                const diags = env
                    ...
                    .map((d) => typescript_1.default.flattenDiagnosticMessageText(d.messageText, ''));
                expect(diags.length).toBe(2);
                expect(diags[0]).toContain(`Node matches the "[foo]" slot of the "Comp" component, but will not be projected`);
                expect(diags[1]).toContain(`Node matches the "[bar]" slot of the "Comp" component, but will not be projected`);
            });
        ...
        `);
                ...
                expect(diags.length).toBe(2);
                expect(diags[0]).toContain(`Node matches the "[foo]" slot of the "Comp" component, but will not be projected`);
                expect(diags[1]).toContain(`Node matches the "[foo]" slot of the "Comp" component, but will not be projected`);
            });
        ...
        `);
                ...
                expect(diags[0]).toContain(`Note: the host component has \`preserveWhitespaces: true\` which may cause ` +
        ...
        `);
            ...
            it('should allow the content projection diagnostic to be disabled individually', () => {
                env.tsconfig({
                    extendedDiagnostics: {
                        checks: {
                            controlFlowPreventingContentProjection: api_1.DiagnosticCategoryLabel.Suppress,
                        },
                    },
                });
                env.write('test.ts', `
        ...
        `);
            ...
            it('should allow the content projection diagnostic to be disabled via `defaultCategory`', () => {
                env.tsconfig({
                    extendedDiagnostics: {
                        defaultCategory: api_1.DiagnosticCategoryLabel.Suppress,
                    },
                });
                env.write('test.ts', `
              import {Component} from '@angular/core';
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toBe('one');
                expect(diags[0].messageText).toBe(`Argument of type 'number' is not assignable to parameter of type 'string'.`);
            });
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect((0, testing_2.getSourceCodeForDiagnostic)(diags[0])).toBe('{} + 1');
                expect(diags[0].messageText).toBe(`Operator '+' cannot be applied to types '{}' and 'number'.`);
            });
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toBe(`Cannot declare @let called 'value' as there is another symbol in the template with the same name.`);
            });
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toBe(`Cannot read @let declaration 'value' before it has been defined.`);
            });
        ...
        `);
            ...
            it('should be able to opt out for checking for unused imports via the tsconfig', () => {
                env.tsconfig({
                    extendedDiagnostics: {
                        checks: {
                            unusedStandaloneImports: api_1.DiagnosticCategoryLabel.Suppress,
                        },
                    },
                });
                env.write('test.ts', `
        ...
        `);
            ...
            it('should unused imports from external modules', () => {
                ...
                env.write('node_modules/fake-common/index.d.ts', `
          import * as i0 from '@angular/core';
