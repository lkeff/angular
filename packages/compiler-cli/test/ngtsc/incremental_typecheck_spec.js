...
(0, testing_1.runInEachFileSystem)(() => {
    describe('ngtsc incremental compilation (template typecheck)', () => {
        ...
        `);
                ...
                expect(diagnostics[0].messageText).toBe(`Type 'string' is not assignable to type 'boolean'.`);
            });
        ...
        `);
                ...
                expect(diags[0].messageText).toContain(`Type 'boolean' is not assignable to type 'string'.`);
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toContain(`Argument of type 'number' is not assignable to parameter of type 'string'.`);
            });
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toContain(`Property 'b' is missing in type 'import("${(0, file_system_1.absoluteFrom)('/node_modules/foo-a/index')}").Foo' but required in type 'import("${(0, file_system_1.absoluteFrom)('/node_modules/foo-b/index')}").Foo'.`);
        ...
        describe('inheritance', () => {
           ...
           `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toContain(`Can't bind to 'parent' since it isn't a known property of 'div'.`);
            });
          ...
          `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toContain(`Can't bind to 'grandparent' since it isn't a known property of 'div'.`);
            });
        ...
        `);
                ...
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toContain(`Can't bind to 'grandgrandparent' since it isn't a known property of 'div'.`);
        ...
        `);
           ...
           `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(1);
                expect(diags[0].messageText).toContain(`Can't bind to 'grandgrandparentA' since it isn't a known property of 'div'.`);
            });
        });
