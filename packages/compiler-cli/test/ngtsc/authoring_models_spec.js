...
(0, testing_1.runInEachFileSystem)(() => {
    describe('initializer-based model() API', () => {
      ...
      `);
            ...
            expect(dts).toContain('static ɵdir: i0.ɵɵDirectiveDeclaration<TestDir, never, never, ' +
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe('Input "value" is also declared as non-signal in @Directive.');
        });
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe('Alias needs to be a string that is statically analyzable.');
        });
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe('Argument needs to be an object literal that is statically analyzable.');
        });
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe('Input "value" is incorrectly declared as static member of "TestDir".');
        });
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe(`Object literal may only specify known properties, ` +
      ...
      `);
        ...
        `);
                const diags = env.driveDiagnostics();
                expect(diags.length).toBe(2);
                expect(diags[0].messageText).toBe(`Type 'InputSignal<number>' is not assignable to type 'number'.`);
                expect(diags[1].messageText).toBe(`Type 'number' is not assignable to type 'InputSignal<number>'.`);
            });
        ...
        `);
                ...
                expect(diagnostics[0].messageText).toBe(`Required input 'value' from directive TestDir must be specified.`);
            });
        ...
        `);
                const diagnostics = env.driveDiagnostics();
                expect(diagnostics.length).toBe(1);
                expect(diagnostics[0].messageText).toBe(`Type 'WritableSignal<number>' is not assignable to type 'number'.`);
            });
        });
      ...
      `);
            const diags = env.driveDiagnostics();
            expect(diags.length).toBe(1);
            expect(diags[0].messageText).toBe(`Type 'WritableSignal<number>' is not assignable to type 'number'.`);
        });
        ...
        `);
                ...
                expect(diagnostics).toEqual([
                    jasmine.objectContaining({
                        messageText: jasmine.objectContaining({
