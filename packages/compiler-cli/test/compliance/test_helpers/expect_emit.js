...
const NUMBER = /\d+/;
const ELLIPSIS = '…';
const TOKEN = new RegExp(`\\s*((${COMMENT_START.source})|(${COMMENT_END.source})|(${IDENTIFIER.source})|(${INLINE_BACKTICK_STRING.source})|(${OPERATOR.source})|(${STRING.source})|${NUMBER.source}|${ELLIPSIS})\\s*`, 'y');
const SKIP = /(?:.|\n|\r)*/;
 ...
 */
function tokenizeBackTickString(str) {
    const pieces = ['`'];
...
function expectEmit(source, expected, description, assertIdentifiers) {
    ...
    if (matches === null) {
        ...
        throw new Error(`Test helper failure: Expected expression failed but the reporting logic could not find where it failed in: ${source}`);
    }
    else {
        if (assertIdentifiers) {
            ...
            for (let i = 0; i < ids.length; i++) {
                const id = ids[i];
                if (groups.has(id)) {
                    const name = matches[groups.get(id)];
                    const regexp = assertIdentifiers[id];
                    if (!regexp.test(name)) {
                        throw Error(`${description}: The matching identifier "${id}" is "${name}" which doesn't match ${regexp}`);
                    }
                }
            }
        }
    ...
    for (const piece of pieces) {
        ...
        if (typeof piece === 'string') {
            if (MATCHING_IDENT.test(piece)) {
                ...
                results.push((0, util_1.escapeRegExp)(piece));
            }
        }
        else {
