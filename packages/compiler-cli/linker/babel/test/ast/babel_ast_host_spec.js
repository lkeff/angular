...
const babel_ast_host_1 = require("../../src/ast/babel_ast_host");
describe('BabelAstHost', () => {
    let host;
    beforeEach(() => (host = new babel_ast_host_1.BabelAstHost()));
    ...
    describe('getRange()', () => {
        it('should extract the range from the expression', () => {
            const file = (0, core_1.parse)("// preamble\nx = 'moo';");
        ...
        it('should error if there is no range information', () => {
            ...
            expect(() => host.getRange(moo)).toThrowError('Unable to read range for node - it is missing location information.');
        });
    });
});
function expr(code) {
    const stmt = core_1.template.ast(code);
...
function assertExpressionStatement(e) {
    if (!core_1.types.isExpressionStatement(e)) {
        throw new Error('Bad test - expected an expression statement');
    }
}
function assertAssignmentExpression(e) {
    if (!core_1.types.isAssignmentExpression(e)) {
