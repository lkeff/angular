...
describe('TypeScriptAstHost', () => {
    let host;
    beforeEach(() => (host = new typescript_ast_host_1.TypeScriptAstHost()));
    ...
    describe('parseStringLiteral()', () => {
        ...
        it('should error if the value is not a string literal', () => {
            expect(() => host.parseStringLiteral(expr('42'))).toThrowError('Unsupported syntax, expected a string literal.');
        });
    });
    ...
    describe('parseNumericLiteral()', () => {
        ...
        it('should error if the value is not a numeric literal', () => {
            expect(() => host.parseNumericLiteral(expr('"moo"'))).toThrowError('Unsupported syntax, expected a numeric literal.');
        });
    });
    ...
    describe('parseBooleanLiteral()', () => {
        ...
        it('should error if the value is not a boolean literal', () => {
            expect(() => host.parseBooleanLiteral(expr('"moo"'))).toThrowError('Unsupported syntax, expected a boolean literal.');
        });
    });
    ...
    describe('parseArrayLiteral()', () => {
        it('should extract the expressions in the array', () => {
            const moo = jasmine.objectContaining({ text: 'moo', kind: typescript_1.default.SyntaxKind.StringLiteral });
        ...
        it('should error if there is an empty item', () => {
            expect(() => host.parseArrayLiteral(expr('[,]'))).toThrowError('Unsupported syntax, expected element in array not to be empty.');
        });
        it('should error if there is a spread element', () => {
            expect(() => host.parseArrayLiteral(expr('[...[0,1]]'))).toThrowError('Unsupported syntax, expected element in array not to use spread syntax.');
        });
    });
    ...
    describe('parseObjectLiteral()', () => {
        ...
        it('should error if there is a method', () => {
            expect(() => host.parseObjectLiteral(rhs('x = { foo() {} }'))).toThrowError('Unsupported syntax, expected a property assignment.');
        });
        it('should error if there is a spread element', () => {
            expect(() => host.parseObjectLiteral(rhs("x = {...{ a: 'moo' }}"))).toThrowError('Unsupported syntax, expected a property assignment.');
        });
    });
    ...
    describe('parseReturnValue()', () => {
        ...
        it('should error if the body has 0 statements', () => {
            expect(() => host.parseReturnValue(rhs('x = function () { }'))).toThrowError('Unsupported syntax, expected a function body with a single return statement.');
            expect(() => host.parseReturnValue(rhs('x = () => { }'))).toThrowError('Unsupported syntax, expected a function body with a single return statement.');
        });
        it('should error if the body has more than 1 statement', () => {
            expect(() => host.parseReturnValue(rhs('x = function () { const x = 10; return x; }'))).toThrowError('Unsupported syntax, expected a function body with a single return statement.');
            expect(() => host.parseReturnValue(rhs('x = () => { const x = 10; return x; }'))).toThrowError('Unsupported syntax, expected a function body with a single return statement.');
        });
        it('should error if the single statement is not a return statement', () => {
            expect(() => host.parseReturnValue(rhs('x = function () { const x = 10; }'))).toThrowError('Unsupported syntax, expected a function body with a single return statement.');
            expect(() => host.parseReturnValue(rhs('x = () => { const x = 10; }'))).toThrowError('Unsupported syntax, expected a function body with a single return statement.');
        });
    });
    describe('parseParameters()', () => {
        it('should return the parameters as an array of expressions', () => {
            const arg1 = jasmine.objectContaining({ text: 'a', kind: typescript_1.default.SyntaxKind.Identifier });
            const arg2 = jasmine.objectContaining({ text: 'b', kind: typescript_1.default.SyntaxKind.Identifier });
        ...
        it('should error if the node is not a function declaration or arrow function', () => {
            expect(() => host.parseParameters(expr('[]'))).toThrowError('Unsupported syntax, expected a function.');
        });
        it('should error if a parameter uses spread syntax', () => {
            expect(() => host.parseParameters(rhs('x = function(a, ...other) {}'))).toThrowError('Unsupported syntax, expected an identifier.');
        });
    });
    ...
    describe('parseCallee()', () => {
        it('should return the callee expression', () => {
            const foo = jasmine.objectContaining({ text: 'foo', kind: typescript_1.default.SyntaxKind.Identifier });
            const bar = jasmine.objectContaining({ kind: typescript_1.default.SyntaxKind.PropertyAccessExpression });
        ...
        it('should error if the node is not a call expression', () => {
            expect(() => host.parseCallee(expr('[]'))).toThrowError('Unsupported syntax, expected a call expression.');
        });
    });
    describe('parseArguments()', () => {
        it('should return the arguments as an array of expressions', () => {
            const arg1 = jasmine.objectContaining({ text: '12', kind: typescript_1.default.SyntaxKind.NumericLiteral });
            const arg2 = jasmine.objectContaining({ kind: typescript_1.default.SyntaxKind.ArrayLiteralExpression });
            expect(host.parseArguments(expr('foo(12, [])'))).toEqual([arg1, arg2]);
            expect(host.parseArguments(expr('foo.bar()'))).toEqual([]);
        });
        it('should error if the node is not a call expression', () => {
            expect(() => host.parseArguments(expr('[]'))).toThrowError('Unsupported syntax, expected a call expression.');
        });
        it('should error if an argument uses spread syntax', () => {
            expect(() => host.parseArguments(expr('foo(1, ...[])'))).toThrowError('Unsupported syntax, expected argument not to use spread syntax.');
        });
    });
    describe('getRange()', () => {
        ...
        it('should error if the nodes do not have attached parents', () => {
            const moo = rhs("// preamble\nx = 'moo';", false);
            expect(() => host.getRange(moo)).toThrowError('Unable to read range for node - it is missing parent information.');
        });
    });
});
function stmt(code, attachParents = true) {
    const sf = typescript_1.default.createSourceFile('test.ts', code, typescript_1.default.ScriptTarget.ES2015, attachParents);
    return sf.statements[0];
}
function expr(code, attachParents = true) {
    return stmt(code, attachParents).expression;
}
function rhs(code, attachParents = true) {
