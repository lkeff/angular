...
const testing_1 = require("../../../../src/ngtsc/logging/testing");
const partial_linker_selector_1 = require("../../../src/file_linker/partial_linkers/partial_linker_selector");
describe('PartialLinkerSelector', () => {
    let logger;
    const linkerA = { name: 'linkerA' };
    const linkerA2 = { name: 'linkerA2' };
    const linkerB = { name: 'linkerB' };
    const linkerB2 = { name: 'linkerB2' };
    beforeEach(() => {
    ...
    describe('getLinker()', () => {
        ...
        it('should return the most recent linker if `version` is `0.0.0-PLACEHOLDER`, regardless of `minVersion`', () => {
            ...
            expect(selector.getLinker('declareA', '0.0.0-PLACEHOLDER', '0.0.0-PLACEHOLDER')).toBe(linkerA2);
        });
        it('should throw an error if there is no linker that matches the given name', () => {
            ...
            expect(() => selector.getLinker('$foo', '11.1.2', '11.2.0')).toThrowError('Unknown partial declaration function $foo.');
        });
        describe('[unknown declaration version]', () => {
            ...
            describe('[unknownDeclarationVersionHandling is "error"]', () => {
                it('should throw an error', () => {
                    const selector = createSelector('error');
                    expect(() => selector.getLinker('declareA', '13.1.0', '14.0.5')).toThrowError(`This application depends upon a library published using Angular version 14.0.5, ` +
                        `which requires Angular version 13.1.0 or newer to work correctly.\n` +
                        `Consider upgrading your application to use a more recent version of Angular.`);
                });
            });
     ...
     */
    function createSelector(unknownDeclarationVersionHandling) {
        const linkerMap = new Map();
        linkerMap.set('declareA', [
            { range: new semver_1.default.Range('<=12.0.0'), linker: linkerA },
            { range: new semver_1.default.Range('<=13.0.0'), linker: linkerA2 },
        ]);
        linkerMap.set('declareB', [
            { range: new semver_1.default.Range('<=12.0.0'), linker: linkerB },
            { range: new semver_1.default.Range('<=12.1.0'), linker: linkerB2 },
        ]);
        return new partial_linker_selector_1.PartialLinkerSelector(linkerMap, logger, unknownDeclarationVersionHandling);
    }
});
