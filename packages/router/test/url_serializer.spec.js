...
describe('url serializer', () => {
    const url = new url_tree_1.DefaultUrlSerializer();
    ...
    it('should parse nodes with only secondary segment', () => {
        ...
        const one = tree.root.children[shared_1.PRIMARY_OUTLET];
    ...
    it('should not parse empty path segments with params', () => {
        expect(() => url.parse('/one/two/(;a=1//right:;b=2)')).toThrowError(/Empty path url segment cannot have parameters/);
    });
    it('should parse scoped secondary segments', () => {
        const tree = url.parse('/one/(two//left:three)');
        const primary = tree.root.children[shared_1.PRIMARY_OUTLET];
    ...
    describe('encoding/decoding', () => {
        it('should encode/decode path segments and parameters', () => {
            const u = `/${(0, url_tree_1.encodeUriSegment)('one two')};${(0, url_tree_1.encodeUriSegment)('p 1')}=${(0, url_tree_1.encodeUriSegment)('v 1')};${(0, url_tree_1.encodeUriSegment)('p 2')}=${(0, url_tree_1.encodeUriSegment)('v 2')}`;
            const tree = url.parse(u);
            expect(tree.root.children[shared_1.PRIMARY_OUTLET].segments[0].path).toEqual('one two');
            expect(tree.root.children[shared_1.PRIMARY_OUTLET].segments[0].parameters).toEqual({
                ['p 1']: 'v 1',
                ['p 2']: 'v 2',
            });
        ...
        it('should encode/decode "slash" in path segments and parameters', () => {
            const u = `/${(0, url_tree_1.encodeUriSegment)('one/two')};${(0, url_tree_1.encodeUriSegment)('p/1')}=${(0, url_tree_1.encodeUriSegment)('v/1')}/three`;
            const tree = url.parse(u);
            const segment = tree.root.children[shared_1.PRIMARY_OUTLET].segments[0];
        ...
        it('should encode/decode query params', () => {
            const u = `/one?${(0, url_tree_1.encodeUriQuery)('p 1')}=${(0, url_tree_1.encodeUriQuery)('v 1')}&${(0, url_tree_1.encodeUriQuery)('p 2')}=${(0, url_tree_1.encodeUriQuery)('v 2')}`;
            const tree = url.parse(u);
        ...
        it('should encode query params leaving sub-delimiters intact', () => {
            ...
            expect(percentCharsEncoded).toEqual((0, url_tree_1.encodeUriQuery)(percentChars));
            expect(intactChars).toEqual((0, url_tree_1.encodeUriQuery)(intactChars));
            // Verify it replaces repeated characters correctly
            expect(paramsEncoded + paramsEncoded).toEqual((0, url_tree_1.encodeUriQuery)(params + params));
            // Verify it doesn't change the case of alpha characters
            expect(mixedCaseString + paramsEncoded).toEqual((0, url_tree_1.encodeUriQuery)(mixedCaseString + params));
        });
        it('should encode/decode fragment', () => {
            const u = `/one#${(0, url_tree_1.encodeUriFragment)('one two=three four')}`;
            const tree = url.parse(u);
    ...
    describe('special character encoding/decoding', () => {
        ...
        it('should encode minimal special characters plus parens and semi-colon in matrix params', () => {
            ...
            const parsed = url.parse('/foo');
            parsed.root.children[shared_1.PRIMARY_OUTLET].segments[0].parameters = { notEncoded, encode };
            expect(url.serialize(parsed)).toBe(`/foo;notEncoded=${notEncoded};encode=${encoded}`);
        });
...
function expectSegment(segment, expected, hasChildren = false) {
    if (segment.segments.filter((s) => s.path === '').length > 0) {
        throw new Error(`UrlSegments cannot be empty ${segment.segments}`);
    }
    const p = segment.segments.map((p) => (0, url_tree_1.serializePath)(p)).join('/');
