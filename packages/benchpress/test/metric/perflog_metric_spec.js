...
(function () {
    let commandLog;
    const eventFactory = new trace_event_factory_1.TraceEventFactory('timeline', 'pid0');
    function createMetric(perfLogs, perfLogFeatures, { microMetrics, forceGc, captureFrames, receivedData, requestCount, ignoreNavigation, } = {}) {
        commandLog = [];
        if (!perfLogFeatures) {
        ...
        const providers = [
            ...
            index_1.PerflogMetric.PROVIDERS,
    ...
    describe('perflog metric', () => {
        ...
        it('should describe itself based on the perfLogFeatrues', () => {
            expect(sortedKeys(createMetric([[]], new index_1.PerfLogFeatures()).describe())).toEqual([
                'pureScriptTime',
                'scriptTime',
            ]);
            expect(sortedKeys(createMetric([[]], new index_1.PerfLogFeatures({ render: true, gc: false })).describe())).toEqual(['pureScriptTime', 'renderTime', 'renderTimeInScript', 'scriptTime']);
            ...
            expect(sortedKeys(createMetric([[]], new index_1.PerfLogFeatures({ render: true, gc: true }), {
                forceGc: true,
            }).describe())).toEqual([
                'forcedGcAmount',
                'forcedGcTime',
            ...
            expect(sortedKeys(createMetric([[]], new index_1.PerfLogFeatures({ userTiming: true }), {
                receivedData: true,
                requestCount: true,
            }).describe())).toEqual(['pureScriptTime', 'receivedData', 'requestCount', 'scriptTime']);
        });
        ...
        it('should describe itself if frame capture is requested and available', () => {
            const description = createMetric([[]], new index_1.PerfLogFeatures({ frameCapture: true }), {
                captureFrames: true,
            }).describe();
        ...
        it('should describe itself if frame capture is requested and not available', () => {
            const description = createMetric([[]], new index_1.PerfLogFeatures({ frameCapture: false }), {
                captureFrames: true,
            }).describe();
        ...
        describe('aggregation', () => {
            function aggregate(events, { microMetrics, captureFrames, receivedData, requestCount, } = {}) {
            ...
            describe('frame metrics', () => {
                ...
                it('should throw if trying to capture twice', (done) => {
                    aggregate([eventFactory.markStart('frameCapture', 3), eventFactory.markStart('frameCapture', 4)], { captureFrames: true }).catch((err) => {
                        expect(() => {
                            throw err;
                ...
                it('should throw if frame capture is enabled, but nothing is captured', (done) => {
                    aggregate([], { captureFrames: true }).catch((err) => {
                        expect(() => {
                            throw err;
            ...
            it('should ignore events from different processed as the start mark', (done) => {
                const otherProcessEventFactory = new trace_event_factory_1.TraceEventFactory('timeline', 'pid1');
...
class MockDriverExtension extends index_1.WebDriverExtension {
    constructor(_perfLogs, _commandLog, _perfLogFeatures) {
        super();
        this._perfLogs = _perfLogs;
        this._commandLog = _commandLog;
        this._perfLogFeatures = _perfLogFeatures;
    }
    timeBegin(name) {
