...
describe('definitions', () => {
    it('gets definition for template reference in overridden template', () => {
       ...
       `,
        ...
        (0, testing_2.assertFileNames)(Array.from(definitions), ['app.html']);
    });
    it('returns the pipe definitions when checkTypeOfPipes is false', () => {
       ...
       `,
        ...
        expect(template.contents.slice(textSpan.start, textSpan.start + textSpan.length)).toEqual('date');
    ...
    it('gets definitions for all inputs when attribute matches more than one', () => {
       ...
       export class MyDir2 {
        ...
        const { textSpan, definitions } = getDefinitionsAndAssertBoundSpan(env, template);
        expect(template.contents.slice(textSpan.start, textSpan.start + textSpan.length)).toEqual('inputA');
        ...
        (0, testing_2.assertFileNames)([def, def2], ['dir2.ts', 'dir.ts']);
    });
    ...
    it('gets definitions for all outputs when attribute matches more than one', () => {
       ...
       `,
        ...
        const { textSpan, definitions } = getDefinitionsAndAssertBoundSpan(env, template);
        expect(template.contents.slice(textSpan.start, textSpan.start + textSpan.length)).toEqual('someEvent');
        ...
        (0, testing_2.assertFileNames)([def4, def3, def2, def], ['dir4.ts', 'dir3.ts', 'dir2.ts', 'dir.ts']);
    });
    it('gets definitions for all model inputs when attribute matches more than one in a static attribute', () => {
        ...
        template.moveCursorToText('inpu¦tA="abc"');
        const { textSpan, definitions } = getDefinitionsAndAssertBoundSpan(env, template);
        expect(template.contents.slice(textSpan.start, textSpan.start + textSpan.length)).toBe('inputA');
    ...
    it('gets definitions for all model inputs when attribute matches more than one in a two-way binding', () => {
        ...
        (0, testing_2.assertFileNames)([def, def2, def3, def4], ['dir2.ts', 'dir.ts', 'dir2.ts', 'dir.ts']);
    });
    it('should go to the pre-compiled style sheet', () => {
       ...
       `,
        ...
        expect(appFile.contents.slice(textSpan.start, textSpan.start + textSpan.length)).toEqual('./style.css');
        expect(definitions.length).toEqual(1);
        (0, testing_2.assertFileNames)(definitions, ['style.scss']);
    });
    ...
    it('gets definition for a let declaration', () => {
       ...
       `,
        ...
        expect(definitions[0].kind).toBe(typescript_1.default.ScriptElementKind.variableElement);
    ...
    it('gets definition for a method in a void expression', () => {
       ...
       `,
        ...
        expect(definitions[0].kind).toBe(typescript_1.default.ScriptElementKind.memberFunctionElement);
        expect(definitions[0].textSpan).toBe('doSomething');
        (0, testing_2.assertFileNames)(Array.from(definitions), ['app.ts']);
    });
    it('gets definition for a tagged template literal expression', () => {
       ...
       `,
        ...
        expect(definitions[0].kind).toBe(typescript_1.default.ScriptElementKind.memberVariableElement);
    ...
    it('gets definition for a host binding value of a directive', () => {
       ...
       `,
        ...
        expect(definitions[0].kind).toBe(typescript_1.default.ScriptElementKind.memberVariableElement);
        expect(definitions[0].textSpan).toBe('myTitle');
        (0, testing_2.assertFileNames)(Array.from(definitions), ['dir.ts']);
    });
    ...
    function getDefinitionsAndAssertBoundSpan(env, file) {
        ...
        return { textSpan, definitions: definitions.map((d) => (0, testing_2.humanizeDocumentSpanLike)(d, env)) };
    }
});
describe('definitions', () => {
    let env;
	 ...
	 export class AppModule {}
       `,
        ...
        it('can get definitions for input', () => {
            ...
            expect(definitions.length).toEqual(1);
            (0, testing_2.assertTextSpans)(definitions, ['obs$']);
            (0, testing_2.assertFileNames)(definitions, ['app.ts']);
        });
        it('can get definitions for component', () => {
            const project = env.addProject('test', files, { strictTemplates: false });
            const definitions = getDefinitionsAndAssertBoundSpan(project, 'app.ts', '<dollar-cm¦p');
            expect(definitions.length).toEqual(1);
            (0, testing_2.assertTextSpans)(definitions, ['DollarCmp']);
            (0, testing_2.assertFileNames)(definitions, ['app.ts']);
        });
    });
    ...
    standalone: false,
	 ...
	 export class AppModule {}
       `,
            ...
            const project = env.addProject('test', files, { strictTemplates: false });
            const definitions = getDefinitionsAndAssertBoundSpan(project, 'app.ts', '[dollar¦$]="greeting"');
            expect(definitions.length).toEqual(2);
            (0, testing_2.assertTextSpans)(definitions, ['dollar$', 'DollarDir']);
            (0, testing_2.assertFileNames)(definitions, ['app.ts']);
        });
    });
    function getDefinitionsAndAssertBoundSpan(project, file, targetText) {
        ...
        return defAndBoundSpan.definitions.map((d) => (0, testing_2.humanizeDocumentSpanLike)(d, env));
    }
});
