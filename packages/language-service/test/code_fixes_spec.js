...
const utils_1 = require("../src/codefixes/utils");
...
describe('code fixes', () => {
    ...
    it('should fix all errors when property does not exist on type', () => {
     ...
     `,
        ...
        const fixesAllSpelling = project.getCombinedCodeFix('app.ts', 'fixSpelling');
        ...
        const fixAllMissingMember = project.getCombinedCodeFix('app.ts', 'fixMissingMember');
    ...
    it('should fix all invalid banana-in-box errors', () => {
     ...
     `,
        ...
        const project = (0, testing_2.createModuleAndProjectWithDeclarations)(env, 'test', files);
        const appFile = project.openFile('app.ts');
        const fixesAllActions = project.getCombinedCodeFix('app.ts', utils_1.FixIdForCodeFixesAll.FIX_INVALID_BANANA_IN_BOX);
        expectIncludeReplacementTextForFileTextChange({
    ...
    describe('should fix missing selector imports', () => {
        it('for a new standalone component import', () => {
         ...
         `,
            ...
            const project = (0, testing_2.createModuleAndProjectWithDeclarations)(env, 'test', {}, {}, standaloneFiles);
    ...
    describe('unused standalone imports', () => {
       ...
       `,
            ...
            const project = (0, testing_2.createModuleAndProjectWithDeclarations)(env, 'test', files);
            const appFile = project.openFile('app.ts');
            const fixesAllActions = project.getCombinedCodeFix('app.ts', utils_1.FixIdForCodeFixesAll.FIX_UNUSED_STANDALONE_IMPORTS);
            expectIncludeReplacementTextForFileTextChange({
                fileTextChanges: fixesAllActions.changes,
                content: appFile.contents,
...
function actionChangesMatch(actionChanges, description, substitutions) {
...
function expectNotIncludeFixAllInfo(codeActions) {
 ...
 * check it.
 */
function expectIncludeReplacementText({ codeActions, content, text, newText, fileName, description, }) {
...
function expectIncludeReplacementTextForFileTextChange({ fileTextChanges, content, text, newText, fileName, }) {
    expect(includeReplacementTextInChanges({ fileTextChanges, content, text, newText, fileName })).toBeTruthy();
}
...
function includeReplacementTextInChanges({ fileTextChanges, content, text, newText, fileName, }) {
    for (const change of fileTextChanges) {
        ...
        for (const textChange of change.textChanges) {
            ...
            const textChangeOldText = content.slice(textChange.span.start, textChange.span.start + textChange.span.length);
...
function includeAddTextInChanges({ fileTextChanges, position, text, fileName, }) {
    for (const change of fileTextChanges) {
        ...
        for (const textChange of change.textChanges) {
            ...
            const includeAddText = (position === null || position === textChange.span.start) && text === textChange.newText;
            if (includeAddText) {
                return true;
            }
        }
