...
describe('type definitions', () => {
    ...
    describe('templates', () => {
        it('should return no definitions for ng-templates', () => {
            ...
            const defs = ngLS.getTypeDefinitionAtPosition(mock_host_1.APP_COMPONENT, position);
            expect(defs).toEqual([]);
        });
    });
    describe('directives', () => {
    ...
    describe('bindings', () => {
        describe('inputs', () => {
            ...
            it('should work for structural directive inputs ngForOf', () => {
                ...
                expectAllDefinitions(definitions, new Set(['Array', 'NgForOf']), new Set([...possibleArrayDefFiles, 'ng_for_of.d.ts']));
            });
        ...
        describe('outputs', () => {
            it('should work for event providers', () => {
                ...
                expect(emitterInterface.contextSpan).toContain('export interface EventEmitter<T> extends Subject<T>');
            ...
            it('should work for native event outputs', () => {
                ...
                expect(definitions[0].contextSpan).toEqual('addEventListener(type: string, listener: EventListenerOrEventListenerObject, options?: boolean | AddEventListenerOptions): void;');
    ...
    describe('expressions', () => {
        ...
        it('should work for variables in structural directives', () => {
            ...
            expectAllDefinitions(definitions, new Set(['Hero', 'Array']), new Set([...possibleArrayDefFiles, 'app.component.ts']));
        });
        ...
        it('should return nothing for the $any() cast function', () => {
            ...
            const definitionAndBoundSpan = ngLS.getTypeDefinitionAtPosition(mock_host_1.APP_COMPONENT, position);
            expect(definitionAndBoundSpan).toBeUndefined();
        });
    });
    function getTypeDefinitions({ templateOverride, }) {
        const { position } = service.overwriteInlineTemplate(mock_host_1.APP_COMPONENT, templateOverride);
        const defs = ngLS.getTypeDefinitionAtPosition(mock_host_1.APP_COMPONENT, position);
        expect(defs).toBeTruthy();
        return defs.map((d) => (0, test_utils_1.humanizeDefinitionInfo)(d, service));
    }
    function expectAllDefinitions(definitions, textSpans, possibleFileNames) {
