...
describe('definitions', () => {
    ...
    describe('templates', () => {
        it('should return no definitions for ng-templates', () => {
            const { position } = service.overwriteInlineTemplate(mock_host_1.APP_COMPONENT, `<ng-templ¦ate></ng-template>`);
            const definitionAndBoundSpan = ngLS.getDefinitionAndBoundSpan(mock_host_1.APP_COMPONENT, position);
    ...
    describe('directives', () => {
        ...
        it('should work for structural directives', () => {
            ...
            expect(definitions[0].contextSpan).toContain('export declare class NgForOf<T, U extends NgIterable<T> = NgIterable<T>> implements DoCheck');
        });
    ...
    describe('bindings', () => {
        describe('inputs', () => {
            ...
            it('should work for structural directive inputs ngForOf', () => {
                ...
                expect(inputDef.contextSpan).toEqual('set ngForOf(ngForOf: (U & NgIterable<T>) | undefined | null);');
            ...
            it('should work for two-way binding providers', () => {
                ...
                expect(outputDef.contextSpan).toEqual(`@Output() modelChange: EventEmitter<string> = new EventEmitter();`);
            });
        });
        describe('outputs', () => {
            ...
            it('should return nothing for $event from EventEmitter', () => {
                const { position } = service.overwriteInlineTemplate(mock_host_1.APP_COMPONENT, `<div string-model (modelChange)="myClick($e¦vent)"></div>`);
                const definitionAndBoundSpan = ngLS.getDefinitionAndBoundSpan(mock_host_1.APP_COMPONENT, position);
                expect(definitionAndBoundSpan).toBeUndefined();
            });
            ...
            it('should work for $event from native element', () => {
                ...
                expect(definitions[0].contextSpan).toContain('addEventListener<K extends keyof HTMLElementEventMap>');
    ...
    describe('references', () => {
        it('should work for element reference declarations', () => {
            const { position } = service.overwriteInlineTemplate(mock_host_1.APP_COMPONENT, `<div #cha¦rt></div>{{chart}}`);
            const definitionAndBoundSpan = ngLS.getDefinitionAndBoundSpan(mock_host_1.APP_COMPONENT, position);
    ...
    describe('expressions', () => {
        ...
        it('should return nothing for the $any() cast function', () => {
            const { position } = service.overwriteInlineTemplate(mock_host_1.APP_COMPONENT, `<div>{{$an¦y(title)}}</div>`);
            const definitionAndBoundSpan = ngLS.getDefinitionAndBoundSpan(mock_host_1.APP_COMPONENT, position);
            expect(definitionAndBoundSpan).toBeUndefined();
        });
    ...
    describe('external resources', () => {
        it('should be able to find a template from a url', () => {
            const { position, text } = service.overwrite(mock_host_1.APP_COMPONENT, `
        import {Component} from '@angular/core';
	      @Component({
	        templateUrl: './tes¦t.ng',
	      })
	      export class AppComponent {}`);
            const result = ngLS.getDefinitionAndBoundSpan(mock_host_1.APP_COMPONENT, position);
        ...
        import {Component} from '@angular/core';
	      ...
	      export class AppComponent {}`);
            ...
            expect(result).toBeDefined();
            const { textSpan, definitions } = result;
            expect(text.substring(textSpan.start, textSpan.start + textSpan.length)).toEqual('./test.css');
            expect(definitions).toBeDefined();
            expect(definitions.length).toBe(1);
            const [def] = definitions;
    ...
    function getDefinitionsAndAssertBoundSpan({ templateOverride, expectedSpanText, }) {
        const { position, text } = service.overwriteInlineTemplate(mock_host_1.APP_COMPONENT, templateOverride);
        const definitionAndBoundSpan = ngLS.getDefinitionAndBoundSpan(mock_host_1.APP_COMPONENT, position);
        expect(definitionAndBoundSpan).toBeTruthy();
        const { textSpan, definitions } = definitionAndBoundSpan;
        expect(text.substring(textSpan.start, textSpan.start + textSpan.length)).toEqual(expectedSpanText);
        expect(definitions).toBeTruthy();
        return definitions.map((d) => (0, test_utils_1.humanizeDefinitionInfo)(d, service));
    }
});
