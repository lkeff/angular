...
describe('get outlining spans', () => {
    ...
    it('should get block outlining spans for an inline template', () => {
      ...
      export class AppCmp {
        ...
        expect(files['app.ts'].substring(textSpan.start, textSpan.start + textSpan.length)).toEqual(' if body ');
    });
    it('should get block outlining spans for an external template', () => {
        ...
        const result = appFile.getOutliningSpans();
        const { textSpan } = result[0];
        expect(files['app.html'].substring(textSpan.start, textSpan.start + textSpan.length)).toEqual(' lazy text ');
    });
    it('should have outlining spans for all defer block parts', () => {
        ...
        expect(getTrimmedSpanText(result[1].textSpan, files['app.ts'])).toEqual('defer placeholder block');
    ...
    it('should have outlining spans for all switch cases, including the main', () => {
        ...
        const appFile = project.openFile('app.ts');
        const result = appFile.getOutliningSpans();
        expect(getTrimmedSpanText(result[0].textSpan, files['app.ts'])).toMatch(/case..'test'.*default.*\}/);
...
function getTrimmedSpanText(span, contents) {
