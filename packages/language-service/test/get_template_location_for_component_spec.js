...
describe('get template location for component', () => {
    ...
    it('finds location of inline template', () => {
      ...
      export class AppCmp {
        ...
        (0, testing_2.assertFileNames)([result], ['app.ts']);
        expect((0, testing_2.humanizeDocumentSpanLike)(result, env).textSpan).toEqual(`'<div>{{ myProp }}</div>'`);
    });
    it('finds location of external template', () => {
        ...
        appFile.moveCursorToText('my¦Prop!: string');
        const result = appFile.getTemplateLocationForComponent();
        (0, testing_2.assertFileNames)([result], ['app.html']);
    });
    ...
    it('returns template when cursor is on `class` keyword', () => {
      ...
      export class MyDir {
        ...
        const result = appFile.getTemplateLocationForComponent();
        (0, testing_2.assertFileNames)([result], ['app.ts']);
        expect((0, testing_2.humanizeDocumentSpanLike)(result, env).textSpan).toEqual(`'abc'`);
    });
});
