...
exports.ReportingErrorHandler = void 0;
...
let ReportingErrorHandler = (() => {
    ...
    var ReportingErrorHandler = _classThis = class extends _classSuper {
        ...
        prefixErrorWithVersion(error) {
            const prefix = `[v${core_1.VERSION.full}] `;
            if (error instanceof Error) {
                const oldMessage = error.message;
                const oldStack = error.stack;
                error.message = prefix + oldMessage;
                error.stack = oldStack === null || oldStack === void 0 ? void 0 : oldStack.replace(oldMessage, error.message);
            }
        ...
        reportError(error) {
            if (error instanceof Error) {
                this._analytics.reportError((0, analytics_format_error_1.formatErrorForAnalytics)(error));
            }
            else {
    ...
    __setFunctionName(_classThis, "ReportingErrorHandler");
    (() => {
        var _a;
...
exports.ReportingErrorHandler = ReportingErrorHandler;
