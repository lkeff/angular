...
exports.EXAMPLES_PATH = void 0;
exports.interpolateCodeExamples = interpolateCodeExamples;
const path_1 = require("path");
const fs_1 = __importDefault(require("fs"));
exports.EXAMPLES_PATH = 'packages/examples';
...
const HTML_COMMENT_REGION_REGEX = /[ \t]*<!--[ \t]*#(docregion|enddocregion)[ \t]*[\w-]*[ \t]*-->(\n|$)/g;
const examplesCache = new Map(); // <file_path, <region, example>>
const MD_CTYPE_MAP = {
    'ts': 'angular-ts',
    'js': 'javascript',
    'html': 'angular-html',
 ...
 */
function interpolateCodeExamples(entries) {
...
function replaceExample(text) {
    ...
    return text.replace(examplesTagRegex, (_, path, __, region) => {
        const example = getExample(path, region);
        if (!example) {
            throw new Error(`Missing code example ${exports.EXAMPLES_PATH}/${path}#${region}`);
        }
        return example;
    });
}
/** Returns the example wrapped in a Markdown code block or an empty string, if the example doesn't exist. */
function getExample(path, region) {
    let fileExamples = examplesCache.get(path);
    const src = `${exports.EXAMPLES_PATH}/${path}`;
    const fullPath = (0, path_1.resolve)((0, path_1.dirname)(src), (0, path_1.basename)(src));
    const fileType = path.split('.').pop();
    if (!fileExamples) {
        const contents = fs_1.default.readFileSync(fullPath, { encoding: 'utf8' });
 ...
 */
function extractExamplesFromContents(contents, fileType) {
    let markerBuffer = '';
    let paramBuffer = '';
            ...
            switch (markerBuffer) {
                ...
                case REGION_END_MARKER:
                    if (regionStack.length) {
                        ...
                        const endIdx = i - REGION_END_MARKER.length - (paramBuffer ? paramBuffer.length + 1 : 0);
...
function removeLeftoverCommentsFromExample(example, fileType) {
