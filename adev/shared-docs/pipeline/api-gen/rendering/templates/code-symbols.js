...
const url_transforms_1 = require("../transforms/url-transforms");
...
function CodeSymbol(props) {
    return (<code>
      {props.code.split(symbolRegex).map((rawSymbol, index) => {
            ...
            const moduleName = (0, symbol_context_1.getModuleName)(symbol);
            if (moduleName) {
                const url = (0, url_transforms_1.getLinkToModule)(moduleName, symbol, subSymbol);
