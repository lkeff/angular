...
exports.getNavItemGenStrategy = getNavItemGenStrategy;
...
function getNavItemGenStrategy(strategy, packageDir) {
    if (SUPPORTED_STRATEGIES.indexOf(strategy) === -1) {
        throw new Error(`Unsupported NavigationItem generation strategy "${strategy}". Supported: ${SUPPORTED_STRATEGIES.join(', ')}`);
    }
    switch (strategy) {
        case 'errors':
