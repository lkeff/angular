...
exports.generatePlaygroundRoutes = generatePlaygroundRoutes;
exports.generateTutorialRoutes = generateTutorialRoutes;
function generatePlaygroundRoutes(configs) {
...
function generateTutorialRoutes(tutorialName, introConfig, stepConfigs) {
    return __awaiter(this, void 0, void 0, function* () {
        const children = Object.entries(stepConfigs)
            // Sort using the number prefix from the step directory name.
            .sort(([pathA], [pathB]) => Number(pathA.split('-')[0]) > Number(pathB.split('-')[0]) ? 1 : -1)
            .map(([path, config], idx) => {
            return {
                label: config.title,
