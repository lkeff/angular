...
const queryPackagesCmd = `${bazelCmd} query --output=label "attr('tags', '\\[.*${releaseTargetTag}.*\\]', //packages/...) ` +
...
export function performNpmReleaseBuild() {
 ...
 */
export function performDefaultSnapshotBuild() {
 ...
 */
function buildReleasePackages(distPath, isSnapshotBuild, additionalTargets = []) {
    console.info('######################################');
    ...
    const getBazelOutputPath = (pkgName) => join(bazelBinPath, 'packages', pkgName, 'npm_package');
    const getDistPath = (pkgName) => join(distPath, pkgName);
...
export function exec(command, captureStdout) {
